...
def find_regressions(
    ...
    *,
    ...
    prefix = "" if scope == "overall" else f"{scope} {dataset}/{split}: "
    if min_holyc_accuracy is not None and summary["holyc_accuracy"] < min_holyc_accuracy:
        regressions.append(
            EvalRegression(
                ...
                message=(
                    f"{prefix}HolyC accuracy {summary['holyc_accuracy']:.4f} "
    ...
    if min_agreement is not None and summary["agreement"] < min_agreement:
        regressions.append(
            EvalRegression(
                ...
                message=f"{prefix}engine agreement {summary['agreement']:.4f} is below minimum {min_agreement:.4f}",
                scope=scope,
                dataset=dataset,
                split=split,
            )
        ...
        if delta < allowed_delta:
            regressions.append(
                EvalRegression(
                    ...
                    message=(
                        f"{prefix}HolyC accuracy delta {delta:.4f} is below allowed drop "
                        f"{allowed_delta:.4f}"
                    ),
                    scope=scope,
                    dataset=dataset,
...
def find_all_regressions(summary: dict[str, Any], args: argparse.Namespace) -> list[EvalRegression]:
...
def write_junit(regressions: list[EvalRegression], path: Path) -> None:
    ...
    if regressions:
        ...
        failure.text = "\n".join(
            f"{regression.scope}:{regression.dataset or '-'}:{regression.split or '-'}:{regression.metric}: "
            f"value={regression.value:.6f} threshold={regression.threshold:.6f}"
...
def write_breakdown_csv_report(path: Path, breakdown: list[dict[str, Any]]) -> None:
    with path.open("w", newline="", encoding="utf-8") as handle:
        fields = [
            "dataset",
            "split",
...
def write_calibration_bins_csv_report(path: Path, summary: dict[str, Any]) -> None:
    with path.open("w", newline="", encoding="utf-8") as handle:
        fields = [
            "engine",
            "bin_index",
        ...
        writer = csv.DictWriter(handle, fieldnames=fields, lineterminator="\n")
        writer.writeheader()
        for engine, key in (("holyc", "holyc_calibration"), ("llama", "llama_calibration")):
            for bin_index, calibration_bin in enumerate(summary[key]["calibration_bins"]):
                writer.writerow(
                    {
                        "engine": engine,
                        "bin_index": bin_index,
                        "lower": calibration_bin["lower"],
                        "upper": calibration_bin["upper"],
                        "count": calibration_bin["count"],
                        "accuracy": calibration_bin["accuracy"],
                        "confidence": calibration_bin["confidence"],
                        "ece_contribution": calibration_bin["ece_contribution"],
                    }
                )
...
def write_report(
    ...
    report = {
        ...
        "gate_dataset_breakdowns": args.gate_dataset_breakdowns,
