...
def extract_elapsed_us(payload: dict[str, Any]) -> int | None:
    for key in ("elapsed_us", "duration_us", "decode_elapsed_us"):
        parsed = parse_int(payload.get(key))
...
def run_prompt(
    ...
    host_elapsed_us = max(1, (time.monotonic_ns() - started) // 1000)
    payload = parse_bench_payload(stdout + "\n" + stderr)
    elapsed_us = extract_elapsed_us(payload) or host_elapsed_us
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument("--qemu-arg", action="append", default=[], help="Extra QEMU argument; repeat per token")
...
def main(argv: list[str] | None = None) -> int:
    ...
    command = build_command(args.qemu_bin, args.image, args.qemu_arg)
