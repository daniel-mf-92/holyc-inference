#!/usr/bin/env python3
"""Smoke test for qemu_benchmark_matrix.py."""
...
def main() -> int:
    with tempfile.TemporaryDirectory() as tmp:
        ...
        status = qemu_benchmark_matrix.main(
            [
                str(matrix),
                "--output-dir",
                "bench/results",
                "--output-stem",
                "qemu_benchmark_matrix_smoke_latest",
            ]
        )
        if status != 0:
            return status
        payload = json.loads(Path("bench/results/qemu_benchmark_matrix_smoke_latest.json").read_text(encoding="utf-8"))
        if payload["summary"]["builds"] != 2 or payload["summary"]["launches"] != 12:
            return 1
        if payload["summary"]["airgap_ok_builds"] != 2:
            return 1
        return 0
