#!/usr/bin/env python3
"""Smoke gate for QEMU serial endpoint audit."""
...
def main() -> int:
    with tempfile.TemporaryDirectory() as tmp_dir:
        ...
        if qemu_serial_endpoint_audit.main(
            [
                str(safe),
                "--output-dir",
                str(safe_out),
                "--output-stem",
                "qemu_serial_endpoint_audit_smoke",
                "--require-top-command",
            ]
        ) != 0:
            return 1
        ...
        write_artifact(
            ...
            [
                ...
                row(["qemu-system-x86_64", "-nic", "none", "-serial", "tcp:127.0.0.1:4444"], 2),
                row(["qemu-system-x86_64", "-nic", "none", "-serial", "stdio", "-monitor", "udp:127.0.0.1:4445"], 3),
            ],
        )
        bad_out = tmp / "bad_out"
        if qemu_serial_endpoint_audit.main(
            ...
            return 1
        report = json.loads((bad_out / "qemu_serial_endpoint_audit_smoke.json").read_text(encoding="utf-8"))
        kinds = {finding["kind"] for finding in report["findings"]}
        if not {"missing_serial_stdio", "socket_endpoint"} <= kinds:
            return 1
    return 0
