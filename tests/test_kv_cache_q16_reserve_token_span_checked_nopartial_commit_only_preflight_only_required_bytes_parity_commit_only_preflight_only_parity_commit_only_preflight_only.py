...
from test_kv_cache_q16_reserve_token_span_checked_nopartial_commit_only_preflight_only_required_bytes_parity_commit_only_preflight_only_parity_commit_only import (
...
def kv_cache_q16_reserve_token_span_checked_nopartial_commit_only_preflight_only_required_bytes_parity_commit_only_preflight_only_parity_commit_only_preflight_only(
    layer_count: int,
    head_count: int,
    head_dim: int,
    token_start: int,
...
def test_source_contains_required_bytes_parity_commit_only_preflight_only_helper() -> None:
    ...
    sig = "I32 KVCacheQ16ReserveTokenSpanCheckedNoPartialCommitOnlyPreflightOnlyRequiredBytesParityCommitOnlyPreflightOnlyParityCommitOnlyPreflightOnly("
...
def test_known_vector_required_cells_bytes_and_last_index() -> None:
    ...
    out_bytes = [222]
    out_last = [333]
    err = kv_cache_q16_reserve_token_span_checked_nopartial_commit_only_preflight_only_required_bytes_parity_commit_only_preflight_only_parity_commit_only_preflight_only(
...
def test_error_and_guard_paths() -> None:
    ...
    out_last = [93]
    assert (
        kv_cache_q16_reserve_token_span_checked_nopartial_commit_only_preflight_only_required_bytes_parity_commit_only_preflight_only_parity_commit_only_preflight_only(
...
def test_randomized_parity_vs_explicit_composition() -> None:
    rng = random.Random(20260422_1089)
    for _ in range(2500):
        ...
        got_bytes = [7002]
        got_last = [7003]
        err_new = kv_cache_q16_reserve_token_span_checked_nopartial_commit_only_preflight_only_required_bytes_parity_commit_only_preflight_only_parity_commit_only_preflight_only(
            layer_count,
            head_count,
            head_dim,
            token_start,
        ...
        if err_new == KV_Q16_OK:
            assert (got_cells[0], got_bytes[0], got_last[0]) == ref_tuple
        else:
