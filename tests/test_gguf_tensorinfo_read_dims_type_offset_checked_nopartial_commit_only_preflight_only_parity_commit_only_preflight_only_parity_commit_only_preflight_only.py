#!/usr/bin/env python3
"""Harness for IQ-1052 dims/type/offset parity-commit preflight-only companion."""
...
def parse_dims_type_offset_checked_nopartial_commit_only_preflight_only_parity_commit_only_preflight_only_parity_commit_only_preflight_only(
    buf: bytes | None,
    size: int,
    cursor: int,
    ...
    if staged_commit_type_value[0] not in KNOWN_TYPES or canonical_type_value[0] not in KNOWN_TYPES:
...
def explicit_checked_composition(*args) -> int:
    return parse_dims_type_offset_checked_nopartial_commit_only_preflight_only_parity_commit_only_preflight_only_parity_commit_only_preflight_only(
        *args
    )


def test_source_contains_iq1052_signature_and_contract() -> None:
    source = Path("src/gguf/tensorinfo.HC").read_text(encoding="utf-8")
    sig = "I64 GGUFTensorInfoReadDimsTypeOffsetCheckedNoPartialCommitOnlyPreflightOnlyParityCommitOnlyPreflightOnlyParityCommitOnlyPreflightOnly("
    assert sig in source
    body = source.split(sig, 1)[1].split("I64 GGUFTensorParseOne(", 1)[0]

    assert "IQ-1052 diagnostics-only zero-write companion for dims/type/offset parity." in source
...
def test_known_vector_success_and_alias_guard() -> None:
    ...
    err = parse_dims_type_offset_checked_nopartial_commit_only_preflight_only_parity_commit_only_preflight_only_parity_commit_only_preflight_only(
        payload,
        len(payload),
        0,
        out_dim_count,
...
def test_adversarial_dim_type_span_vectors() -> None:
    ...
    bad_type_payload = dims_type_offset_entry([2], 999, 7)
    err = parse_dims_type_offset_checked_nopartial_commit_only_preflight_only_parity_commit_only_preflight_only_parity_commit_only_preflight_only(
        bad_type_payload,
...
def test_fuzz_parity_matches_explicit_composition() -> None:
    rng = random.Random(20260422_1052)

    for _ in range(512):
        dim_count = rng.randint(0, 6)
        dims = [rng.randint(0, 2048) for _ in range(dim_count)]

        if rng.random() < 0.15:
            tensor_type = rng.choice([0, 3, 11, 777, 2**32 - 1])
        else:
            tensor_type = rng.choice(sorted(KNOWN_TYPES))

        tensor_offset = rng.randint(0, 2**63 - 1)
        payload = dims_type_offset_entry(dims, tensor_type, tensor_offset)

        size = len(payload)
        if rng.random() < 0.1:
            size = rng.randint(0, max(0, len(payload) - 1))
        cursor = 0
        if rng.random() < 0.2:
            cursor = rng.randint(0, size + 3)

        out_a = [[111], [222], [333], [444], [555], [666]]
        out_b = [[111], [222], [333], [444], [555], [666]]

        err_a = parse_dims_type_offset_checked_nopartial_commit_only_preflight_only_parity_commit_only_preflight_only_parity_commit_only_preflight_only(
            payload,
            size,
            cursor,
            out_a[0],
