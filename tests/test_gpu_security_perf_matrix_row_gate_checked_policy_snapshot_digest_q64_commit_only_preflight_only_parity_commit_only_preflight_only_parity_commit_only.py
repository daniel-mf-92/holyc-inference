#!/usr/bin/env python3
"""Harness for IQ-1525 commit-only hardening over digest preflight/parity wrappers."""
...
def gpu_security_perf_matrix_row_gate_checked_policy_snapshot_digest_q64_commit_only_preflight_only_parity_commit_only_preflight_only_parity_commit_only(
    secure_local_mode: int,
    iommu_active: int,
    book_of_truth_gpu_hooks: int,
    policy_digest_parity: int,
...
def test_source_contains_iq1525_symbols() -> None:
    src = Path("src/gpu/security_perf_matrix.HC").read_text(encoding="utf-8")

    assert "I32 GPUSecurityPerfMatrixRowGateCheckedPolicySnapshotDigestQ64CommitOnlyPreflightOnlyParityCommitOnlyPreflightOnlyParityCommitOnly(" in src
    ...
    assert "// IQ-1525 commit-only hardening wrapper:" in src
...
def test_secure_on_overhead_budget_vectors() -> None:
    ...
    for prompt_tokens, batch_size, quant_profile in vectors:
        ...
        status, digest = gpu_security_perf_matrix_row_gate_checked_policy_snapshot_digest_q64_commit_only_preflight_only_parity_commit_only_preflight_only_parity_commit_only(
            secure_local_mode=GPU_SEC_PERF_PROFILE_SECURE_LOCAL,
            iommu_active=1,
            book_of_truth_gpu_hooks=1,
            policy_digest_parity=1,
...
if __name__ == "__main__":
    test_source_contains_iq1525_symbols()
