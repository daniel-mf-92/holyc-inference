#!/usr/bin/env python3
"""Host-side harness for IQ-1322 PrefixCacheReplayGuardCheckedNoPartialCommitOnlyPreflightOnlyParity."""
...
def test_preflight_only_parity_secure_local_monotonic_roundtrip():
...
def test_preflight_only_parity_dev_local_allows_rollback():
...
def test_preflight_only_parity_secure_local_policy_failure_keeps_output_slots_unchanged():
...
def test_preflight_only_parity_rejects_bad_vectors():
    cache = _cache_with_tick(1)
    out = [0, 0, 0]

    assert (
        replay_guard_commit_only_preflight_only_parity(
            None,
            1,
            1,
...
def test_holyc_function_body_and_contract_markers_present():
    ...
    sig = "I32 PrefixCacheReplayGuardCheckedNoPartialCommitOnlyPreflightOnlyParity(PrefixCache *cache,"
    assert sig in source
    body = source.split(sig, 1)[1]
    assert "status_preflight = PrefixCacheReplayGuardCheckedNoPartialCommitOnlyPreflightOnly(cache," in body
    assert "cache->entries[entry_index].last_used_tick = snapshot_previous_tick;" in body
    assert "status_commit = PrefixCacheReplayGuardCheckedNoPartialCommitOnly(cache," in body
...
if __name__ == "__main__":
    test_preflight_only_parity_secure_local_monotonic_roundtrip()
    test_preflight_only_parity_dev_local_allows_rollback()
    test_preflight_only_parity_secure_local_policy_failure_keeps_output_slots_unchanged()
    test_preflight_only_parity_rejects_bad_vectors()
