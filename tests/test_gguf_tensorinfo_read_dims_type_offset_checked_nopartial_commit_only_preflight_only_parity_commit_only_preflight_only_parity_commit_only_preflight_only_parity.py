#!/usr/bin/env python3
"""Harness for IQ-1056 dims/type/offset parity gate over commit+preflight wrappers."""
...
from test_gguf_tensorinfo_read_dims_type_offset_checked_nopartial_commit_only_preflight_only_parity_commit_only_preflight_only_parity_commit_only_preflight_only import (
    parse_dims_type_offset_checked_nopartial_commit_only_preflight_only_parity_commit_only_preflight_only_parity_commit_only_preflight_only,
)
...
def parse_dims_type_offset_checked_nopartial_commit_only_preflight_only_parity_commit_only_preflight_only_parity_commit_only_preflight_only_parity(
    buf: bytes | None,
    size: int,
    cursor: int,
    ...
    status = parse_dims_type_offset_checked_nopartial_commit_only_preflight_only_parity_commit_only_preflight_only_parity_commit_only_preflight_only(
        buf,
        size,
        cursor,
    ...
    pre_end = u64_add(cursor, staged_pre_required_bytes[0])
    ...
    if staged_pre_dim_count[0] == 0 or staged_commit_dim_count[0] == 0:
        return GGUF_TENSOR_PARSE_ERR_BAD_DIMS
    if staged_pre_dim_count[0] > 8 or staged_commit_dim_count[0] > 8:
        return GGUF_TENSOR_PARSE_ERR_BAD_DIMS
    ...
    if staged_pre_last_dim_index[0] != staged_pre_dim_count[0] - 1:
        return GGUF_TENSOR_PARSE_ERR_TRUNCATED
    if staged_commit_last_dim_index[0] != staged_commit_dim_count[0] - 1:
        return GGUF_TENSOR_PARSE_ERR_TRUNCATED

    if staged_pre_type_value[0] not in KNOWN_TYPES or staged_commit_type_value[0] not in KNOWN_TYPES:
        return GGUF_TENSOR_PARSE_ERR_BAD_TYPE

    if pre_end != commit_end:
        return GGUF_TENSOR_PARSE_ERR_TRUNCATED

    out_dim_count[0] = staged_pre_dim_count[0]
    out_dims_cells[0] = staged_pre_dims_cells[0]
    out_required_bytes[0] = staged_pre_required_bytes[0]
    out_type_value[0] = staged_pre_type_value[0]
    out_tensor_offset[0] = staged_pre_tensor_offset[0]
    out_last_dim_index[0] = staged_pre_last_dim_index[0]
    return GGUF_TENSOR_PARSE_OK


def explicit_checked_composition(*args) -> int:
    return parse_dims_type_offset_checked_nopartial_commit_only_preflight_only_parity_commit_only_preflight_only_parity_commit_only_preflight_only_parity(
        *args
    )


def test_source_contains_iq1056_signature_and_contract() -> None:
    source = Path("src/gguf/tensorinfo.HC").read_text(encoding="utf-8")
    sig = "I64 GGUFTensorInfoReadDimsTypeOffsetCheckedNoPartialCommitOnlyPreflightOnlyParityCommitOnlyPreflightOnlyParityCommitOnlyPreflightOnlyParity("
    assert sig in source
    body = source.rsplit(sig, 1)[1].split("\nI64 GGUFTensorParseOne(", 1)[0]

    assert "IQ-1056 diagnostics-only parity gate for dims/type/offset commit+preflight wrappers." in source
    assert "GGUFTensorInfoReadDimsTypeOffsetCheckedNoPartialCommitOnlyPreflightOnlyParityCommitOnlyPreflightOnlyParityCommitOnlyPreflightOnly(" in body
...
def test_known_vector_success_and_alias_guard() -> None:
    ...
    err = parse_dims_type_offset_checked_nopartial_commit_only_preflight_only_parity_commit_only_preflight_only_parity_commit_only_preflight_only_parity(
        payload,
        len(payload),
        0,
        out_dim_count,
...
def test_randomized_parity_vs_explicit_composition() -> None:
    rng = random.Random(20260422_1056)
    ...
    for _ in range(900):
        ...
        got = parse_dims_type_offset_checked_nopartial_commit_only_preflight_only_parity_commit_only_preflight_only_parity_commit_only_preflight_only_parity(
            buf,
            size,
            cursor,
            out_a[0],
...
if __name__ == "__main__":
    test_source_contains_iq1056_signature_and_contract()
