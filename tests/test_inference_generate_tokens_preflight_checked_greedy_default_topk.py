#!/usr/bin/env python3
"""Reference checks for InferenceGenerateTokensPreflightCheckedGreedyDefaultTopK (IQ-786)."""
...
def inference_generate_tokens_preflight_greedy_default_topk_reference(
    *,
    step_logits_capacity: int,
    vocab_size: int,
    max_new_tokens: int,
...
def test_source_contains_preflight_greedy_default_topk_wrapper() -> None:
    source = Path("src/model/sampling.HC").read_text(encoding="utf-8")
    assert "I32 InferenceGenerateTokensPreflightCheckedGreedyDefaultTopK(" in source
    assert "return InferenceGenerateTokensPreflightChecked(" in source
    assert "out_required_topk_capacity" in source


def test_source_greedy_default_topk_wrapper_is_direct_forwarder() -> None:
    source = Path("src/model/sampling.HC").read_text(encoding="utf-8")
    signature = "I32 InferenceGenerateTokensPreflightCheckedGreedyDefaultTopK("
...
def test_preflight_greedy_default_topk_matches_explicit_reference_randomized() -> None:
    rng = random.Random(20260420_786)
    for _ in range(800):
        ...
        status_default, diag_default = (
            inference_generate_tokens_preflight_greedy_default_topk_reference(
...
def test_preflight_greedy_default_topk_preserves_failure_classification() -> None:
    cases = [
        dict(
    ...
    for case in cases:
        ...
        status_default, diag_default = (
            inference_generate_tokens_preflight_greedy_default_topk_reference(**case)
        )
        status_explicit, diag_explicit = inference_generate_tokens_preflight_reference(
            **case
        )
...
def test_preflight_greedy_default_topk_success_diagnostics_exact() -> None:
    status, diagnostics = (
        inference_generate_tokens_preflight_greedy_default_topk_reference(
...
if __name__ == "__main__":
    test_source_contains_preflight_greedy_default_topk_wrapper()
    test_source_greedy_default_topk_wrapper_is_direct_forwarder()
    test_preflight_greedy_default_topk_matches_explicit_reference_randomized()
    test_preflight_greedy_default_topk_preserves_failure_classification()
    test_preflight_greedy_default_topk_success_diagnostics_exact()
    print("inference_generate_tokens_preflight_checked_greedy_default_topk_reference_checks=ok")
