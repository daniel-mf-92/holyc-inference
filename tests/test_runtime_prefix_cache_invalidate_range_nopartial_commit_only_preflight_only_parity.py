#!/usr/bin/env python3
"""Host-side harness for IQ-1338 PrefixCacheInvalidateRangeCheckedNoPartialCommitOnlyPreflightOnlyParity."""
...
def test_iq1338_preflight_only_parity_matches_commit_only_tuple():
    ...
    status, removed = invalidate_range_commit_only_preflight_only_parity(cache, 12, 30)
...
def test_iq1338_preflight_only_parity_zero_length_invalidate_is_noop():
    ...
    status, removed = invalidate_range_commit_only_preflight_only_parity(cache, 12, 0)
    assert status == PREFIX_CACHE_OK
    assert removed == 0
...
def test_iq1338_preflight_only_parity_preflight_guard_keeps_state_unchanged_on_error():
    ...
    status, removed = invalidate_range_commit_only_preflight_only_parity(cache, 0, 10)
...
def test_holyc_function_body_and_contract_markers_present():
    ...
    sig = "I32 PrefixCacheInvalidateRangeCheckedNoPartialCommitOnlyPreflightOnlyParity(PrefixCache *cache,"
    assert sig in source
    body = source.split(sig, 1)[1]
    assert (
        "status_preflight = PrefixCacheInvalidateRangeCheckedNoPartialCommitOnlyPreflightOnly(cache," in body
    )
    assert "cache->entries[idx] = snapshot_entries[idx];" in body
    assert "status_commit = PrefixCacheInvalidateRangeCheckedNoPartialCommitOnly(cache," in body
...
if __name__ == "__main__":
    test_iq1338_preflight_only_parity_matches_commit_only_tuple()
    test_iq1338_preflight_only_parity_zero_length_invalidate_is_noop()
    test_iq1338_preflight_only_parity_preflight_guard_keeps_state_unchanged_on_error()
