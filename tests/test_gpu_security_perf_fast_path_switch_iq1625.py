#!/usr/bin/env python3
"""Harness for IQ-1625 zero-write diagnostics companion over IQ-1625 and IQ-1623."""
...
def _target_iq1624_commit_only(
    secure_local_mode: int,
    iommu_active: int,
    book_of_truth_gpu_hooks: int,
    policy_digest_parity: int,
...
def fast_path_switch_checked_audit_parity_iq1625(
    secure_local_mode: int,
    iommu_active: int,
    book_of_truth_gpu_hooks: int,
    policy_digest_parity: int,
    ...
    *,
    ...
    status_commit_only, staged_enabled, staged_reason = _target_iq1624_commit_only(
        secure_local_mode,
        iommu_active,
        book_of_truth_gpu_hooks,
        policy_digest_parity,
...
def test_source_contains_iq1625_symbols() -> None:
...
def test_gate_missing_and_audit_parity_breach_vectors() -> None:
    status, enabled, reason = fast_path_switch_checked_audit_parity_iq1625(
...
def test_status_domain_drift_and_no_write_parity_vectors() -> None:
    ...
    status, enabled, reason = fast_path_switch_checked_audit_parity_iq1625(
        ...
        policy_digest_parity=1,
        dispatch_transcript_parity=1,
        current_fast_path_enabled=515,
        current_disable_reason_code=516,
    )
    assert (status, enabled, reason) == (GPU_SEC_PERF_OK, 515, 516)


def test_deterministic_tuple_parity_vectors() -> None:
    first = fast_path_switch_checked_audit_parity_iq1625(
        secure_local_mode=GPU_SEC_PERF_PROFILE_SECURE_LOCAL,
        iommu_active=1,
        book_of_truth_gpu_hooks=1,
        policy_digest_parity=1,
    ...
    second = fast_path_switch_checked_audit_parity_iq1625(
        secure_local_mode=GPU_SEC_PERF_PROFILE_SECURE_LOCAL,
        iommu_active=1,
        book_of_truth_gpu_hooks=1,
        policy_digest_parity=1,
...
if __name__ == "__main__":
    test_source_contains_iq1625_symbols()
