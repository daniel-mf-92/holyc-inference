#!/usr/bin/env python3
"""Reference checks for GGUFMetadataReadI16QuadCheckedNoPartial semantics."""
...
def gguf_metadata_read_i16_quad_checked_no_partial(
    buf: list[int] | None,
    buf_nbytes: int,
    cursor_ref: list[int] | None,
    table_end: int,
...
def test_null_ptr_and_no_partial_write() -> None:
    ...
    assert (
        gguf_metadata_read_i16_quad_checked_no_partial(
            None,
            16,
            cursor,
            16,
...
def test_fourth_scalar_fail_does_not_commit_prior_lanes_or_cursor() -> None:
    ...
    err = gguf_metadata_read_i16_quad_checked_no_partial(
        buf,
        len(buf),
        cursor,
        len(buf),
...
def test_randomized_parity_against_staged_quad() -> None:
    rng = random.Random(20260417_266)
    ...
    for _ in range(6000):
        ...
        err_a = gguf_metadata_read_i16_quad_checked_no_partial(
            buf,
            n,
            cursor_a,
            table_end,
        ...
        err_b = gguf_metadata_read_i16_quad_checked(
            buf,
            n,
            cursor_b,
            table_end,
...
if __name__ == "__main__":
    ...
    test_randomized_parity_against_staged_quad()
    print("ok")
