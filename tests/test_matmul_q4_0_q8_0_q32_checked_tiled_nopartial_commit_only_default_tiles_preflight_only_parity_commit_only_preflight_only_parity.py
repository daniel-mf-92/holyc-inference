#!/usr/bin/env python3
"""Parity harness for IQ-1034 default-tiles preflight-only parity gate."""
...
from test_matmul_q4_0_q8_0_q32_checked_tiled_nopartial_commit_only_default_tiles_preflight_only_parity_commit_only import (  # noqa: E402
...
def matmul_q4_0_q8_0_q32_checked_tiled_nopartial_commit_only_default_tiles_preflight_only_parity_commit_only_preflight_only_parity(
    lhs_q4_blocks,
...
def explicit_checked_composition(*args):
    return matmul_q4_0_q8_0_q32_checked_tiled_nopartial_commit_only_default_tiles_preflight_only_parity_commit_only_preflight_only_parity(
        *args
    )


def test_source_contains_iq1034_signature_and_parity_contract() -> None:
    source = pathlib.Path("src/matmul/q4_0_q8_0_matmul.HC").read_text(encoding="utf-8")
    sig = (
        "I32 "
        "MatMulQ4_0Q8_0Q32CheckedTiledNoPartialCommitOnlyDefaultTilesPreflightOnlyParityCommitOnlyPreflightOnlyParity("
    )
    assert sig in source

    body = source.rsplit(sig, 1)[1].split("\nI32 ", 1)[0]

    assert "// IQ-1034 diagnostics-only parity gate:" in body
...
def test_known_vector_success_and_alias_rejection() -> None:
    rng = random.Random(202604221034)
    ...
    bad = matmul_q4_0_q8_0_q32_checked_tiled_nopartial_commit_only_default_tiles_preflight_only_parity_commit_only_preflight_only_parity(
        lhs,
        len(lhs),
        row_count,
        lhs_stride,
...
def test_randomized_parity_vs_explicit_composition() -> None:
    rng = random.Random(2026042210342)
    ...
    for i in range(1000):
        ...
        local_rng = random.Random(202604221034200 + i)
        ...
        err_a = matmul_q4_0_q8_0_q32_checked_tiled_nopartial_commit_only_default_tiles_preflight_only_parity_commit_only_preflight_only_parity(
            lhs,
            lhs_capacity,
            row_count,
            lhs_stride,
...
if __name__ == "__main__":
    test_source_contains_iq1034_signature_and_parity_contract()
    test_known_vector_success_and_alias_rejection()
    test_randomized_parity_vs_explicit_composition()
    print("matmul_q4_0_q8_0_q32_checked_tiled_nopartial_commit_only_default_tiles_preflight_only_parity_commit_only_preflight_only_parity=ok")
