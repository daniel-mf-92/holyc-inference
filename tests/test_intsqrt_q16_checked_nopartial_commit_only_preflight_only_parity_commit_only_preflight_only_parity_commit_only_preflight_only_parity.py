#!/usr/bin/env python3
"""Harness for FPQ16Sqrt...PreflightOnlyParity (IQ-1242)."""
...
def fpq16_sqrt_checked_nopartial_commit_only_preflight_only_parity_commit_only_preflight_only_parity_commit_only_preflight_only_parity_reference(
    x_q16: int,
...
def test_source_contains_iq1242_function() -> None:
    source = Path("src/math/intsqrt.HC").read_text(encoding="utf-8")
    sig = "I32 FPQ16SqrtCheckedNoPartialCommitOnlyPreflightOnlyParityCommitOnlyPreflightOnlyParityCommitOnlyPreflightOnlyParity("
...
def test_snapshot_and_tuple_parity_guards() -> None:
    out = [11]
    err = fpq16_sqrt_checked_nopartial_commit_only_preflight_only_parity_commit_only_preflight_only_parity_commit_only_preflight_only_parity_reference(
        q16_from_float(49.0),
        out,
...
def test_success_and_explicit_composition_parity() -> None:
    ...
    for x_q16 in vectors:
        ...
        err_a = fpq16_sqrt_checked_nopartial_commit_only_preflight_only_parity_commit_only_preflight_only_parity_commit_only_preflight_only_parity_reference(
            x_q16,
            out_a,
        )
...
def test_randomized_parity_against_components() -> None:
    rng = random.Random(20260423_1242)

    for _ in range(12000):
        x_q16 = q16_from_float(rng.uniform(0.0, 50000.0))

        out_new = [9]
        err_new = fpq16_sqrt_checked_nopartial_commit_only_preflight_only_parity_commit_only_preflight_only_parity_commit_only_preflight_only_parity_reference(
            x_q16,
            out_new,
        )
...
if __name__ == "__main__":
    test_source_contains_iq1242_function()
    ...
    print(
        "intsqrt_q16_checked_nopartial_commit_only_preflight_only_parity_commit_only_preflight_only_parity_commit_only_preflight_only_parity=ok"
    )
