...
def row_gate_checked_nopartial_preflight_only_parity_commit_only_preflight_only_parity_commit_only_preflight_only(
    secure_local_mode: int,
    iommu_active: int,
    book_of_truth_gpu_hooks: int,
    policy_digest_parity: int,
...
def test_profile_guard_preserves_outputs() -> None:
    status, reason, allowed = row_gate_checked_nopartial_preflight_only_parity_commit_only_preflight_only_parity_commit_only_preflight_only(
        ...
        row_batch_size=1,
        row_quant_profile=GPU_SEC_PERF_QUANT_Q4_0,
        current_gate_reason_code=131,
        current_row_allowed=132,
    )
    assert (status, reason, allowed) == (GPU_SEC_PERF_ERR_POLICY_GUARD, 131, 132)
