#!/usr/bin/env python3
"""Reference checks for GGUFMetadataReadI64LEChecked semantics."""
...
def gguf_metadata_read_i64le_checked(
    ...
    out_value_ref[0] = reinterpret_u64_as_i64(raw[0])
    cursor_ref[0] = cur[0]
    return GGUF_META_TABLE_OK
...
def test_null_ptr_and_no_partial_write() -> None:
    ...
    assert (
        gguf_metadata_read_i64le_checked(None, 32, cursor, 16, out)
    ...
    assert (
        gguf_metadata_read_i64le_checked([0] * 32, 32, None, 16, out)
        == GGUF_META_TABLE_ERR_NULL_PTR
    )
    assert out[0] == 123456789

    assert (
        gguf_metadata_read_i64le_checked([0] * 32, 32, cursor, 16, None)
        == GGUF_META_TABLE_ERR_NULL_PTR
    )
    assert cursor[0] == 3
...
def test_short_reads_do_not_advance_or_write() -> None:
    ...
    for table_end in range(0, 8):
        cursor = [0]
        err = gguf_metadata_read_i64le_checked(buf, len(buf), cursor, table_end, out)
        assert err in (GGUF_META_TABLE_ERR_OUT_OF_BOUNDS, GGUF_META_TABLE_ERR_BAD_PARAM)
        assert cursor[0] == 0
        assert out[0] == -987654321
...
def test_success_signed_interpretation_and_cursor_advance() -> None:
    ...
    err = gguf_metadata_read_i64le_checked(buf, len(buf), cursor, len(buf), out)
    assert err == GGUF_META_TABLE_OK
    assert out[0] == -1
    assert cursor[0] == 10


def test_min_i64_boundary() -> None:
    # 0x8000000000000000 little-endian => -9223372036854775808
    buf = [0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x80]
    cursor = [0]
    out = [1]
...
def test_randomized_parity() -> None:
    rng = random.Random(20260417_197)
    ...
    for _ in range(5000):
        ...
        cursor = [cursor0]
        out = [314159265]
        err = gguf_metadata_read_i64le_checked(buf, n, cursor, table_end, out)

        if cursor0 + 8 <= table_end and cursor0 + 8 <= n:
            raw = 0
            for i in range(8):
                raw |= buf[cursor0 + i] << (8 * i)
            expect = reinterpret_u64_as_i64(raw)
            assert err == GGUF_META_TABLE_OK
            assert out[0] == expect
            assert cursor[0] == cursor0 + 8
        else:
            assert err in (GGUF_META_TABLE_ERR_OUT_OF_BOUNDS, GGUF_META_TABLE_ERR_BAD_PARAM)
            assert out[0] == 314159265
...
def run() -> None:
    ...
    test_min_i64_boundary()
    test_randomized_parity()
    print("gguf_metadata_read_i64le_checked_reference_checks=ok")
