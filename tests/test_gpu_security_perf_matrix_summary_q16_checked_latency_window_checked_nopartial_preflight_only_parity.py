#!/usr/bin/env python3
"""Harness for IQ-1505 latency-window no-partial preflight-only parity gate."""
...
def gpu_security_perf_matrix_summary_q16_checked_latency_window_checked_nopartial_preflight_only_parity(
...
def test_source_contains_iq1505_symbols() -> None:
    src = Path("src/gpu/security_perf_matrix.HC").read_text(encoding="utf-8")

    assert "I32 GPUSecurityPerfMatrixSummaryQ16CheckedLatencyWindowCheckedNoPartialPreflightOnlyParity(" in src
...
def test_parity_ok_zero_write_outputs_duplicate_latency_vector() -> None:
    rows = [
    ...
    status, outputs_after, diag = gpu_security_perf_matrix_summary_q16_checked_latency_window_checked_nopartial_preflight_only_parity(
        rows,
        rows_capacity=5,
        out_capacity=4,
    ...
    assert diag == (10_000, 10_000, 30_000, 80_000)
...
def test_null_alias_capacity_gate_and_parity_drift_vectors() -> None:
    ...
    status, *_ = gpu_security_perf_matrix_summary_q16_checked_latency_window_checked_nopartial_preflight_only_parity(
        rows,
        rows_capacity=1,
        out_capacity=4,
        secure_local_mode=GPU_SEC_PERF_PROFILE_SECURE_LOCAL,
