#!/usr/bin/env python3
"""Harness for IQ-1744 fast-path batch audit commit-only preflight-only companion."""
...
def iq1744_model(
    ...
    dispatch_transcript_parity: int,
    *,
    initial_out: tuple[int, int, int, int] = (313, 317, 331, 337),
    ...
    if status_staged == GPU_SEC_PERF_OK:
        if tuple_staged[0] < 0 or tuple_staged[1] < 0 or tuple_staged[2] < 0:
            return GPU_SEC_PERF_ERR_BAD_PARAM, saved_out
    ...
    if status_canonical == GPU_SEC_PERF_OK:
        if tuple_canonical[0] < 0 or tuple_canonical[1] < 0 or tuple_canonical[2] < 0:
            return GPU_SEC_PERF_ERR_BAD_PARAM, saved_out
...
def test_source_contains_iq1744_symbols() -> None:
    src = Path("src/gpu/security_perf_matrix.HC").read_text(encoding="utf-8")
    assert f"I32 {HC_FN_IQ1744}(" in src
    assert f"status_commit_only = {HC_FN_IQ1743}(" in src
    assert f"status_canonical = {HC_FN_IQ1742}(" in src
    assert "if (status_commit_only != status_canonical)" in src
    assert "// IQ-1744 zero-write diagnostics companion:" in src


def test_gate_missing_vector() -> None:
    status, out_values = iq1744_model(
    ...
    assert out_values == (313, 317, 331, 337)
...
def test_no_write_parity_vector() -> None:
    status, out_values = iq1744_model(
        ...
        policy_digest_parity=1,
        dispatch_transcript_parity=1,
        initial_out=(401, 409, 419, 421),
    )
    assert status == GPU_SEC_PERF_OK
    assert out_values == (401, 409, 419, 421)
...
def test_deterministic_secure_on_vector() -> None:
    ...
    assert first[1] == (313, 317, 331, 337)
    assert second == first


if __name__ == "__main__":
    test_source_contains_iq1744_symbols()
