#!/usr/bin/env python3
"""Parity harness for ...PreflightOnlyCommitOnlyParityPreflightOnly (IQ-849)."""
...
def inference_generate_tokens_checked_topk_default_preflight_only_commit_only_parity_preflight_only_reference(
    *,
    ...
    parity_required_step_logits_cells = [out_required_step_logits_cells[0]]
    parity_required_generated_capacity = [out_required_generated_capacity[0]]
    parity_required_random_capacity = [out_required_random_capacity[0]]
    ...
    status = inference_generate_tokens_checked_topk_default_preflight_only_commit_only_parity_reference(
        ...
        out_required_step_logits_cells=parity_required_step_logits_cells,
        out_required_generated_capacity=parity_required_generated_capacity,
        out_required_random_capacity=parity_required_random_capacity,
    )
    ...
    if parity_required_step_logits_cells[0] != diagnostics["required_step_logits_cells"]:
        return SAMPLING_Q16_ERR_BAD_PARAM
    if parity_required_generated_capacity[0] != diagnostics["required_generated_capacity"]:
        return SAMPLING_Q16_ERR_BAD_PARAM
    if parity_required_random_capacity[0] != diagnostics["required_random_capacity"]:
        return SAMPLING_Q16_ERR_BAD_PARAM

    out_required_step_logits_cells[0] = parity_required_step_logits_cells[0]
    out_required_generated_capacity[0] = parity_required_generated_capacity[0]
    out_required_random_capacity[0] = parity_required_random_capacity[0]
    return SAMPLING_Q16_OK


def explicit_parity_preflight_only_composition(
    *,
    step_logits_q16: list[int] | None,
    step_logits_capacity: int,
    vocab_size: int,
    ...
    parity_required_step_logits_cells = [0]
    parity_required_generated_capacity = [0]
    parity_required_random_capacity = [0]
...
def test_source_contains_commit_only_parity_preflight_only_helper() -> None:
    ...
    sig = "I32 InferenceGenerateTokensCheckedTopKDefaultPreflightOnlyCommitOnlyParityPreflightOnly("
    assert sig in source
    assert "InferenceGenerateTokensCheckedTopKDefaultPreflightOnlyCommitOnlyParity(" in source
    assert "InferenceGenerateTokensCheckedTopKDefaultPreflightOnly(" in source
    assert "if (parity_required_step_logits_cells != canonical_required_step_logits_cells)" in source
...
def test_randomized_parity_vs_explicit() -> None:
    rng = random.Random(20260421_849)
    ...
    for _ in range(800):
        ...
        status_new = inference_generate_tokens_checked_topk_default_preflight_only_commit_only_parity_preflight_only_reference(
        ...
        status_ref = explicit_parity_preflight_only_composition(
            step_logits_q16=[0] * max(1, step_logits_capacity),
            step_logits_capacity=step_logits_capacity,
            vocab_size=vocab_size,
            max_new_tokens=max_new_tokens,
...
if __name__ == "__main__":
    test_source_contains_commit_only_parity_preflight_only_helper()
    test_known_vector()
    test_error_no_publish()
    test_randomized_parity_vs_explicit()
    print("inference_generate_tokens_checked_topk_default_preflight_only_commit_only_parity_preflight_only=ok")
