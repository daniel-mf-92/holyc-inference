#!/usr/bin/env python3
"""Host-side harness for IQ-1294 PrefixCacheInvalidateRangeCheckedNoPartialCommitOnly."""
...
def test_iq1294_commit_only_invalidates_overlap_and_updates_count_atomically():
    ...
    status, removed = invalidate_range_commit_only(cache, 12, 30)
...
def test_iq1294_commit_only_preflight_rejects_negative_entry_without_mutation():
    ...
    before = [Entry(**vars(entry)) for entry in entries]

    status, removed = invalidate_range_commit_only(cache, 0, 10)
...
def test_iq1294_commit_only_zero_length_invalidation_is_noop():
    ...
    status, removed = invalidate_range_commit_only(cache, 12, 0)
    assert status == PREFIX_CACHE_OK
    assert removed == 0
    assert cache.count == 2
...
def test_iq1294_source_contains_signature_and_atomic_staging():
    ...
    assert "staged_live_count" in source
    assert "staged_removed_count" in source
    assert "status = PrefixCacheInvalidateRangeChecked(cache," in source


def main():
    test_iq1294_commit_only_invalidates_overlap_and_updates_count_atomically()
    test_iq1294_commit_only_preflight_rejects_negative_entry_without_mutation()
    test_iq1294_commit_only_zero_length_invalidation_is_noop()
    test_iq1294_source_contains_signature_and_atomic_staging()
    print("ok")
