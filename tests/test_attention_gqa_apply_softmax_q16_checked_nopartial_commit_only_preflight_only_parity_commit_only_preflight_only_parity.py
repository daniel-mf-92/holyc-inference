...
from test_attention_gqa_apply_softmax_q16_checked_nopartial_commit_only_preflight_only_parity_commit_only import (
...
def gqa_attention_apply_softmax_q16_checked_nopartial_commit_only_preflight_only_parity_commit_only_preflight_only_parity(
    scores_q32,
    scores_capacity: int,
    query_rows: int,
    key_rows: int,
    ...
    staged_preflight = [0] * required_out_cells
    ...
    for idx in range(required_out_cells):
        if staged_preflight[idx] != staged_commit[idx]:
            return ATTN_Q16_ERR_BAD_PARAM
