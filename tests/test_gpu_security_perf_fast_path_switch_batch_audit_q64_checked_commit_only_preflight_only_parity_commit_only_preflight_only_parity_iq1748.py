#!/usr/bin/env python3
"""Harness for IQ-1748 fast-path batch audit strict diagnostics parity gate."""
...
import test_gpu_security_perf_fast_path_switch_batch_audit_q64_checked_commit_only_preflight_only_parity_commit_only_preflight_only_iq1747 as iq1747

GPU_SEC_PERF_OK = iq1747.GPU_SEC_PERF_OK
GPU_SEC_PERF_ERR_BAD_PARAM = iq1747.GPU_SEC_PERF_ERR_BAD_PARAM
GPU_SEC_PERF_PROFILE_SECURE_LOCAL = iq1747.GPU_SEC_PERF_PROFILE_SECURE_LOCAL

Scenario = iq1747.Scenario

HC_FN_IQ1748 = "GPUSecurityPerfFastPathSwitchBatchAuditQ64CheckedCommitOnlyPreflightOnlyParityCommitOnlyPreflightOnlyParity"
HC_FN_IQ1747 = iq1747.HC_FN_IQ1747
HC_FN_IQ1746 = iq1747.HC_FN_IQ1746
...
def iq1748_model(
    ...
    *,
    ...
    force_status_domain_drift_preflight_only: bool = False,
    ...
    status_commit_only, canonical_tuple = iq1747.iq1746.iq1746_model(
        scenarios=scenarios,
    ...
    status_preflight_only, staged_tuple = iq1747.iq1747_model(
        scenarios=scenarios,
        secure_local_mode=secure_local_mode,
        iommu_active=iommu_active,
        book_of_truth_gpu_hooks=book_of_truth_gpu_hooks,
        ...
        force_status_domain_drift_commit_only=force_status_domain_drift_preflight_only,
    )

    if not iq1747.iq1746._status_is_valid(status_preflight_only):
        return GPU_SEC_PERF_ERR_BAD_PARAM, saved_out
    if not iq1747.iq1746._status_is_valid(status_commit_only):
        return GPU_SEC_PERF_ERR_BAD_PARAM, saved_out
...
def test_source_contains_iq1748_symbols() -> None:
    src = Path("src/gpu/security_perf_matrix.HC").read_text(encoding="utf-8")
    assert f"I32 {HC_FN_IQ1748}(" in src
    assert f"status_preflight_only = {HC_FN_IQ1747}(" in src
    assert f"status_commit_only = {HC_FN_IQ1746}(" in src
    assert "if (status_preflight_only != status_commit_only)" in src
    assert "// IQ-1748 strict diagnostics parity gate:" in src


def test_gate_missing_vector() -> None:
    saved_out = (42, 52, 62, 72)
    status, out_values = iq1748_model(
...
def test_status_domain_drift_vector() -> None:
    ...
    status, out_values = iq1748_model(
        ...
        dispatch_transcript_parity=1,
        initial_out=saved_out,
        force_status_domain_drift_preflight_only=True,
    )
    assert status == GPU_SEC_PERF_ERR_BAD_PARAM
    assert out_values == saved_out
...
if __name__ == "__main__":
    test_source_contains_iq1748_symbols()
