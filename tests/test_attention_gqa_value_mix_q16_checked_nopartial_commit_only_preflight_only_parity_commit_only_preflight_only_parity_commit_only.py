#!/usr/bin/env python3
"""Reference checks for ...ParityCommitOnlyPreflightOnlyParityCommitOnly (IQ-1394)."""
...
from test_attention_gqa_value_mix_q16_checked_nopartial_commit_only_preflight_only_parity_commit_only_preflight_only import (
    gqa_attention_value_mix_q16_checked_nopartial_commit_only_preflight_only_parity_commit_only_preflight_only,
)
from test_attention_gqa_value_mix_q16_checked_nopartial_commit_only_preflight_only_parity_commit_only_preflight_only_parity import (
    gqa_attention_value_mix_q16_checked_nopartial_commit_only_preflight_only_parity_commit_only_preflight_only_parity,
)


def gqa_attention_value_mix_q16_checked_nopartial_commit_only_preflight_only_parity_commit_only_preflight_only_parity_commit_only(
    scores_q16,
    scores_capacity: int,
    query_rows: int,
    key_rows: int,
    ...
    err = gqa_attention_value_mix_q16_checked_nopartial_commit_only_preflight_only_parity_commit_only_preflight_only_parity(
        scores_q16,
        scores_capacity,
        query_rows,
        key_rows,
...
def test_fixed_vector_reference_atomic_publish() -> None:
    ...
    err = (
        gqa_attention_value_mix_q16_checked_nopartial_commit_only_preflight_only_parity_commit_only_preflight_only_parity_commit_only(
            scores_q16,
            len(scores_q16),
            query_rows,
            key_rows,
...
def test_error_contract_null_alias_capacity_overflow() -> None:
    ...
    assert scores == [1 << 16, 2 << 16, 3 << 16, 4 << 16]
...
def test_randomized_parity_commit_preflight_publish_path() -> None:
    rng = random.Random(20260425_1394)

    for _ in range(220):
        head_groups = rng.randint(1, 4)
        query_rows = head_groups * rng.randint(1, 4)
        ...
        scores = [rng.randint(-(8 << 16), (8 << 16)) for _ in range(required_scores)]
        values = [rng.randint(-(8 << 16), (8 << 16)) for _ in range(required_values)]
        out = [rng.randint(-7000, 7000) for _ in range(max(1, required_out))]
...
if __name__ == "__main__":
    ...
    print(
        "gqa_attention_value_mix_q16_checked_nopartial_commit_only_preflight_only_"
