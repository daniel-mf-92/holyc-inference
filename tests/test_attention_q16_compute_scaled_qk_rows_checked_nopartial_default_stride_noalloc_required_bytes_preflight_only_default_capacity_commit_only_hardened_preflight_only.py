#!/usr/bin/env python3
"""Parity harness for ...CommitOnlyHardenedPreflightOnly."""
...
from test_attention_q16_compute_scaled_qk_rows_checked_nopartial_default_stride_noalloc_required_bytes_preflight_only_default_capacity_commit_only_hardened import (
    attention_q16_compute_scaled_qk_rows_checked_nopartial_default_stride_noalloc_required_bytes_preflight_only_default_capacity_commit_only_hardened,
)


def attention_q16_compute_scaled_qk_rows_checked_nopartial_default_stride_noalloc_required_bytes_preflight_only_default_capacity_commit_only_hardened_preflight_only(
    q_rows_q16,
    q_rows_capacity: int,
    query_row_count: int,
    k_rows_q16,
...
def explicit_checked_composition(*args, **kwargs) -> int:
    return attention_q16_compute_scaled_qk_rows_checked_nopartial_default_stride_noalloc_required_bytes_preflight_only_default_capacity_commit_only_hardened_preflight_only(
        *args, **kwargs
    )


def test_source_contains_hardened_preflight_only_wrapper() -> None:
    source = Path("src/model/attention.HC").read_text(encoding="utf-8")
    signature = "I32 AttentionQ16ComputeScaledQKRowsCheckedNoPartialDefaultStrideNoAllocRequiredBytesPreflightOnlyDefaultCapacityCommitOnlyHardenedPreflightOnly("
    assert signature in source
    body = source.split(signature, 1)[1]

    assert "AttentionQ16ComputeScaledQKRowsCheckedNoPartialDefaultStrideNoAllocRequiredBytesPreflightOnlyDefaultCapacityCommitOnlyHardened(" in body
...
def test_randomized_parity_and_no_partial() -> None:
    rng = random.Random(20260420_728)
    ...
    for _ in range(2800):
        ...
        err_got = attention_q16_compute_scaled_qk_rows_checked_nopartial_default_stride_noalloc_required_bytes_preflight_only_default_capacity_commit_only_hardened_preflight_only(
            q_rows,
            q_capacity,
            query_row_count,
            k_rows,
...
if __name__ == "__main__":
    test_source_contains_hardened_preflight_only_wrapper()
