#!/usr/bin/env python3
"""Harness for IQ-1768 fast-path batch audit commit-only hardening wrapper."""
...
import test_gpu_security_perf_fast_path_switch_batch_audit_q64_checked_commit_only_preflight_only_parity_commit_only_preflight_only_parity_commit_only_preflight_only_parity_commit_only_preflight_only_parity_commit_only_iq1765 as iq1765
...
import test_gpu_security_perf_fast_path_switch_batch_audit_q64_checked_commit_only_preflight_only_parity_commit_only_preflight_only_parity_commit_only_preflight_only_parity_commit_only_preflight_only_parity_commit_only_preflight_only_iq1766 as iq1766

GPU_SEC_PERF_OK = iq1767.GPU_SEC_PERF_OK
GPU_SEC_PERF_ERR_BAD_PARAM = iq1767.GPU_SEC_PERF_ERR_BAD_PARAM
GPU_SEC_PERF_PROFILE_SECURE_LOCAL = iq1767.GPU_SEC_PERF_PROFILE_SECURE_LOCAL

Scenario = iq1767.Scenario

HC_FN_IQ1768 = "GPUSecurityPerfFastPathSwitchBatchAuditQ64CheckedCommitOnlyPreflightOnlyParityCommitOnlyPreflightOnlyParityCommitOnlyPreflightOnlyParityCommitOnlyPreflightOnlyParityCommitOnlyPreflightOnlyParityCommitOnly"
HC_FN_IQ1767 = iq1767.HC_FN_IQ1767
HC_FN_IQ1766 = iq1766.HC_FN_IQ1766
HC_FN_IQ1765 = iq1765.HC_FN_IQ1765
...
def iq1768_model(
    ...
    *,
    ...
    status_seed, seeded_tuple = iq1765.iq1765_model(
        scenarios=scenarios,
    ...
    if not status_is_valid(status_seed):
        return GPU_SEC_PERF_ERR_BAD_PARAM, (0, 0, 0, 0)

    status_primary, staged_tuple = iq1767.iq1767_model(
        scenarios=scenarios,
        secure_local_mode=secure_local_mode,
        iommu_active=iommu_active,
        book_of_truth_gpu_hooks=book_of_truth_gpu_hooks,
    ...
    status_parity, parity_tuple = iq1766.iq1766_model(
        scenarios=scenarios,
        secure_local_mode=secure_local_mode,
        iommu_active=iommu_active,
        book_of_truth_gpu_hooks=book_of_truth_gpu_hooks,
    ...
    if not status_is_valid(status_primary):
        return GPU_SEC_PERF_ERR_BAD_PARAM, (0, 0, 0, 0)
    if not status_is_valid(status_parity):
        return GPU_SEC_PERF_ERR_BAD_PARAM, (0, 0, 0, 0)
...
def test_source_contains_iq1768_symbols() -> None:
    src = Path("src/gpu/security_perf_matrix.HC").read_text(encoding="utf-8")
    assert f"I32 {HC_FN_IQ1768}(" in src
    assert f"status_seed = {HC_FN_IQ1765}(" in src
    assert f"status_primary = {HC_FN_IQ1767}(" in src
    assert f"status_parity = {HC_FN_IQ1766}(" in src
    assert "if (status_primary != status_parity)" in src
    assert "// IQ-1768 commit-only hardening wrapper refresh:" in src


def test_gate_missing_vector() -> None:
    status, out_values = iq1768_model(
...
if __name__ == "__main__":
    test_source_contains_iq1768_symbols()
