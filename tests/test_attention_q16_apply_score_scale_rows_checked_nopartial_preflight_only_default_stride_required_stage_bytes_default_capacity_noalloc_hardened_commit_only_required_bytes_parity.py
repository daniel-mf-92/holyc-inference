#!/usr/bin/env python3
"""Parity harness for ...NoAllocHardenedCommitOnlyRequiredBytesParity (IQ-798)."""
...
from test_attention_q16_apply_score_scale_rows_checked_nopartial_preflight_only_default_stride_required_stage_bytes_default_capacity_noalloc_hardened_commit_only_required_bytes import (
    attention_q16_apply_score_scale_rows_checked_nopartial_preflight_only_default_stride_required_stage_bytes_default_capacity_noalloc_hardened_commit_only_required_bytes,
)
...
def attention_q16_apply_score_scale_rows_checked_nopartial_preflight_only_default_stride_required_stage_bytes_default_capacity_noalloc_hardened_commit_only_required_bytes_parity(
    in_scores_q32,
...
def test_source_contains_commit_only_required_bytes_parity() -> None:
    source = Path("src/model/attention.HC").read_text(encoding="utf-8")
    assert (
        "I32 AttentionQ16ApplyScoreScaleRowsCheckedNoPartialPreflightOnlyDefaultStride"
        "RequiredStageBytesDefaultCapacityNoAllocHardenedCommitOnlyRequiredBytesParity("
    ) in source
    assert (
    ...
    assert "IQ-798 diagnostics-only parity gate:" in source
...
def test_commit_only_required_bytes_parity_rejects_tuple_that_matches_but_violates_geometry() -> None:
    original_commit = (
        attention_q16_apply_score_scale_rows_checked_nopartial_preflight_only_default_stride_required_stage_bytes_default_capacity_noalloc_hardened_commit_only_required_bytes
    )
    original_preflight = (
    ...
    try:
        ...
        err = attention_q16_apply_score_scale_rows_checked_nopartial_preflight_only_default_stride_required_stage_bytes_default_capacity_noalloc_hardened_commit_only_required_bytes_parity(
            [0] * 6,
            6,
            2,
            3,
...
def test_commit_only_required_bytes_parity_nullptr_and_alias_contracts() -> None:
    out_a = [111]
    out_b = [222]
    out_c = [333]
    out_d = [444]
...
def test_commit_only_required_bytes_parity_overflow_propagation() -> None:
    out_required_in_cells = [0]
    out_required_out_cells = [0]
    out_required_stage_cells = [0]
    out_required_stage_bytes = [0]
...
def test_commit_only_required_bytes_parity_matches_explicit_staged_composition_randomized() -> None:
    ...
    for _ in range(1000):
        ...
        err_a = attention_q16_apply_score_scale_rows_checked_nopartial_preflight_only_default_stride_required_stage_bytes_default_capacity_noalloc_hardened_commit_only_required_bytes_parity(
            in_scores,
            in_scores_capacity,
            row_count,
            token_count,
...
if __name__ == "__main__":
    test_source_contains_commit_only_required_bytes_parity()
    test_commit_only_required_bytes_parity_rejects_tuple_that_matches_but_violates_geometry()
    test_commit_only_required_bytes_parity_nullptr_and_alias_contracts()
    test_commit_only_required_bytes_parity_overflow_propagation()
    test_commit_only_required_bytes_parity_matches_explicit_staged_composition_randomized()
    print("ok")
