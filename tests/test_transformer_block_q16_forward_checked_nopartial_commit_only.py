#!/usr/bin/env python3
"""Parity harness for IQ-1224 TransformerBlockQ16ForwardCheckedNoPartialCommitOnly."""
...
def try_mul_i64_checked(lhs: int, rhs: int) -> tuple[int, int]:
    ...
    if lhs > 0:
        if rhs > 0:
            if lhs > I64_MAX // rhs:
                return BLOCK_Q16_ERR_OVERFLOW, 0
        elif rhs < I64_MIN // lhs:
            return BLOCK_Q16_ERR_OVERFLOW, 0
    elif rhs > 0:
        if lhs < I64_MIN // rhs:
            return BLOCK_Q16_ERR_OVERFLOW, 0
    elif lhs != 0 and rhs < I64_MAX // lhs:
        return BLOCK_Q16_ERR_OVERFLOW, 0
    return BLOCK_Q16_OK, lhs * rhs
...
def transformer_block_q16_forward_checked_nopartial_commit_only_model(
    input_q16: list[int],
    ...
    if row_count <= 0 or lane_count <= 0:
        return BLOCK_Q16_OK, list(out_seed)
    ...
    if (
        ...
        or out_capacity < dense
        or len(out_seed) < out_capacity
    ):
        return BLOCK_Q16_ERR_BAD_PARAM, list(out_seed)
    ...
    residual_stage = [0] * dense
    for idx in range(dense):
        err, summed = try_add_i64_checked(input_q16[idx], ffn_out[idx])
        if err != BLOCK_Q16_OK:
            return err, list(out_seed)
        residual_stage[idx] = summed

    # Commit gate preflight: validate each destination row span first.
    for row_idx in range(row_count):
        row_start = row_idx * lane_count
        row_end = row_start + lane_count - 1
        if row_start < 0 or row_end < row_start:
            return BLOCK_Q16_ERR_BAD_PARAM, list(out_seed)
        if row_end >= dense:
            return BLOCK_Q16_ERR_BAD_PARAM, list(out_seed)
        if row_end >= out_capacity:
            return BLOCK_Q16_ERR_BAD_PARAM, list(out_seed)

    out = list(out_seed)
    for idx in range(dense):
        out[idx] = residual_stage[idx]
    return BLOCK_Q16_OK, out


def test_source_contains_iq1224_signature_and_commit_gate() -> None:
    source = Path("src/model/block.HC").read_text(encoding="utf-8")
    signature = "I32 TransformerBlockQ16ForwardCheckedNoPartialCommitOnly("
    ...
    assert "out_row_start" in body
    assert "out_row_end" in body
    assert "stage_row_start" in body
    assert "stage_row_end" in body


def test_deterministic_fixture_commit_only_matches_expected() -> None:
    row_count = 2
    lane_count = 4
    dense = row_count * lane_count
    ...
    out_seed = [777] * (dense + 3)
    status, out = transformer_block_q16_forward_checked_nopartial_commit_only_model(
        input_q16,
        gamma_q16,
        16,
        int(0.75 * Q16_ONE),
    ...
    assert out[:dense] != out_seed[:dense]
    assert out[dense:] == out_seed[dense:]


def test_commit_only_preserves_out_when_capacity_fails() -> None:
    row_count = 2
    lane_count = 4
    dense = row_count * lane_count

    rng = random.Random(1224)
    input_q16 = [rng.randint(-12000, 12000) for _ in range(dense)]
    gamma_q16 = [rng.randint(52000, 76000) for _ in range(lane_count)]
    ffn_gate_q16 = [rng.randint(26000, 70000) for _ in range(dense)]
    ffn_up_q16 = [rng.randint(26000, 70000) for _ in range(dense)]

    out_seed = [rng.randint(-999, 999) for _ in range(dense)]
    snapshot = list(out_seed)
