#!/usr/bin/env python3
"""Parity harness for TokenizerBPEEncodePromptCheckedNoAllocFromLensCommitOnlyPreflightOnly."""
...
from test_tokenizer_bpe_encode_prompt_checked_noalloc_from_lens_preflight_only import (
    tokenizer_bpe_encode_prompt_checked_noalloc_from_lens_preflight_only,
)


def tokenizer_bpe_encode_prompt_checked_noalloc_from_lens_commit_only_preflight_only(
    data: list[int] | None,
    byte_len: int,
...
def explicit_checked_composition(*args):
    return tokenizer_bpe_encode_prompt_checked_noalloc_from_lens_commit_only_preflight_only(*args)


def test_source_contains_commit_only_preflight_only_signature_and_atomic_publish() -> None:
    source = Path("src/tokenizer/bpe.HC").read_text(encoding="utf-8")
    sig = "I32 TokenizerBPEEncodePromptCheckedNoAllocFromLensCommitOnlyPreflightOnly("
...
def test_fuzz_parity() -> None:
    ...
    for _ in range(128):
        ...
        cursor = random.randint(0, nbytes) if random.random() > 0.25 else random.randint(nbytes + 1, nbytes + 3)
        ...
        err_a = tokenizer_bpe_encode_prompt_checked_noalloc_from_lens_commit_only_preflight_only(
            payload,
            nbytes,
            cur_a,
            prompt_nbytes,
