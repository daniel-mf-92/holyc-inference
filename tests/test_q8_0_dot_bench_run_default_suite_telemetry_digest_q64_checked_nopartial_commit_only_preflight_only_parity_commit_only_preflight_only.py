#!/usr/bin/env python3
"""Zero-write preflight checks for IQ-1330 telemetry digest wrapper."""
...
def telemetry_digest_q64_checked_nopartial_commit_only_preflight_only_parity_commit_only(
    total_ops: int,
    total_cycles: int,
    cycles_per_op: int,
    remainder_cycles: int,
...
def telemetry_digest_q64_checked_nopartial_commit_only_preflight_only_parity_commit_only_preflight_only(
    ...
    *,
    ...
    status, commit_digest = telemetry_digest_q64_checked_nopartial_commit_only_preflight_only_parity_commit_only(
        total_ops,
        total_cycles,
        cycles_per_op,
        remainder_cycles,
    ...
    status, parity_digest = telemetry_digest_q64_checked_nopartial_commit_only_preflight_only_parity(
        total_ops,
        total_cycles,
        cycles_per_op,
        remainder_cycles,
    ...
    if out_ptr[0] != out_snapshot:
        return ERR_BAD_PARAM
    if commit_digest != parity_digest:
        return ERR_BAD_PARAM
    return ERR_OK


def test_source_contains_iq1330_function_and_guards() -> None:
    ...
    assert "if (total_ops < 0 || total_cycles < 0 || cycles_per_op < 0 ||" in body
    assert "snapshot_out_digest_value = *out_digest_q64;" in body
    assert "status = Q8_0DotBenchRunDefaultSuiteTelemetryDigestQ64CheckedNoPartialCommitOnlyPreflightOnlyParityCommitOnly(" in body
    assert "status = Q8_0DotBenchRunDefaultSuiteTelemetryDigestQ64CheckedNoPartialCommitOnlyPreflightOnlyParity(" in body
    assert "if (snapshot_out_digest_value != *out_digest_q64)" in body
    assert "if (parity_commit_digest_q64 != parity_digest_q64)" in body
    assert "return Q8_0_DOT_BENCH_OK;" in body


def test_happy_path_zero_write_success() -> None:
    out = [0xCAFEBABE]
...
if __name__ == "__main__":
    test_source_contains_iq1330_function_and_guards()
    test_happy_path_zero_write_success()
