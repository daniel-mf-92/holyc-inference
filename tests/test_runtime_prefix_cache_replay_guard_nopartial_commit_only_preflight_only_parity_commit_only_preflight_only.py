#!/usr/bin/env python3
"""Host-side harness for IQ-1336 PrefixCacheReplayGuardCheckedNoPartialCommitOnlyPreflightOnlyParityCommitOnlyPreflightOnly."""
...
def test_parity_commit_only_preflight_only_secure_local_monotonic_roundtrip():
...
def test_parity_commit_only_preflight_only_dev_local_allows_rollback():
...
def test_parity_commit_only_preflight_only_secure_local_policy_failure_keeps_output_slots_unchanged():
...
def test_parity_commit_only_preflight_only_rejects_bad_vectors():
    cache = _cache_with_tick(3)
    out = [0, 0, 0]

    assert (
        replay_guard_commit_only_preflight_only_parity_commit_only_preflight_only(
            None,
            1,
            2,
...
def test_holyc_function_body_and_contract_markers_present():
    ...
    sig = "I32 PrefixCacheReplayGuardCheckedNoPartialCommitOnlyPreflightOnlyParityCommitOnlyPreflightOnly(PrefixCache *cache,"
    ...
    assert "*out_previous_tick = commit_previous_tick;" in body
    assert "*out_guard_passed = commit_guard_passed;" in body


if __name__ == "__main__":
    test_parity_commit_only_preflight_only_secure_local_monotonic_roundtrip()
    test_parity_commit_only_preflight_only_dev_local_allows_rollback()
    test_parity_commit_only_preflight_only_secure_local_policy_failure_keeps_output_slots_unchanged()
    test_parity_commit_only_preflight_only_rejects_bad_vectors()
