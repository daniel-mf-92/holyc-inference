#!/usr/bin/env python3
"""Parity harness for ...RequiredStageBytesDefaultCapacityNoAllocHardened."""
...
from test_attention_q16_apply_score_scale_rows_checked_nopartial_preflight_only_default_stride_required_stage_bytes_default_capacity import (
    attention_q16_apply_score_scale_rows_checked_nopartial_preflight_only_default_stride_required_stage_bytes_default_capacity,
)

I64_SIZE = 8


def attention_q16_apply_score_scale_rows_checked_nopartial_preflight_only_default_stride_required_stage_bytes_default_capacity_noalloc_hardened(
    in_scores_q32,
...
def test_source_contains_required_stage_bytes_default_capacity_noalloc_hardened_helper() -> None:
    source = Path("src/model/attention.HC").read_text(encoding="utf-8")
    sig = "I32 AttentionQ16ApplyScoreScaleRowsCheckedNoPartialPreflightOnlyDefaultStrideRequiredStageBytesDefaultCapacityNoAllocHardened("
    assert sig in source
    body = source.split(sig, 1)[1]

    assert "AttentionQ16ApplyScoreScaleRowsCheckedNoPartialPreflightOnlyDefaultStrideRequiredStageBytesDefaultCapacity(" in body
...
def test_randomized_parity_and_no_write_on_error() -> None:
    ...
    for _ in range(500):
        ...
        err_a = attention_q16_apply_score_scale_rows_checked_nopartial_preflight_only_default_stride_required_stage_bytes_default_capacity_noalloc_hardened(
            in_scores,
            in_capacity,
            row_count,
            token_count,
...
if __name__ == "__main__":
    test_source_contains_required_stage_bytes_default_capacity_noalloc_hardened_helper()
