#!/usr/bin/env python3
"""Preflight-only harness for InferenceForwardQ16...Parity...CommitOnlyPreflightOnly (IQ-1062)."""
...
def parity_commit_only_preflight_only_parity_commit_only_preflight_only_reference(
    *,
    embed_capacity: int,
    ...
    parity_fn=parity_commit_only_preflight_only_parity_reference,
) -> int:
    if (
...
def test_source_contains_iq_1062_symbol() -> None:
    ...
    assert (
        "status =\n"
        "        InferenceForwardQ16CheckedNoPartialCommitOnlyPreflightOnlyParityCommitOnlyPreflightOnlyParity("
        in body
    )
...
def test_known_vector_success() -> None:
    ...
    status = parity_commit_only_preflight_only_parity_commit_only_preflight_only_reference(
        embed_capacity=96,
...
def test_no_partial_publish_on_canonical_mismatch() -> None:
    ...
    def _bad_parity(**kwargs: int) -> int:
    ...
    assert out_dense == [501]
    assert out_workspace == [502]
    assert out_block == [503]
    assert out_logits == [504]


def test_randomized_geometry_capacity_overflow_vectors() -> None:
    rng = random.Random(20260422_1062)
