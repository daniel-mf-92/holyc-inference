#!/usr/bin/env python3
"""Reference checks for GQAAttentionApplySoftmaxQ16CheckedNoPartialPreflightDefaultStrideParity (IQ-1412)."""
...
from test_attention_gqa_apply_softmax_q16_checked_nopartial_preflight_default_stride import (
    gqa_attention_apply_softmax_q16_checked_nopartial_preflight,
    gqa_attention_apply_softmax_q16_checked_nopartial_preflight_default_stride,
)


def gqa_attention_apply_softmax_q16_checked_nopartial_preflight_default_stride_parity(
    scores_q32,
    scores_capacity: int,
    query_rows: int,
    key_rows: int,
...
def test_randomized_equivalence_with_explicit_composition() -> None:
    rng = random.Random(20260425_1412)
    ...
    for _ in range(240):
        ...
        err_a = gqa_attention_apply_softmax_q16_checked_nopartial_preflight_default_stride_parity(
            scores,
            len(scores),
            query_rows,
            key_rows,
...
def test_source_contract_markers() -> None:
    ...
    assert "status = GQAAttentionApplySoftmaxQ16CheckedNoPartialPreflightDefaultStride(" in parity_body
    assert "status = GQAAttentionApplySoftmaxQ16CheckedNoPartialPreflight(" in parity_body
    assert "key_rows,\n        out_probs_q16," in parity_body
...
if __name__ == "__main__":
    ...
    print("gqa_attention_apply_softmax_q16_checked_nopartial_preflight_default_stride_parity_reference_checks=ok")
