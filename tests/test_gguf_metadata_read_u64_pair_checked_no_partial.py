#!/usr/bin/env python3
"""Reference checks for GGUFMetadataReadU64PairCheckedNoPartial semantics."""
...
def test_null_ptr_and_no_partial_write() -> None:
    ...
    assert (
        gguf_metadata_read_u64_pair_checked_no_partial(
            None,
            64,
            cursor,
            64,
...
def test_second_scalar_fail_does_not_commit_first_or_cursor() -> None:
    ...
    err = gguf_metadata_read_u64_pair_checked_no_partial(
        buf,
        len(buf),
        cursor,
        len(buf),
...
def test_randomized_parity() -> None:
    rng = random.Random(20260417_232)
...
def run() -> None:
    ...
    test_success_reads_two_u64_and_advances()
    test_overflow_passthrough_and_no_commit()
    test_randomized_parity()
    print("gguf_metadata_read_u64_pair_checked_no_partial_reference_checks=ok")
