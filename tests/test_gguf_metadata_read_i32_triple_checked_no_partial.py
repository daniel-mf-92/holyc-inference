#!/usr/bin/env python3
"""Reference checks for GGUFMetadataReadI32TripleCheckedNoPartial semantics."""
...
def gguf_metadata_read_i32_triple_checked_no_partial(
    buf: list[int] | None,
    buf_nbytes: int,
    cursor_ref: list[int] | None,
    table_end: int,
...
def test_null_ptr_and_no_partial_write() -> None:
    ...
    assert (
        gguf_metadata_read_i32_triple_checked_no_partial(
            None,
            16,
            cursor,
            16,
...
def test_third_scalar_fail_does_not_commit_prior_lanes_or_cursor() -> None:
    ...
    err = gguf_metadata_read_i32_triple_checked_no_partial(
        buf,
        len(buf),
        cursor,
        len(buf),
