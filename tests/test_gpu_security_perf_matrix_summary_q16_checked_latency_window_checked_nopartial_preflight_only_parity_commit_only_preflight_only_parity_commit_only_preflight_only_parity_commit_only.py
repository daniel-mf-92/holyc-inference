#!/usr/bin/env python3
"""Harness for IQ-1548 commit-only latency-window wrapper over strict parity + preflight diagnostics."""
...
def _latency_window_nopartial_preflight_only_parity_commit_only_preflight_only_parity(
    rows: list[RowOutput],
    rows_capacity: int,
    out_capacity: int,
    secure_local_mode: int,
    ...
    commit_status, canonical_tuple = _latency_window_nopartial_preflight_only_parity_commit_only(
        rows,
        rows_capacity,
        out_capacity,
        secure_local_mode,
...
def gpu_security_perf_matrix_summary_q16_checked_latency_window_checked_nopartial_preflight_only_parity_commit_only_preflight_only_parity_commit_only_preflight_only_parity_commit_only(
    ...
    parity_status, parity_tuple = _latency_window_nopartial_preflight_only_parity_commit_only_preflight_only_parity(
        rows,
        rows_capacity,
        out_capacity,
        secure_local_mode,
...
def test_source_contains_iq1548_symbols() -> None:
    src = Path("src/gpu/security_perf_matrix.HC").read_text(encoding="utf-8")

    assert "I32 GPUSecurityPerfMatrixSummaryQ16CheckedLatencyWindowCheckedNoPartialPreflightOnlyParityCommitOnlyPreflightOnlyParityCommitOnlyPreflightOnlyParityCommitOnly(" in src
    assert "status_parity = GPUSecurityPerfMatrixSummaryQ16CheckedLatencyWindowCheckedNoPartialPreflightOnlyParityCommitOnlyPreflightOnlyParityCommitOnlyPreflightOnlyParity(" in src
    assert "status_preflight_only = GPUSecurityPerfMatrixSummaryQ16CheckedLatencyWindowCheckedNoPartialPreflightOnlyParityCommitOnlyPreflightOnlyParityCommitOnlyPreflightOnly(" in src
...
def test_commit_only_success_duplicate_latency_publishes_tuple() -> None:
    rows = [
    ...
    status, published, staged = gpu_security_perf_matrix_summary_q16_checked_latency_window_checked_nopartial_preflight_only_parity_commit_only_preflight_only_parity_commit_only_preflight_only_parity_commit_only(
        rows,
        rows_capacity=5,
        out_capacity=4,
...
def test_null_alias_capacity_gate_missing_and_parity_vectors() -> None:
    ...
    assert (status, published, staged) == (GPU_SEC_PERF_ERR_NULL_PTR, (1, 2, 3, 4), (0, 0, 0, 0))
