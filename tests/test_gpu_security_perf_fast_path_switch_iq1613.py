#!/usr/bin/env python3
"""Harness for IQ-1613 commit-only hardening wrapper over parity + preflight-only wrappers."""
...
def _target_parity(
    ...
    status_a, enabled_a, reason_a = _fast_path_switch_checked_audit_parity(
        secure_local_mode,
        iommu_active,
        book_of_truth_gpu_hooks,
        policy_digest_parity,
    ...
    status_b, _, _ = _target_preflight_only(
        secure_local_mode,
        iommu_active,
        book_of_truth_gpu_hooks,
        policy_digest_parity,
        dispatch_transcript_parity,
        current_fast_path_enabled=enabled_a,
        current_disable_reason_code=reason_a,
    )

    if status_a != status_b:
        return GPU_SEC_PERF_ERR_BAD_PARAM, current_fast_path_enabled, current_disable_reason_code
    return status_a, enabled_a, reason_a


def fast_path_switch_checked_audit_parity_iq1613(
    secure_local_mode: int,
    iommu_active: int,
    book_of_truth_gpu_hooks: int,
    policy_digest_parity: int,
    ...
    *,
    ...
    status_preflight_only, staged_enabled, staged_reason = _target_preflight_only(
        secure_local_mode,
        iommu_active,
        book_of_truth_gpu_hooks,
        policy_digest_parity,
...
def test_source_contains_iq1613_symbols() -> None:
    src = Path("src/gpu/security_perf_matrix.HC").read_text(encoding="utf-8")

    assert "I32 GPUSecurityPerfFastPathSwitchCheckedAuditParityCommitOnlyPreflightOnlyParityCommitOnlyPreflightOnlyParityCommitOnlyPreflightOnlyParityCommitOnly(" in src
    assert "status_parity = GPUSecurityPerfFastPathSwitchCheckedAuditParityCommitOnlyPreflightOnlyParityCommitOnlyPreflightOnlyParityCommitOnlyPreflightOnlyParity(" in src
...
def test_gate_missing_and_audit_parity_breach_vectors() -> None:
    status, enabled, reason = fast_path_switch_checked_audit_parity_iq1613(
        secure_local_mode=GPU_SEC_PERF_PROFILE_SECURE_LOCAL,
        iommu_active=0,
        book_of_truth_gpu_hooks=1,
        policy_digest_parity=1,
        dispatch_transcript_parity=1,
        current_fast_path_enabled=42,
        current_disable_reason_code=24,
    )
...
def test_status_domain_drift_and_tuple_parity_vectors() -> None:
    status, enabled, reason = fast_path_switch_checked_audit_parity_iq1613(
        ...
        policy_digest_parity=1,
        dispatch_transcript_parity=1,
        current_fast_path_enabled=13,
        current_disable_reason_code=17,
    ...
    status, enabled, reason = fast_path_switch_checked_audit_parity_iq1613(
        ...
        policy_digest_parity=1,
        dispatch_transcript_parity=1,
        current_fast_path_enabled=5,
        current_disable_reason_code=6,
...
def test_deterministic_tuple_parity_vector() -> None:
    first = fast_path_switch_checked_audit_parity_iq1613(
        secure_local_mode=GPU_SEC_PERF_PROFILE_SECURE_LOCAL,
        iommu_active=1,
        book_of_truth_gpu_hooks=1,
        policy_digest_parity=1,
    ...
    second = fast_path_switch_checked_audit_parity_iq1613(
        secure_local_mode=GPU_SEC_PERF_PROFILE_SECURE_LOCAL,
        iommu_active=1,
        book_of_truth_gpu_hooks=1,
        policy_digest_parity=1,
...
if __name__ == "__main__":
    test_source_contains_iq1613_symbols()
