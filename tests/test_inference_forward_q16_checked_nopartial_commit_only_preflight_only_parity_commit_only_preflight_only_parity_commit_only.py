#!/usr/bin/env python3
"""Commit-only harness for InferenceForwardQ16...ParityCommitOnlyPreflightOnlyParityCommitOnly (IQ-1060)."""
...
def parity_commit_only_preflight_only_parity_commit_only_reference(
    *,
    embed_capacity: int,
    ...
    preflight_only_fn=preflight_only_reference,
) -> int:
    if (
...
def test_source_contains_iq_1060_symbol() -> None:
...
def test_known_vector_success() -> None:
    ...
    status = parity_commit_only_preflight_only_parity_commit_only_reference(
        embed_capacity=96,
...
def test_no_partial_publish_on_parity_failure() -> None:
    out_dense = [201]
    out_workspace = [202]
    out_block = [203]
    out_logits = [204]
    ...
    def _bad_parity(**kwargs: int) -> int:
        # Return canonical-looking tuple but intentionally skew one cell.
...
def test_randomized_geometry_capacity_overflow_vectors() -> None:
    rng = random.Random(20260422_1060)
...
if __name__ == "__main__":
    test_source_contains_iq_1060_symbol()
    test_known_vector_success()
    test_no_partial_publish_on_parity_failure()
