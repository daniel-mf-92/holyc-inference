#!/usr/bin/env python3
"""Harness for IQ-1513 commit-only row-gate wrapper over strict preflight parity diagnostics."""
...
def row_gate_checked_nopartial_preflight_only_commit_only_preflight_only_parity_commit_only(
    secure_local_mode: int,
    iommu_active: int,
    book_of_truth_gpu_hooks: int,
    policy_digest_parity: int,
...
def test_source_contains_iq1513_symbols() -> None:
    src = Path("src/gpu/security_perf_matrix.HC").read_text(encoding="utf-8")

    assert "I32 GPUSecurityPerfMatrixRowGateCheckedNoPartialPreflightOnlyCommitOnlyPreflightOnlyParityCommitOnly(" in src
...
def test_null_alias_capacity_vectors() -> None:
    status, reason, allowed = row_gate_checked_nopartial_preflight_only_commit_only_preflight_only_parity_commit_only(
...
def test_gate_missing_and_reason_code_parity_vectors() -> None:
    ...
    for idx, vector in enumerate(vectors):
        ...
        canonical_status, canonical_reason, canonical_allowed = _row_gate_checked_nopartial_preflight_only_commit_only_preflight_only_parity(
            secure_local_mode,
            iommu_active,
            hooks,
            parity,
