#!/usr/bin/env python3
"""Harness for IQ-1629 strict diagnostics parity gate over IQ-1628 and IQ-1627."""
...
def fast_path_switch_checked_audit_parity_iq1629(
    secure_local_mode: int,
    iommu_active: int,
    book_of_truth_gpu_hooks: int,
    policy_digest_parity: int,
    ...
    *,
    ...
    status_commit_only, canonical_enabled, canonical_reason = _iq1627_commit_only(
        secure_local_mode,
        iommu_active,
        book_of_truth_gpu_hooks,
        policy_digest_parity,
...
def test_source_contains_iq1629_symbols() -> None:
    ...
    assert "// IQ-1629 strict diagnostics parity gate over preflight-only + commit-only:" in src


def test_gate_missing_and_audit_parity_breach_vectors() -> None:
    status, enabled, reason = fast_path_switch_checked_audit_parity_iq1629(
...
def test_null_and_alias_outputs_are_rejected() -> None:
    ...
    assert (status, enabled, reason) == (GPU_SEC_PERF_ERR_NULL_PTR, 51, 52)
