#!/usr/bin/env python3
"""Harness for IQ-1760 fast-path batch audit commit-only hardening wrapper."""
...
HC_FN_IQ1760 = "GPUSecurityPerfFastPathSwitchBatchAuditQ64CheckedCommitOnlyPreflightOnlyParityCommitOnlyPreflightOnlyParityCommitOnlyPreflightOnlyParityCommitOnlyPreflightOnlyParityCommitOnlyPreflightOnlyParityCommitOnlyPreflightOnlyParityCommitOnly"
HC_FN_IQ1759 = iq1759.HC_FN_IQ1759
HC_FN_IQ1758 = iq1759.HC_FN_IQ1758
...
def test_source_contains_iq1760_symbols() -> None:
    src = Path("src/gpu/security_perf_matrix.HC").read_text(encoding="utf-8")
    assert f"I32 {HC_FN_IQ1760}(" in src
    assert f"status_primary = {HC_FN_IQ1759}(" in src
    assert f"status_parity = {HC_FN_IQ1758}(" in src
    assert "if (status_primary != status_parity)" in src
    assert "// IQ-1760 commit-only hardening wrapper:" in src


def test_gate_missing_vector() -> None:
    status, out_values = iq1760_model(
...
if __name__ == "__main__":
    test_source_contains_iq1760_symbols()
