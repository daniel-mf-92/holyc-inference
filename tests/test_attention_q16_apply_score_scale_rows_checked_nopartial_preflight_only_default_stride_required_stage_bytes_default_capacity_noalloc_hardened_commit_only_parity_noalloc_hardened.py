#!/usr/bin/env python3
"""Parity harness for ...DefaultCapacityNoAllocHardenedCommitOnlyParityNoAllocHardened."""
...
from test_attention_q16_apply_score_scale_rows_checked_nopartial_preflight_only_default_stride_required_stage_bytes_default_capacity_noalloc_hardened_commit_only_parity import (
    attention_q16_apply_score_scale_rows_checked_nopartial_preflight_only_default_stride_required_stage_bytes_default_capacity_noalloc_hardened_commit_only_parity,
)


def attention_q16_apply_score_scale_rows_checked_nopartial_preflight_only_default_stride_required_stage_bytes_default_capacity_noalloc_hardened_commit_only_parity_noalloc_hardened(
    in_scores_q32,
...
def test_source_contains_commit_only_parity_noalloc_hardened() -> None:
    source = Path("src/model/attention.HC").read_text(encoding="utf-8")
    sig = "I32 AttentionQ16ApplyScoreScaleRowsCheckedNoPartialPreflightOnlyDefaultStrideRequiredStageBytesDefaultCapacityNoAllocHardenedCommitOnlyParityNoAllocHardened("
    assert sig in source
    body = source.split(sig, 1)[1]

    assert "AttentionQ16ApplyScoreScaleRowsCheckedNoPartialPreflightOnlyDefaultStrideRequiredStageBytesDefaultCapacityNoAllocHardenedCommitOnlyParity(" in body
...
def test_randomized_parity() -> None:
    ...
    for _ in range(200):
        ...
        err_a = attention_q16_apply_score_scale_rows_checked_nopartial_preflight_only_default_stride_required_stage_bytes_default_capacity_noalloc_hardened_commit_only_parity_noalloc_hardened(
            in_scores,
            len(in_scores),
            row_count,
            token_count,
...
if __name__ == "__main__":
    test_source_contains_commit_only_parity_noalloc_hardened()
