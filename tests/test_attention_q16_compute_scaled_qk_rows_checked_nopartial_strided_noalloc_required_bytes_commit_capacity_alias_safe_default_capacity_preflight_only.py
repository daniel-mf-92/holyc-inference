...
def attention_q16_compute_scaled_qk_rows_checked_nopartial_strided_noalloc_required_bytes_commit_capacity_alias_safe_default_capacity_preflight_only(
    ...
    *,
    ...
    out_required_q_cells[0] = canonical_required_q_cells[0]
    out_required_k_cells[0] = canonical_required_k_cells[0]
...
def explicit_checked_composition(
    ...
    *,
    ...
    if required_out_cells[0] != commit_required_out_cells[0]:
        return ATTN_Q16_ERR_BAD_PARAM
    if required_stage_cells[0] != commit_required_stage_cells[0]:
        return ATTN_Q16_ERR_BAD_PARAM
    if required_stage_bytes[0] != commit_required_stage_bytes[0]:
        return ATTN_Q16_ERR_BAD_PARAM
...
def test_source_contains_required_bytes_commit_capacity_alias_safe_default_capacity_preflight_helper() -> None:
    ...
    assert (
        "AttentionQ16ComputeScaledQKRowsCheckedNoPartialStridedNoAllocCommitCapacityAliasSafePreflightOnly(" in body
    )
    assert "AttentionQ16ComputeScaledQKRowsCheckedNoPartialStridedNoAllocPreflightOnly(" in body
    assert "if (commit_required_out_cells != canonical_required_out_cells)" in body
    assert "if (commit_required_stage_cells != canonical_required_stage_cells)" in body
    assert "if (commit_required_stage_bytes != canonical_required_stage_bytes)" in body
    ...
    assert "*out_required_q_cells = canonical_required_q_cells;" in body
    assert "*out_required_k_cells = canonical_required_k_cells;" in body
    assert "*out_required_out_cells = canonical_required_out_cells;" in body
    assert "*out_required_stage_cells = canonical_required_stage_cells;" in body
...
def test_randomized_parity() -> None:
    rng = random.Random(20260420_661)

    for _ in range(3000):
        query_row_count = rng.randint(0, 8)
        token_count = rng.randint(0, 10)
        head_dim = rng.randint(0, 12)
        query_row_stride_q16 = rng.randint(0, 16)
        k_row_stride_q16 = rng.randint(0, 16)
        out_row_stride = rng.randint(0, 16)
        ...
        if query_row_count > 0 and token_count > 0 and out_row_stride < token_count:
            out_row_stride = token_count + rng.randint(0, 5)
        ...
        q_capacity = max(0, q_need + rng.randint(-3, 4))
        k_capacity = max(0, k_need + rng.randint(-3, 4))
        out_capacity = max(0, out_need + rng.randint(-3, 4))
        stage_capacity = max(0, stage_need + rng.randint(-3, 4))
        ...
        if rng.randint(0, 9) < 4:
            alias_base = rng.choice([0x100000, 0x200000, 0x300000]) + rng.randint(0, 56)
        else:
            alias_base = 0x400000 + rng.randint(0, 512)
