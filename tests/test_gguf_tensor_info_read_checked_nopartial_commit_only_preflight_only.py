...
def test_source_contains_iq1010_signature_and_parity_contract() -> None:
...
def test_known_vector_and_alias_rejection() -> None:
    payload = tensor_entry("blk.0.ffn_up.weight", [64, 32, 16], 8, 4096)

    out_name_len = [0x11]
    out_dim_count = [0x12]
    out_dims_cells = [0x13]
    out_required_bytes = [0x14]
    out_type_value = [0x15]
    out_tensor_offset = [0x16]
    out_next_cursor = [0x17]
    ...
    assert out_name_len == [len("blk.0.ffn_up.weight")]
    assert out_dim_count == [3]
    assert out_dims_cells == [3]
    assert out_required_bytes == [8 + len("blk.0.ffn_up.weight") + 4 + 3 * 8 + 4 + 8]
...
def test_adversarial_name_dim_type_and_span_vectors() -> None:
    ...
    out_dim_count = [302]
    out_dims_cells = [303]
    out_required_bytes = [304]
    out_type_value = [305]
    out_tensor_offset = [306]
    out_next_cursor = [307]
    ...
    trunc_offset = tensor_entry("trunc", [2, 2, 2], 8, 0)[:-7]
    err = parse_one_checked_nopartial_commit_only_preflight_only(
        trunc_offset,
        len(trunc_offset),
        0,
...
def test_randomized_parity_against_explicit_composition() -> None:
    ...
    for _ in range(2200):
        ...
        err_a = parse_one_checked_nopartial_commit_only_preflight_only(
            payload,
            len(payload),
            cursor,
            a_name_len,
        ...
        err_b = parse_one_checked_nopartial_commit_only(
            payload,
            len(payload),
            cursor,
            b_name_len,
...
if __name__ == "__main__":
    test_source_contains_iq1010_signature_and_parity_contract()
