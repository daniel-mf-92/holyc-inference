#!/usr/bin/env python3
"""Harness for IQ-1610 commit-only hardening wrapper over IQ-1609 and IQ-1608."""
...
def fast_path_switch_checked_audit_parity_iq1610(
    secure_local_mode: int,
    iommu_active: int,
    book_of_truth_gpu_hooks: int,
    policy_digest_parity: int,
...
def test_source_contains_iq1610_symbols() -> None:
    src = Path("src/gpu/security_perf_matrix.HC").read_text(encoding="utf-8")

    assert "I32 GPUSecurityPerfFastPathSwitchCheckedAuditParityCommitOnlyPreflightOnlyParityCommitOnlyPreflightOnlyParityCommitOnlyPreflightOnlyParityCommitOnlyPreflightOnlyParityCommitOnlyPreflightOnlyParityCommitOnly(" in src
...
def test_gate_missing_and_audit_parity_breach_vectors() -> None:
    status, enabled, reason = fast_path_switch_checked_audit_parity_iq1610(
...
def test_status_domain_drift_no_write_parity_and_deterministic_vectors() -> None:
    ...
    # Commit-only parity vector: secure-pass tuple currently fails closed in the chained guards,
    # so IQ-1610 must preserve caller slots exactly on non-OK status.
    status, enabled, reason = fast_path_switch_checked_audit_parity_iq1610(
        secure_local_mode=GPU_SEC_PERF_PROFILE_SECURE_LOCAL,
        iommu_active=1,
        book_of_truth_gpu_hooks=1,
