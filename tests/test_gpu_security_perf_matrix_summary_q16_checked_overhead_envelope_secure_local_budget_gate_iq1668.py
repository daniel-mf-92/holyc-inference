#!/usr/bin/env python3
"""Harness for IQ-1668 zero-write diagnostics companion over IQ-1667 + IQ-1666 canonical parity."""
...
def secure_local_budget_gate_iq1668(
    rows: list[RowOutput],
    rows_capacity: int,
    secure_local_mode: int,
    iommu_active: int,
    ...
    *,
    ...
    status_commit_only, staged_tuple = secure_local_budget_gate_iq1667(
        rows,
        rows_capacity,
        secure_local_mode,
        iommu_active,
    ...
    status_canonical, canonical_tuple = secure_local_budget_gate_iq1666(
        rows,
        rows_capacity,
        secure_local_mode,
        iommu_active,
...
def test_source_contains_iq1668_symbols() -> None:
    src = Path("src/gpu/security_perf_matrix.HC").read_text(encoding="utf-8")
    assert "I32 GPUSecurityPerfMatrixRowOutputsSnapshotDigestQ64(" in src
    assert "I32 (" in src
    ...
    assert "status_canonical = (" in src
    assert "status_snapshot_digest = GPUSecurityPerfMatrixRowOutputsSnapshotDigestQ64(" in src
    assert "// IQ-1668 zero-write diagnostics companion over IQ-1667 commit-only hardening + IQ-1666 canonical parity gate:" in src
...
def test_gate_missing_and_threshold_breach_vectors() -> None:
    ...
    status, outputs = secure_local_budget_gate_iq1668(
        rows_gate_missing,
        rows_capacity=3,
...
if __name__ == "__main__":
    test_source_contains_iq1668_symbols()
