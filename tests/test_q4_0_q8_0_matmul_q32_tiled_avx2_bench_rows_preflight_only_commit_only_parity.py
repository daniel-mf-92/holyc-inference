#!/usr/bin/env python3
"""Parity harness for Q4_0Q8_0MatMulQ32TiledAVX2BenchRowsPreflightOnlyCommitOnlyParity (IQ-828)."""
...
from test_q4_0_q8_0_matmul_q32_tiled_avx2_bench_rows_preflight_only_commit_only import (
    q4_0_q8_0_matmul_q32_tiled_avx2_bench_rows_preflight_only_commit_only,
)


def q4_0_q8_0_matmul_q32_tiled_avx2_bench_rows_preflight_only_commit_only_parity(
    lhs_q4_blocks,
    ...
    canonical_cells_per_iter = [out_cells_per_iter[0]]
    canonical_block_dots_per_iter = [out_block_dots_per_iter[0]]
    canonical_total_cells = [out_total_cells[0]]
    canonical_total_block_dots = [out_total_block_dots[0]]
    ...
    if commit_cells_per_iter[0] != canonical_cells_per_iter[0]:
        return Q4_0_Q8_0_AVX2_ERR_BAD_LEN
    if commit_block_dots_per_iter[0] != canonical_block_dots_per_iter[0]:
        return Q4_0_Q8_0_AVX2_ERR_BAD_LEN
    if commit_total_cells[0] != canonical_total_cells[0]:
        return Q4_0_Q8_0_AVX2_ERR_BAD_LEN
    if commit_total_block_dots[0] != canonical_total_block_dots[0]:
        return Q4_0_Q8_0_AVX2_ERR_BAD_LEN
...
def explicit_checked_composition(*args):
    return q4_0_q8_0_matmul_q32_tiled_avx2_bench_rows_preflight_only_commit_only_parity(*args)


def test_source_contains_preflight_only_commit_only_parity_signature_and_parity_checks() -> None:
    source = Path("src/matmul/q4_0_q8_0_matmul.HC").read_text(encoding="utf-8")
    sig = "I32 Q4_0Q8_0MatMulQ32TiledAVX2BenchRowsPreflightOnlyCommitOnlyParity("
    assert sig in source
    body = source.split(sig, 1)[1]
    assert "Q4_0Q8_0MatMulQ32TiledAVX2BenchRowsPreflightOnlyCommitOnly(" in body
...
def test_known_vector_and_no_publish_on_failure() -> None:
    ...
    rng = random.Random(20260421_8281)
...
def test_fuzz_parity_against_explicit_checked_composition() -> None:
    ...
    for i in range(2400):
        ...
        rng = random.Random(20260421_928000 + i)
        ...
        err_a = q4_0_q8_0_matmul_q32_tiled_avx2_bench_rows_preflight_only_commit_only_parity(
            lhs,
            lhs_capacity,
            row_count,
            lhs_stride,
...
if __name__ == "__main__":
    test_source_contains_preflight_only_commit_only_parity_signature_and_parity_checks()
