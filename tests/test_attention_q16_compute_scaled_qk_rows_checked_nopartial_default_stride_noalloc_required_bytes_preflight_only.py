...
def attention_q16_compute_scaled_qk_rows_checked_nopartial_default_stride_noalloc_required_bytes_preflight_only(
    ...
    tmp_required_k = [0]
    tmp_required_out = [0]
    tmp_required_stage_cells = [0]
    tmp_required_stage_bytes = [0]
    ...
    err = attention_q16_compute_scaled_qk_rows_checked_nopartial_strided_noalloc_preflight_only(
        ...
        tmp_required_k,
        tmp_required_out,
        tmp_required_stage_cells,
        tmp_required_stage_bytes,
    )
    if err != ATTN_Q16_OK:
        return err

    out_required_q_cells[0] = tmp_required_q[0]
    out_required_k_cells[0] = tmp_required_k[0]
    out_required_out_cells[0] = tmp_required_out[0]
    out_required_stage_cells[0] = tmp_required_stage_cells[0]
    out_required_stage_bytes[0] = tmp_required_stage_bytes[0]
    return ATTN_Q16_OK
...
def explicit_checked_composition(
    ...
    return attention_q16_compute_scaled_qk_rows_checked_nopartial_strided_noalloc_preflight_only(
        q_rows_q16,
        q_rows_capacity,
        query_row_count,
        default_query_row_stride_q16,
...
def test_source_contains_default_stride_required_bytes_preflight_only_wrapper() -> None:
    source = Path("src/model/attention.HC").read_text(encoding="utf-8")
    signature = "I32 AttentionQ16ComputeScaledQKRowsCheckedNoPartialDefaultStrideNoAllocRequiredBytesPreflightOnly("
...
def test_known_vector_success_and_null_contract() -> None:
    q_rows = [0] * 64
    k_rows = [0] * 96
    out_scores = [0] * 32
    ...
    assert got_q[0] == ((3 - 1) * 8) + 8
    assert got_k[0] == ((4 - 1) * 8) + 8
    assert got_out[0] == ((3 - 1) * 4) + 4
    assert got_stage_cells[0] == 3 * 4
    assert got_stage_bytes[0] == (3 * 4) * 8
...
def test_randomized_parity_vs_explicit_composition() -> None:
    ...
    for _ in range(500):
        ...
        q_capacity = rng.randint(0, 2048)
        k_capacity = rng.randint(0, 2048)
        out_capacity = rng.randint(0, 2048)
        ...
        err_got = attention_q16_compute_scaled_qk_rows_checked_nopartial_default_stride_noalloc_required_bytes_preflight_only(
            q_rows,
            q_capacity,
            query_row_count,
            k_rows,
