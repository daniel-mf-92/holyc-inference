#!/usr/bin/env python3
"""Parity harness for ...DefaultCapacityValidateCursorNoAllocFromMaxPieceNoPartial."""
...
def tokenizer_bpe_decode_prompt_checked_default_capacity_validate_cursor_noalloc_from_max_piece_nopartial(
...
def test_source_contains_wrapper_and_delegation() -> None:
    ...
    assert "TokenizerBPEDecodePromptCheckedDefaultCapacityValidateCursorNoAllocFromMaxPieceNoPartial" in source
    body = source.split(
...
def test_no_partial_on_adversarial_failures() -> None:
    ...
    cursor = [8]
    err = tokenizer_bpe_decode_prompt_checked_default_capacity_validate_cursor_noalloc_from_max_piece_nopartial(
        [0, 1, 2],
        3,
        cursor,
        blob,
...
def test_null_and_overflow_domains() -> None:
    ...
    assert (
        tokenizer_bpe_decode_prompt_checked_default_capacity_validate_cursor_noalloc_from_max_piece_nopartial(
            None,
            0,
            cursor,
            blob,
...
def run() -> None:
    ...
    print("tokenizer_bpe_decode_prompt_checked_default_capacity_validate_cursor_noalloc_from_max_piece_nopartial=ok")
