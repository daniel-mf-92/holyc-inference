#!/usr/bin/env python3
"""Harness for IQ-1616 zero-write diagnostics companion over IQ-1610 and IQ-1609."""
...
def fast_path_switch_checked_audit_parity_iq1616(
    secure_local_mode: int,
    iommu_active: int,
    book_of_truth_gpu_hooks: int,
    policy_digest_parity: int,
...
def test_source_contains_iq1616_symbols() -> None:
    src = Path("src/gpu/security_perf_matrix.HC").read_text(encoding="utf-8")

    assert "I32 GPUSecurityPerfFastPathSwitchCheckedAuditParityCommitOnlyPreflightOnlyParityCommitOnlyPreflightOnlyParityCommitOnlyPreflightOnlyParityCommitOnlyPreflightOnlyParityCommitOnlyPreflightOnlyParityCommitOnlyPreflightOnly(" in src
    assert "status_commit_only = GPUSecurityPerfFastPathSwitchCheckedAuditParityCommitOnlyPreflightOnlyParityCommitOnlyPreflightOnlyParityCommitOnlyPreflightOnlyParityCommitOnlyPreflightOnlyParityCommitOnlyPreflightOnlyParityCommitOnly(" in src
    assert "status_parity = GPUSecurityPerfFastPathSwitchCheckedAuditParityCommitOnlyPreflightOnlyParityCommitOnlyPreflightOnlyParityCommitOnlyPreflightOnlyParityCommitOnlyPreflightOnlyParityCommitOnlyPreflightOnlyParity(" in src
    assert "if (*out_fast_path_enabled != saved_fast_path_enabled" in src


def test_gate_missing_and_audit_parity_breach_vectors() -> None:
    status, enabled, reason = fast_path_switch_checked_audit_parity_iq1616(
    ...
    assert (status, enabled, reason) == (GPU_SEC_PERF_ERR_POLICY_GUARD, 101, 102)
    ...
    status, enabled, reason = fast_path_switch_checked_audit_parity_iq1616(
        ...
        current_fast_path_enabled=103,
        current_disable_reason_code=104,
    )
    assert (status, enabled, reason) == (GPU_SEC_PERF_ERR_POLICY_GUARD, 103, 104)
...
def test_status_domain_drift_and_no_write_parity_vectors() -> None:
    ...
    status, enabled, reason = fast_path_switch_checked_audit_parity_iq1616(
        ...
        policy_digest_parity=1,
        dispatch_transcript_parity=1,
        current_fast_path_enabled=113,
        current_disable_reason_code=114,
        force_tuple_drift=True,
    )
    assert (status, enabled, reason) == (GPU_SEC_PERF_ERR_BAD_PARAM, 113, 114)


def test_secure_on_overhead_budget_vector_and_determinism() -> None:
    first = fast_path_switch_checked_audit_parity_iq1616(
        secure_local_mode=GPU_SEC_PERF_PROFILE_SECURE_LOCAL,
        iommu_active=1,
        book_of_truth_gpu_hooks=1,
        policy_digest_parity=1,
    ...
    second = fast_path_switch_checked_audit_parity_iq1616(
        secure_local_mode=GPU_SEC_PERF_PROFILE_SECURE_LOCAL,
        iommu_active=1,
        book_of_truth_gpu_hooks=1,
        policy_digest_parity=1,
    ...
    assert first == second
    assert first == (GPU_SEC_PERF_OK, 121, 122)


if __name__ == "__main__":
    test_source_contains_iq1616_symbols()
