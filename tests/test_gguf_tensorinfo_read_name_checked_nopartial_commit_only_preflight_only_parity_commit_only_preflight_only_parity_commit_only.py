#!/usr/bin/env python3
"""Harness for IQ-1031 tensor-name parity preflight commit-only wrapper."""
...
def explicit_checked_composition(
    ...
    err = parse_name_checked_nopartial_commit_only_preflight_only_parity_commit_only_preflight_only_parity(
        buf,
        size,
        cursor,
        parity_name_len,
    ...
    err = parse_name_checked_nopartial_commit_only_preflight_only_parity_commit_only_preflight_only(
        buf,
        size,
        cursor,
        canonical_name_len,
    ...
    parity_required_check = try_add_u64(8, parity_name_len[0])
    if parity_required_check is None:
        return GGUF_TENSOR_PARSE_ERR_TRUNCATED
    canonical_required_check = try_add_u64(8, canonical_name_len[0])
    if canonical_required_check is None:
        return GGUF_TENSOR_PARSE_ERR_TRUNCATED

    parity_computed_end = try_add_u64(cursor, parity_required_bytes[0])
    if parity_computed_end is None:
        return GGUF_TENSOR_PARSE_ERR_TRUNCATED
    canonical_computed_end = try_add_u64(cursor, canonical_required_bytes[0])
    if canonical_computed_end is None:
        return GGUF_TENSOR_PARSE_ERR_TRUNCATED

    span_remaining = size - cursor
    if parity_required_bytes[0] > span_remaining or canonical_required_bytes[0] > span_remaining:
        return GGUF_TENSOR_PARSE_ERR_TRUNCATED

    if parity_computed_end > size or canonical_computed_end > size:
        return GGUF_TENSOR_PARSE_ERR_TRUNCATED
    if parity_computed_end != parity_next_cursor[0] or canonical_computed_end != canonical_next_cursor[0]:
        return GGUF_TENSOR_PARSE_ERR_TRUNCATED

    if (
        parity_name_len[0] != canonical_name_len[0]
        or parity_name_len[0] > GGUF_TENSOR_MAX_NAME_BYTES
        or canonical_name_len[0] > GGUF_TENSOR_MAX_NAME_BYTES
        or parity_required_bytes[0] != canonical_required_bytes[0]
        or parity_next_cursor[0] != canonical_next_cursor[0]
        or parity_required_bytes[0] != parity_required_check
        or canonical_required_bytes[0] != canonical_required_check
    ):
        return GGUF_TENSOR_PARSE_ERR_TRUNCATED

    out_name_len[0] = parity_name_len[0]
    out_required_bytes[0] = parity_required_bytes[0]
    out_next_cursor[0] = parity_next_cursor[0]
    return GGUF_TENSOR_PARSE_OK


def test_source_contains_iq1031_signature_and_contract() -> None:
    ...
    assert "IQ-1031 commit-only diagnostics wrapper for tensor-name parity-preflight gate." in source
    ...
    assert "staged_parity_span_remaining = size - cursor;" in body
    assert "canonical_span_remaining = size - cursor;" in body
    assert "staged_parity_required_bytes > staged_parity_span_remaining" in body
    assert "canonical_required_bytes > canonical_span_remaining" in body
    ...
    assert "staged_parity_name_len > GGUF_TENSOR_MAX_NAME_BYTES" in body
    assert "canonical_name_len > GGUF_TENSOR_MAX_NAME_BYTES" in body
...
def test_randomized_parity_vs_explicit_composition() -> None:
    rng = random.Random(20260422_1031)
...
if __name__ == "__main__":
    test_source_contains_iq1031_signature_and_contract()
