#!/usr/bin/env python3
"""Harness for IQ-1523 overhead-envelope commit-only parity commit-only wrapper."""
...
def gpu_security_perf_matrix_summary_q16_checked_overhead_envelope_checked_nopartial_commit_only_preflight_only_parity_commit_only(
...
def test_source_contains_iq1523_symbols() -> None:
    src = Path("src/gpu/security_perf_matrix.HC").read_text(encoding="utf-8")

    assert "I32 GPUSecurityPerfMatrixSummaryQ16CheckedOverheadEnvelopeCheckedNoPartialCommitOnlyPreflightOnlyParityCommitOnly(" in src
    ...
    assert "status_preflight_only = GPUSecurityPerfMatrixSummaryQ16CheckedOverheadEnvelopeCheckedNoPartialCommitOnlyPreflightOnly(" in src
...
def test_commit_only_success_publishes_tuple() -> None:
    ...
    status, persisted, staged = gpu_security_perf_matrix_summary_q16_checked_overhead_envelope_checked_nopartial_commit_only_preflight_only_parity_commit_only(
        rows,
        rows_capacity=3,
        out_capacity=3,
...
def test_null_alias_capacity_gate_monotonicity_and_parity_vectors() -> None:
    ...
    status, *_ = gpu_security_perf_matrix_summary_q16_checked_overhead_envelope_checked_nopartial_commit_only_preflight_only_parity_commit_only(
        rows,
        rows_capacity=1,
        out_capacity=3,
        secure_local_mode=GPU_SEC_PERF_PROFILE_SECURE_LOCAL,
