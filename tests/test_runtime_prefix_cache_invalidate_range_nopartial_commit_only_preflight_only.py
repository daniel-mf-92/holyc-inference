#!/usr/bin/env python3
"""Host-side harness for IQ-1323 PrefixCacheInvalidateRangeCheckedNoPartialCommitOnlyPreflightOnly."""
...
def invalidate_range_commit_only(
    ...
    status, canonical_removed = invalidate_range_checked(
        cache, invalidate_start_token, invalidate_token_count
    )
...
def test_iq1323_preflight_only_matches_commit_and_canonical_tuples():
    ...
    cache = Cache(entries=entries, capacity=5, count=4)

    status, removed = invalidate_range_commit_only_preflight_only(cache, 12, 30)
...
def test_iq1323_preflight_only_zero_length_invalidate_is_noop():
    ...
    status, removed = invalidate_range_commit_only_preflight_only(cache, 12, 0)
    assert status == PREFIX_CACHE_OK
    assert removed == 0
...
def test_iq1323_preflight_only_preflight_guard_keeps_state_unchanged_on_error():
    ...
    status, removed = invalidate_range_commit_only_preflight_only(cache, 0, 10)
...
def test_iq1323_source_contains_preflight_only_signature_and_parity_guards():
    ...
    assert "status_preflight = PrefixCacheInvalidateRangeCheckedNoPartialCommitOnly(cache," in source
    assert "status_canonical = PrefixCacheInvalidateRangeChecked(cache," in source
    assert "preflight_removed_count + preflight_remaining_count != snapshot_entry_count" in source
    assert "preflight_removed_count != canonical_removed_count" in source


def main():
    test_iq1323_preflight_only_matches_commit_and_canonical_tuples()
    test_iq1323_preflight_only_zero_length_invalidate_is_noop()
    test_iq1323_preflight_only_preflight_guard_keeps_state_unchanged_on_error()
    test_iq1323_source_contains_preflight_only_signature_and_parity_guards()
    print("ok")
