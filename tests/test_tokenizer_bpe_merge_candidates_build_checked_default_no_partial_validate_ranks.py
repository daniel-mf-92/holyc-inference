#!/usr/bin/env python3
"""Parity harness for TokenizerBPEMergeCandidatesBuildCheckedDefaultNoPartialValidateRanks."""
...
from test_tokenizer_bpe_merge_candidates_build_checked_default_no_partial import (
    I64_MAX,
    TOKENIZER_BPE_ERR_NULL_PTR,
    TOKENIZER_BPE_ERR_OVERFLOW,
    TOKENIZER_BPE_OK,
    tokenizer_bpe_merge_candidates_build_checked_default_no_partial,
)
from test_tokenizer_bpe_merge_candidates_build_checked import TOKENIZER_BPE_ERR_BAD_PARAM


def tokenizer_bpe_merge_candidates_build_checked_default_no_partial_validate_ranks(
    ...
    return tokenizer_bpe_merge_candidates_build_checked_default_no_partial(
        token_ids,
        token_count,
        rank_left_tokens,
        rank_right_tokens,
...
def _run_case(
    ...
    err_a = tokenizer_bpe_merge_candidates_build_checked_default_no_partial_validate_ranks(
        token_ids,
        token_count,
        rank_left,
        rank_right,
    ...
    err_b = tokenizer_bpe_merge_candidates_build_checked_default_no_partial(
        token_ids,
        token_count,
        rank_left,
        rank_right,
    ...
    assert out_count_a[0] == out_count_b[0]
...
def test_unsorted_rank_table_rejected_without_partial_commit() -> None:
    ...
    out_left = [111, 112, 113, 114, 115]
    out_right = [211, 212, 213, 214, 215]
    out_indices = [311, 312, 313, 314, 315]
    out_ranks = [411, 412, 413, 414, 415]
    out_count = [511]

    err = tokenizer_bpe_merge_candidates_build_checked_default_no_partial_validate_ranks(
        tokens,
        len(tokens),
        rank_left,
        rank_right,
    ...
    assert out_left == [111, 112, 113, 114, 115]
    assert out_right == [211, 212, 213, 214, 215]
    assert out_indices == [311, 312, 313, 314, 315]
    assert out_ranks == [411, 412, 413, 414, 415]
    assert out_count[0] == 511


def test_duplicate_key_ties_and_min_rank_parity_on_sorted_tables() -> None:
    rows = sorted(
        [
            (10, 20, 7),
            (10, 20, 2),
...
def test_randomized_sorted_parity_vs_default_no_partial() -> None:
    rng = random.Random(20260418_408)
...
if __name__ == "__main__":
    ...
    test_duplicate_key_ties_and_min_rank_parity_on_sorted_tables()
    test_null_and_overflow_contracts()
    test_randomized_sorted_parity_vs_default_no_partial()
    print(
        "tokenizer_bpe_merge_candidates_build_checked_default_no_partial_validate_ranks_reference_checks=ok"
    )
