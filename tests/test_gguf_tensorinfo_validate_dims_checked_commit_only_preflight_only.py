#!/usr/bin/env python3
"""Parity harness for GGUFTensorInfoValidateDimsCheckedCommitOnlyPreflightOnly (IQ-1196)."""
...
def validate_dims_checked_commit_only_preflight_only(
    n_dims: int,
    dims: list[int] | None,
    ggml_type: int,
    out_n_elements: list[int] | None,
    ...
    canonical_n = [0]
    status = validate_dims_checked(snapshot_n_dims, snapshot_dims, snapshot_ggml_type, canonical_n)
    if status != GGUF_TENSOR_PARSE_OK:
        return status

    canonical_b = [0]
    status = tensor_type_block_elems_checked(snapshot_ggml_type, canonical_b)
    if status != GGUF_TENSOR_PARSE_OK:
        return status
    if canonical_b[0] == 0:
        return GGUF_TENSOR_PARSE_ERR_BAD_TYPE
    if canonical_n[0] % canonical_b[0] != 0:
        return GGUF_TENSOR_PARSE_ERR_BAD_DIM_SIZE

    if snapshot_n_dims != n_dims or snapshot_dims is not dims or snapshot_ggml_type != ggml_type:
        return GGUF_TENSOR_PARSE_ERR_BAD_DIMS
    if staged_n[0] != canonical_n[0] or staged_b[0] != canonical_b[0]:
        return GGUF_TENSOR_PARSE_ERR_BAD_DIMS
...
def test_source_contains_iq1196_validate_dims_commit_only_preflight_only() -> None:
    source = Path("src/gguf/tensorinfo.HC").read_text(encoding="utf-8")
    sig = "I64 GGUFTensorInfoValidateDimsCheckedCommitOnlyPreflightOnly(U32 n_dims,"
    assert sig in source
    body = source.split(sig, 1)[1].split(
        "I64 GGUFTensorInfoValidateDimsCheckedCommitOnlyPreflightOnlyParity(U32 n_dims,",
        1,
    )[0]
    ...
    assert "if (staged_n_elements != canonical_n_elements ||" in body
    assert "*out_n_elements = staged_n_elements;" in body
    assert "*out_block_elems = staged_block_elems;" in body


def test_commit_only_preflight_only_error_contracts_and_no_partial_publish() -> None:
    n_out = [111]
    b_out = [222]

    status = validate_dims_checked_commit_only_preflight_only(2, None, 2, n_out, b_out)
    assert status == GGUF_TENSOR_PARSE_ERR_NULL_PTR
    assert n_out == [111]
    assert b_out == [222]

    alias_out = [333]
    status = validate_dims_checked_commit_only_preflight_only(2, [32, 1], 2, alias_out, alias_out)
    assert status == GGUF_TENSOR_PARSE_ERR_NULL_PTR
    assert alias_out == [333]

    status = validate_dims_checked_commit_only_preflight_only(2, [0, 32], 2, n_out, b_out)
    assert status == GGUF_TENSOR_PARSE_ERR_BAD_DIM_SIZE
    assert n_out == [111]
    assert b_out == [222]

    status = validate_dims_checked_commit_only_preflight_only(2, [1 << 63, 3], 2, n_out, b_out)
    assert status == GGUF_TENSOR_PARSE_ERR_DIM_OVERFLOW
    assert n_out == [111]
    assert b_out == [222]

    status = validate_dims_checked_commit_only_preflight_only(2, [32, 1], 999, n_out, b_out)
    assert status == GGUF_TENSOR_PARSE_ERR_BAD_TYPE
    assert n_out == [111]
    assert b_out == [222]


def test_commit_only_preflight_only_success_and_randomized_vectors() -> None:
    status = validate_dims_checked_commit_only_preflight_only(2, [32, 4], 2, [0], [0])
    ...
    for _ in range(500):
        ...
        out_n = [987654321]
        out_b = [123456789]
        status = validate_dims_checked_commit_only_preflight_only(
            n_dims,
            dims,
            ggml_type,
            out_n,
...
if __name__ == "__main__":
    test_source_contains_iq1196_validate_dims_commit_only_preflight_only()
    test_commit_only_preflight_only_error_contracts_and_no_partial_publish()
    test_commit_only_preflight_only_success_and_randomized_vectors()
    print("ok")
