#!/usr/bin/env python3
"""Harness for IQ-1757 fast-path batch audit commit-only hardening wrapper."""
...
import test_gpu_security_perf_fast_path_switch_batch_audit_q64_checked_commit_only_preflight_only_parity_commit_only_preflight_only_parity_commit_only_preflight_only_parity_commit_only_preflight_only_parity_commit_only_preflight_only_parity_iq1756 as iq1756

GPU_SEC_PERF_OK = iq1756.GPU_SEC_PERF_OK
GPU_SEC_PERF_ERR_BAD_PARAM = iq1756.GPU_SEC_PERF_ERR_BAD_PARAM
GPU_SEC_PERF_PROFILE_SECURE_LOCAL = iq1756.GPU_SEC_PERF_PROFILE_SECURE_LOCAL

Scenario = iq1756.Scenario

HC_FN_IQ1757 = "GPUSecurityPerfFastPathSwitchBatchAuditQ64CheckedCommitOnlyPreflightOnlyParityCommitOnlyPreflightOnlyParityCommitOnlyPreflightOnlyParityCommitOnlyPreflightOnlyParityCommitOnlyPreflightOnlyParityCommitOnly"
HC_FN_IQ1756 = iq1756.HC_FN_IQ1756
HC_FN_IQ1755 = iq1756.HC_FN_IQ1755
...
def iq1757_model(
    ...
    *,
    ...
    status_seed, seeded_tuple = iq1756.iq1755.iq1754.iq1754_model(
        scenarios=scenarios,
    ...
    if not iq1756.iq1755.iq1754.iq1753.iq1752.iq1751.iq1750.iq1749.iq1748.iq1747.iq1746._status_is_valid(status_seed):
        return GPU_SEC_PERF_ERR_BAD_PARAM, (0, 0, 0, 0)

    status_primary, staged_tuple = iq1756.iq1756_model(
        scenarios=scenarios,
        secure_local_mode=secure_local_mode,
        iommu_active=iommu_active,
        book_of_truth_gpu_hooks=book_of_truth_gpu_hooks,
    ...
    status_parity, parity_tuple = iq1756.iq1755.iq1755_model(
        scenarios=scenarios,
        secure_local_mode=secure_local_mode,
        iommu_active=iommu_active,
        book_of_truth_gpu_hooks=book_of_truth_gpu_hooks,
    ...
    if not iq1756.iq1755.iq1754.iq1753.iq1752.iq1751.iq1750.iq1749.iq1748.iq1747.iq1746._status_is_valid(status_primary):
        return GPU_SEC_PERF_ERR_BAD_PARAM, (0, 0, 0, 0)
    if not iq1756.iq1755.iq1754.iq1753.iq1752.iq1751.iq1750.iq1749.iq1748.iq1747.iq1746._status_is_valid(status_parity):
        return GPU_SEC_PERF_ERR_BAD_PARAM, (0, 0, 0, 0)
...
def test_source_contains_iq1757_symbols() -> None:
    src = Path("src/gpu/security_perf_matrix.HC").read_text(encoding="utf-8")
    assert f"I32 {HC_FN_IQ1757}(" in src
    assert f"status_primary = {HC_FN_IQ1756}(" in src
    assert f"status_parity = {HC_FN_IQ1755}(" in src
    assert "if (status_primary != status_parity)" in src
    assert "// IQ-1757 commit-only hardening wrapper:" in src


def test_gate_missing_vector() -> None:
    status, out_values = iq1757_model(
...
if __name__ == "__main__":
    test_source_contains_iq1757_symbols()
