#!/usr/bin/env python3
"""Reference checks for ...ParityCommitOnlyPreflightOnlyParityCommitOnlyPreflightOnlyParity (IQ-1411)."""
...
from test_attention_gqa_apply_softmax_q16_checked_nopartial_commit_only_preflight_only_parity_commit_only_preflight_only_parity_commit_only import (
...
from test_attention_gqa_apply_softmax_q16_checked_nopartial_commit_only_preflight_only_parity_commit_only_preflight_only_parity_commit_only_preflight_only import (
    gqa_attention_apply_softmax_q16_checked_nopartial_commit_only_preflight_only_parity_commit_only_preflight_only_parity_commit_only_preflight_only,
)


def gqa_attention_apply_softmax_q16_checked_nopartial_commit_only_preflight_only_parity_commit_only_preflight_only_parity_commit_only_preflight_only_parity(
    scores_q32,
    scores_capacity: int,
    query_rows: int,
    key_rows: int,
    ...
    err = gqa_attention_apply_softmax_q16_checked_nopartial_commit_only_preflight_only_parity_commit_only_preflight_only_parity_commit_only_preflight_only(
        scores_q32,
        scores_capacity,
        query_rows,
        key_rows,
    ...
    err = gqa_attention_apply_softmax_q16_checked_nopartial_commit_only_preflight_only_parity_commit_only_preflight_only_parity_commit_only(
        scores_q32,
        scores_capacity,
        query_rows,
        key_rows,
    ...
    for idx in range(required_out_cells):
        out_probs_q16[idx] = staged_commit[idx]
...
def test_fixed_vector_reference_atomic_publish() -> None:
    ...
    out = [6262] * (query_rows * key_rows)
    baseline = [6262] * (query_rows * key_rows)

    err = (
        gqa_attention_apply_softmax_q16_checked_nopartial_commit_only_preflight_only_parity_commit_only_preflight_only_parity_commit_only_preflight_only_parity(
            scores_q32,
            len(scores_q32),
            query_rows,
            key_rows,
...
def test_randomized_parity_commit_preflight_publish_path() -> None:
    rng = random.Random(20260425_1411)
...
def test_source_contract_markers() -> None:
    ...
    assert "status = GQAAttentionApplySoftmaxQ16CheckedNoPartialCommitOnlyPreflightOnlyParityCommitOnly(" in body
    assert "if (out_begin < scores_end && scores_begin < out_end)" in body
    assert "if (staged_preflight_probs_q16[copy_index] != staged_commit_probs_q16[copy_index])" in body
...
if __name__ == "__main__":
    ...
    print(
        ...
        "parity_commit_only_preflight_only_parity_reference_checks=ok"
    )
