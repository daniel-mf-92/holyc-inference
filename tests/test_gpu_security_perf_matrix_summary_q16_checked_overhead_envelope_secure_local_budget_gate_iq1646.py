#!/usr/bin/env python3
"""Harness for IQ-1646 strict diagnostics parity gate over preflight-only + commit-only."""
...
def secure_local_budget_gate_iq1646(
    rows: list[RowOutput],
    rows_capacity: int,
    secure_local_mode: int,
    iommu_active: int,
...
def test_source_contains_iq1646_symbols() -> None:
    src = Path("src/gpu/security_perf_matrix.HC").read_text(encoding="utf-8")
    assert "I32 GPUSecurityPerfMatrixSummaryQ16CheckedOverheadEnvelopeSecureLocalBudgetGateCommitOnlyPreflightOnlyParityCommitOnlyPreflightOnlyParityCommitOnlyPreflightOnlyParityCommitOnlyPreflightOnlyParityCommitOnlyPreflightOnlyParityCommitOnlyPreflightOnlyParity(" in src
...
def test_gate_missing_and_threshold_breach_vectors() -> None:
    ...
    status, outputs = secure_local_budget_gate_iq1646(
        rows_gate_missing,
        rows_capacity=3,
...
if __name__ == "__main__":
    test_source_contains_iq1646_symbols()
