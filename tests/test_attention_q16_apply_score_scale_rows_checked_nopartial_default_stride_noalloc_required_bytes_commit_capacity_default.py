#!/usr/bin/env python3
"""Parity harness for ...NoAllocRequiredBytesCommitCapacityDefault."""
...
def attention_q16_apply_score_scale_rows_checked_nopartial_default_stride_noalloc_required_bytes_commit_capacity_default(
    in_scores_q32,
    ...
    err, commit_stage_cell_capacity = try_mul_i64_checked(row_count, token_count)
    if err != ATTN_Q16_OK:
        return err

    return attention_q16_apply_score_scale_rows_checked_nopartial_default_stride_noalloc_required_bytes_commit_capacity(
        in_scores_q32,
        in_scores_capacity,
        row_count,
        token_count,
...
def explicit_checked_commit_capacity_default_composition(
    in_scores_q32,
    in_scores_capacity: int,
    row_count: int,
    token_count: int,
...
def test_source_contains_noalloc_required_bytes_commit_capacity_default_helper() -> None:
    ...
    signature = "I32 AttentionQ16ApplyScoreScaleRowsCheckedNoPartialDefaultStrideNoAllocRequiredBytesCommitCapacityDefault("
...
def test_known_vectors_and_zero_case() -> None:
    ...
    in_scores = [3] * required_cells
    out_scores = [0] * required_cells

    out_commit_cells = [111]
    out_commit_bytes = [222]
    out_out_cells = [333]

    err = attention_q16_apply_score_scale_rows_checked_nopartial_default_stride_noalloc_required_bytes_commit_capacity_default(
        in_scores,
        len(in_scores),
        row_count,
        token_count,
    ...
    assert out_commit_cells == [required_cells]
    assert out_commit_bytes == [required_bytes]
    ...
    assert err == ATTN_Q16_OK
    assert out_commit_cells == [0]
    assert out_commit_bytes == [0]
...
def test_error_paths() -> None:
    ...
    assert (
        attention_q16_apply_score_scale_rows_checked_nopartial_default_stride_noalloc_required_bytes_commit_capacity_default(
            [0], 1, 1, 1, [0], 1, 8, None, out_commit_bytes, out_out_cells
        )
        == ATTN_Q16_ERR_NULL_PTR
    )
...
def test_randomized_parity_against_explicit_composition() -> None:
    ...
    for _ in range(7000):
        ...
        if row_count >= 0 and token_count >= 0 and row_count < (1 << 31) and token_count < (1 << 31):
            required_bytes = row_count * token_count * 8
            commit_stage_byte_capacity = max(0, required_bytes + rng.randint(-256, 1024))
        else:
            commit_stage_byte_capacity = rng.randint(0, 1 << 24)

        if rng.random() < 0.04:
            commit_stage_byte_capacity = -rng.randint(1, 300)
        ...
        got_commit_cells = [0xAA]
        got_commit_bytes = [0xBB]
        got_out_cells = [0xCC]

        exp_commit_cells = [0xAA]
        exp_commit_bytes = [0xBB]
        exp_out_cells = [0xCC]

        err_new = attention_q16_apply_score_scale_rows_checked_nopartial_default_stride_noalloc_required_bytes_commit_capacity_default(
            in_scores,
            in_capacity,
            row_count,
            token_count,
        ...
        err_ref = explicit_checked_commit_capacity_default_composition(
            in_scores,
            in_capacity,
            row_count,
            token_count,
