#!/usr/bin/env python3
"""Parity harness for ...PreflightOnlyDefaultCapacityCommitOnlyParity."""
...
from test_tokenizer_bpe_encode_prompt_checked_noalloc_from_lens_commit_only_required_bytes_commit_only_default_capacity_preflight_only import (
    tokenizer_bpe_encode_prompt_checked_noalloc_from_lens_commit_only_required_bytes_commit_only_default_capacity_preflight_only,
)
...
def tokenizer_bpe_encode_prompt_checked_noalloc_from_lens_commit_only_required_bytes_preflight_only_default_capacity_commit_only_parity(
    data: list[int] | None,
    byte_len: int,
    ...
    err = tokenizer_bpe_encode_prompt_checked_noalloc_from_lens_commit_only_required_bytes_commit_only_default_capacity_preflight_only(
        data,
        byte_len,
        [io_cursor[0]],
        prompt_nbytes,
...
def test_source_contains_signature_and_parity_calls() -> None:
    source = Path("src/tokenizer/bpe.HC").read_text(encoding="utf-8")
    sig = "I32 TokenizerBPEEncodePromptCheckedNoAllocFromLensCommitOnlyRequiredBytesPreflightOnlyDefaultCapacityCommitOnlyParity("
    assert sig in source
    body = source.split(sig, 1)[1].split(
        "I32 TokenizerBPEEncodePromptCheckedNoAllocFromLensCommitOnlyRequiredBytesCommitOnlyDefaultCapacityPreflightOnlyHardened(",
        1,
    )[0]
...
def test_success_error_and_no_write_parity() -> None:
    ...
    err_a = tokenizer_bpe_encode_prompt_checked_noalloc_from_lens_commit_only_required_bytes_preflight_only_default_capacity_commit_only_parity(
        payload,
        len(payload),
        cur_a,
        len(payload),
...
def test_null_overflow_and_fuzz_parity() -> None:
    ...
    err = tokenizer_bpe_encode_prompt_checked_noalloc_from_lens_commit_only_required_bytes_preflight_only_default_capacity_commit_only_parity(
        None,
        0,
        [0],
        0,
    ...
    rng = random.Random(0x706C0DE)
    for _ in range(220):
        payload_len = rng.randint(0, 64)
        payload = [rng.randint(0, 255) for _ in range(payload_len)]
        cursor0 = rng.randint(0, payload_len)
        prompt_nbytes = rng.randint(cursor0, payload_len)

        rank_count = len(ranks)
        rank_cap = rank_count if rng.random() < 0.9 else rank_count + rng.randint(0, 3)

        vocab_count = rng.randint(1, 12)
        vocab_lens = [rng.randint(1, 16) for _ in range(vocab_count)]
        vocab_cap = vocab_count if rng.random() < 0.9 else vocab_count + rng.randint(0, 3)

        req_a = [0xAAAA_AAAA_AAAA_AAAA]
        max_a = [0xBBBB_BBBB_BBBB_BBBB]
        bytes_a = [0xCCCC_CCCC_CCCC_CCCC]
        req_b = [0xAAAA_AAAA_AAAA_AAAA]
        max_b = [0xBBBB_BBBB_BBBB_BBBB]
        bytes_b = [0xCCCC_CCCC_CCCC_CCCC]
