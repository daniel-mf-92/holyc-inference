#!/usr/bin/env python3
"""Parity harness for ...FromMaxPieceValidateCursor wrapper."""
...
from test_tokenizer_bpe_decode_prompt_checked_default_no_partial_from_max_piece import (
...
def tokenizer_bpe_decode_prompt_checked_default_no_partial_from_max_piece_validate_cursor(
...
def run_case(token_ids: list[int], token_cursor: int, pieces: list[bytes], max_piece_bytes: int) -> None:
    ...
    err_wrapped = tokenizer_bpe_decode_prompt_checked_default_no_partial_from_max_piece_validate_cursor(
        token_ids,
        len(token_ids),
        cursor_wrapped,
        blob,
...
def test_validate_cursor_rejects_overflow_and_oob() -> None:
    ...
    cursor = [4]
    err = tokenizer_bpe_decode_prompt_checked_default_no_partial_from_max_piece_validate_cursor(
        [0, 1, 2],
        3,
        cursor,
        blob,
    ...
    assert cursor[0] == 2 and count[0] == 0x9090
    assert out == [0x44] * 64


def test_no_partial_output_on_delegate_failure() -> None:
    pieces = [b"A", b"BC", b"DEF"]
    blob, offsets, lens = build_vocab_tables(pieces)
...
def test_randomized_parity_against_explicit_staged_composition() -> None:
    rng = random.Random(20260419_446)
    pieces = [
        b"a",
        b"bc",
        b"def",
...
if __name__ == "__main__":
    ...
    test_no_partial_output_on_delegate_failure()
    test_capacity_and_null_contracts()
    test_randomized_parity_against_explicit_staged_composition()
    print(
        "test_tokenizer_bpe_decode_prompt_checked_default_no_partial_from_max_piece_validate_cursor: ok"
    )
