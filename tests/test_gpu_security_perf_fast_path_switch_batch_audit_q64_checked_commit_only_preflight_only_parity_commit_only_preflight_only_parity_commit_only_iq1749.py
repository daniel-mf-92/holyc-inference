#!/usr/bin/env python3
"""Harness for IQ-1749 fast-path batch audit commit-only hardening wrapper."""
...
import test_gpu_security_perf_fast_path_switch_batch_audit_q64_checked_commit_only_preflight_only_parity_commit_only_preflight_only_parity_iq1748 as iq1748

GPU_SEC_PERF_OK = iq1748.GPU_SEC_PERF_OK
GPU_SEC_PERF_ERR_BAD_PARAM = iq1748.GPU_SEC_PERF_ERR_BAD_PARAM
GPU_SEC_PERF_PROFILE_SECURE_LOCAL = iq1748.GPU_SEC_PERF_PROFILE_SECURE_LOCAL

Scenario = iq1748.Scenario

HC_FN_IQ1749 = "GPUSecurityPerfFastPathSwitchBatchAuditQ64CheckedCommitOnlyPreflightOnlyParityCommitOnlyPreflightOnlyParityCommitOnly"
HC_FN_IQ1748 = iq1748.HC_FN_IQ1748
HC_FN_IQ1747 = iq1748.HC_FN_IQ1747
...
def iq1749_model(
    ...
    *,
    ...
    status_seed, seeded_tuple = iq1748.iq1747.iq1746.iq1746_model(
        scenarios=scenarios,
    ...
    if not iq1748.iq1747.iq1746._status_is_valid(status_seed):
        return GPU_SEC_PERF_ERR_BAD_PARAM, (0, 0, 0, 0)

    status_primary, staged_tuple = iq1748.iq1748_model(
        scenarios=scenarios,
        secure_local_mode=secure_local_mode,
        iommu_active=iommu_active,
        book_of_truth_gpu_hooks=book_of_truth_gpu_hooks,
        ...
        force_status_domain_drift_preflight_only=force_status_domain_drift_primary,
    )

    status_parity, parity_tuple = iq1748.iq1747.iq1747_model(
        scenarios=scenarios,
        secure_local_mode=secure_local_mode,
        iommu_active=iommu_active,
        book_of_truth_gpu_hooks=book_of_truth_gpu_hooks,
    ...
    if not iq1748.iq1747.iq1746._status_is_valid(status_primary):
        return GPU_SEC_PERF_ERR_BAD_PARAM, (0, 0, 0, 0)
    if not iq1748.iq1747.iq1746._status_is_valid(status_parity):
        return GPU_SEC_PERF_ERR_BAD_PARAM, (0, 0, 0, 0)
...
def test_source_contains_iq1749_symbols() -> None:
    src = Path("src/gpu/security_perf_matrix.HC").read_text(encoding="utf-8")
    assert f"I32 {HC_FN_IQ1749}(" in src
    assert f"status_primary = {HC_FN_IQ1748}(" in src
    assert f"status_parity = {HC_FN_IQ1747}(" in src
    assert "if (status_primary != status_parity)" in src
    assert "// IQ-1749 commit-only hardening wrapper:" in src


def test_gate_missing_vector() -> None:
    status, out_values = iq1749_model(
...
if __name__ == "__main__":
    test_source_contains_iq1749_symbols()
