#!/usr/bin/env python3
"""Harness for IQ-1493 overhead-envelope no-partial commit-only hardening wrapper."""
...
def gpu_security_perf_matrix_summary_q16_checked_overhead_envelope_checked_nopartial_commit_only(
    rows: list[RowOutput],
    rows_capacity: int,
    out_capacity: int,
    secure_local_mode: int,
    ...
    parity_status, parity_min, parity_max, parity_mean = parity
    ...
    preflight = _overhead_envelope_nopartial_preflight_only(
        ...
        book_of_truth_gpu_hooks,
        policy_digest_parity,
        caller_outputs=(parity_min, parity_max, parity_mean),
    )
    preflight_status, _seed_preserved, staged_tuple = preflight
    ...
    if parity_status != preflight_status:
        return GPU_SEC_PERF_ERR_BAD_PARAM, caller_outputs, (0, 0, 0)
    if staged_tuple != (parity_min, parity_max, parity_mean):
        return GPU_SEC_PERF_ERR_BAD_PARAM, caller_outputs, (0, 0, 0)

    if parity_status != GPU_SEC_PERF_OK:
        return parity_status, caller_outputs, (0, 0, 0)

    return GPU_SEC_PERF_OK, (parity_min, parity_max, parity_mean), (parity_min, parity_max, parity_mean)


def test_source_contains_iq1493_symbols() -> None:
    src = Path("src/gpu/security_perf_matrix.HC").read_text(encoding="utf-8")

    assert "I32 GPUSecurityPerfMatrixSummaryQ16CheckedOverheadEnvelopeCheckedNoPartialCommitOnly(" in src
...
def test_null_alias_capacity_vectors() -> None:
    ...
    status, outputs_after, diag = gpu_security_perf_matrix_summary_q16_checked_overhead_envelope_checked_nopartial_commit_only(
        rows,
        rows_capacity=1,
        out_capacity=3,
...
def test_parity_vectors() -> None:
    ...
    assert outputs_after == (1_000, 3_000, 2_000)
    assert diag == (1_000, 3_000, 2_000)

    drift_status, outputs_after, drift_diag = gpu_security_perf_matrix_summary_q16_checked_overhead_envelope_checked_nopartial_commit_only(
        rows,
        rows_capacity=3,
        out_capacity=3,
        secure_local_mode=GPU_SEC_PERF_PROFILE_SECURE_LOCAL,
