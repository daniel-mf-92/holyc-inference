#!/usr/bin/env python3
"""Harness for IQ-1750 fast-path batch audit zero-write diagnostics companion."""
...
import test_gpu_security_perf_fast_path_switch_batch_audit_q64_checked_commit_only_preflight_only_parity_commit_only_preflight_only_parity_commit_only_iq1749 as iq1749

GPU_SEC_PERF_OK = iq1749.GPU_SEC_PERF_OK
GPU_SEC_PERF_ERR_BAD_PARAM = iq1749.GPU_SEC_PERF_ERR_BAD_PARAM
GPU_SEC_PERF_PROFILE_SECURE_LOCAL = iq1749.GPU_SEC_PERF_PROFILE_SECURE_LOCAL

Scenario = iq1749.Scenario

HC_FN_IQ1750 = "GPUSecurityPerfFastPathSwitchBatchAuditQ64CheckedCommitOnlyPreflightOnlyParityCommitOnlyPreflightOnlyParityCommitOnlyPreflightOnlyParity"
HC_FN_IQ1749 = iq1749.HC_FN_IQ1749
HC_FN_IQ1748 = iq1749.HC_FN_IQ1748
...
def iq1750_model(
    ...
    *,
    ...
    status_commit_only, staged_tuple = iq1749.iq1749_model(
        scenarios=scenarios,
    ...
    status_canonical, canonical_tuple = iq1749.iq1748.iq1748_model(
        scenarios=scenarios,
        secure_local_mode=secure_local_mode,
        iommu_active=iommu_active,
        book_of_truth_gpu_hooks=book_of_truth_gpu_hooks,
    ...
    if not iq1749.iq1748.iq1747.iq1746._status_is_valid(status_commit_only):
        return GPU_SEC_PERF_ERR_BAD_PARAM, saved_out
    if not iq1749.iq1748.iq1747.iq1746._status_is_valid(status_canonical):
        return GPU_SEC_PERF_ERR_BAD_PARAM, saved_out
...
def test_source_contains_iq1750_symbols() -> None:
    src = Path("src/gpu/security_perf_matrix.HC").read_text(encoding="utf-8")
    assert f"I32 {HC_FN_IQ1750}(" in src
    assert f"status_commit_only = {HC_FN_IQ1749}(" in src
    assert f"status_canonical = {HC_FN_IQ1748}(" in src
    assert "if (status_commit_only != status_canonical)" in src
    assert "// IQ-1750 zero-write diagnostics companion:" in src


def test_gate_missing_vector() -> None:
    saved_out = (45, 46, 47, 48)
    status, out_values = iq1750_model(
...
if __name__ == "__main__":
    test_source_contains_iq1750_symbols()
