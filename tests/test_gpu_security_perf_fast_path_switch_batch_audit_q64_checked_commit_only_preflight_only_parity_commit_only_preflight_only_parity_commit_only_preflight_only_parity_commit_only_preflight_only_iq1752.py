#!/usr/bin/env python3
"""Harness for IQ-1752 fast-path batch audit zero-write diagnostics companion."""
...
import test_gpu_security_perf_fast_path_switch_batch_audit_q64_checked_commit_only_preflight_only_parity_commit_only_preflight_only_parity_commit_only_preflight_only_parity_commit_only_iq1751 as iq1751

GPU_SEC_PERF_OK = iq1751.GPU_SEC_PERF_OK
GPU_SEC_PERF_ERR_BAD_PARAM = iq1751.GPU_SEC_PERF_ERR_BAD_PARAM
GPU_SEC_PERF_PROFILE_SECURE_LOCAL = iq1751.GPU_SEC_PERF_PROFILE_SECURE_LOCAL

Scenario = iq1751.Scenario

HC_FN_IQ1752 = "GPUSecurityPerfFastPathSwitchBatchAuditQ64CheckedCommitOnlyPreflightOnlyParityCommitOnlyPreflightOnlyParityCommitOnlyPreflightOnlyParityCommitOnlyPreflightOnly"
HC_FN_IQ1751 = iq1751.HC_FN_IQ1751
HC_FN_IQ1750 = iq1751.HC_FN_IQ1750
...
def iq1752_model(
    ...
    *,
    ...
    status_companion, staged_tuple = iq1751.iq1751_model(
        scenarios=scenarios,
    ...
    status_canonical, canonical_tuple = iq1751.iq1750.iq1750_model(
        scenarios=scenarios,
        secure_local_mode=secure_local_mode,
        iommu_active=iommu_active,
        book_of_truth_gpu_hooks=book_of_truth_gpu_hooks,
    ...
    if not iq1751.iq1750.iq1749.iq1748.iq1747.iq1746._status_is_valid(status_companion):
        return GPU_SEC_PERF_ERR_BAD_PARAM, saved_out
    if not iq1751.iq1750.iq1749.iq1748.iq1747.iq1746._status_is_valid(status_canonical):
        return GPU_SEC_PERF_ERR_BAD_PARAM, saved_out
...
def test_source_contains_iq1752_symbols() -> None:
    src = Path("src/gpu/security_perf_matrix.HC").read_text(encoding="utf-8")
    assert f"I32 {HC_FN_IQ1752}(" in src
    assert f"status_companion = {HC_FN_IQ1751}(" in src
    assert f"status_canonical = {HC_FN_IQ1750}(" in src
    assert "if (status_companion != status_canonical)" in src
    assert "// IQ-1752 zero-write diagnostics companion:" in src


def test_gate_missing_vector() -> None:
    saved_out = (45, 46, 47, 48)
    status, out_values = iq1752_model(
...
if __name__ == "__main__":
    test_source_contains_iq1752_symbols()
