#!/usr/bin/env python3
"""Parity harness for ...CommitCapacityDefaultCapacityAliasSafePreflightOnly."""
...
def attention_q16_compute_scaled_qk_rows_checked_nopartial_strided_noalloc_commit_capacity_default_capacity_alias_safe_preflight_only(
    q_rows_q16,
    q_rows_capacity: int,
    query_row_count: int,
    ...
    *,
    ...
    status = attention_q16_compute_scaled_qk_rows_checked_nopartial_strided_noalloc_commit_capacity_alias_safe_preflight_only(
        q_rows_q16,
        q_rows_capacity,
        query_row_count,
...
def test_source_contains_default_capacity_alias_safe_preflight_helper() -> None:
    source = Path("src/model/attention.HC").read_text(encoding="utf-8")
    signature = "I32 AttentionQ16ComputeScaledQKRowsCheckedNoPartialStridedNoAllocCommitCapacityDefaultCapacityAliasSafePreflightOnly("
...
def test_known_vectors_and_alias_rejection() -> None:
    ...
    got_commit_cells = [11]
    got_commit_bytes = [22]
    got_required_cells = [33]
    got_required_bytes = [44]
    got_out_cells = [55]

    exp_commit_cells = [66]
    exp_commit_bytes = [77]
    exp_required_cells = [88]
    exp_required_bytes = [99]
    exp_out_cells = [111]

    err_got = attention_q16_compute_scaled_qk_rows_checked_nopartial_strided_noalloc_commit_capacity_default_capacity_alias_safe_preflight_only(
        q_rows,
        len(q_rows),
        3,
        8,
    ...
    assert got_required_cells == exp_required_cells == [12]
    assert got_required_bytes == exp_required_bytes == [96]
    assert got_out_cells == exp_out_cells == [16]

    err_alias = attention_q16_compute_scaled_qk_rows_checked_nopartial_strided_noalloc_commit_capacity_default_capacity_alias_safe_preflight_only(
        q_rows,
        len(q_rows),
        3,
        8,
...
def test_overflow_and_null_contracts() -> None:
    ...
    assert (
        attention_q16_compute_scaled_qk_rows_checked_nopartial_strided_noalloc_commit_capacity_default_capacity_alias_safe_preflight_only(
            q_rows,
            len(q_rows),
            1,
            1,
...
def test_randomized_parity() -> None:
    rng = random.Random(20260420_652)
    ...
    for _ in range(2500):
        ...
        got_commit_cells = [777]
        got_commit_bytes = [888]
        got_required_cells = [999]
        got_required_bytes = [111]
        got_out_cells = [222]

        exp_commit_cells = [777]
        exp_commit_bytes = [888]
        exp_required_cells = [999]
        exp_required_bytes = [111]
        exp_out_cells = [222]
        ...
        assert got_required_cells == exp_required_cells
        assert got_required_bytes == exp_required_bytes
        assert got_out_cells == exp_out_cells


if __name__ == "__main__":
    test_source_contains_default_capacity_alias_safe_preflight_helper()
