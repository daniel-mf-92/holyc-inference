#!/usr/bin/env python3
"""Reference checks for GGUFHeaderParseChecked fixed-header decode semantics."""
...
def gguf_header_parse_checked_reference(
...
def test_null_ptr_contracts() -> None:
    err, _, _ = gguf_header_parse_checked_reference(None)
    assert err == GGUF_HEADER_ERR_NULL_PTR

    err, _, _ = gguf_header_parse_checked_reference(make_header_blob(), out_header_present=False)
    assert err == GGUF_HEADER_ERR_NULL_PTR

    err, _, _ = gguf_header_parse_checked_reference(make_header_blob(), out_header_bytes_present=False)
    assert err == GGUF_HEADER_ERR_NULL_PTR


def test_bad_param_and_overflow_contracts() -> None:
    blob = make_header_blob()

    err, _, _ = gguf_header_parse_checked_reference(blob, header_nbytes_override=-1)
    assert err == GGUF_HEADER_ERR_BAD_PARAM

    err, _, _ = gguf_header_parse_checked_reference(blob, header_nbytes_override=I64_MAX_VALUE + 1)
    assert err == GGUF_HEADER_ERR_OVERFLOW


def test_truncation_for_every_short_prefix() -> None:
    blob = make_header_blob()

    for prefix_len in range(GGUF_HEADER_BYTES):
        err, _, _ = gguf_header_parse_checked_reference(blob[:prefix_len])
        assert err == GGUF_HEADER_ERR_TRUNCATED


def test_magic_and_version_validation() -> None:
    err, _, _ = gguf_header_parse_checked_reference(make_header_blob(magic=0xDEADBEEF))
    assert err == GGUF_HEADER_ERR_BAD_MAGIC

    err, _, _ = gguf_header_parse_checked_reference(make_header_blob(version=1))
    assert err == GGUF_HEADER_ERR_BAD_VERSION


def test_success_v2_and_v3_field_decode() -> None:
    for version in (GGUF_VERSION_V2, GGUF_VERSION_V3):
        blob = make_header_blob(version=version, tensor_count=42, metadata_kv_count=99)
        err, header, consumed = gguf_header_parse_checked_reference(blob)
...
def run() -> None:
    ...
    test_bad_param_and_overflow_contracts()
    test_truncation_for_every_short_prefix()
    test_magic_and_version_validation()
    test_success_v2_and_v3_field_decode()
    print("gguf_header_parse_reference_checks=ok")
