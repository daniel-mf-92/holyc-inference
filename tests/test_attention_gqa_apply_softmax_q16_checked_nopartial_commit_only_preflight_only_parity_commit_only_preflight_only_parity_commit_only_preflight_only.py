#!/usr/bin/env python3
"""Reference checks for ...ParityCommitOnlyPreflightOnlyParityCommitOnlyPreflightOnly (IQ-1396)."""
...
from test_attention_gqa_apply_softmax_q16_checked_nopartial_commit_only_preflight_only_parity_commit_only_preflight_only_parity import (
...
def gqa_attention_apply_softmax_q16_checked_nopartial_commit_only_preflight_only_parity_commit_only_preflight_only_parity_commit_only_preflight_only(
    scores_q32,
    scores_capacity: int,
    query_rows: int,
    key_rows: int,
    ...
    err = gqa_attention_apply_softmax_q16_checked_nopartial_commit_only_preflight_only_parity_commit_only_preflight_only_parity(
        scores_q32,
        scores_capacity,
        query_rows,
        key_rows,
    ...
    for idx in range(required_out_cells):
        if staged_commit[idx] != staged_parity[idx]:
            return ATTN_Q16_ERR_BAD_PARAM
...
def test_fixed_vector_reference_zero_write() -> None:
    query_rows = 4
    key_rows = 3
    head_groups = 2
    row_stride = 5
...
def test_randomized_zero_write_parity_path() -> None:
    rng = random.Random(20260425_1396)
...
def test_source_contract_markers() -> None:
    ...
    assert "if (staged_commit_probs_q16[copy_index] != staged_parity_probs_q16[copy_index])" in body
    assert "if (out_probs_q16[copy_index] != snapshot_out_probs_q16_values[copy_index])" in body


if __name__ == "__main__":
    test_fixed_vector_reference_zero_write()
    test_error_contract_null_alias_capacity_overflow()
    test_randomized_zero_write_parity_path()
    test_source_contract_markers()
    print(
        "gqa_attention_apply_softmax_q16_checked_nopartial_commit_only_preflight_only_"
        "parity_commit_only_preflight_only_parity_commit_only_preflight_only_reference_checks=ok"
    )
