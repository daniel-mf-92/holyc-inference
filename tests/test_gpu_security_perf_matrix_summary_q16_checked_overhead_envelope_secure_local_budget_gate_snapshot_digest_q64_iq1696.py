#!/usr/bin/env python3
"""Harness for IQ-1696 snapshot-digest commit-only hardening wrapper."""
...
def matrix_summary_snapshot_digest_q64_checked_commit_only_iq1696(
    rows: list[RowOutput],
...
def test_source_contains_iq1696_symbols() -> None:
    ...
    assert "// IQ-1696 commit-only hardening wrapper:" in src


def test_gate_missing_vector() -> None:
    status, p50, p95, gate, digest_q64 = matrix_summary_snapshot_digest_q64_checked_commit_only_iq1696(
...
def test_deterministic_tuple_parity_vector() -> None:
    ...
    first = matrix_summary_snapshot_digest_q64_checked_commit_only_iq1696(
        rows=rows,
        secure_local_mode=GPU_SEC_PERF_PROFILE_SECURE_LOCAL,
        iommu_active=1,
        book_of_truth_gpu_hooks=1,
    ...
    second = matrix_summary_snapshot_digest_q64_checked_commit_only_iq1696(
        rows=rows,
        secure_local_mode=GPU_SEC_PERF_PROFILE_SECURE_LOCAL,
        iommu_active=1,
        book_of_truth_gpu_hooks=1,
...
if __name__ == "__main__":
    test_source_contains_iq1696_symbols()
