#!/usr/bin/env python3
"""Parity harness for TokenizerBPEDecodePromptCheckedDefaultCapacityValidateCursorNoPartial."""
...
def tokenizer_bpe_decode_prompt_checked_default_capacity_validate_cursor_no_partial(
    ...
    if staged_cursor[0] != token_count or staged_count[0] > out_byte_capacity:
        return TOKENIZER_BPE_ERR_BAD_PARAM
...
def run_case(token_ids: list[int], token_cursor: int, pieces: list[bytes], out_capacity: int) -> None:
    ...
    err_wrapped = tokenizer_bpe_decode_prompt_checked_default_capacity_validate_cursor_no_partial(
        token_ids,
        len(token_ids),
        cursor_wrapped,
        blob,
...
def test_cursor_overflow_and_output_capacity_reject_no_partial() -> None:
    ...
    cursor = [4]
    err = tokenizer_bpe_decode_prompt_checked_default_capacity_validate_cursor_no_partial(
        [0, 1, 2],
        3,
        cursor,
        blob,
    ...
    assert err == TOKENIZER_BPE_ERR_BAD_PARAM
    assert cursor[0] == 0 and count[0] == 0x9393
    assert out == [0x37] * 64
...
def test_randomized_parity_against_explicit_staged_core_composition() -> None:
    rng = random.Random(20260418_435)
    pieces = [
        b"a",
        b"bc",
        b"def",
...
if __name__ == "__main__":
    ...
    test_cursor_overflow_and_output_capacity_reject_no_partial()
    test_null_and_domain_contracts()
    test_randomized_parity_against_explicit_staged_core_composition()
    print("test_tokenizer_bpe_decode_prompt_checked_default_capacity_validate_cursor_no_partial: ok")
