#!/usr/bin/env python3
"""Harness for IQ-1619 strict diagnostics parity gate over IQ-1616 and IQ-1610."""
...
def fast_path_switch_checked_audit_parity_iq1619(
    secure_local_mode: int,
    iommu_active: int,
    book_of_truth_gpu_hooks: int,
    policy_digest_parity: int,
...
def test_source_contains_iq1619_symbols() -> None:
    src = Path("src/gpu/security_perf_matrix.HC").read_text(encoding="utf-8")

    assert "I32 GPUSecurityPerfFastPathSwitchCheckedAuditParityCommitOnlyPreflightOnlyParityCommitOnlyPreflightOnlyParityCommitOnlyPreflightOnlyParityCommitOnlyPreflightOnlyParityCommitOnlyPreflightOnlyParityCommitOnlyPreflightOnlyParity(" in src
...
def test_gate_missing_and_audit_parity_breach_vectors() -> None:
    status, enabled, reason = fast_path_switch_checked_audit_parity_iq1619(
        secure_local_mode=GPU_SEC_PERF_PROFILE_SECURE_LOCAL,
        iommu_active=0,
        book_of_truth_gpu_hooks=1,
        policy_digest_parity=1,
        dispatch_transcript_parity=1,
        current_fast_path_enabled=201,
        current_disable_reason_code=202,
    )
    assert (status, enabled, reason) == (GPU_SEC_PERF_ERR_POLICY_GUARD, 201, 202)
    ...
    status, enabled, reason = fast_path_switch_checked_audit_parity_iq1619(
        ...
        current_fast_path_enabled=203,
        current_disable_reason_code=204,
    )
    assert (status, enabled, reason) == (GPU_SEC_PERF_ERR_POLICY_GUARD, 203, 204)
...
def test_status_domain_drift_and_no_write_parity_vectors() -> None:
    status, enabled, reason = fast_path_switch_checked_audit_parity_iq1619(
        ...
        policy_digest_parity=1,
        dispatch_transcript_parity=1,
        current_fast_path_enabled=211,
        current_disable_reason_code=212,
        force_status_domain_drift=True,
    )
    assert (status, enabled, reason) == (GPU_SEC_PERF_ERR_BAD_PARAM, 211, 212)
    ...
    status, enabled, reason = fast_path_switch_checked_audit_parity_iq1619(
        ...
        policy_digest_parity=1,
        dispatch_transcript_parity=1,
        current_fast_path_enabled=213,
        current_disable_reason_code=214,
        force_tuple_drift=True,
    )
    assert (status, enabled, reason) == (GPU_SEC_PERF_ERR_BAD_PARAM, 213, 214)


def test_deterministic_parity_vector() -> None:
    first = fast_path_switch_checked_audit_parity_iq1619(
        secure_local_mode=GPU_SEC_PERF_PROFILE_SECURE_LOCAL,
        iommu_active=1,
        book_of_truth_gpu_hooks=1,
        policy_digest_parity=1,
        dispatch_transcript_parity=1,
        current_fast_path_enabled=221,
        current_disable_reason_code=222,
    )
    second = fast_path_switch_checked_audit_parity_iq1619(
        secure_local_mode=GPU_SEC_PERF_PROFILE_SECURE_LOCAL,
        iommu_active=1,
        book_of_truth_gpu_hooks=1,
        policy_digest_parity=1,
    ...
    assert first == (GPU_SEC_PERF_OK, 221, 222)
    assert second == first
