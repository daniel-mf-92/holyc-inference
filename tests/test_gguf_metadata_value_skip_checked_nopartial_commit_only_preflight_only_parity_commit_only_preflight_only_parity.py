#!/usr/bin/env python3
"""Harness for ...ParityCommitOnlyPreflightOnlyParity (IQ-1252)."""
...
from test_gguf_metadata_value_skip_checked_nopartial_commit_only_preflight_only_parity_commit_only import (
    gguf_metadata_value_skip_checked_nopartial_commit_only_preflight_only_parity_commit_only_reference,
)
from test_gguf_metadata_value_skip_checked_nopartial_commit_only_preflight_only_parity_commit_only_preflight_only import (
    gguf_metadata_value_skip_checked_nopartial_commit_only_preflight_only_parity_commit_only_preflight_only_reference,
)


def gguf_metadata_value_skip_checked_nopartial_commit_only_preflight_only_parity_commit_only_preflight_only_parity_reference(
    *,
    buf: list[int] | None,
    buf_nbytes: int,
    ...
    staged_preflight_commit_cursor = [snapshot_cursor]
    staged_preflight_commit_next = [0]
    err = gguf_metadata_value_skip_checked_nopartial_commit_only_preflight_only_parity_commit_only_preflight_only_reference(
        buf=buf,
        buf_nbytes=buf_nbytes,
        cursor_ref=staged_preflight_commit_cursor,
        table_end=table_end,
        value_type=value_type,
        out_next_cursor_ref=staged_preflight_commit_next,
    )
    if err != GGUF_META_TABLE_OK:
        return err

    if staged_preflight_commit_cursor[0] != staged_preflight_commit_next[0]:
        return GGUF_META_TABLE_ERR_BAD_PARAM
    if staged_preflight_commit_next[0] > buf_nbytes or staged_preflight_commit_next[0] > table_end:
        return GGUF_META_TABLE_ERR_OUT_OF_BOUNDS

    staged_canonical_commit_cursor = [snapshot_cursor]
    staged_canonical_commit_next = [0]
    err = gguf_metadata_value_skip_checked_nopartial_commit_only_preflight_only_parity_commit_only_reference(
        buf=buf,
        buf_nbytes=buf_nbytes,
        cursor_ref=staged_canonical_commit_cursor,
        table_end=table_end,
        value_type=value_type,
        out_next_cursor_ref=staged_canonical_commit_next,
    )
    if err != GGUF_META_TABLE_OK:
        return err

    if staged_canonical_commit_cursor[0] != staged_canonical_commit_next[0]:
        return GGUF_META_TABLE_ERR_BAD_PARAM
    if staged_canonical_commit_next[0] > buf_nbytes or staged_canonical_commit_next[0] > table_end:
        return GGUF_META_TABLE_ERR_OUT_OF_BOUNDS

    if staged_preflight_commit_next[0] != staged_canonical_commit_next[0]:
        return GGUF_META_TABLE_ERR_BAD_PARAM
    ...
    out_next_cursor_ref[0] = staged_preflight_commit_next[0]
    cursor_ref[0] = staged_preflight_commit_next[0]
    return GGUF_META_TABLE_OK


def test_source_contains_iq1252_function_and_strict_publish() -> None:
    source = Path("src/gguf/metadata.HC").read_text(encoding="utf-8")

    sig = "I32 GGUFMetadataValueSkipCheckedNoPartialCommitOnlyPreflightOnlyParityCommitOnlyPreflightOnlyParity(U8 *buf,"
    sig_def = (
        "I32 GGUFMetadataValueSkipCheckedNoPartialCommitOnlyPreflightOnlyParityCommitOnlyPreflightOnlyParity(U8 *buf,\n"
        "                                                                                                     U64 buf_nbytes,\n"
        "                                                                                                     U64 *cursor,\n"
        "                                                                                                     U64 table_end,\n"
        "                                                                                                     U32 value_type,\n"
    ...
    assert "GGUFMetadataValueSkipCheckedNoPartialCommitOnlyPreflightOnlyParityCommitOnly(" in body
    assert "snapshot_cursor = *cursor;" in body
    assert "staged_preflight_commit_next_cursor !=" in body
    assert "staged_canonical_commit_next_cursor" in body
    assert "*out_next_cursor = staged_preflight_commit_next_cursor;" in body
    assert "*cursor = staged_preflight_commit_next_cursor;" in body
...
def test_randomized_parity_vectors() -> None:
    rng = random.Random(20260424_1252)
    ...
    for _ in range(1200):
        ...
        cursor_a = [cursor0]
        out_a = [0xAA]
        status_a = gguf_metadata_value_skip_checked_nopartial_commit_only_preflight_only_parity_commit_only_preflight_only_parity_reference(
            buf=buf,
            buf_nbytes=len(buf),
            cursor_ref=cursor_a,
            table_end=len(buf),
        ...
        cursor_b = [cursor0]
        out_b = [0xBB]
        status_b = gguf_metadata_value_skip_checked_nopartial_commit_only_preflight_only_parity_commit_only_reference(
            buf=buf,
            buf_nbytes=len(buf),
            cursor_ref=cursor_b,
            table_end=len(buf),
...
if __name__ == "__main__":
    test_source_contains_iq1252_function_and_strict_publish()
