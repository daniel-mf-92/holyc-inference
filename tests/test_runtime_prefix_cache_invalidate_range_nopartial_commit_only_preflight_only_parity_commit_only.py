#!/usr/bin/env python3
"""Host-side harness for IQ-1339 PrefixCacheInvalidateRangeCheckedNoPartialCommitOnlyPreflightOnlyParityCommitOnly."""
...
def test_iq1339_parity_commit_only_matches_preflight_only_tuple():
    ...
    status, removed = invalidate_range_commit_only_preflight_only_parity_commit_only(cache, 12, 30)
...
def test_iq1339_parity_commit_only_zero_length_invalidate_is_noop():
    ...
    status, removed = invalidate_range_commit_only_preflight_only_parity_commit_only(cache, 12, 0)
    assert status == PREFIX_CACHE_OK
    assert removed == 0
...
def test_iq1339_parity_commit_only_preflight_guard_keeps_state_unchanged_on_error():
    ...
    status, removed = invalidate_range_commit_only_preflight_only_parity_commit_only(cache, 0, 10)
...
def test_holyc_function_body_and_contract_markers_present():
    ...
    assert (
        "status_commit = PrefixCacheInvalidateRangeCheckedNoPartialCommitOnlyPreflightOnly(cache," in body
    )
...
if __name__ == "__main__":
    test_iq1339_parity_commit_only_matches_preflight_only_tuple()
    test_iq1339_parity_commit_only_zero_length_invalidate_is_noop()
    test_iq1339_parity_commit_only_preflight_guard_keeps_state_unchanged_on_error()
