#!/usr/bin/env python3
"""Harness for IQ-1565 preflight-only digest-locked latency-window diagnostics companion."""
...
def gpu_security_perf_matrix_summary_q16_checked_latency_window_checked_nopartial_preflight_only_parity_commit_only_preflight_only_digest_locked_parity_commit_only_preflight_only(
    ...
    status_canonical, preserved_outputs, canonical_tuple = _latency_window_nopartial_preflight_only(
        rows,
        rows_capacity,
        out_capacity,
        secure_local_mode,
    ...
    if status_commit_only not in valid_status or status_canonical not in valid_status:
        return GPU_SEC_PERF_ERR_BAD_PARAM, caller_outputs, (0, 0, 0, 0)
    ...
    if status_commit_only != GPU_SEC_PERF_OK:
        return status_commit_only, caller_outputs, (0, 0, 0, 0)

    return GPU_SEC_PERF_OK, preserved_outputs, canonical_tuple


def test_source_contains_iq1565_symbols() -> None:
    src = Path("src/gpu/security_perf_matrix.HC").read_text(encoding="utf-8")

    assert "I32 GPUSecurityPerfMatrixSummaryQ16CheckedLatencyWindowCheckedNoPartialPreflightOnlyParityCommitOnlyPreflightOnlyDigestLockedParityCommitOnlyPreflightOnly(" in src
    ...
    assert "status_canonical = GPUSecurityPerfMatrixSummaryQ16CheckedLatencyWindowCheckedNoPartialPreflightOnlyParityCommitOnlyPreflightOnlyDigestLockedParity(" in src
...
def test_duplicate_latency_success_zero_write_diagnostics() -> None:
    ...
    status, preserved, published = gpu_security_perf_matrix_summary_q16_checked_latency_window_checked_nopartial_preflight_only_parity_commit_only_preflight_only_digest_locked_parity_commit_only_preflight_only(
        rows,
        rows_capacity=5,
        out_capacity=4,
...
def test_null_alias_capacity_gate_missing_digest_drift_vectors() -> None:
    ...
    status, *_ = gpu_security_perf_matrix_summary_q16_checked_latency_window_checked_nopartial_preflight_only_parity_commit_only_preflight_only_digest_locked_parity_commit_only_preflight_only(
        rows,
        rows_capacity=1,
        out_capacity=4,
        secure_local_mode=GPU_SEC_PERF_PROFILE_SECURE_LOCAL,
...
if __name__ == "__main__":
    test_source_contains_iq1565_symbols()
