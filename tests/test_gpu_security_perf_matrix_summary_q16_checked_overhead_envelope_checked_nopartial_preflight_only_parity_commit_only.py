#!/usr/bin/env python3
"""Harness for IQ-1495 overhead-envelope no-partial preflight-only parity commit-only wrapper."""
...
def _overhead_envelope_nopartial_preflight_only_parity(
    rows: list[RowOutput],
    rows_capacity: int,
    out_capacity: int,
    secure_local_mode: int,
...
def gpu_security_perf_matrix_summary_q16_checked_overhead_envelope_checked_nopartial_preflight_only_parity_commit_only(
    ...
    parity_status, parity_tuple = _overhead_envelope_nopartial_preflight_only_parity(
        rows,
        rows_capacity,
        out_capacity,
        secure_local_mode,
...
def test_source_contains_iq1495_symbols() -> None:
    src = Path("src/gpu/security_perf_matrix.HC").read_text(encoding="utf-8")

    assert "I32 GPUSecurityPerfMatrixSummaryQ16CheckedOverheadEnvelopeCheckedNoPartialPreflightOnlyParityCommitOnly(" in src
    assert "status_parity = GPUSecurityPerfMatrixSummaryQ16CheckedOverheadEnvelopeCheckedNoPartialPreflightOnlyParity(" in src
...
def test_commit_only_success_publishes_tuple() -> None:
    ...
    status, persisted, staged = gpu_security_perf_matrix_summary_q16_checked_overhead_envelope_checked_nopartial_preflight_only_parity_commit_only(
        rows,
        rows_capacity=4,
        out_capacity=3,
...
def test_null_rows_rejected() -> None:
    status, persisted, staged = gpu_security_perf_matrix_summary_q16_checked_overhead_envelope_checked_nopartial_preflight_only_parity_commit_only(
        None,
        rows_capacity=1,
        out_capacity=3,
...
def test_alias_rejected() -> None:
    ...
    status, persisted, staged = gpu_security_perf_matrix_summary_q16_checked_overhead_envelope_checked_nopartial_preflight_only_parity_commit_only(
        ...
        book_of_truth_gpu_hooks=1,
        policy_digest_parity=1,
        caller_outputs=(5, 5, 5),
        outputs_alias=True,
    )

    assert status == GPU_SEC_PERF_ERR_BAD_PARAM
    assert persisted == (5, 5, 5)
    assert staged == (0, 0, 0)


def test_policy_gate_preserves_outputs() -> None:
    rows = [RowOutput(100_000, 1_500, 10_000)]
    status, persisted, staged = gpu_security_perf_matrix_summary_q16_checked_overhead_envelope_checked_nopartial_preflight_only_parity_commit_only(
        rows,
        rows_capacity=1,
    ...
    assert persisted == (10, 11, 12)
    assert staged == (0, 0, 0)


def test_capacity_rejected() -> None:
    rows = [RowOutput(100_000, 1_500, 10_000)]
    status, persisted, staged = gpu_security_perf_matrix_summary_q16_checked_overhead_envelope_checked_nopartial_preflight_only_parity_commit_only(
        rows,
        rows_capacity=1,
...
def test_parity_drift_detected() -> None:
