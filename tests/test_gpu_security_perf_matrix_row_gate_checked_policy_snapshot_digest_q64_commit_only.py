#!/usr/bin/env python3
"""Harness for IQ-1472 commit-only row-gate policy snapshot digest helper."""
...
def gpu_security_perf_matrix_row_gate_checked_policy_snapshot_digest_q64_commit_only(
    secure_local_mode: int,
    iommu_active: int,
    book_of_truth_gpu_hooks: int,
    policy_digest_parity: int,
...
def test_source_contains_iq1472_symbols() -> None:
    src = Path("src/gpu/security_perf_matrix.HC").read_text(encoding="utf-8")

    assert "I32 GPUSecurityPerfMatrixRowGateCheckedPolicySnapshotDigestQ64CommitOnly(" in src
    assert "status_primary = GPUSecurityPerfMatrixRowGateCheckedPolicySnapshotDigestQ64(" in src
    assert "status_parity = GPUSecurityPerfMatrixRowGateCheckedPolicySnapshotDigestQ64(" in src
    assert "staged_snapshot_digest_q64" in src
    assert "parity_snapshot_digest_q64" in src


def test_gate_missing_vectors() -> None:
    status, digest = gpu_security_perf_matrix_row_gate_checked_policy_snapshot_digest_q64_commit_only(
...
def test_bit_flip_sensitivity_vectors() -> None:
    status_base, digest_base = gpu_security_perf_matrix_row_gate_checked_policy_snapshot_digest_q64_commit_only(
        secure_local_mode=GPU_SEC_PERF_PROFILE_SECURE_LOCAL,
        iommu_active=1,
        book_of_truth_gpu_hooks=1,
        policy_digest_parity=1,
    ...
    status_prompt, digest_prompt = gpu_security_perf_matrix_row_gate_checked_policy_snapshot_digest_q64_commit_only(
        secure_local_mode=GPU_SEC_PERF_PROFILE_SECURE_LOCAL,
        iommu_active=1,
        book_of_truth_gpu_hooks=1,
        policy_digest_parity=1,
    ...
    status_batch, digest_batch = gpu_security_perf_matrix_row_gate_checked_policy_snapshot_digest_q64_commit_only(
        secure_local_mode=GPU_SEC_PERF_PROFILE_SECURE_LOCAL,
        iommu_active=1,
        book_of_truth_gpu_hooks=1,
        policy_digest_parity=1,
    ...
    status_quant, digest_quant = gpu_security_perf_matrix_row_gate_checked_policy_snapshot_digest_q64_commit_only(
        secure_local_mode=GPU_SEC_PERF_PROFILE_SECURE_LOCAL,
        iommu_active=1,
        book_of_truth_gpu_hooks=1,
        policy_digest_parity=1,
    ...
    assert digest_base != digest_batch
    assert digest_base != digest_quant


def test_overflow_boundary_vectors() -> None:
    status_overflow, digest_overflow = gpu_security_perf_matrix_row_gate_checked_policy_snapshot_digest_q64_commit_only(
        secure_local_mode=GPU_SEC_PERF_PROFILE_SECURE_LOCAL,
        iommu_active=1,
        book_of_truth_gpu_hooks=1,
        policy_digest_parity=1,
    ...
    status_ok, digest_ok = gpu_security_perf_matrix_row_gate_checked_policy_snapshot_digest_q64_commit_only(
        secure_local_mode=GPU_SEC_PERF_PROFILE_SECURE_LOCAL,
        iommu_active=1,
        book_of_truth_gpu_hooks=1,
        policy_digest_parity=1,
    ...
    assert status_ok == GPU_SEC_PERF_OK
    assert digest_ok > 0


if __name__ == "__main__":
    test_source_contains_iq1472_symbols()
