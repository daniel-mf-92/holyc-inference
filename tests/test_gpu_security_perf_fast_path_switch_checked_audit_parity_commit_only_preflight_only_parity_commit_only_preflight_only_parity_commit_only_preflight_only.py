#!/usr/bin/env python3
"""Harness for IQ-1585 zero-write diagnostics companion over fast-path parity wrappers."""
...
def fast_path_switch_checked_audit_parity_commit_only_preflight_only_parity_commit_only_preflight_only_parity_commit_only_preflight_only(
    secure_local_mode: int,
    iommu_active: int,
    book_of_truth_gpu_hooks: int,
    policy_digest_parity: int,
    ...
    *,
    ...
    if force_parity_status_domain_drift:
        status_parity = 95
...
def test_source_contains_iq1585_symbols() -> None:
...
def test_gate_missing_and_audit_parity_breach_vectors() -> None:
    status, enabled, reason = fast_path_switch_checked_audit_parity_commit_only_preflight_only_parity_commit_only_preflight_only_parity_commit_only_preflight_only(
