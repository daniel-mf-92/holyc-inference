#!/usr/bin/env python3
"""Parity harness for ...CommitOnlyDefaultCapacityPreflightOnlyNoAlloc."""
...
def tokenizer_bpe_encode_prompt_checked_noalloc_from_lens_commit_only_required_bytes_commit_only_default_capacity_preflight_only_noalloc(
    data: list[int] | None,
    byte_len: int,
...
def test_source_contains_signature_and_noalloc_calls() -> None:
    source = Path("src/tokenizer/bpe.HC").read_text(encoding="utf-8")
    sig = "I32 TokenizerBPEEncodePromptCheckedNoAllocFromLensCommitOnlyRequiredBytesCommitOnlyDefaultCapacityPreflightOnlyNoAlloc("
    ...
    assert "canonical_cursor = *io_cursor;" in body
...
def test_success_error_and_no_write_parity() -> None:
    ...
    err_a = tokenizer_bpe_encode_prompt_checked_noalloc_from_lens_commit_only_required_bytes_commit_only_default_capacity_preflight_only_noalloc(
        payload,
        len(payload),
        cur_a,
        len(payload),
...
if __name__ == "__main__":
    test_source_contains_signature_and_noalloc_calls()
    test_success_error_and_no_write_parity()
