#!/usr/bin/env python3
"""Parity harness for GGUFTensorInfoReadNameCheckedNoPartialCommitOnly (IQ-1007)."""
...
def parse_name_checked_nopartial_commit_only(
    buf: bytes | None,
    size: int,
    cursor: int,
    ...
    out_next_cursor[0] = staged_next[0]
    return GGUF_TENSOR_PARSE_OK


def test_source_contains_iq1007_commit_only_signature_and_publish_tuple() -> None:
    source = Path("src/gguf/tensorinfo.HC").read_text(encoding="utf-8")
    sig = "I64 GGUFTensorInfoReadNameCheckedNoPartialCommitOnly(U8 *buf,"
    ...
    assert "GGUFTensorFreeString(&staged_name);" in body
...
def test_success_and_randomized_tuple_parity() -> None:
    ...
    rng = random.Random(202604221007)
    for i in range(1200):
...
if __name__ == "__main__":
    test_source_contains_iq1007_commit_only_signature_and_publish_tuple()
    ...
    print("gguf_tensorinfo_read_name_checked_nopartial_commit_only=ok")
