...
def explicit_commit_only_parity_commit_only_composition(
    query_row_count: int,
    token_count: int,
    staged_scores_q32,
    staged_scores_capacity: int,
...
def test_known_vector_commit_only_parity_commit_only_outputs() -> None:
    query_row_count = 7
    token_count = 6
...
def test_error_paths_preserve_outputs() -> None:
    ...
    assert got_stage_cells == [911]
    assert got_stage_bytes == [912]
    assert got_out_cells == [913]
    assert got_last_out == [914]
...
def test_overflow_vectors_preserve_outputs() -> None:
    ...
    for query_row_count, token_count in (
        ...
        err_ref = explicit_commit_only_parity_commit_only_composition(
            query_row_count,
            token_count,
            staged,
            i64_max,
...
def test_randomized_commit_only_parity_commit_only_vs_explicit_composition() -> None:
    rng = random.Random(20260421_846)
    ...
    for _ in range(1200):
        ...
        err_new = attention_q16_compute_scaled_qk_rows_checked_default_stride_nopartial_commit_only_preflight_only_required_bytes_parity_commit_only_parity_commit_only(
            query_row_count,
            token_count,
            staged,
            staged_cap,
...
if __name__ == "__main__":
    ...
    test_known_vector_commit_only_parity_commit_only_outputs()
    test_error_paths_preserve_outputs()
    test_overflow_vectors_preserve_outputs()
    test_randomized_commit_only_parity_commit_only_vs_explicit_composition()
    print(
        "attention_q16_compute_scaled_qk_rows_checked_default_stride_nopartial_commit_only_preflight_only_required_bytes_parity_commit_only_parity_commit_only=ok"
    )
