...
def tokenizer_bpe_decode_token_span_checked_default_capacity_validate_cursor_noalloc_from_max_piece(
    ...
    return tokenizer_bpe_decode_token_span_checked_default_capacity_validate_cursor_noalloc(
        token_ids,
        token_count,
        io_token_cursor,
        span_token_count,
        ...
        vocab_piece_count,
        out_bytes,
        span_token_count * max_piece_bytes,
        out_byte_count,
    )
...
def run_case(
    ...
    err_new = tokenizer_bpe_decode_token_span_checked_default_capacity_validate_cursor_noalloc_from_max_piece(
        token_ids,
        len(token_ids),
        cursor_new,
        span_count,
...
def test_source_contains_expected_wrapper_shape() -> None:
    ...
    assert "TokenizerBPEDecodeTokenSpanCheckedDefaultCapacityValidateCursorNoAllocFromMaxPiece" in source
    section = source.split(
        "I32 TokenizerBPEDecodeTokenSpanCheckedDefaultCapacityValidateCursorNoAllocFromMaxPiece", 1
    )[1].split("I32 TokenizerBPEDecodePromptChecked", 1)[0]
    assert "TokenizerBPEComputeMaxPieceBytesChecked" in section
    assert "TokenizerBPEDecodeTokenSpanCheckedDefaultCapacityValidateCursorNoAlloc(" in section
    assert "span_token_count * max_piece_bytes" in section
...
def test_null_overflow_and_cursor_window_rejects() -> None:
...
def test_randomized_parity_against_staged_composition() -> None:
    rng = random.Random(20260419_496)
    pieces = [
        b"",
        b"a",
        b"bc",
