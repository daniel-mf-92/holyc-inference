#!/usr/bin/env python3
"""Harness for IQ-1600 commit-only hardening wrapper over IQ-1599 and IQ-1598."""
...
def secure_local_budget_gate_commit_only_preflight_only_parity_commit_only(
    rows: list[RowOutput],
    rows_capacity: int,
    secure_local_mode: int,
    iommu_active: int,
    ...
    *,
    ...
    status_preflight, _ = _secure_local_budget_gate_commit_only_preflight_only(
        rows,
        rows_capacity,
        secure_local_mode,
        iommu_active,
    ...
    if force_tuple_parity_drift:
        staged_tuple = (staged_tuple[0] + 1, staged_tuple[1], staged_tuple[2])
    ...
    if status_parity != status_preflight or parity_tuple != staged_tuple:
        return GPU_SEC_PERF_ERR_BAD_PARAM, caller_outputs
...
def test_source_contains_iq1600_symbols() -> None:
    src = Path("src/gpu/security_perf_matrix.HC").read_text(encoding="utf-8")
    assert "I32 GPUSecurityPerfMatrixSummaryQ16CheckedOverheadEnvelopeSecureLocalBudgetGateCommitOnlyPreflightOnlyParityCommitOnly(" in src
    ...
    assert "status_preflight_only = GPUSecurityPerfMatrixSummaryQ16CheckedOverheadEnvelopeSecureLocalBudgetGateCommitOnlyPreflightOnly(" in src
...
def test_gate_missing_and_threshold_breach_vectors() -> None:
    ...
    status, outputs = secure_local_budget_gate_commit_only_preflight_only_parity_commit_only(
        rows_gate_missing,
        rows_capacity=3,
...
if __name__ == "__main__":
    test_source_contains_iq1600_symbols()
