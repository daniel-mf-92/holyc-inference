#!/usr/bin/env python3
"""Harness for IQ-1023 tensor-name preflight parity gate."""
...
def parse_name_checked_nopartial_commit_only_preflight_only_parity_commit_only_preflight_only_parity(
    buf: bytes | None,
    size: int,
    cursor: int,
    ...
    pre_required = try_add_u64(8, staged_pre_name_len[0])
    if pre_required is None:
        return GGUF_TENSOR_PARSE_ERR_TRUNCATED
    staged_pre_required_check[0] = pre_required
    ...
    pre_end = try_add_u64(cursor, staged_pre_required_bytes[0])
    if pre_end is None:
        return GGUF_TENSOR_PARSE_ERR_TRUNCATED
    staged_pre_computed_end[0] = pre_end
    ...
    if staged_pre_computed_end[0] > size or staged_parity_computed_end[0] > size:
        return GGUF_TENSOR_PARSE_ERR_TRUNCATED
    if staged_pre_computed_end[0] != staged_pre_next_cursor[0] or staged_parity_computed_end[0] != staged_parity_next_cursor[0]:
        return GGUF_TENSOR_PARSE_ERR_TRUNCATED
    ...
    if (
        staged_pre_name_len[0] != staged_parity_name_len[0]
        or staged_pre_required_bytes[0] != staged_parity_required_bytes[0]
        or staged_pre_next_cursor[0] != staged_parity_next_cursor[0]
...
def explicit_checked_composition(*args) -> int:
    return parse_name_checked_nopartial_commit_only_preflight_only_parity_commit_only_preflight_only_parity(*args)


def test_source_contains_iq1023_signature_and_contract() -> None:
    source = Path("src/gguf/tensorinfo.HC").read_text(encoding="utf-8")
    sig = "I64 GGUFTensorInfoReadNameCheckedNoPartialCommitOnlyPreflightOnlyParityCommitOnlyPreflightOnlyParity("
    assert sig in source
    body = source.rsplit(sig, 1)[1].split("\nI64 ", 1)[0]

    assert "IQ-1023 diagnostics-only parity gate for tensor-name preflight parity wrappers." in source
    ...
    assert "staged_preflight_name_len != staged_parity_name_len" in body
    assert "staged_preflight_required_bytes != staged_parity_required_bytes" in body
    assert "staged_preflight_next_cursor != staged_parity_next_cursor" in body
...
def test_randomized_parity_vs_explicit_composition() -> None:
    rng = random.Random(20260422_1023)
    ...
    for _ in range(900):
        ...
        err_a = parse_name_checked_nopartial_commit_only_preflight_only_parity_commit_only_preflight_only_parity(
            payload,
            size,
            prefix_len,
            out_a_name,
...
if __name__ == "__main__":
    test_source_contains_iq1023_signature_and_contract()
    test_known_vector_success_and_alias_guard()
    test_randomized_parity_vs_explicit_composition()
    print("gguf_tensorinfo_read_name_checked_nopartial_commit_only_preflight_only_parity_commit_only_preflight_only_parity=ok")
