...
def telemetry_digest_q64_checked_nopartial_commit_only_preflight_only_parity_commit_only_preflight_only_parity_commit_only_preflight_only_parity_commit_only_preflight_only(
    total_ops: int,
    total_cycles: int,
    cycles_per_op: int,
    remainder_cycles: int,
...
def telemetry_digest_q64_checked_nopartial_commit_only_preflight_only_parity_commit_only_preflight_only_parity_commit_only_preflight_only_parity_commit_only_preflight_only_parity(
    total_ops: int,
    total_cycles: int,
    cycles_per_op: int,
    remainder_cycles: int,
    ...
    *,
    ...
    preflight_probe = [4919131752989213764]
    status = telemetry_digest_q64_checked_nopartial_commit_only_preflight_only_parity_commit_only_preflight_only_parity_commit_only_preflight_only_parity_commit_only_preflight_only(
        total_ops,
        total_cycles,
        cycles_per_op,
        remainder_cycles,
    ...
    status = telemetry_digest_q64_checked_nopartial_commit_only_preflight_only_parity_commit_only_preflight_only_parity_commit_only_preflight_only_parity_commit_only(
        ...
        cpu_hz,
        commit_digest,
        force_parity_digest=force_commit_digest,
    )
    if status != ERR_OK:
        return status
    ...
    if preflight_probe[0] != 4919131752989213764:
        return ERR_BAD_PARAM
...
def test_source_contains_iq1361_function_and_guards() -> None:
    ...
    sig = (
        ...
        "PreflightOnlyParityCommitOnlyPreflightOnlyParity("
    )
    assert sig in source
    body = source.split(sig, 1)[1]
    assert "snapshot_out_digest_value = *out_digest_q64;" in body
    assert "status = Q8_0DotBenchRunDefaultSuiteTelemetryDigestQ64CheckedNoPartialCommitOnlyPreflightOnlyParityCommitOnlyPreflightOnlyParityCommitOnlyPreflightOnlyParityCommitOnlyPreflightOnly(" in body
    assert "status = Q8_0DotBenchRunDefaultSuiteTelemetryDigestQ64CheckedNoPartialCommitOnlyPreflightOnlyParityCommitOnlyPreflightOnlyParityCommitOnlyPreflightOnlyParityCommitOnly(" in body
    ...
    assert "if (preflight_probe_digest_q64 != 4919131752989213764)" in body
...
def test_happy_path_publishes_digest() -> None:
    ...
    expected_status, expected_digest = telemetry_digest_q64_checked(
        12_288,
        319_488,
        26,
        0,
...
def test_commit_parity_mismatch_rejects() -> None:
    out = [0xBEEF]
    status = telemetry_digest_q64_checked_nopartial_commit_only_preflight_only_parity_commit_only_preflight_only_parity_commit_only_preflight_only_parity_commit_only_preflight_only_parity(
        total_ops=4_096,
        total_cycles=106_496,
        cycles_per_op=26,
        remainder_cycles=0,
        suite_checksum=0x17,
        cpu_hz=2_700_000_000,
        out_ptr=out,
        force_commit_digest=13,
    )
    assert status == ERR_BAD_PARAM
    assert out[0] == 0xBEEF


def test_parity_canonical_mismatch_rejects() -> None:
    out = [0x99]
    status = telemetry_digest_q64_checked_nopartial_commit_only_preflight_only_parity_commit_only_preflight_only_parity_commit_only_preflight_only_parity_commit_only_preflight_only_parity(
        total_ops=2_048,
        total_cycles=53_248,
        ...
        remainder_cycles=0,
        suite_checksum=0x11,
        cpu_hz=2_300_000_000,
        out_ptr=out,
        force_parity_digest=77,
    )
    assert status == ERR_BAD_PARAM
    assert out[0] == 0x99
