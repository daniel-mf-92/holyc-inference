#!/usr/bin/env python3
"""Harness for IQ-1642 secure-local overhead-budget cross-gate strict parity wrapper."""
...
def fast_path_switch_secure_local_overhead_budget_cross_gate_checked_parity_iq1642(
    secure_local_mode: int,
    iommu_active: int,
    book_of_truth_gpu_hooks: int,
    policy_digest_parity: int,
...
def test_source_contains_iq1642_symbols() -> None:
    src = Path("src/gpu/security_perf_matrix.HC").read_text(encoding="utf-8")

    assert "I32 GPUSecurityPerfFastPathSwitchSecureLocalOverheadBudgetCrossGateCheckedParity(" in src
    assert (
        "status_commit_only =\n        GPUSecurityPerfFastPathSwitchSecureLocalOverheadBudgetCrossGateCheckedCommitOnly(" in src
    )
    assert (
        "status_preflight_only =\n        GPUSecurityPerfFastPathSwitchSecureLocalOverheadBudgetCrossGateCheckedCommitOnlyPreflightOnly(" in src
    )


def test_gate_missing_vector() -> None:
    status, out_enabled, out_reason = fast_path_switch_secure_local_overhead_budget_cross_gate_checked_parity_iq1642(
    ...
    assert (status, out_enabled, out_reason) == (GPU_SEC_PERF_ERR_POLICY_GUARD, 333, 444)
...
def test_budget_breach_vector() -> None:
    status, out_enabled, out_reason = fast_path_switch_secure_local_overhead_budget_cross_gate_checked_parity_iq1642(
        ...
        max_p50_overhead_q16=500,
        max_p95_overhead_q16=700,
        caller_fast_path_enabled=7001,
        caller_disable_reason=7002,
    )
    assert (status, out_enabled, out_reason) == (GPU_SEC_PERF_ERR_POLICY_GUARD, 7001, 7002)
...
def test_status_domain_drift_vector() -> None:
    status, out_enabled, out_reason = fast_path_switch_secure_local_overhead_budget_cross_gate_checked_parity_iq1642(
        ...
        max_p50_overhead_q16=500,
        max_p95_overhead_q16=700,
        caller_fast_path_enabled=808,
        caller_disable_reason=909,
        force_status_domain_drift=True,
    )
    assert (status, out_enabled, out_reason) == (GPU_SEC_PERF_ERR_BAD_PARAM, 808, 909)
...
def test_no_write_parity_vector() -> None:
    status, out_enabled, out_reason = fast_path_switch_secure_local_overhead_budget_cross_gate_checked_parity_iq1642(
        ...
        max_p50_overhead_q16=500,
        max_p95_overhead_q16=700,
        caller_fast_path_enabled=1234,
        caller_disable_reason=5678,
    )
    assert status == GPU_SEC_PERF_OK
    assert out_enabled == 1234
    assert out_reason == 5678


def test_deterministic_secure_on_parity_vectors() -> None:
    first = fast_path_switch_secure_local_overhead_budget_cross_gate_checked_parity_iq1642(
        secure_local_mode=GPU_SEC_PERF_PROFILE_SECURE_LOCAL,
        iommu_active=1,
        book_of_truth_gpu_hooks=1,
        policy_digest_parity=1,
        ...
        max_p50_overhead_q16=500,
        max_p95_overhead_q16=700,
        caller_fast_path_enabled=91,
        caller_disable_reason=92,
    )
    second = fast_path_switch_secure_local_overhead_budget_cross_gate_checked_parity_iq1642(
        secure_local_mode=GPU_SEC_PERF_PROFILE_SECURE_LOCAL,
        iommu_active=1,
        book_of_truth_gpu_hooks=1,
        policy_digest_parity=1,
...
if __name__ == "__main__":
    test_source_contains_iq1642_symbols()
