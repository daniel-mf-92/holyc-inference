#!/usr/bin/env python3
"""Harness for GGUFMetadataValueSkipCheckedNoPartialCommitOnlyPreflightOnlyParity (IQ-1249)."""
...
def gguf_metadata_value_skip_checked_nopartial_commit_only_preflight_only_parity_reference(
    *,
    buf: list[int] | None,
    buf_nbytes: int,
    ...
    if staged_preflight_next[0] != staged_commit_next[0]:
        return GGUF_META_TABLE_ERR_BAD_PARAM
    ...
    out_next_cursor_ref[0] = staged_preflight_next[0]
    cursor_ref[0] = staged_preflight_next[0]
    return GGUF_META_TABLE_OK


def test_source_contains_iq1249_function_and_dual_path_parity() -> None:
    source = Path("src/gguf/metadata.HC").read_text(encoding="utf-8")

    sig = "I32 GGUFMetadataValueSkipCheckedNoPartialCommitOnlyPreflightOnlyParity(U8 *buf,"
    sig_def = (
        "I32 GGUFMetadataValueSkipCheckedNoPartialCommitOnlyPreflightOnlyParity(U8 *buf,\n"
        "                                                                       U64 buf_nbytes,\n"
        "                                                                       U64 *cursor,\n"
        "                                                                       U64 table_end,\n"
        "                                                                       U32 value_type,\n"
    ...
    assert "GGUFMetadataValueSkipCheckedNoPartialCommitOnlyPreflightOnly(buf," in body
    ...
    assert "*out_next_cursor = staged_preflight_next_cursor;" in body
    assert "*cursor = staged_preflight_next_cursor;" in body
...
def test_array_string_span_success_and_truncation() -> None:
    ...
    assert err == GGUF_META_TABLE_OK
    assert cursor[0] == 5 + len(payload)
    assert out_next[0] == 5 + len(payload)

    truncated = buf[: 5 + len(payload) - 1]
    cursor2 = [5]
    out_next2 = [444]
    err = gguf_metadata_value_skip_checked_nopartial_commit_only_preflight_only_parity_reference(
        buf=truncated,
    ...
    assert err != GGUF_META_TABLE_OK
    assert cursor2[0] == 5
    assert out_next2[0] == 444


def test_randomized_parity_vectors() -> None:
    rng = random.Random(20260423_1249)
    ...
    for _ in range(1200):
        ...
        cursor_a = [cursor0]
        out_a = [0xAA]
        status_a = gguf_metadata_value_skip_checked_nopartial_commit_only_preflight_only_parity_reference(
            buf=buf,
            buf_nbytes=len(buf),
            cursor_ref=cursor_a,
            table_end=len(buf),
        ...
        cursor_b = [cursor0]
        out_b = [0xBB]
        status_b = gguf_metadata_value_skip_checked_nopartial_commit_only_preflight_only_reference(
            buf=buf,
            buf_nbytes=len(buf),
            cursor_ref=cursor_b,
            table_end=len(buf),
...
if __name__ == "__main__":
    test_source_contains_iq1249_function_and_dual_path_parity()
