#!/usr/bin/env python3
"""Harness for IQ-1487 commit-only preflight-only parity digest wrapper."""
...
def gpu_security_perf_matrix_row_gate_checked_policy_snapshot_digest_q64_commit_only_preflight_only_parity(
    secure_local_mode: int,
    iommu_active: int,
    book_of_truth_gpu_hooks: int,
    policy_digest_parity: int,
...
def test_source_contains_iq1487_symbols() -> None:
    src = Path("src/gpu/security_perf_matrix.HC").read_text(encoding="utf-8")

    assert "I32 GPUSecurityPerfMatrixRowGateCheckedPolicySnapshotDigestQ64CommitOnlyPreflightOnlyParity(" in src
    ...
    assert "if (status_commit_only != status_preflight_only)" in src
...
def test_gate_missing_vectors() -> None:
    ...
    for idx, vector in enumerate(vectors):
        ...
        seed = 8100 + idx
        status, digest = gpu_security_perf_matrix_row_gate_checked_policy_snapshot_digest_q64_commit_only_preflight_only_parity(
...
def test_bit_flip_sensitivity_vectors() -> None:
    status_base, digest_base = gpu_security_perf_matrix_row_gate_checked_policy_snapshot_digest_q64_commit_only_preflight_only_parity(
        secure_local_mode=GPU_SEC_PERF_PROFILE_SECURE_LOCAL,
        iommu_active=1,
    ...
    status_prompt, digest_prompt = gpu_security_perf_matrix_row_gate_checked_policy_snapshot_digest_q64_commit_only_preflight_only_parity(
        secure_local_mode=GPU_SEC_PERF_PROFILE_SECURE_LOCAL,
        iommu_active=1,
        book_of_truth_gpu_hooks=1,
        policy_digest_parity=1,
    ...
    status_d0, d0 = _row_gate_policy_snapshot_digest_q64_commit_only(1, 1, 1, 1, 256, 8, GPU_SEC_PERF_QUANT_Q8_0)
    status_d1, d1 = _row_gate_policy_snapshot_digest_q64_commit_only(1, 1, 1, 1, 257, 8, GPU_SEC_PERF_QUANT_Q8_0)

    assert status_base == GPU_SEC_PERF_OK
    assert status_prompt == GPU_SEC_PERF_OK
    assert (digest_base, digest_prompt) == (9000, 9000)
    assert status_d0 == GPU_SEC_PERF_OK and status_d1 == GPU_SEC_PERF_OK
    assert d0 != d1


def test_overflow_parity_vectors() -> None:
    status, digest = gpu_security_perf_matrix_row_gate_checked_policy_snapshot_digest_q64_commit_only_preflight_only_parity(
        secure_local_mode=GPU_SEC_PERF_PROFILE_SECURE_LOCAL,
        iommu_active=1,
        book_of_truth_gpu_hooks=1,
...
if __name__ == "__main__":
    test_source_contains_iq1487_symbols()
    test_gate_missing_vectors()
    test_bit_flip_sensitivity_vectors()
    test_overflow_parity_vectors()
    test_drift_rejected()
    print("ok")
