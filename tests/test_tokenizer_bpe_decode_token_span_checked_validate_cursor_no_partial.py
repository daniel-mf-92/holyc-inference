#!/usr/bin/env python3
"""Parity harness for TokenizerBPEDecodeTokenSpanCheckedValidateCursorNoPartial."""
...
def tokenizer_bpe_decode_token_span_checked_validate_cursor_no_partial(
...
def run_case(
    ...
    err_wrapped = tokenizer_bpe_decode_token_span_checked_validate_cursor_no_partial(
        token_ids,
        len(token_ids),
        cursor_wrapped,
        span_count,
...
def test_validate_cursor_rejects_overflow_and_oob() -> None:
    ...
    cursor = [4]
    err = tokenizer_bpe_decode_token_span_checked_validate_cursor_no_partial(
        [0, 1, 2],
        3,
        cursor,
        0,
...
def test_malformed_token_id_and_no_partial_outputs() -> None:
    ...
    assert err == TOKENIZER_BPE_ERR_BAD_PARAM
    assert cursor[0] == 0 and count[0] == 0x9999
    assert out == [0x33] * 64
...
def test_capacity_and_null_contracts() -> None:
    ...
    assert (
        tokenizer_bpe_decode_token_span_checked_validate_cursor_no_partial(
            None,
            0,
            cursor,
            0,
...
def test_randomized_parity_against_explicit_staged_composition() -> None:
    rng = random.Random(20260418_424)
    pieces = [
        b"a",
        b"bc",
        b"def",
...
if __name__ == "__main__":
    ...
    test_malformed_token_id_and_no_partial_outputs()
    test_capacity_and_null_contracts()
    test_randomized_parity_against_explicit_staged_composition()
    print("test_tokenizer_bpe_decode_token_span_checked_validate_cursor_no_partial: ok")
