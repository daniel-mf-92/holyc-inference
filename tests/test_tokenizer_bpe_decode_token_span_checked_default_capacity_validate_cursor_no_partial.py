#!/usr/bin/env python3
"""Parity harness for TokenizerBPEDecodeTokenSpanCheckedDefaultCapacityValidateCursorNoPartial."""
...
def tokenizer_bpe_decode_token_span_checked_default_capacity_validate_cursor_no_partial(
...
def run_case(
    ...
    err_wrapper = tokenizer_bpe_decode_token_span_checked_default_capacity_validate_cursor_no_partial(
        token_ids,
        len(token_ids),
        cursor_wrapper,
        span_count,
...
def test_validate_cursor_and_window_guards_no_partial() -> None:
    ...
    cursor = [4]
    err = tokenizer_bpe_decode_token_span_checked_default_capacity_validate_cursor_no_partial(
        [0, 1, 2],
        3,
        cursor,
        0,
    ...
    assert cursor[0] == 4 and count[0] == 0x6B6B
    assert out == [0x33] * 64
    ...
    assert err == TOKENIZER_BPE_ERR_BAD_PARAM
    assert cursor[0] == 2 and count[0] == 0x6B6B
    assert out == [0x33] * 64


def test_capacity_and_contract_guards() -> None:
    pieces = [b"A", b"BC"]
    blob, offsets, lens = build_vocab_tables(pieces)
    ...
    assert err == TOKENIZER_BPE_ERR_BAD_PARAM
    assert cursor[0] == 0 and count[0] == 0x7878
    assert out == [0x44] * 16
...
def test_randomized_parity_against_explicit_staged_core_composition() -> None:
    rng = random.Random(20260418_432)
    pieces = [
        b"a",
        b"bc",
        b"def",
...
if __name__ == "__main__":
    ...
    test_validate_cursor_and_window_guards_no_partial()
    test_capacity_and_contract_guards()
    test_randomized_parity_against_explicit_staged_core_composition()
    print(
        "test_tokenizer_bpe_decode_token_span_checked_default_capacity_validate_cursor_no_partial: ok"
    )
