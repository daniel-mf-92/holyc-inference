#!/usr/bin/env python3
"""Harness for IQ-1231 one-step preflight parity commit-only wrapper."""
...
from test_inference_generate_step_checked_topk_topp_nopartial_commit_only_preflight_only import (
    inference_generate_step_checked_topk_topp_nopartial_commit_only_preflight_only_reference,
)


def inference_generate_step_checked_topk_topp_nopartial_commit_only_preflight_only_parity_commit_only_reference(
    *,
    ...
    staged_step = [out_required_step_logits_cells[0]]
    staged_hist = [out_required_history_capacity[0]]
    staged_stage = [out_required_stage_logits_capacity[0]]
    staged_topk = [out_required_topk_capacity[0]]
    ...
    status = inference_generate_step_checked_topk_topp_nopartial_commit_only_preflight_only_reference(
        ...
        workspace_history_stage=workspace_history_stage,
        workspace_history_stage_capacity=workspace_history_stage_capacity,
        out_required_step_logits_cells=canonical_step,
        out_required_history_capacity=canonical_hist,
        out_required_stage_logits_capacity=canonical_stage,
        out_required_topk_capacity=canonical_topk,
    )
    if status != SAMPLING_Q16_OK:
        return status
    ...
    if staged_step[0] != canonical_step[0]:
        return SAMPLING_Q16_ERR_BAD_PARAM
    if staged_hist[0] != canonical_hist[0]:
        return SAMPLING_Q16_ERR_BAD_PARAM
    if staged_stage[0] != canonical_stage[0]:
        return SAMPLING_Q16_ERR_BAD_PARAM
    if staged_topk[0] != canonical_topk[0]:
        return SAMPLING_Q16_ERR_BAD_PARAM
...
def test_source_contains_iq_1231_signature_and_contract() -> None:
    source = Path("src/model/inference.HC").read_text(encoding="utf-8")
    signature = "I32 InferenceGenerateStepCheckedTopKTopPNoPartialCommitOnlyPreflightOnlyParityCommitOnly("
    ...
    assert "IQ-1231 commit-only diagnostics wrapper for one-step preflight parity gate." in source
    ...
    assert "if (staged_required_topk_capacity != canonical_required_topk_capacity)" in body
...
def test_success_and_no_publish_on_error() -> None:
    ...
    status = inference_generate_step_checked_topk_topp_nopartial_commit_only_preflight_only_parity_commit_only_reference(
        logits_q16=[0] * 64,
        logits_capacity=64,
        vocab_size=64,
...
if __name__ == "__main__":
    test_source_contains_iq_1231_signature_and_contract()
