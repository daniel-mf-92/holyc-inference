#!/usr/bin/env python3
"""Reference checks for InferenceGenerateTokensPreflightCheckedGreedyDefault (IQ-758)."""
...
def inference_generate_tokens_preflight_greedy_default_reference(
    *,
    step_logits_capacity: int,
    vocab_size: int,
    max_new_tokens: int,
...
def inference_generate_tokens_preflight_checked_commit_only_reference(
    *,
    step_logits_capacity: int,
    vocab_size: int,
    max_new_tokens: int,
...
def inference_generate_tokens_preflight_greedy_default_commit_only_reference(
    *,
    step_logits_capacity: int,
    vocab_size: int,
    max_new_tokens: int,
    ...
    return inference_generate_tokens_preflight_checked_commit_only_reference(
        step_logits_capacity=step_logits_capacity,
        vocab_size=vocab_size,
        max_new_tokens=max_new_tokens,
        token_history_capacity=token_history_capacity,
...
def test_source_contains_preflight_greedy_default_wrapper() -> None:
    source = Path("src/model/sampling.HC").read_text(encoding="utf-8")
    assert "I32 InferenceGenerateTokensPreflightCheckedGreedyDefault(" in source
    assert "temperature_q16 = SAMPLING_Q16_ONE" in source
    assert "top_k = 1" in source
    assert "top_p_q16 = SAMPLING_Q16_ONE" in source
    assert "return InferenceGenerateTokensPreflightChecked(" in source


def test_preflight_greedy_default_matches_explicit_reference_randomized() -> None:
    rng = random.Random(20260420_758)
    for _ in range(800):
        ...
        status_default, diag_default = (
            inference_generate_tokens_preflight_greedy_default_reference(
...
def test_preflight_greedy_default_preserves_failure_classification() -> None:
    cases = [
        dict(
    ...
    for case in cases:
        ...
        status_default, diag_default = (
            inference_generate_tokens_preflight_greedy_default_reference(**case)
        )
        status_explicit, diag_explicit = inference_generate_tokens_preflight_reference(
            **case
        )
...
def test_preflight_greedy_default_nullptr_contract_in_source() -> None:
    source = Path("src/model/sampling.HC").read_text(encoding="utf-8")
    wrapper_anchor = source.index(
        "I32 InferenceGenerateTokensPreflightCheckedGreedyDefault("
    )
    body = source[wrapper_anchor : wrapper_anchor + 2000]
    assert "out_required_step_logits_cells" in body
    assert "out_required_generated_capacity" in body


def test_preflight_greedy_default_success_diagnostics_exact() -> None:
    status, diagnostics = inference_generate_tokens_preflight_greedy_default_reference(
...
def test_preflight_greedy_default_commit_only_no_partial_write_parity() -> None:
    sentinel_default = {
    ...
    sentinel_explicit = dict(sentinel_default)

    # BAD_PARAM: step logits under-capacity for required vocab*tokens.
    failing = dict(
        step_logits_capacity=31,
        vocab_size=8,
        max_new_tokens=4,
        token_history_capacity=9,
        token_history_count=5,
    ...
    status_default = inference_generate_tokens_preflight_greedy_default_commit_only_reference(
        **failing,
        outputs=sentinel_default,
    )
    status_explicit = inference_generate_tokens_preflight_checked_commit_only_reference(
        **failing,
        outputs=sentinel_explicit,
    )
    assert status_default == SAMPLING_Q16_ERR_BAD_PARAM
    assert status_default == status_explicit
    assert sentinel_default == sentinel_explicit
    assert sentinel_default == {
        "required_step_logits_cells": -11,
        "required_history_capacity": -22,
        "required_stage_logits_capacity": -33,
        "required_topk_capacity": -44,
    ...
    # OVERFLOW: multiplication overflow in required_step_logits_cells.
    overflow = dict(
        step_logits_capacity=I64_MAX,
        vocab_size=(1 << 62),
        max_new_tokens=3,
        token_history_capacity=100,
        token_history_count=0,
        workspace_stage_logits_capacity=(1 << 62),
        workspace_topk_logits_capacity=(1 << 62),
        workspace_topk_index_capacity=(1 << 62),
    ...
    assert status_default == SAMPLING_Q16_ERR_OVERFLOW
    assert status_default == status_explicit
    assert sentinel_default == sentinel_explicit

    # NULL_PTR: explicit pointer contract parity.
    status_default = inference_generate_tokens_preflight_greedy_default_commit_only_reference(
        step_logits_capacity=48,
        vocab_size=12,
        max_new_tokens=4,
    ...
    assert status_default == SAMPLING_Q16_ERR_NULL_PTR
    assert status_default == status_explicit

    # Success: both paths atomically publish identical diagnostics.
    out_default = {
        "required_step_logits_cells": -1,
        "required_history_capacity": -1,
        "required_stage_logits_capacity": -1,
        "required_topk_capacity": -1,
        "required_random_capacity": -1,
        "required_generated_capacity": -1,
    }
    out_explicit = dict(out_default)
    success = dict(
        step_logits_capacity=48,
        vocab_size=12,
        max_new_tokens=4,
    ...
    status_explicit = inference_generate_tokens_preflight_checked_commit_only_reference(
        **success,
        outputs=out_explicit,
    )
    assert status_default == SAMPLING_Q16_OK
    assert status_default == status_explicit
...
if __name__ == "__main__":
    test_source_contains_preflight_greedy_default_wrapper()
    test_preflight_greedy_default_matches_explicit_reference_randomized()
    test_preflight_greedy_default_preserves_failure_classification()
    test_preflight_greedy_default_nullptr_contract_in_source()
    test_preflight_greedy_default_success_diagnostics_exact()
    test_preflight_greedy_default_commit_only_no_partial_write_parity()
    print("inference_generate_tokens_preflight_checked_greedy_default_reference_checks=ok")
