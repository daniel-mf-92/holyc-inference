...
def gguf_metadata_value_skip_checked_nopartial_commit_only_preflight_only_parity_commit_only_preflight_only_parity_commit_only_reference(
    *,
    buf: list[int] | None,
    buf_nbytes: int,
    ...
    if staged_parity_cursor[0] != staged_preflight_commit_next[0]:
    ...
    if staged_canonical_cursor[0] != staged_canonical_commit_next[0]:
        return GGUF_META_TABLE_ERR_BAD_PARAM
...
def test_source_contains_iq1259_function_and_strict_publish() -> None:
    ...
    sig_def = (
        "I32 GGUFMetadataValueSkipCheckedNoPartialCommitOnlyPreflightOnlyParityCommitOnlyPreflightOnlyParityCommitOnly(U8 *buf,\n"
        "                                                                                                              U64 buf_nbytes,\n"
        "                                                                                                              U64 *cursor,\n"
        "                                                                                                              U64 table_end,\n"
        "                                                                                                              U32 value_type,\n"
...
def test_randomized_parity_vectors() -> None:
    ...
    for _ in range(1200):
        ...
        cursor_a = [cursor0]
        out_a = [0xAA]
        status_a = gguf_metadata_value_skip_checked_nopartial_commit_only_preflight_only_parity_commit_only_preflight_only_parity_commit_only_reference(
            buf=buf,
            buf_nbytes=len(buf),
            cursor_ref=cursor_a,
            table_end=len(buf),
