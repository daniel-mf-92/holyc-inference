#!/usr/bin/env python3
"""Parity harness for GGUFReadTensorDataQ4_0CheckedNoPartialCommitOnlyPreflightOnly (IQ-936)."""
...
def test_source_contains_iq936_q4_preflight_function_and_no_write_contract() -> None:
    source = Path("src/gguf/reader.HC").read_text(encoding="utf-8")
    sig = "I32 GGUFReadTensorDataQ4_0CheckedNoPartialCommitOnlyPreflightOnly("
    assert sig in source
    body = source.split(sig, 1)[1].split("\nI32 ", 1)[0]

    assert "// Preflight-only: never writes out_q4_0_blocks, only diagnostics." in body
...
def test_null_and_bad_input_vectors_do_not_publish_or_mutate_output() -> None:
    ...
    bad_type = TensorInfoQ40(1, [64, 0, 0, 0], 8, 0)
    err = gguf_read_tensor_data_q4_0_checked_no_partial_commit_only_preflight_only(
        b"\x00" * 128, 128, 0, bad_type, out, len(out), out_blocks, out_bytes
    )
    ...
    bad_dim = TensorInfoQ40(1, [65, 0, 0, 0], GGUF_READER_GGML_TYPE_Q4_0, 0)
    err = gguf_read_tensor_data_q4_0_checked_no_partial_commit_only_preflight_only(
...
def test_capacity_snapshot_mismatch_and_success_publish_exact_counts() -> None:
    payload = bytes((idx * 5 + 11) & 0xFF for idx in range(36))
    blob = b"\x10\x11\x12\x13" + payload + b"\xEE" * 7
    out = bytearray(b"\xCC" * 96)
    out_before = bytes(out)
    ...
    info2 = TensorInfoQ40(1, [64, 0, 0, 0], GGUF_READER_GGML_TYPE_Q4_0, 4)
    err = gguf_read_tensor_data_q4_0_checked_no_partial_commit_only_preflight_only(
        blob,
        len(blob),
        0,
    ...
    info3 = TensorInfoQ40(1, [64, 0, 0, 0], GGUF_READER_GGML_TYPE_Q4_0, 4)
    err = gguf_read_tensor_data_q4_0_checked_no_partial_commit_only_preflight_only(
        blob,
        len(blob),
        0,
...
def run() -> None:
    test_source_contains_iq936_q4_preflight_function_and_no_write_contract()
    ...
    print("gguf_read_tensor_data_q4_0_checked_nopartial_commit_only_preflight_only=ok")
