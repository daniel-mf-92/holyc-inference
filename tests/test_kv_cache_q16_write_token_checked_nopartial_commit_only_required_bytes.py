#!/usr/bin/env python3
"""Parity harness for KVCacheQ16WriteToken...CommitOnlyRequiredBytes (IQ-891)."""
...
def kv_cache_q16_write_token_checked_nopartial_commit_only_required_bytes(
...
def test_source_contains_required_bytes_helper() -> None:
    ...
    sig = "I32 KVCacheQ16WriteTokenCheckedNoPartialCommitOnlyRequiredBytes("
...
def test_known_vector_required_bytes_and_commit_semantics() -> None:
    layer_count = 4
    token_capacity = 3
    kv_heads = 2
    head_dim = 6
    ...
    err = kv_cache_q16_write_token_checked_nopartial_commit_only_required_bytes(
        k_cache,
        len(k_cache),
        v_cache,
        len(v_cache),
...
def test_alias_null_and_no_partial_output_contracts() -> None:
    ...
    assert (
        kv_cache_q16_write_token_checked_nopartial_commit_only_required_bytes(
            k_cache,
            len(k_cache),
            v_cache,
            len(v_cache),
...
def test_randomized_parity_vs_explicit_composition_adversarial() -> None:
    ...
    for _ in range(800):
        ...
        err_a = kv_cache_q16_write_token_checked_nopartial_commit_only_required_bytes(
            k_cache,
            k_cache_capacity,
            v_cache,
            v_cache_capacity,
...
if __name__ == "__main__":
    ...
    test_known_vector_required_bytes_and_commit_semantics()
