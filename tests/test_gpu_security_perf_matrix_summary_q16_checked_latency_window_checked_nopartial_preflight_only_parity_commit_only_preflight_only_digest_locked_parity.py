#!/usr/bin/env python3
"""Harness for IQ-1536 latency-window strict diagnostics parity gate wrapper."""
...
def gpu_security_perf_matrix_summary_q16_checked_latency_window_checked_nopartial_preflight_only_parity_commit_only_preflight_only_digest_locked_parity(
...
def test_source_contains_iq1536_symbols() -> None:
    src = Path("src/gpu/security_perf_matrix.HC").read_text(encoding="utf-8")

    assert "I32 GPUSecurityPerfMatrixSummaryQ16CheckedLatencyWindowCheckedNoPartialPreflightOnlyParityCommitOnlyPreflightOnlyDigestLockedParity(" in src
    ...
    assert "status_canonical = GPUSecurityPerfMatrixSummaryQ16CheckedLatencyWindowCheckedNoPartialPreflightOnlyParityCommitOnlyPreflightOnlyParity(" in src
...
def test_preflight_only_success_preserves_outputs_duplicate_latency_tuple() -> None:
    ...
    status, preserved, staged = gpu_security_perf_matrix_summary_q16_checked_latency_window_checked_nopartial_preflight_only_parity_commit_only_preflight_only_digest_locked_parity(
        rows,
        rows_capacity=5,
        out_capacity=4,
...
def test_null_alias_capacity_gate_missing_and_digest_drift_vectors() -> None:
    ...
    status, *_ = gpu_security_perf_matrix_summary_q16_checked_latency_window_checked_nopartial_preflight_only_parity_commit_only_preflight_only_digest_locked_parity(
        rows,
        rows_capacity=1,
        out_capacity=4,
        secure_local_mode=GPU_SEC_PERF_PROFILE_SECURE_LOCAL,
