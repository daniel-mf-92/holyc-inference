#!/usr/bin/env python3
"""Parity harness for IQ-1183 ...PreflightOnly...ParityCommitOnlyPreflightOnly."""
...
def q8_0_dot_bench_run_default_suite_preflight_only_commit_only_parity_commit_only_preflight_only_parity_commit_only_preflight_only(
    cpu_hz: int,
...
def explicit_checked_composition(
    ...
    return q8_0_dot_bench_run_default_suite_preflight_only_commit_only_parity_commit_only_preflight_only_parity_commit_only_preflight_only(
        cpu_hz,
        out_shape_count,
        out_max_blocks,
        out_max_rows,
...
def test_source_contains_iq1183_function_and_guards() -> None:
    source = Path("src/bench/q8_0_dot_bench.HC").read_text(encoding="utf-8")
    sig = "I32 Q8_0DotBenchRunDefaultSuitePreflightOnlyCommitOnlyParityCommitOnlyPreflightOnlyParityCommitOnlyPreflightOnly("
...
def test_preflight_only_companion_no_partial_on_error() -> None:
    assert (
...
def test_preflight_only_companion_happy_path() -> None:
    out_shape = [-1]
    out_blocks = [-1]
    out_rows = [-1]
    out_row_matrix = [-1]
...
def test_preflight_only_companion_randomized_parity() -> None:
    rng = random.Random(20260422_1183)
    ...
    for _ in range(3000):
        ...
        want_status = q8_0_dot_bench_run_default_suite_preflight_only_commit_only_parity_commit_only_preflight_only_parity_commit_only(
            cpu_hz,
            want_shape,
            want_blocks,
            want_rows,
...
if __name__ == "__main__":
    test_source_contains_iq1183_function_and_guards()
    test_preflight_only_companion_no_partial_on_error()
    test_preflight_only_companion_happy_path()
    test_preflight_only_companion_randomized_parity()
    print("ok")
