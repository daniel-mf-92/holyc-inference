#!/usr/bin/env python3
"""Harness for IQ-1459 secure-on summary preflight-only diagnostics wrapper."""
...
def _summary_checked(
    ...
    for row in rows:
        if row.tok_per_sec_q16 < 0 or row.audit_overhead_delta_q16 < 0 or row.secure_cycles_per_token_q16 < 0:
            return GPU_SEC_PERF_ERR_BAD_PARAM, 0, 0, 0, 0
        tok_next = _add_checked(tok_total, row.tok_per_sec_q16)
        overhead_next = _add_checked(overhead_total, row.audit_overhead_delta_q16)
        if tok_next is None or overhead_next is None:
            return GPU_SEC_PERF_ERR_OVERFLOW, 0, 0, 0, 0
        tok_total = tok_next
        overhead_total = overhead_next
...
def gpu_security_perf_matrix_summary_q16_checked_nopartial_preflight_only(
    rows: list[RowOutput],
    rows_capacity: int,
    out_capacity: int,
    secure_local_mode: int,
...
def test_source_contains_iq1459_symbols() -> None:
    src = Path("src/gpu/security_perf_matrix.HC").read_text(encoding="utf-8")

    assert "I32 GPUSecurityPerfMatrixSummaryQ16CheckedNoPartialPreflightOnly(" in src
    assert "saved_tok_per_sec_q16" in src
    assert "status_nopartial" in src
    assert "status_canonical" in src
    assert "snapshot_after_digest_q64" in src


def test_preflight_only_preserves_outputs_and_matches_secure_on_tuple() -> None:
    rows = [
        RowOutput(100_000, 1_000, 10_000),
        RowOutput(110_000, 3_000, 20_000),
        RowOutput(120_000, 2_000, 30_000),
    ]
    status, outputs_after, diag_tuple = gpu_security_perf_matrix_summary_q16_checked_nopartial_preflight_only(
        rows,
        rows_capacity=3,
        out_capacity=4,
    ...
    assert diag_tuple == (330_000, 2_000, 20_000, 30_000)


def test_preflight_only_fail_closed_gate_vectors() -> None:
...
if __name__ == "__main__":
    test_source_contains_iq1459_symbols()
    test_preflight_only_preserves_outputs_and_matches_secure_on_tuple()
    test_preflight_only_fail_closed_gate_vectors()
    print("ok")
