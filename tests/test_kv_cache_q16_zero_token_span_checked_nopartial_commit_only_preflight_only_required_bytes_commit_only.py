#!/usr/bin/env python3
"""Parity harness for KVCacheQ16ZeroTokenSpan...RequiredBytesCommitOnly (IQ-898)."""
...
def kv_cache_q16_zero_token_span_checked_nopartial_commit_only_preflight_only_required_bytes_commit_only(
    ...
    if staged_required_span_cells[0] < 0:
        return KV_Q16_ERR_BAD_PARAM

    err, staged_required_token_bytes_recomputed = try_mul_i64_checked(
        staged_required_span_cells[0],
        8,
    )
    if err != KV_Q16_OK:
    ...
    if staged_required_token_bytes_recomputed != staged_required_token_bytes[0]:
        return KV_Q16_ERR_BAD_PARAM
...
def explicit_commit_only_composition(*args, **kwargs) -> tuple[int, tuple[int, int, int, int, int]]:
    span_out = [0]
    bytes_out = [0]
    k_base_out = [0]
    v_base_out = [0]
    ...
    err = kv_cache_q16_zero_token_span_checked_nopartial_commit_only_preflight_only_required_bytes(
        *args,
        ...
        out_required_token_bytes=bytes_out,
    ...
    err, bytes_recomputed = try_mul_i64_checked(span_out[0], 8)
    if err != KV_Q16_OK:
        return err, (0, 0, 0, 0, 0)

    if bytes_recomputed != bytes_out[0]:
        return KV_Q16_ERR_BAD_PARAM, (0, 0, 0, 0, 0)

    err, k_end = try_add_i64_checked(k_base_out[0], span_out[0])
    if err != KV_Q16_OK:
        return err, (0, 0, 0, 0, 0)

    err, v_end = try_add_i64_checked(v_base_out[0], span_out[0])
    if err != KV_Q16_OK:
        return err, (0, 0, 0, 0, 0)

    if k_end != end_out[0] or v_end != end_out[0]:
        return KV_Q16_ERR_BAD_PARAM, (0, 0, 0, 0, 0)
    if end_out[0] < k_base_out[0] or end_out[0] < v_base_out[0]:
        return KV_Q16_ERR_BAD_PARAM, (0, 0, 0, 0, 0)

    return KV_Q16_OK, (span_out[0], bytes_out[0], k_base_out[0], v_base_out[0], end_out[0])


def test_source_contains_required_bytes_commit_only_helper() -> None:
    source = Path("src/model/kv_cache.HC").read_text(encoding="utf-8")
    sig = "I32 KVCacheQ16ZeroTokenSpanCheckedNoPartialCommitOnlyPreflightOnlyRequiredBytesCommitOnly("
    ...
    assert "if (staged_required_token_bytes_recomputed != staged_required_token_bytes)" in body
...
def test_known_vector_commit_only_required_bytes_and_zero_write() -> None:
    layer_count = 3
    token_capacity = 4
    kv_heads = 2
    head_dim = 5
...
def test_randomized_parity_vs_explicit_composition_adversarial() -> None:
    ...
    for _ in range(1000):
        ...
        err_a = kv_cache_q16_zero_token_span_checked_nopartial_commit_only_preflight_only_required_bytes_commit_only(
            k_cache,
            k_cache_capacity,
            v_cache,
            v_cache_capacity,
...
if __name__ == "__main__":
    test_source_contains_required_bytes_commit_only_helper()
    test_known_vector_commit_only_required_bytes_and_zero_write()
