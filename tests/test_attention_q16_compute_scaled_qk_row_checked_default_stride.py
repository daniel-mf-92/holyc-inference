...
def attention_q16_compute_scaled_qk_row_checked_default_stride(
    ...
    return attention_q16_compute_scaled_qk_row_checked(
        q_row_q16,
        q_row_capacity,
        k_rows_q16,
        k_rows_capacity,
...
def test_source_contains_default_stride_wrapper() -> None:
    ...
    assert "return AttentionQ16ComputeScaledQKRowChecked(" in body
...
def test_error_parity() -> None:
    ...
    k_big = [1 << 16, 2 << 16, 3 << 16, 4 << 16, 5 << 16, 6 << 16, 7 << 16, 8 << 16, 9 << 16]
    out_a = [111] * 6
    out_b = out_a.copy()
...
if __name__ == "__main__":
    test_source_contains_default_stride_wrapper()
