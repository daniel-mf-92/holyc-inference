#!/usr/bin/env python3
"""Parity harness for KVCacheInitQ16CheckedNoAllocCommitOnly (IQ-1189)."""
...
from test_kv_cache_q16_total_cells_checked import kv_cache_q16_compute_total_cells_checked


# Mirrors HolyC wrapper in src/model/kv_cache.HC.
def kv_cache_q16_init_checked_noalloc_commit_only(
    ...
    err, staged_layer_span_cells = try_mul_i64_checked(
        token_capacity,
        staged_span_cells[0],
    )
    if err != KV_Q16_OK:
    ...
    if staged_total_cells[0] > k_cache_capacity or staged_total_cells[0] > v_cache_capacity:
        return KV_Q16_ERR_BAD_PARAM
    ...
    commit_used_tokens = [0x55]
    commit_layer_span_cells = [0x66]
    commit_total_cells = [0x77]
    ...
    if commit_used_tokens[0] != 0:
        return KV_Q16_ERR_BAD_PARAM

    if (
        commit_layer_span_cells[0] != staged_layer_span_cells
        or commit_total_cells[0] != staged_total_cells[0]
    ):
        return KV_Q16_ERR_BAD_PARAM

    out_used_tokens[0] = commit_used_tokens[0]
    out_layer_span_cells[0] = commit_layer_span_cells[0]
    out_total_cells[0] = commit_total_cells[0]
    return KV_Q16_OK


def test_source_contains_commit_only_init_symbol() -> None:
    source = Path("src/model/kv_cache.HC").read_text(encoding="utf-8")
    sig = "I32 KVCacheInitQ16CheckedNoAllocCommitOnly("
    ...
    assert "if (commit_used_tokens != 0)" in body
    assert "*out_used_tokens = commit_used_tokens;" in body
    assert "*out_layer_span_cells = commit_layer_span_cells;" in body
...
def run() -> None:
    test_source_contains_commit_only_init_symbol()
    ...
    print("kv_cache_init_q16_checked_noalloc_commit_only_reference_checks=ok")
