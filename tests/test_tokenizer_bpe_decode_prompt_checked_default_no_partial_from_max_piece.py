#!/usr/bin/env python3
"""Parity harness for TokenizerBPEDecodePromptCheckedDefaultNoPartialFromMaxPiece."""
...
from test_tokenizer_bpe_decode_token_span_checked_default_no_partial import (
...
def tokenizer_bpe_decode_prompt_checked_default_no_partial_from_max_piece(
...
def run_case(
    ...
    out_explicit = [0xA3] * 1024
    out_wrapper = out_explicit.copy()
    count_explicit = [0x6262]
    count_wrapper = [0x6262]
    ...
    if len(token_ids) and max_piece_bytes > I64_MAX // len(token_ids):
        ...
        err_explicit = tokenizer_bpe_decode_token_span_checked_default_no_partial(
            token_ids,
            len(token_ids),
            staged_cursor,
        ...
        if err_explicit == TOKENIZER_BPE_OK:
            ...
            elif staged_count[0] > derived_capacity:
                err_explicit = TOKENIZER_BPE_ERR_BAD_PARAM
            else:
                for idx in range(staged_count[0]):
    ...
    err_wrapper = tokenizer_bpe_decode_prompt_checked_default_no_partial_from_max_piece(
        token_ids,
        len(token_ids),
        cursor_wrapper,
        blob,
...
def test_multilingual_parity_vs_span_wrapper_composition() -> None:
    pieces = [
        b"hello",
        b" ",
        b"world",
    ...
    run_case(token_ids, 5, pieces, max_piece_bytes=16)


def test_cursor_and_malformed_token_no_partial_guarantees() -> None:
    ...
    cursor = [4]
    err = tokenizer_bpe_decode_prompt_checked_default_no_partial_from_max_piece(
        [0, 1, 2],
        3,
        cursor,
        blob,
    ...
    assert cursor[0] == 4 and count[0] == 0x8888
    assert out == [0x5E] * 64
    ...
    assert err == TOKENIZER_BPE_ERR_BAD_PARAM
    assert cursor[0] == 0 and count[0] == 0x8888
    assert out == [0x5E] * 64
...
def test_overflow_and_null_contracts() -> None:
    ...
    assert (
        tokenizer_bpe_decode_prompt_checked_default_no_partial_from_max_piece(
            None,
            0,
            cursor,
            blob,
...
def test_randomized_parity() -> None:
    rng = random.Random(20260418_431)
    pieces = [b"a", b"bb", b"ccc", "世界".encode("utf-8"), "🙂".encode("utf-8"), b"\n"]
...
if __name__ == "__main__":
    test_multilingual_parity_vs_span_wrapper_composition()
    test_cursor_and_malformed_token_no_partial_guarantees()
    test_overflow_and_null_contracts()
    test_randomized_parity()
    print("test_tokenizer_bpe_decode_prompt_checked_default_no_partial_from_max_piece: ok")
