#!/usr/bin/env python3
"""Reference checks for GGUFMetadataReadU8TripleChecked semantics."""
...
def test_null_ptr_and_no_partial_write() -> None:
    ...
    assert (
        gguf_metadata_read_u8_triple_checked(
            None,
            8,
            cursor,
            8,
...
def test_short_read_does_not_commit_first_second_or_cursor() -> None:
    buf = [0xAA, 0xBB, 0xCC]
    cursor = [1]
    out_first = [0x44]
    out_second = [0x55]
    ...
    assert out_third == [0x66]


def test_success_reads_three_bytes_and_advances() -> None:
    buf = [10, 20, 30, 40, 50]
    cursor = [1]
    out_first = [0]
    out_second = [0]
    ...
    assert out_third[0] == 40
    assert cursor[0] == 4


def test_randomized_parity() -> None:
    rng = random.Random(20260417_245)
    ...
    for _ in range(6000):
        ...
        if cursor0 + 3 <= table_end and cursor0 + 3 <= n:
            ...
            assert out_third[0] == 77
            assert cursor[0] == cursor0


if __name__ == "__main__":
    test_null_ptr_and_no_partial_write()
    test_short_read_does_not_commit_first_second_or_cursor()
    test_success_reads_three_bytes_and_advances()
    test_randomized_parity()
    print("gguf_metadata_read_u8_triple_checked_reference_checks=ok")
