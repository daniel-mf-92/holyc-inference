#!/usr/bin/env python3
"""Parity harness for ...PreflightOnlyDefaultCapacityCommitOnlyParityNoAllocHardenedDefault."""
...
from test_tokenizer_bpe_encode_prompt_checked_noalloc_from_lens_commit_only_required_bytes_preflight_only_default_capacity_commit_only_parity_noalloc_hardened import (
    tokenizer_bpe_encode_prompt_checked_noalloc_from_lens_commit_only_required_bytes_preflight_only_default_capacity_commit_only_parity_noalloc_hardened,
)


def tokenizer_bpe_encode_prompt_checked_noalloc_from_lens_commit_only_required_bytes_preflight_only_default_capacity_commit_only_parity_noalloc_hardened_default(
    data: list[int] | None,
    byte_len: int,
    ...
    err = tokenizer_bpe_encode_prompt_checked_noalloc_from_lens_commit_only_required_bytes_preflight_only_default_capacity_commit_only_parity_noalloc_hardened(
        data,
        byte_len,
        [initial_cursor],
        prompt_nbytes,
...
def test_source_contains_signature_and_default_hardening_wrapper() -> None:
    source = Path("src/tokenizer/bpe.HC").read_text(encoding="utf-8")
    sig = "I32 TokenizerBPEEncodePromptCheckedNoAllocFromLensCommitOnlyRequiredBytesPreflightOnlyDefaultCapacityCommitOnlyParityNoAllocHardenedDefault("
    ...
    assert "TokenizerBPEEncodePromptCheckedNoAllocFromLensCommitOnlyRequiredBytesPreflightOnlyDefaultCapacityCommitOnlyParityNoAllocHardened(" in body
...
def test_success_error_no_write_parity_and_sentinel_preservation() -> None:
    ...
    err_a = tokenizer_bpe_encode_prompt_checked_noalloc_from_lens_commit_only_required_bytes_preflight_only_default_capacity_commit_only_parity_noalloc_hardened_default(
        payload,
        len(payload),
        cur_a,
        good_prompt_nbytes,
...
def test_null_overflow_and_fuzz_parity() -> None:
    ...
    err = tokenizer_bpe_encode_prompt_checked_noalloc_from_lens_commit_only_required_bytes_preflight_only_default_capacity_commit_only_parity_noalloc_hardened_default(
        None,
        0,
        [0],
        0,
...
if __name__ == "__main__":
    test_source_contains_signature_and_default_hardening_wrapper()
