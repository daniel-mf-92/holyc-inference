#!/usr/bin/env python3
"""Parity harness for ...NoAllocHardenedCommitOnlyPreflightOnlyRequiredBytes (IQ-795)."""
...
from test_attention_q16_apply_score_scale_rows_checked_nopartial_preflight_only_default_stride_required_stage_bytes_default_capacity_noalloc_hardened_commit_only_preflight_only import (
    attention_q16_apply_score_scale_rows_checked_nopartial_preflight_only_default_stride_required_stage_bytes_default_capacity_noalloc_hardened_commit_only_preflight_only,
)

I64_SIZE = 8


def attention_q16_apply_score_scale_rows_checked_nopartial_preflight_only_default_stride_required_stage_bytes_default_capacity_noalloc_hardened_commit_only_preflight_only_required_bytes(
    in_scores_q32,
    ...
    err, recomputed_required_stage_bytes = try_mul_i64(
...
def explicit_checked_composition(
    ...
    commit_preflight_required_in_cells = [0]
    commit_preflight_required_out_cells = [0]
    commit_preflight_required_stage_cells = [0]
    commit_preflight_required_stage_bytes = [0]
    commit_preflight_last_in_index = [0]
    commit_preflight_last_out_index = [0]
    ...
    if commit_preflight_required_in_cells[0] != recomputed_required_stage_cells:
        return ATTN_Q16_ERR_BAD_PARAM
    if commit_preflight_required_out_cells[0] != recomputed_required_stage_cells:
        return ATTN_Q16_ERR_BAD_PARAM
    if commit_preflight_required_stage_cells[0] != recomputed_required_stage_cells:
        return ATTN_Q16_ERR_BAD_PARAM
    if commit_preflight_required_stage_bytes[0] != recomputed_required_stage_bytes:
        return ATTN_Q16_ERR_BAD_PARAM
    if commit_preflight_last_in_index[0] != expected_last_index:
        return ATTN_Q16_ERR_BAD_PARAM
    if commit_preflight_last_out_index[0] != expected_last_index:
        return ATTN_Q16_ERR_BAD_PARAM

    out_required_in_cells[0] = commit_preflight_required_in_cells[0]
    out_required_out_cells[0] = commit_preflight_required_out_cells[0]
    out_required_stage_cells[0] = commit_preflight_required_stage_cells[0]
    out_required_stage_bytes[0] = commit_preflight_required_stage_bytes[0]
    out_last_in_index[0] = commit_preflight_last_in_index[0]
    out_last_out_index[0] = commit_preflight_last_out_index[0]
    return ATTN_Q16_OK


def test_source_contains_default_capacity_noalloc_hardened_commit_only_preflight_only_required_bytes() -> None:
    source = Path("src/model/attention.HC").read_text(encoding="utf-8")
    assert (
        "I32 AttentionQ16ApplyScoreScaleRowsCheckedNoPartialPreflightOnlyDefaultStride"
        "RequiredStageBytesDefaultCapacityNoAllocHardenedCommitOnlyPreflightOnlyRequiredBytes("
    ) in source
    assert (
    ...
    assert "IQ-795 required-bytes diagnostics contract" in source


def test_commit_only_preflight_only_required_bytes_nullptr_and_alias_contracts() -> None:
    out_a = [111]
    out_b = [222]
    out_c = [333]
    out_d = [444]
...
def test_commit_only_preflight_only_required_bytes_overflow_propagation() -> None:
...
def test_commit_only_preflight_only_required_bytes_matches_explicit_checked_composition_randomized() -> None:
    rng = random.Random(20260420_795)

    for _ in range(1000):
        row_count = rng.randint(0, 96)
        token_count = rng.randint(0, 96)
        required = row_count * token_count
        slack_in = rng.randint(0, 6)
        slack_out = rng.randint(0, 6)
        ...
        err_a = attention_q16_apply_score_scale_rows_checked_nopartial_preflight_only_default_stride_required_stage_bytes_default_capacity_noalloc_hardened_commit_only_preflight_only_required_bytes(
            in_scores,
            in_scores_capacity,
            row_count,
            token_count,
        ...
        if err_a == ATTN_Q16_OK:
            ...
            assert b_required_in == [888]
            assert b_required_out == [888]
            assert b_required_stage == [888]
            assert b_required_bytes == [888]
            assert b_last_in == [888]
            assert b_last_out == [888]


if __name__ == "__main__":
    test_source_contains_default_capacity_noalloc_hardened_commit_only_preflight_only_required_bytes()
    test_commit_only_preflight_only_required_bytes_nullptr_and_alias_contracts()
    test_commit_only_preflight_only_required_bytes_overflow_propagation()
    test_commit_only_preflight_only_required_bytes_matches_explicit_checked_composition_randomized()
    print(
        "attention_q16_apply_score_scale_rows_checked_nopartial_preflight_only_"
        "default_stride_required_stage_bytes_default_capacity_noalloc_"
        "hardened_commit_only_preflight_only_required_bytes=ok"
    )
