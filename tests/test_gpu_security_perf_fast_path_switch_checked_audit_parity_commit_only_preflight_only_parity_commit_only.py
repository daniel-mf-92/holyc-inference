#!/usr/bin/env python3
"""Harness for IQ-1580 commit-only hardening wrapper over fast-path preflight parity gates."""
...
def fast_path_switch_checked_audit_parity_commit_only_preflight_only_parity_commit_only(
    secure_local_mode: int,
    iommu_active: int,
    book_of_truth_gpu_hooks: int,
    policy_digest_parity: int,
    ...
    *,
    ...
    if force_tuple_parity_drift:
        staged_reason = GPU_SEC_PERF_FAST_PATH_DISABLE_REASON_BOOK_GUARD
...
def test_source_contains_iq1580_symbols() -> None:
    src = Path("src/gpu/security_perf_matrix.HC").read_text(encoding="utf-8")

    assert "I32 GPUSecurityPerfFastPathSwitchCheckedAuditParityCommitOnlyPreflightOnlyParityCommitOnly(" in src
...
def test_gate_missing_and_audit_parity_breach_vectors() -> None:
    status, enabled, reason = fast_path_switch_checked_audit_parity_commit_only_preflight_only_parity_commit_only(
    ...
    assert (status, enabled, reason) == (GPU_SEC_PERF_ERR_POLICY_GUARD, 61, 62)
...
def test_null_and_alias_vectors() -> None:
    ...
    assert (status, enabled, reason) == (GPU_SEC_PERF_ERR_NULL_PTR, 101, 102)
