#!/usr/bin/env python3
"""Reference checks for GQAAttentionValueMixQ16CheckedNoPartialCommitOnlyPreflightDefaultStrideParity (IQ-1416)."""
...
def gqa_attention_value_mix_q16_checked_nopartial_commit_only_preflight(
    scores_q16,
    scores_capacity: int,
    query_rows: int,
    key_rows: int,
    ...
    # IQ-1416 compatibility helper: commit-only preflight maps to the checked
    # immutable tuple preflight helper semantics.
    return gqa_attention_value_mix_q16_checked_nopartial_preflight(
        scores_q16,
        scores_capacity,
        query_rows,
        key_rows,
...
def gqa_attention_value_mix_q16_checked_nopartial_commit_only_preflight_default_stride(
    scores_q16,
    scores_capacity: int,
    query_rows: int,
    key_rows: int,
    ...
    # IQ-1416 compatibility helper: default-stride commit-only preflight maps
    # to existing default-stride immutable tuple preflight helper.
    return gqa_attention_value_mix_q16_checked_nopartial_preflight_default_stride(
        scores_q16,
        scores_capacity,
        query_rows,
        key_rows,
...
def gqa_attention_value_mix_q16_checked_nopartial_commit_only_preflight_default_stride_parity(
    scores_q16,
    scores_capacity: int,
    query_rows: int,
    key_rows: int,
    ...
    err = gqa_attention_value_mix_q16_checked_nopartial_commit_only_preflight_default_stride(
        scores_q16,
        scores_capacity,
        query_rows,
        key_rows,
    ...
    staged_canonical_required_score_cells = [0]
    staged_canonical_required_value_cells = [0]
    staged_canonical_required_out_cells = [0]
    err = gqa_attention_value_mix_q16_checked_nopartial_commit_only_preflight(
        scores_q16,
        scores_capacity,
        query_rows,
        key_rows,
    ...
    if (
        staged_default_required_score_cells[0] != staged_canonical_required_score_cells[0]
        or staged_default_required_value_cells[0] != staged_canonical_required_value_cells[0]
        or staged_default_required_out_cells[0] != staged_canonical_required_out_cells[0]
    ):
        return ATTN_Q16_ERR_BAD_PARAM

    if (
        staged_default_required_score_cells[0] != recomputed_required_score_cells
        or staged_default_required_value_cells[0] != recomputed_required_value_cells
        or staged_default_required_out_cells[0] != recomputed_required_out_cells
    ):
        return ATTN_Q16_ERR_BAD_PARAM

    if (
        staged_default_required_score_cells[0] > snapshot_scores_capacity
        or staged_default_required_value_cells[0] > snapshot_values_capacity
        or staged_default_required_out_cells[0] > snapshot_out_capacity
    ):
        return ATTN_Q16_ERR_BAD_PARAM
    ...
    out_required_score_cells[0] = staged_default_required_score_cells[0]
    out_required_value_cells[0] = staged_default_required_value_cells[0]
    out_required_out_cells[0] = staged_default_required_out_cells[0]
    return ATTN_Q16_OK


def explicit_commit_only_preflight_default_stride_parity_composition(*args) -> int:
    scores_q16 = args[0]
...
def test_fixed_vector_reference_tuple_publish_no_writes() -> None:
    ...
    err = gqa_attention_value_mix_q16_checked_nopartial_commit_only_preflight_default_stride_parity(
        scores,
        len(scores),
        query_rows,
        key_rows,
...
def test_null_alias_capacity_tail_stride_vectors() -> None:
    out = [99] * 8
    required_score = [1]
    required_value = [2]
    required_out = [3]
    ...
    # Tail-stride sanity vector: canonical helper with extra row tail should
    # require more score cells than the default-stride parity gate permits.
    scores = [1] * 16
    values = [2] * 12
    out_values = [3] * 8
    default_score = [0]
    ...
    assert err == ATTN_Q16_OK
    assert tail_score[0] > default_score[0]
    assert tail_value[0] == default_value[0]
    assert tail_out[0] == default_out[0]


def test_randomized_equivalence_with_explicit_composition() -> None:
    rng = random.Random(20260425_1416)
    ...
    for _ in range(240):
        ...
        err_a = gqa_attention_value_mix_q16_checked_nopartial_commit_only_preflight_default_stride_parity(
            scores,
            len(scores),
            query_rows,
            key_rows,
        ...
        err_b = explicit_commit_only_preflight_default_stride_parity_composition(
            scores,
            len(scores),
            query_rows,
            key_rows,
...
def test_source_contract_markers() -> None:
    ...
    marker_default = "I32 GQAAttentionValueMixQ16CheckedNoPartialCommitOnlyPreflightDefaultStride("
    marker_canonical = "I32 GQAAttentionValueMixQ16CheckedNoPartialCommitOnlyPreflight("
    marker_parity = "I32 GQAAttentionValueMixQ16CheckedNoPartialCommitOnlyPreflightDefaultStrideParity("

    assert marker_default in source
    assert marker_canonical in source
    assert marker_parity in source

    parity_body = source.split(marker_parity, 1)[1]
    assert "status = GQAAttentionValueMixQ16CheckedNoPartialCommitOnlyPreflightDefaultStride(" in parity_body
    assert "status = GQAAttentionValueMixQ16CheckedNoPartialCommitOnlyPreflight(" in parity_body
    assert "head_groups," in parity_body
    assert "key_rows," in parity_body


if __name__ == "__main__":
    test_fixed_vector_reference_tuple_publish_no_writes()
    test_null_alias_capacity_tail_stride_vectors()
    test_randomized_equivalence_with_explicit_composition()
    test_source_contract_markers()
    print("gqa_attention_value_mix_q16_checked_nopartial_commit_only_preflight_default_stride_parity_reference_checks=ok")
