#!/usr/bin/env python3
"""Reference checks for GQAAttentionValueMixQ16CheckedNoPartialCommitOnlyPreflightOnly (IQ-1378)."""
...
def gqa_attention_value_mix_q16_checked_nopartial_commit_only_preflight_only(
    scores_q16,
    scores_capacity: int,
    query_rows: int,
    key_rows: int,
    ...
    snapshot_out = out_values_q16[:required_out_cells]
    ...
    for idx in range(required_out_cells):
        if staged_commit[idx] != staged_canonical[idx]:
            return ATTN_Q16_ERR_BAD_PARAM

    for idx in range(required_out_cells):
        if out_values_q16[idx] != snapshot_out[idx]:
            return ATTN_Q16_ERR_BAD_PARAM
...
def explicit_preflight_only_composition(*args, **kwargs) -> int:
    return gqa_attention_value_mix_q16_checked_nopartial_commit_only_preflight_only(*args, **kwargs)
...
def test_fixed_vector_reference() -> None:
    ...
    seed = [9090] * (query_rows * value_dim)
    out_a = seed.copy()
    out_b = seed.copy()

    err_a = gqa_attention_value_mix_q16_checked_nopartial_commit_only_preflight_only(
        scores_q16,
        len(scores_q16),
        query_rows,
        key_rows,
...
def test_null_capacity_overflow_and_no_write_contract() -> None:
    seed = [9191] * 8
...
def test_randomized_preflight_only_parity() -> None:
    rng = random.Random(20260425_1378)
    ...
    for _ in range(220):
        ...
        seed = [rng.randint(-777, 777) for _ in range(out_cells)]
...
def test_source_contract_markers() -> None:
    ...
    marker = "I32 GQAAttentionValueMixQ16CheckedNoPartialCommitOnlyPreflightOnly("
...
if __name__ == "__main__":
    ...
    test_null_capacity_overflow_and_no_write_contract()
    test_randomized_preflight_only_parity()
    test_source_contract_markers()
    print("gqa_attention_value_mix_q16_checked_nopartial_commit_only_preflight_only_reference_checks=ok")
