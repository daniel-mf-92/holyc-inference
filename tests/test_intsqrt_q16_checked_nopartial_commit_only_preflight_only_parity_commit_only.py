#!/usr/bin/env python3
"""Harness for FPQ16SqrtCheckedNoPartialCommitOnlyPreflightOnlyParityCommitOnly (IQ-1237)."""
...
def fpq16_sqrt_checked_nopartial_commit_only_preflight_only_parity_commit_only_reference(
    x_q16: int,
    ...
    *,
    ...
    if staged_required[0] != canonical_required_out_cells:
        return FP_Q16_ERR_BAD_PARAM
...
def test_source_contains_iq1237_function() -> None:
    source = Path("src/math/intsqrt.HC").read_text(encoding="utf-8")
    sig = "I32 FPQ16SqrtCheckedNoPartialCommitOnlyPreflightOnlyParityCommitOnly("
...
def test_null_domain_overflow_and_no_publish() -> None:
    err = fpq16_sqrt_checked_nopartial_commit_only_preflight_only_parity_commit_only_reference(
        123,
        None,
    )
...
def test_success_and_explicit_composition_parity() -> None:
    ...
    for x_q16 in vectors:
        ...
        err_a = fpq16_sqrt_checked_nopartial_commit_only_preflight_only_parity_commit_only_reference(
            x_q16,
            out_a,
        )
...
def test_randomized_parity_against_component() -> None:
    rng = random.Random(20260423_1237)
...
if __name__ == "__main__":
    test_source_contains_iq1237_function()
    ...
    print("intsqrt_q16_checked_nopartial_commit_only_preflight_only_parity_commit_only=ok")
