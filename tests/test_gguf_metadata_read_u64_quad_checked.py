#!/usr/bin/env python3
"""Reference checks for GGUFMetadataReadU64QuadChecked semantics."""
...
def test_null_ptr_and_no_partial_write() -> None:
    ...
    assert (
        gguf_metadata_read_u64_quad_checked(
            None,
            64,
            cursor,
            64,
...
def test_fourth_scalar_fail_does_not_commit_or_advance() -> None:
    first = 0x0123456789ABCDEF
    second = 0x0F1E2D3C4B5A6978
    third = 0xFFEEDDCCBBAA9988
    ...
    err = gguf_metadata_read_u64_quad_checked(
        buf,
        len(buf),
        cursor,
        len(buf),
...
def test_randomized_parity() -> None:
    rng = random.Random(20260417_277)
    ...
    for _ in range(6000):
        ...
        if cursor0 + 32 <= table_end:
            ...
            for i in range(8):
                ...
                third |= buf[cursor0 + 16 + i] << (8 * i)
                fourth |= buf[cursor0 + 24 + i] << (8 * i)
            assert out_first[0] == first
            assert out_second[0] == second
            assert out_third[0] == third
            assert out_fourth[0] == fourth
            ...
            assert out_first[0] == 0x11
            assert out_second[0] == 0x22
            assert out_third[0] == 0x33
            assert out_fourth[0] == 0x44
