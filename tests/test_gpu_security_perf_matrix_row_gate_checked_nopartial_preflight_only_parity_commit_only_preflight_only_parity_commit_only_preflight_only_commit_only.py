#!/usr/bin/env python3
"""Harness for IQ-1545 commit-only wrapper over preflight-only parity + commit-only parity row-gate wrappers."""
...
def _row_gate_checked_nopartial_preflight_only_parity_commit_only_preflight_only_parity_commit_only(
    secure_local_mode: int,
    iommu_active: int,
    book_of_truth_gpu_hooks: int,
    policy_digest_parity: int,
...
def _row_gate_checked_nopartial_preflight_only_parity_commit_only_preflight_only_parity_commit_only_preflight_only(
    secure_local_mode: int,
    iommu_active: int,
    book_of_truth_gpu_hooks: int,
    policy_digest_parity: int,
    ...
    status_commit_only, staged_reason, staged_allowed = _row_gate_checked_nopartial_preflight_only_parity_commit_only_preflight_only_parity_commit_only(
        secure_local_mode,
        iommu_active,
        book_of_truth_gpu_hooks,
        policy_digest_parity,
    ...
    status_parity, canonical_reason, canonical_allowed = _row_gate_checked_nopartial_preflight_only_parity_commit_only_preflight_only_parity(
        secure_local_mode,
        iommu_active,
        book_of_truth_gpu_hooks,
        policy_digest_parity,
...
def row_gate_checked_nopartial_preflight_only_parity_commit_only_preflight_only_parity_commit_only_preflight_only_commit_only(
    secure_local_mode: int,
    iommu_active: int,
    book_of_truth_gpu_hooks: int,
    policy_digest_parity: int,
    ...
    status_preflight_only, parity_reason, parity_allowed = _row_gate_checked_nopartial_preflight_only_parity_commit_only_preflight_only_parity_commit_only_preflight_only(
        secure_local_mode,
        iommu_active,
        book_of_truth_gpu_hooks,
        policy_digest_parity,
    ...
    if not _is_valid_status(status_preflight_only) or not _is_valid_status(status_commit_only):
        return GPU_SEC_PERF_ERR_BAD_PARAM, current_gate_reason_code, current_row_allowed
...
def test_source_contains_iq1545_symbols() -> None:
    src = Path("src/gpu/security_perf_matrix.HC").read_text(encoding="utf-8")

    assert "I32 GPUSecurityPerfMatrixRowGateCheckedNoPartialPreflightOnlyParityCommitOnlyPreflightOnlyParityCommitOnlyPreflightOnlyCommitOnly(" in src
    assert "status_preflight_only = GPUSecurityPerfMatrixRowGateCheckedNoPartialPreflightOnlyParityCommitOnlyPreflightOnlyParityCommitOnlyPreflightOnly(" in src
    assert "status_commit_only = GPUSecurityPerfMatrixRowGateCheckedNoPartialPreflightOnlyParityCommitOnlyPreflightOnlyParityCommitOnly(" in src
...
def test_null_alias_capacity_vectors() -> None:
    status, reason, allowed = row_gate_checked_nopartial_preflight_only_parity_commit_only_preflight_only_parity_commit_only_preflight_only_commit_only(
...
def test_gate_missing_status_drift_and_reason_parity_vectors() -> None:
    status, reason, allowed = row_gate_checked_nopartial_preflight_only_parity_commit_only_preflight_only_parity_commit_only_preflight_only_commit_only(
        secure_local_mode=GPU_SEC_PERF_PROFILE_SECURE_LOCAL,
        iommu_active=0,
        book_of_truth_gpu_hooks=1,
...
if __name__ == "__main__":
    test_source_contains_iq1545_symbols()
    test_null_alias_capacity_vectors()
    test_gate_missing_status_drift_and_reason_parity_vectors()
