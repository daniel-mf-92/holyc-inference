#!/usr/bin/env python3
"""Harness for IQ-1746 fast-path batch audit commit-only parity wrapper."""
...
HC_FN_IQ1746 = "GPUSecurityPerfFastPathSwitchBatchAuditQ64CheckedCommitOnlyPreflightOnlyParityCommitOnly"
...
def _iq1745_model(
    ...
    *,
    ...
    return status_preflight_only, canonical_tuple
...
def iq1746_model(
    ...
    *,
    ...
    status_primary, staged_tuple = _iq1745_model(
        scenarios=scenarios,
        secure_local_mode=secure_local_mode,
        iommu_active=iommu_active,
        book_of_truth_gpu_hooks=book_of_truth_gpu_hooks,
        policy_digest_parity=policy_digest_parity,
        dispatch_transcript_parity=dispatch_transcript_parity,
        force_status_domain_drift_canonical=force_status_domain_drift_primary,
    )

    status_parity, parity_saved = _iq1744_model(
        scenarios=scenarios,
        secure_local_mode=secure_local_mode,
        iommu_active=iommu_active,
        book_of_truth_gpu_hooks=book_of_truth_gpu_hooks,
        ...
        force_digest_drift_staged=force_digest_drift_parity,
    )
...
def test_source_contains_iq1746_symbols() -> None:
    src = Path("src/gpu/security_perf_matrix.HC").read_text(encoding="utf-8")
    assert f"I32 {HC_FN_IQ1746}(" in src
    assert f"status_primary = {HC_FN_IQ1745}(" in src
    assert f"status_parity = {HC_FN_IQ1744}(" in src
    assert "if (status_primary != status_parity)" in src
    assert "// IQ-1746 commit-only hardening wrapper:" in src


def test_gate_missing_vector() -> None:
    status, out_values = iq1746_model(
...
if __name__ == "__main__":
    test_source_contains_iq1746_symbols()
