#!/usr/bin/env python3
"""Reference checks for GQAAttentionApplySoftmaxQ16CheckedNoPartialPreflightDefaultStrideCommitOnly (IQ-1408)."""
...
def gqa_attention_apply_softmax_q16_checked_nopartial_preflight_default_stride_commit_only(
    scores_q32,
    scores_capacity: int,
    query_rows: int,
    key_rows: int,
...
def explicit_default_stride_commit_only_composition(*args) -> int:
    scores_q32 = args[0]
...
def test_randomized_equivalence_with_explicit_composition() -> None:
    rng = random.Random(20260425_1408)
    ...
    for _ in range(240):
        ...
        err_a = gqa_attention_apply_softmax_q16_checked_nopartial_preflight_default_stride_commit_only(
            scores,
            len(scores),
            query_rows,
            key_rows,
        ...
        err_b = explicit_default_stride_commit_only_composition(
            scores,
            len(scores),
            query_rows,
            key_rows,
...
def test_source_contract_markers() -> None:
    ...
    marker_commit_only = "I32 GQAAttentionApplySoftmaxQ16CheckedNoPartialPreflightDefaultStrideCommitOnly("
    ...
    assert "status = GQAAttentionApplySoftmaxQ16CheckedNoPartialPreflight(" in commit_body
    assert "key_rows,\n        out_probs_q16," in commit_body
...
if __name__ == "__main__":
    ...
    print("gqa_attention_apply_softmax_q16_checked_nopartial_preflight_default_stride_commit_only_reference_checks=ok")
