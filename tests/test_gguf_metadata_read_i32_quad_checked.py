#!/usr/bin/env python3
"""Reference checks for GGUFMetadataReadI32QuadChecked semantics."""
...
def test_null_ptr_and_no_partial_write() -> None:
    ...
    assert (
        gguf_metadata_read_i32_quad_checked(
            None,
            16,
            cursor,
            16,
...
def test_fourth_scalar_fail_does_not_commit_prior_lanes_or_cursor() -> None:
    ...
    second = -1
    third = (1 << 31) - 1
    buf = _le_i32(first) + _le_i32(second) + _le_i32(third) + [0x11, 0x22, 0x33]
...
def test_success_reads_four_i32_and_advances() -> None:
    ...
    third = 123_456_789
    fourth = -42
    buf = [0xAB, 0xCD] + _le_i32(first) + _le_i32(second) + _le_i32(third) + _le_i32(fourth) + [0xEF]
