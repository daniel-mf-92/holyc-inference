#!/usr/bin/env python3
"""Parity harness for FPQ16ExpApproxCheckedNoPartialArrayRequiredBytesCommitOnlyPreflightOnly (IQ-911)."""
...
def fpq16_exp_approx_checked_no_partial_array_required_bytes_commit_only_preflight_only(
    x_q16: list[int] | None,
    ...
    snapshot_out_q16 = out_q16.copy()

    for i in range(count):
        status, _ = fpq16_exp_approx_checked(x_q16[i])
        if status != FP_Q16_OK:
            return status

    if out_q16 != snapshot_out_q16:
        return FP_Q16_ERR_BAD_PARAM
    ...
    out_required_output_bytes_slot[0] = required
    return FP_Q16_OK
...
def explicit_preflight_only_composition(
    ...
    if out_q16 != out_snapshot:
        return FP_Q16_ERR_BAD_PARAM

    if count > (I64_MAX_VALUE >> 3):
        return FP_Q16_ERR_OVERFLOW

    out_required_output_bytes_slot[0] = count * 8
    return FP_Q16_OK


def test_source_contains_iq911_function() -> None:
    source = Path("src/math/fixedpoint.HC").read_text(encoding="utf-8")
    sig = "I32 FPQ16ExpApproxCheckedNoPartialArrayRequiredBytesCommitOnlyPreflightOnly(I64 *x_q16,"
    assert sig in source
    body = source.split(sig, 1)[1]
    assert "FPQ16ExpApproxChecked(x_q16[i]," in body
...
def test_null_bad_param_paths() -> None:
    ...
    assert fpq16_exp_approx_checked_no_partial_array_required_bytes_commit_only_preflight_only(None, out, 1, required) == FP_Q16_ERR_NULL_PTR
    assert fpq16_exp_approx_checked_no_partial_array_required_bytes_commit_only_preflight_only(x, None, 1, required) == FP_Q16_ERR_NULL_PTR
    assert fpq16_exp_approx_checked_no_partial_array_required_bytes_commit_only_preflight_only(x, out, 1, None) == FP_Q16_ERR_NULL_PTR
    assert fpq16_exp_approx_checked_no_partial_array_required_bytes_commit_only_preflight_only(x, out, -1, required) == FP_Q16_ERR_BAD_PARAM
    assert fpq16_exp_approx_checked_no_partial_array_required_bytes_commit_only_preflight_only(x, x, 1, required) == FP_Q16_ERR_BAD_PARAM


def test_no_publish_and_no_write_on_hard_failure() -> None:
    x = [0, I64_MAX_VALUE, 1]
    ...
    status = fpq16_exp_approx_checked_no_partial_array_required_bytes_commit_only_preflight_only(x, out, len(x), required)
...
def test_zero_count_commit() -> None:
    ...
    status = fpq16_exp_approx_checked_no_partial_array_required_bytes_commit_only_preflight_only(x, out, 0, required)
...
def test_known_vectors_parity() -> None:
    ...
    got = fpq16_exp_approx_checked_no_partial_array_required_bytes_commit_only_preflight_only(
        x,
        out_impl,
        len(x),
        req_impl,
    )
    exp = explicit_preflight_only_composition(
        x,
        out_ref,
        len(x),
        req_ref,
...
def test_randomized_parity() -> None:
    rng = random.Random(20260421_911)
    ...
    for _ in range(3000):
        ...
        expected_status = explicit_preflight_only_composition(
            x,
            out_expected,
            count,
            required_expected,
        )
        got_status = fpq16_exp_approx_checked_no_partial_array_required_bytes_commit_only_preflight_only(
            x,
            out_got,
            count,
            required_got,
...
def run() -> None:
    test_source_contains_iq911_function()
    test_null_bad_param_paths()
    test_no_publish_and_no_write_on_hard_failure()
    test_zero_count_commit()
    test_known_vectors_parity()
    test_randomized_parity()
    print("fixedpoint_q16_exp_approx_checked_no_partial_array_required_bytes_commit_only_preflight_only=ok")
