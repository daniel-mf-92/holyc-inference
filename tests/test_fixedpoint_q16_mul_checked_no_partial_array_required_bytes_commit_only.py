#!/usr/bin/env python3
"""Parity harness for FPQ16MulCheckedNoPartialArrayRequiredBytesCommitOnly (IQ-970)."""
...
def fpq16_mul_checked_no_partial_array_required_bytes_commit_only(
    ...
    *,
    ...
    status, staged_required_cells, staged_required_bytes = (
        fpq16_mul_checked_no_partial_array_required_bytes(
            lhs_q16,
            rhs_q16,
            out_q16,
...
def explicit_commit_only_composition(
    ...
    status, required_cells, required_bytes = fpq16_mul_checked_no_partial_array_required_bytes(
        lhs_q16,
        rhs_q16,
        out_q16,
        count,
...
def test_source_contains_iq970_function() -> None:
    source = Path("src/math/fixedpoint.HC").read_text(encoding="utf-8")
    sig = "I32 FPQ16MulCheckedNoPartialArrayRequiredBytesCommitOnly(I64 *lhs_q16,"
    assert sig in source
    body = source.split(sig, 1)[1].split("I32 FPQ16MulSatCheckedNoPartialArray", 1)[0]
    assert "snapshot_lhs_q16 = lhs_q16;" in body
...
def test_null_alias_and_no_publish_on_fail() -> None:
    ...
    out_cells = [111]
    out_bytes = [222]
    err = fpq16_mul_checked_no_partial_array_required_bytes_commit_only(
        lhs,
        rhs,
        out,
        2,
...
def test_randomized_commit_only_matches_explicit_composition() -> None:
    rng = random.Random(20260421_970)
    ...
    for _ in range(4000):
        ...
        err_got = fpq16_mul_checked_no_partial_array_required_bytes_commit_only(
            lhs.copy(),
            rhs.copy(),
            out_a,
            count,
...
def run() -> None:
    test_source_contains_iq970_function()
    test_null_alias_and_no_publish_on_fail()
    test_publish_and_mutation_guard()
    test_randomized_commit_only_matches_explicit_composition()
    print("fixedpoint_q16_mul_checked_no_partial_array_required_bytes_commit_only=ok")
