#!/usr/bin/env python3
"""Parity harness for FPQ16LnArrayCheckedNoPartialCommitOnlyPreflightOnlyParityCommitOnly (IQ-1161)."""
...
from test_intlog_ln_array_checked_nopartial_commit_only_preflight_only_parity import (
    fpq16_ln_array_checked_nopartial_commit_only_preflight_only_parity,
)


def fpq16_ln_array_checked_nopartial_commit_only_preflight_only_parity_commit_only(
...
def test_source_contains_iq1161_signature_and_commit_only_contract() -> None:
    src = Path("src/math/intlog.HC").read_text(encoding="utf-8")
    sig = (
        "I32 FPQ16LnArrayCheckedNoPartialCommitOnlyPreflightOnlyParityCommitOnly("
        "I64 *input_q16,"
    )
    assert sig in src
...
def test_capacity_failure_and_success_atomic_publish() -> None:
    ...
    out = [0xABCDEF1234567890] * len(inp)
    need = [123]
    ...
    assert st == INTLOG_STATUS_BADCOUNT
    assert need[0] == 0
    assert out == [0xABCDEF1234567890] * len(inp)
...
def test_randomized_required_bytes_parity() -> None:
    ...
    for _ in range(300):
        ...
        assert out == [0x1337133713371337] * count
...
def run() -> None:
    ...
    print(
        "intlog_ln_array_checked_nopartial_commit_only_preflight_only_parity_commit_only_reference_checks=ok"
    )
