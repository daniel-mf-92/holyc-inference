#!/usr/bin/env python3
"""Parity harness for KVCacheQ16ReserveTokenSpan...RequiredBytesParityCommitOnlyPreflightOnly (IQ-1071)."""
...
def kv_cache_q16_reserve_token_span_checked_nopartial_commit_only_preflight_only_required_bytes_parity_preflight_only(
    layer_count: int,
    head_count: int,
    head_dim: int,
    token_start: int,
    ...
    if staged_cells[0] < 0 or staged_bytes[0] < 0 or staged_last[0] < 0:
        return KV_Q16_ERR_BAD_PARAM
    if required_cells < 0 or required_bytes < 0 or last_cell_index < 0:
        return KV_Q16_ERR_BAD_PARAM

    if (staged_bytes[0] % 8) != 0:
        return KV_Q16_ERR_BAD_PARAM
    if (staged_bytes[0] // 8) != staged_cells[0]:
        return KV_Q16_ERR_BAD_PARAM

    if (required_bytes % 8) != 0:
        return KV_Q16_ERR_BAD_PARAM
    if (required_bytes // 8) != required_cells:
        return KV_Q16_ERR_BAD_PARAM

    if staged_cells[0] == 0 and staged_last[0] != 0:
        return KV_Q16_ERR_BAD_PARAM
    if required_cells == 0 and last_cell_index != 0:
        return KV_Q16_ERR_BAD_PARAM

    if staged_cells[0] != required_cells:
        return KV_Q16_ERR_BAD_PARAM
    if staged_bytes[0] != required_bytes:
        return KV_Q16_ERR_BAD_PARAM
    if staged_last[0] != last_cell_index:
        return KV_Q16_ERR_BAD_PARAM
...
def kv_cache_q16_reserve_token_span_checked_nopartial_commit_only_preflight_only_required_bytes_parity_commit_only_preflight_only(
    layer_count: int,
    head_count: int,
    head_dim: int,
    token_start: int,
    ...
    canonical_bytes = [0]
    canonical_last = [0]
    err = kv_cache_q16_reserve_token_span_checked_nopartial_commit_only_preflight_only_required_bytes_parity_preflight_only(
        layer_count,
        head_count,
        head_dim,
        token_start,
    ...
    if (canonical_bytes[0] % 8) != 0:
        return KV_Q16_ERR_BAD_PARAM
    if (canonical_bytes[0] // 8) != canonical_cells[0]:
        return KV_Q16_ERR_BAD_PARAM
    ...
    if staged_cells[0] != canonical_cells[0]:
        return KV_Q16_ERR_BAD_PARAM
    if staged_bytes[0] != canonical_bytes[0]:
        return KV_Q16_ERR_BAD_PARAM
    if staged_last[0] != canonical_last[0]:
        return KV_Q16_ERR_BAD_PARAM
...
def test_source_contains_required_bytes_parity_commit_only_preflight_only_helpers() -> None:
    source = Path("src/model/kv_cache.HC").read_text(encoding="utf-8")

    sig_preflight = "I32 KVCacheQ16ReserveTokenSpanCheckedNoPartialCommitOnlyPreflightOnlyRequiredBytesParityPreflightOnly("
    sig_target = "I32 KVCacheQ16ReserveTokenSpanCheckedNoPartialCommitOnlyPreflightOnlyRequiredBytesParityCommitOnlyPreflightOnly("

    assert sig_preflight in source
    assert sig_target in source

    body_preflight = source.split(sig_preflight, 1)[1]
    assert "KVCacheQ16ReserveTokenSpanCheckedNoPartialCommitOnlyPreflightOnlyRequiredBytesParity(" in body_preflight
    assert "KVTryMulI64Checked(snapshot_layer_count," in body_preflight

    body_target = source.split(sig_target, 1)[1]
    assert "KVCacheQ16ReserveTokenSpanCheckedNoPartialCommitOnlyPreflightOnlyRequiredBytesParityCommitOnly(" in body_target
    assert "KVCacheQ16ReserveTokenSpanCheckedNoPartialCommitOnlyPreflightOnlyRequiredBytesParityPreflightOnly(" in body_target
    assert "if (staged_required_cells != canonical_required_cells)" in body_target
    assert "if (staged_required_cells != recomputed_required_cells)" in body_target
...
def test_randomized_parity_vs_explicit_composition() -> None:
    rng = random.Random(20260422_1071)
    ...
    for _ in range(2000):
        ...
        err_new = kv_cache_q16_reserve_token_span_checked_nopartial_commit_only_preflight_only_required_bytes_parity_commit_only_preflight_only(
            layer_count,
            head_count,
            head_dim,
            token_start,
...
if __name__ == "__main__":
    test_source_contains_required_bytes_parity_commit_only_preflight_only_helpers()
    ...
    print(
        "kv_cache_q16_reserve_token_span_checked_nopartial_commit_only_preflight_only_required_bytes_parity_commit_only_preflight_only=ok"
    )
