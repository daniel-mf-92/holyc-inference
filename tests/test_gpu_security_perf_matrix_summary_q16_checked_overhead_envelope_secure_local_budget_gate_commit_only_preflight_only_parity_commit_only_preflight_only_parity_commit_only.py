...
def secure_local_budget_gate_commit_only_preflight_only_parity_commit_only_preflight_only_parity_commit_only(
    rows: list[RowOutput],
    rows_capacity: int,
    secure_local_mode: int,
    iommu_active: int,
    ...
    *,
    ...
    status_preflight_only, preflight_outputs, _ = _secure_local_budget_gate_commit_only_preflight_only(
        rows,
        rows_capacity,
        secure_local_mode,
        iommu_active,
    ...
    if force_tuple_parity_drift:
        preflight_outputs = (preflight_outputs[0] + 1, preflight_outputs[1], preflight_outputs[2])

    if not _status_is_valid(status_parity) or not _status_is_valid(status_preflight_only):
        return GPU_SEC_PERF_ERR_BAD_PARAM, caller_outputs
    if not _budget_gate_status_is_valid(parity_tuple[2]) or not _budget_gate_status_is_valid(preflight_outputs[2]):
        return GPU_SEC_PERF_ERR_BAD_PARAM, caller_outputs

    if status_parity != status_preflight_only or parity_tuple != preflight_outputs:
        return GPU_SEC_PERF_ERR_BAD_PARAM, caller_outputs
...
def test_gate_missing_and_threshold_breach_vectors() -> None:
    ...
    status, outputs = secure_local_budget_gate_commit_only_preflight_only_parity_commit_only_preflight_only_parity_commit_only(
        rows_gate_missing,
        rows_capacity=3,
