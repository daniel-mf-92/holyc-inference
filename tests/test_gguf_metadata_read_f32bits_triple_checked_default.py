#!/usr/bin/env python3
"""Parity checks for GGUFMetadataReadF32BitsTripleCheckedDefault."""
...
def gguf_metadata_read_f32bits_triple_checked_default(
    buf: list[int] | None,
    buf_nbytes: int,
    cursor_ref: list[int] | None,
    out_first_ref: list[int] | None,
    ...
    return gguf_metadata_read_f32bits_triple_checked(
        buf,
        buf_nbytes,
        cursor_ref,
        buf_nbytes,
...
def test_null_ptr_and_no_partial_write() -> None:
    ...
    err = gguf_metadata_read_f32bits_triple_checked_default(
        None,
        64,
        cursor,
        out_first,
...
def test_success_matches_checked_with_default_end() -> None:
    ...
    buf = [0xAA] + _le_u32(first) + _le_u32(second) + _le_u32(third) + [0x55]
    ...
    err_default = gguf_metadata_read_f32bits_triple_checked_default(
        buf,
        len(buf),
        cur_default,
        out_default[0],
    ...
    err_checked = gguf_metadata_read_f32bits_triple_checked(
        buf,
        len(buf),
        cur_checked,
        len(buf),
    ...
    assert cur_default == cur_checked == [13]
...
def test_truncation_matches_checked_and_no_commit() -> None:
    ...
    second = struct.unpack("<I", struct.pack("<f", -0.25))[0]
    buf = _le_u32(first) + _le_u32(second) + [0x10, 0x20, 0x30]

    cur_default = [0]
    cur_checked = [0]
    out_default = [[0xAAAA0001], [0xBBBB0002], [0xCCCC0003]]
    out_checked = [[0xAAAA0001], [0xBBBB0002], [0xCCCC0003]]
    ...
    assert out_default == out_checked == [[0xAAAA0001], [0xBBBB0002], [0xCCCC0003]]
...
def test_cursor_past_end_bad_param_propagates() -> None:
    ...
    assert out_default == out_checked == [[0], [0], [0]]


def test_randomized_default_wrapper_parity() -> None:
    rng = random.Random(20260418_303)
    ...
    for _ in range(6000):
        ...
        out_default = [[0x10000000], [0x20000000], [0x30000000]]
        out_checked = [[0x10000000], [0x20000000], [0x30000000]]
