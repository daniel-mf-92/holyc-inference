#!/usr/bin/env python3
"""Harness for IQ-1615 zero-write diagnostics companion over IQ-1606 and IQ-1605."""
...
def _secure_local_budget_gate_commit_only_preflight_only(
    ...
    *,
    ...
    return status_commit_only, caller_outputs, staged
...
def secure_local_budget_gate_commit_only_preflight_only_parity_commit_only_preflight_only_parity_commit_only_preflight_only_parity_commit_only_preflight_only_parity_commit_only_preflight_only(
    rows: list[RowOutput],
    rows_capacity: int,
    secure_local_mode: int,
    iommu_active: int,
    ...
    *,
    ...
    status_commit_only, _, staged_tuple = _secure_local_budget_gate_commit_only_preflight_only(
        rows,
        rows_capacity,
        secure_local_mode,
        iommu_active,
        ...
        max_p50_overhead_q16,
        max_p95_overhead_q16,
        (31, 37, GPU_SEC_PERF_BUDGET_GATE_STATUS_PASS),
        force_commit_only_status_domain_drift=force_commit_status_domain_drift,
    )

    canonical_tuple = staged_tuple
    status_parity, canonical_tuple = _secure_local_budget_gate_commit_only_preflight_only_parity(
        rows,
        rows_capacity,
        secure_local_mode,
        iommu_active,
    ...
    if status_commit_only != status_parity or staged_tuple != canonical_tuple:
        return GPU_SEC_PERF_ERR_BAD_PARAM, caller_outputs

    return status_commit_only, caller_outputs


def test_source_contains_iq1615_symbols() -> None:
    src = Path("src/gpu/security_perf_matrix.HC").read_text(encoding="utf-8")
    assert "I32 GPUSecurityPerfMatrixSummaryQ16CheckedOverheadEnvelopeSecureLocalBudgetGateCommitOnlyPreflightOnlyParityCommitOnlyPreflightOnlyParityCommitOnlyPreflightOnlyParityCommitOnlyPreflightOnlyParityCommitOnlyPreflightOnly(" in src
    assert "status_commit_only = GPUSecurityPerfMatrixSummaryQ16CheckedOverheadEnvelopeSecureLocalBudgetGateCommitOnlyPreflightOnlyParityCommitOnlyPreflightOnlyParityCommitOnlyPreflightOnlyParityCommitOnlyPreflightOnly(" in src
    assert "status_parity = GPUSecurityPerfMatrixSummaryQ16CheckedOverheadEnvelopeSecureLocalBudgetGateCommitOnlyPreflightOnlyParityCommitOnlyPreflightOnlyParityCommitOnlyPreflightOnlyParityCommitOnly(" in src
...
def test_gate_missing_and_threshold_breach_vectors() -> None:
    ...
    status, outputs = secure_local_budget_gate_commit_only_preflight_only_parity_commit_only_preflight_only_parity_commit_only_preflight_only_parity_commit_only_preflight_only_parity_commit_only_preflight_only(
        rows_gate_missing,
        rows_capacity=3,
...
if __name__ == "__main__":
    test_source_contains_iq1615_symbols()
