#!/usr/bin/env python3
"""Parity harness for TokenizerBPEDecodeTokenSpanCheckedValidateCursor."""
...
def tokenizer_bpe_decode_token_span_checked_validate_cursor(
...
def run_case(
    ...
    err_wrapped = tokenizer_bpe_decode_token_span_checked_validate_cursor(
        token_ids,
        len(token_ids),
        cursor_wrapped,
        span_count,
...
def test_validate_cursor_rejects_overflow_and_oob() -> None:
    ...
    cursor = [4]
    err = tokenizer_bpe_decode_token_span_checked_validate_cursor(
        [0, 1, 2],
        3,
        cursor,
        0,
...
def test_capacity_and_null_contracts() -> None:
    ...
    assert (
        tokenizer_bpe_decode_token_span_checked_validate_cursor(
            None,
            0,
            cursor,
            0,
...
def test_randomized_parity_against_explicit_guarded_composition() -> None:
    rng = random.Random(20260419_439)
    pieces = [
        b"a",
        b"bc",
        b"def",
...
if __name__ == "__main__":
    ...
    print("test_tokenizer_bpe_decode_token_span_checked_validate_cursor: ok")
