#!/usr/bin/env python3
"""Reference checks for FPQ16ExpArrayFromClampedInputChecked semantics."""
...
def fpq16_exp_array_from_clamped_input_checked(
    ...
    *,
    ...
    # HolyC helper pre-scans for domain violations before writing output.
...
def test_null_pointer_surfaces() -> None:
    err, _ = fpq16_exp_array_from_clamped_input_checked(None, [0], 1)
    assert err == FP_Q16_ERR_NULL_PTR

    err, _ = fpq16_exp_array_from_clamped_input_checked([0], None, 1)
    assert err == FP_Q16_ERR_NULL_PTR


def test_bad_param_negative_count() -> None:
    out = [111, 222]
    err, got = fpq16_exp_array_from_clamped_input_checked([1, 2], out, -1)
...
def test_zero_count_no_write() -> None:
    out = [111, 222, 333]
    err, got = fpq16_exp_array_from_clamped_input_checked([1, 2, 3], out, 0)
...
def test_overflow_pointer_span_guards() -> None:
    ...
    err, _ = fpq16_exp_array_from_clamped_input_checked([0], [0], huge_count)
    assert err == FP_Q16_ERR_OVERFLOW

    count = 4
    near_end = U64_MAX_VALUE - ((count - 1) << 3) + 1
    err, _ = fpq16_exp_array_from_clamped_input_checked(
        [0] * count,
        [0] * count,
        count,
...
def test_rejects_unclamped_lanes_without_partial_writes() -> None:
    inputs = [0, 42, EXP_Q16_MAX_INPUT + 1, -5]
    sentinel = [999, 999, 999, 999]

    err, got = fpq16_exp_array_from_clamped_input_checked(inputs, sentinel, len(inputs))
    assert err == FP_Q16_ERR_BAD_PARAM
    assert got == [999, 999, 999, 999]
...
def test_boundary_and_scalar_composition_parity() -> None:
    ...
    out = [0] * len(inputs)
    err, got = fpq16_exp_array_from_clamped_input_checked(inputs, out, len(inputs))
    assert err == FP_Q16_OK
    assert got is not None

    for i, value in enumerate(inputs):
        scalar_err, scalar_out = fpq16_exp_from_clamped_input_checked(value)
        assert scalar_err == FP_Q16_OK
        assert got[i] == scalar_out


def test_randomized_parity_with_scalar_lane_reference() -> None:
    rng = random.Random(20260417_167)
    ...
    for _ in range(1500):
        ...
        err, got = fpq16_exp_array_from_clamped_input_checked(inputs, out, count)
        assert err == FP_Q16_OK
        assert got is not None

        for i in range(count):
            scalar_err, scalar_out = fpq16_exp_from_clamped_input_checked(inputs[i])
            assert scalar_err == FP_Q16_OK
            assert got[i] == scalar_out
...
def run() -> None:
    ...
    test_rejects_unclamped_lanes_without_partial_writes()
    test_boundary_and_scalar_composition_parity()
    test_randomized_parity_with_scalar_lane_reference()
    print("intexp_exp_array_from_clamped_input_checked_reference_checks=ok")
