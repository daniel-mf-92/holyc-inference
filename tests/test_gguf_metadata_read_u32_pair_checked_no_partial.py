#!/usr/bin/env python3
"""Reference checks for GGUFMetadataReadU32PairCheckedNoPartial semantics."""
...
def gguf_metadata_read_u32_pair_checked_no_partial(
    buf: list[int] | None,
    buf_nbytes: int,
    cursor_ref: list[int] | None,
    table_end: int,
...
def test_null_ptr_and_no_partial_write() -> None:
    ...
    assert (
        gguf_metadata_read_u32_pair_checked_no_partial(
            None,
            32,
            cursor,
            32,
...
def test_second_scalar_fail_does_not_commit_anything() -> None:
    first = 0x89ABCDEF
    buf = _le_u32(first) + [0x11, 0x22, 0x33]

    cursor = [0]
    out_first = [0xAAAAAAAA]
    out_second = [0xBBBBBBBB]

    err = gguf_metadata_read_u32_pair_checked_no_partial(
        buf,
        len(buf),
        cursor,
        len(buf),
...
def test_randomized_parity_against_checked() -> None:
    rng = random.Random(20260418_294)
    ...
    for _ in range(9000):
        ...
        err_no_partial = gguf_metadata_read_u32_pair_checked_no_partial(
            buf,
            n,
            cursor_no_partial,
            table_end,
        ...
        err_checked = gguf_metadata_read_u32_pair_checked(
            buf,
            n,
            cursor_checked,
            table_end,
        ...
        assert out_no_partial_a == out_checked_a
        assert out_no_partial_b == out_checked_b
...
def run() -> None:
    ...
    test_randomized_parity_against_checked()
    print("gguf_metadata_read_u32_pair_checked_no_partial_reference_checks=ok")
