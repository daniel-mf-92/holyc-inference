#!/usr/bin/env python3
"""Parity harness for GGUFTensorInfoValidateDimsCheckedCommitOnly (IQ-1154)."""
...
def validate_dims_checked_commit_only(
    ...
    staged_block = [0]
    status = tensor_type_block_elems_checked(snapshot_ggml_type, staged_block)
    if status != GGUF_TENSOR_PARSE_OK:
        return status
    if staged_block[0] == 0:
        return GGUF_TENSOR_PARSE_ERR_BAD_TYPE
    if staged_n[0] % staged_block[0] != 0:
        return GGUF_TENSOR_PARSE_ERR_BAD_DIM_SIZE
    ...
    parity_block = [0]
    status = tensor_type_block_elems_checked(ggml_type, parity_block)
    if status != GGUF_TENSOR_PARSE_OK:
        return status
    if parity_block[0] == 0:
        return GGUF_TENSOR_PARSE_ERR_BAD_TYPE
    if parity_n[0] % parity_block[0] != 0:
        return GGUF_TENSOR_PARSE_ERR_BAD_DIM_SIZE

    if snapshot_n_dims != n_dims or snapshot_dims is not dims or snapshot_ggml_type != ggml_type:
        return GGUF_TENSOR_PARSE_ERR_BAD_DIMS
    if staged_n[0] != parity_n[0] or staged_block[0] != parity_block[0]:
        return GGUF_TENSOR_PARSE_ERR_BAD_DIMS

    out_n_elements[0] = staged_n[0]
    out_block_elems[0] = staged_block[0]
    return GGUF_TENSOR_PARSE_OK


def test_source_contains_iq1154_function_and_contract() -> None:
    source = Path("src/gguf/tensorinfo.HC").read_text(encoding="utf-8")
    sig = "I64 GGUFTensorInfoValidateDimsCheckedCommitOnly(U32 n_dims,"
    ...
    assert "status = GGUFTensorInfoValidateDimsChecked(n_dims," in body
    assert "status = GGUFTensorTypeBlockElemsChecked(ggml_type, &parity_block_elems);" in body
...
def test_commit_only_error_paths_preserve_outputs() -> None:
    out_n = [0xAA55]
    out_b = [0x55AA]

    status = validate_dims_checked_commit_only(2, None, 2, out_n, out_b)
    assert status == GGUF_TENSOR_PARSE_ERR_NULL_PTR
    assert out_n == [0xAA55]
    assert out_b == [0x55AA]

    status = validate_dims_checked_commit_only(0, [32], 2, out_n, out_b)
    assert status == GGUF_TENSOR_PARSE_ERR_BAD_DIMS
    assert out_n == [0xAA55]
    assert out_b == [0x55AA]

    status = validate_dims_checked_commit_only(2, [32, 0], 2, out_n, out_b)
    assert status == GGUF_TENSOR_PARSE_ERR_BAD_DIM_SIZE
    assert out_n == [0xAA55]
    assert out_b == [0x55AA]

    status = validate_dims_checked_commit_only(2, [1 << 63, 3], 2, out_n, out_b)
    assert status == GGUF_TENSOR_PARSE_ERR_DIM_OVERFLOW
    assert out_n == [0xAA55]
    assert out_b == [0x55AA]

    status = validate_dims_checked_commit_only(2, [32, 32], 999, out_n, out_b)
    assert status == GGUF_TENSOR_PARSE_ERR_BAD_TYPE
    assert out_n == [0xAA55]
    assert out_b == [0x55AA]

    status = validate_dims_checked_commit_only(2, [33, 1], 2, out_n, out_b)
    assert status == GGUF_TENSOR_PARSE_ERR_BAD_DIM_SIZE
    assert out_n == [0xAA55]
    assert out_b == [0x55AA]


def test_commit_only_success_and_randomized_parity() -> None:
    out_n = [0]
    out_b = [0]

    status = validate_dims_checked_commit_only(3, [32, 16, 8], 2, out_n, out_b)
    assert status == GGUF_TENSOR_PARSE_OK
    assert out_n == [4096]
    assert out_b == [32]

    status = validate_dims_checked_commit_only(1, [13], 0, out_n, out_b)
    assert status == GGUF_TENSOR_PARSE_OK
    assert out_n == [13]
    assert out_b == [1]

    rng = random.Random(20260423_1154)
    ...
    for _ in range(400):
        ...
        expect_n = [123456]
        status_expect = validate_dims_checked(n_dims, dims, ggml_type, expect_n)
        expect_b = [0]
        assert tensor_type_block_elems_checked(ggml_type, expect_b) == GGUF_TENSOR_PARSE_OK

        got_n = [789012]
        got_b = [210987]
        status = validate_dims_checked_commit_only(n_dims, dims, ggml_type, got_n, got_b)

        assert status == status_expect == GGUF_TENSOR_PARSE_OK
        assert got_n == expect_n
        assert got_b == expect_b


if __name__ == "__main__":
    test_source_contains_iq1154_function_and_contract()
    test_commit_only_error_paths_preserve_outputs()
    test_commit_only_success_and_randomized_parity()
    print("ok")
