#!/usr/bin/env python3
"""Harness for IQ-1570 digest-locked overhead-envelope strict diagnostics parity gate (preflight-only vs commit-only)."""
...
def gpu_security_perf_matrix_summary_q16_checked_overhead_envelope_checked_nopartial_commit_only_preflight_only_parity_commit_only_preflight_only_digest_locked_parity_commit_only_preflight_only_parity_commit_only_preflight_only_parity(
    rows: list[RowOutput] | None,
    rows_capacity: int,
    out_capacity: int,
    secure_local_mode: int,
    ...
    status_parity, canonical_tuple = _overhead_envelope_preflight_only_parity_commit_only_preflight_only_digest_locked_parity_commit_only(
        rows,
        rows_capacity,
        out_capacity,
        secure_local_mode,
...
def test_source_contains_iq1570_symbols() -> None:
    src = Path("src/gpu/security_perf_matrix.HC").read_text(encoding="utf-8")
    assert (
        "I32 GPUSecurityPerfMatrixSummaryQ16CheckedOverheadEnvelopeCheckedNoPartialCommitOnlyPreflightOnlyParityCommitOnlyPreflightOnlyDigestLockedParityCommitOnlyPreflightOnlyParityCommitOnlyPreflightOnlyParity("
        in src
    )
    assert (
        "status_preflight_only = GPUSecurityPerfMatrixSummaryQ16CheckedOverheadEnvelopeCheckedNoPartialCommitOnlyPreflightOnlyParityCommitOnlyPreflightOnlyDigestLockedParityCommitOnlyPreflightOnlyParityCommitOnlyPreflightOnly("
        in src
    )
    assert (
        "status_parity = GPUSecurityPerfMatrixSummaryQ16CheckedOverheadEnvelopeCheckedNoPartialCommitOnlyPreflightOnlyParityCommitOnlyPreflightOnlyDigestLockedParityCommitOnlyPreflightOnlyParityCommitOnly("
        in src
    )
...
def test_secure_on_success_atomic_publish() -> None:
    ...
    status, preserved, published = gpu_security_perf_matrix_summary_q16_checked_overhead_envelope_checked_nopartial_commit_only_preflight_only_parity_commit_only_preflight_only_digest_locked_parity_commit_only_preflight_only_parity_commit_only_preflight_only_parity(
        rows,
        rows_capacity=3,
        out_capacity=3,
...
def test_null_alias_capacity_gate_and_monotonicity_vectors() -> None:
    ...
    status, _, _ = gpu_security_perf_matrix_summary_q16_checked_overhead_envelope_checked_nopartial_commit_only_preflight_only_parity_commit_only_preflight_only_digest_locked_parity_commit_only_preflight_only_parity_commit_only_preflight_only_parity(
        rows,
        rows_capacity=1,
        out_capacity=3,
        secure_local_mode=GPU_SEC_PERF_PROFILE_SECURE_LOCAL,
