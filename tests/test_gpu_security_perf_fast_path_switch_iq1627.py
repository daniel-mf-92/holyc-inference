#!/usr/bin/env python3
"""Harness for IQ-1627 commit-only hardening wrapper over IQ-1629 and IQ-1628."""
...
def _fast_path_switch_checked_audit_parity_commit_only_preflight_only_parity_commit_only_preflight_only_parity_commit_only_preflight_only_parity_commit_only(
    ...
    # Commit-only baseline semantics: publish parity tuple only on fully clean secure-local tuple.
...
def _fast_path_switch_checked_audit_parity_commit_only_preflight_only_parity_commit_only_preflight_only_parity_commit_only_preflight_only_parity_commit_only_preflight_only(
    ...
    # IQ-1628 semantics: zero-write diagnostics wrapper preserving caller outputs.
...
def _fast_path_switch_checked_audit_parity_iq1629(
    secure_local_mode: int,
    iommu_active: int,
    book_of_truth_gpu_hooks: int,
    policy_digest_parity: int,
...
def fast_path_switch_checked_audit_parity_iq1627(
    secure_local_mode: int,
    iommu_active: int,
    book_of_truth_gpu_hooks: int,
    policy_digest_parity: int,
    ...
    *,
    ...
    status_parity, parity_enabled, parity_reason = _fast_path_switch_checked_audit_parity_iq1629(
        secure_local_mode,
        iommu_active,
        book_of_truth_gpu_hooks,
        policy_digest_parity,
...
def test_source_contains_iq1627_symbols() -> None:
...
def test_gate_missing_and_audit_parity_breach_vectors() -> None:
    status, enabled, reason = fast_path_switch_checked_audit_parity_iq1627(
...
def test_status_domain_drift_and_deterministic_tuple_parity_vectors() -> None:
    ...
    # Deterministic tuple-parity vector: IQ-1629 is fail-closed under secure-pass tuple,
    # so IQ-1627 preserves caller slots and returns the same fail-closed status.
    status, enabled, reason = fast_path_switch_checked_audit_parity_iq1627(
        secure_local_mode=GPU_SEC_PERF_PROFILE_SECURE_LOCAL,
        iommu_active=1,
        book_of_truth_gpu_hooks=1,
