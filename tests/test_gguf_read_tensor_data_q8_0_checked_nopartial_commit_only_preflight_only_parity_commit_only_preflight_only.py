#!/usr/bin/env python3
"""Parity harness for GGUFReadTensorDataQ8_0CheckedNoPartialCommitOnlyPreflightOnlyParityCommitOnlyPreflightOnly (IQ-942)."""
...
_BASE_PATH = Path(
    "tests/test_gguf_read_tensor_data_q8_0_checked_nopartial_commit_only_preflight_only_parity_commit_only.py"
)
...
GGUF_READER_GGML_TYPE_Q8_0 = _base.GGUF_READER_GGML_TYPE_Q8_0
TensorInfoQ80 = _base.TensorInfoQ80


def gguf_read_tensor_data_q8_0_checked_no_partial_commit_only_preflight_only_parity_commit_only_preflight_only(
    ...
    staged_blocks = [0]
    staged_bytes = [0]
    err = _base.gguf_read_tensor_data_q8_0_checked_no_partial_commit_only_preflight_only_parity_commit_only(
        gguf_bytes,
        gguf_nbytes,
        tensor_data_base,
        tensor_info,
    ...
    recomputed_blocks = [0]
    recomputed_bytes = [0]
    err = _base.gguf_read_tensor_data_q8_0_checked_no_partial_commit_only_preflight_only(
        gguf_bytes,
        gguf_nbytes,
        tensor_data_base,
        tensor_info,
...
def test_source_contains_iq942_q8_preflight_only_commit_parity_contract() -> None:
    source = Path("src/gguf/reader.HC").read_text(encoding="utf-8")
    sig = "I32 GGUFReadTensorDataQ8_0CheckedNoPartialCommitOnlyPreflightOnlyParityCommitOnlyPreflightOnly("
    assert sig in source
    body = source.split(sig, 1)[1]
    if "\n\n\nI32 " in body:
        body = body.split("\n\n\nI32 ", 1)[0]

    assert "status = GGUFReadTensorDataQ8_0CheckedNoPartialCommitOnlyPreflightOnlyParityCommitOnly(" in body
    assert "status = GGUFReadTensorDataQ8_0CheckedNoPartialCommitOnlyPreflightOnly(" in body
...
def test_bad_vectors_and_no_publish_behavior() -> None:
    ...
    err = gguf_read_tensor_data_q8_0_checked_no_partial_commit_only_preflight_only_parity_commit_only_preflight_only(
        None,
        0,
        0,
        info,
...
def run() -> None:
    test_source_contains_iq942_q8_preflight_only_commit_parity_contract()
    test_bad_vectors_and_no_publish_behavior()
    test_success_preflight_only_and_capacity_rejection()
    print("gguf_read_tensor_data_q8_0_checked_nopartial_commit_only_preflight_only_parity_commit_only_preflight_only=ok")
