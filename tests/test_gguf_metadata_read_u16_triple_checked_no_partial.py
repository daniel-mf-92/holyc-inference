#!/usr/bin/env python3
"""Reference checks for GGUFMetadataReadU16TripleCheckedNoPartial semantics."""
...
def gguf_metadata_read_u16_triple_checked_no_partial(
    buf: list[int] | None,
    buf_nbytes: int,
    cursor_ref: list[int] | None,
    table_end: int,
...
def test_null_ptr_and_no_partial_write() -> None:
    ...
    assert (
        gguf_metadata_read_u16_triple_checked_no_partial(
            None,
            12,
            cursor,
            12,
...
def test_third_scalar_fail_does_not_commit_anything() -> None:
    ...
    err = gguf_metadata_read_u16_triple_checked_no_partial(
        buf,
        len(buf),
        cursor,
        len(buf),
...
def test_success_reads_three_u16_and_advances() -> None:
    first = 0xBEEF
    second = 0x1234
    third = 0xCAFE
    buf = [0x77] + _le_u16(first) + _le_u16(second) + _le_u16(third) + [0x88]
...
def test_randomized_parity() -> None:
    rng = random.Random(20260417_276)
...
def run() -> None:
    ...
    print("gguf_metadata_read_u16_triple_checked_no_partial_reference_checks=ok")
