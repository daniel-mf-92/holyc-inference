#!/usr/bin/env python3
"""Parity harness for TokenizerBPEPromptSpanScanCheckedDefaultCapacityNoPartial."""
...
def tokenizer_bpe_prompt_span_scan_checked_default_capacity_no_partial(
    data: list[int] | None,
    byte_len: int,
    ...
    stage_cap = max(derived_capacity, 1)
    staged_starts = [0] * stage_cap
    staged_lengths = [0] * stage_cap
    staged_classes = [0] * stage_cap
...
def run_case(payload: list[int] | None, byte_len: int, cursor_start: int, prompt_nbytes: int) -> None:
    ...
    starts_expected = [0x1010] * cap
    lengths_expected = [0x2020] * cap
    classes_expected = [0x33] * cap
    count_expected = [0x4040]
    ...
    staged_cursor = [cursor_expected[0]]
    staged_count = [count_expected[0]]
    staged_starts = [0] * cap
    staged_lengths = [0] * cap
    staged_classes = [0] * cap
    ...
    if err_expected == TOKENIZER_BPE_OK:
        for idx in range(staged_count[0]):
            starts_expected[idx] = staged_starts[idx]
            lengths_expected[idx] = staged_lengths[idx]
            classes_expected[idx] = staged_classes[idx]
        count_expected[0] = staged_count[0]
        cursor_expected[0] = staged_cursor[0]

    err_wrapper = tokenizer_bpe_prompt_span_scan_checked_default_capacity_no_partial(
        payload,
        byte_len,
        cursor_wrapper,
        prompt_nbytes,
...
def test_error_paths_keep_outputs_unchanged() -> None:
    ...
    err = tokenizer_bpe_prompt_span_scan_checked_default_capacity_no_partial(
        None,
        0,
        cursor,
        0,
...
def test_randomized_parity_vs_explicit_staged_composition() -> None:
    rng = random.Random(20260418_401)
...
if __name__ == "__main__":
    ...
    print("test_tokenizer_bpe_prompt_span_scan_checked_default_capacity_no_partial: ok")
