#!/usr/bin/env python3
"""Harness for IQ-1751 fast-path batch audit commit-only hardening wrapper."""
...
import test_gpu_security_perf_fast_path_switch_batch_audit_q64_checked_commit_only_preflight_only_parity_commit_only_preflight_only_parity_commit_only_preflight_only_iq1750 as iq1750

GPU_SEC_PERF_OK = iq1750.GPU_SEC_PERF_OK
GPU_SEC_PERF_ERR_BAD_PARAM = iq1750.GPU_SEC_PERF_ERR_BAD_PARAM
GPU_SEC_PERF_PROFILE_SECURE_LOCAL = iq1750.GPU_SEC_PERF_PROFILE_SECURE_LOCAL

Scenario = iq1750.Scenario

HC_FN_IQ1751 = "GPUSecurityPerfFastPathSwitchBatchAuditQ64CheckedCommitOnlyPreflightOnlyParityCommitOnlyPreflightOnlyParityCommitOnlyPreflightOnlyParityCommitOnly"
HC_FN_IQ1750 = iq1750.HC_FN_IQ1750
HC_FN_IQ1749 = iq1750.HC_FN_IQ1749
...
def iq1751_model(
    ...
    *,
    ...
    status_seed, seeded_tuple = iq1750.iq1749.iq1749_model(
        scenarios=scenarios,
    ...
    if not iq1750.iq1749.iq1748.iq1747.iq1746._status_is_valid(status_seed):
        return GPU_SEC_PERF_ERR_BAD_PARAM, (0, 0, 0, 0)

    status_primary, staged_tuple = iq1750.iq1750_model(
        scenarios=scenarios,
        secure_local_mode=secure_local_mode,
        iommu_active=iommu_active,
        book_of_truth_gpu_hooks=book_of_truth_gpu_hooks,
        ...
        force_status_domain_drift_commit_only=force_status_domain_drift_primary,
    )

    status_parity, parity_tuple = iq1750.iq1749.iq1749_model(
        scenarios=scenarios,
        secure_local_mode=secure_local_mode,
        iommu_active=iommu_active,
        book_of_truth_gpu_hooks=book_of_truth_gpu_hooks,
    ...
    if not iq1750.iq1749.iq1748.iq1747.iq1746._status_is_valid(status_primary):
        return GPU_SEC_PERF_ERR_BAD_PARAM, (0, 0, 0, 0)
    if not iq1750.iq1749.iq1748.iq1747.iq1746._status_is_valid(status_parity):
        return GPU_SEC_PERF_ERR_BAD_PARAM, (0, 0, 0, 0)
...
def test_source_contains_iq1751_symbols() -> None:
    src = Path("src/gpu/security_perf_matrix.HC").read_text(encoding="utf-8")
    assert f"I32 {HC_FN_IQ1751}(" in src
    assert f"status_primary = {HC_FN_IQ1750}(" in src
    assert f"status_parity = {HC_FN_IQ1749}(" in src
    assert "if (status_primary != status_parity)" in src
    assert "// IQ-1751 commit-only hardening wrapper:" in src


def test_gate_missing_vector() -> None:
    status, out_values = iq1751_model(
...
if __name__ == "__main__":
    test_source_contains_iq1751_symbols()
