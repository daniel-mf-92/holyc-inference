#!/usr/bin/env python3
"""Parity harness for ...DefaultCapacityNoAllocHardenedCommitOnly (IQ-722)."""
...
def attention_q16_apply_score_scale_rows_checked_nopartial_preflight_only_default_stride_required_stage_bytes_default_capacity_noalloc_hardened_commit_only(
    in_scores_q32,
...
def test_source_contains_default_capacity_noalloc_hardened_commit_only() -> None:
    ...
    assert "staged_required_stage_bytes" in source
    assert "snapshot_in_scores_capacity" in source


def test_commit_only_nullptr_and_alias_contracts() -> None:
    out_a = [111]
    out_b = [222]
    out_c = [333]
    out_d = [444]
...
def test_commit_only_overflow_propagation() -> None:
...
def test_commit_only_matches_explicit_checked_composition_randomized() -> None:
    rng = random.Random(20260420_722)
    ...
    for _ in range(900):
        ...
        err_a = attention_q16_apply_score_scale_rows_checked_nopartial_preflight_only_default_stride_required_stage_bytes_default_capacity_noalloc_hardened_commit_only(
            in_scores,
            in_scores_capacity,
            row_count,
            token_count,
...
if __name__ == "__main__":
    test_source_contains_default_capacity_noalloc_hardened_commit_only()
    test_commit_only_nullptr_and_alias_contracts()
    test_commit_only_overflow_propagation()
    test_commit_only_matches_explicit_checked_composition_randomized()
    print(
