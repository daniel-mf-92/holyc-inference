#!/usr/bin/env python3
"""Harness for IQ-1532 latency-window zero-write diagnostics companion over parity-commit-only latency-window wrapper."""
...
def gpu_security_perf_matrix_summary_q16_checked_latency_window_checked_nopartial_preflight_only_parity_commit_only_preflight_only_parity_commit_only_preflight_only(
    ...
    commit_only_status, staged_tuple = _latency_window_nopartial_preflight_only_parity_commit_only_preflight_only(
        rows,
        rows_capacity,
        out_capacity,
        secure_local_mode,
    ...
    if commit_only_status != GPU_SEC_PERF_OK:
        return commit_only_status, caller_outputs, (0, 0, 0, 0)
    if staged_tuple != canonical_tuple:
        return GPU_SEC_PERF_ERR_BAD_PARAM, caller_outputs, (0, 0, 0, 0)
...
def test_source_contains_iq1532_symbols() -> None:
    src = Path("src/gpu/security_perf_matrix.HC").read_text(encoding="utf-8")

    assert "I32 GPUSecurityPerfMatrixSummaryQ16CheckedLatencyWindowCheckedNoPartialPreflightOnlyParityCommitOnlyPreflightOnlyParityCommitOnlyPreflightOnly(" in src
...
def test_preflight_only_success_preserves_outputs_duplicate_latency_tuple() -> None:
    ...
    status, preserved, staged = gpu_security_perf_matrix_summary_q16_checked_latency_window_checked_nopartial_preflight_only_parity_commit_only_preflight_only_parity_commit_only_preflight_only(
        rows,
        rows_capacity=5,
        out_capacity=4,
...
def test_null_alias_capacity_gate_missing_and_parity_drift_vectors() -> None:
    ...
    status, *_ = gpu_security_perf_matrix_summary_q16_checked_latency_window_checked_nopartial_preflight_only_parity_commit_only_preflight_only_parity_commit_only_preflight_only(
        rows,
        rows_capacity=1,
        out_capacity=4,
        secure_local_mode=GPU_SEC_PERF_PROFILE_SECURE_LOCAL,
