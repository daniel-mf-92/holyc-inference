#!/usr/bin/env python3
"""Harness for IQ-1598 zero-write diagnostics companion over IQ-1583 and IQ-1573."""
...
def secure_local_budget_gate_commit_only_preflight_only(
    rows: list[RowOutput],
    rows_capacity: int,
    secure_local_mode: int,
    iommu_active: int,
    ...
    *,
    ...
    status_commit_only, staged_tuple = _secure_local_budget_gate_commit_only(
        rows,
        rows_capacity,
        secure_local_mode,
        iommu_active,
    ...
    status_canonical, canonical_tuple = _secure_local_budget_gate_checked(
        rows,
        rows_capacity,
        secure_local_mode,
        iommu_active,
...
def test_source_contains_iq1598_symbols() -> None:
    src = Path("src/gpu/security_perf_matrix.HC").read_text(encoding="utf-8")
    assert "I32 GPUSecurityPerfMatrixSummaryQ16CheckedOverheadEnvelopeSecureLocalBudgetGateCommitOnlyPreflightOnly(" in src
...
def test_gate_missing_and_threshold_breach_vectors() -> None:
    ...
    status, outputs = secure_local_budget_gate_commit_only_preflight_only(
        rows_gate_missing,
        rows_capacity=3,
...
if __name__ == "__main__":
    test_source_contains_iq1598_symbols()
