#!/usr/bin/env python3
"""Harness for IQ-1695 snapshot-digest zero-write diagnostics companion."""
...
_BASE_IQ1693_SPEC = importlib.util.spec_from_file_location("iq1693_models", _BASE_IQ1693_PATH)
assert _BASE_IQ1693_SPEC is not None and _BASE_IQ1693_SPEC.loader is not None
_BASE_IQ1693_MOD = importlib.util.module_from_spec(_BASE_IQ1693_SPEC)
sys.modules[_BASE_IQ1693_SPEC.name] = _BASE_IQ1693_MOD
_BASE_IQ1693_SPEC.loader.exec_module(_BASE_IQ1693_MOD)
...
def matrix_summary_snapshot_digest_q64_checked_commit_only_preflight_only_parity_commit_only_preflight_only_iq1695(
    rows: list[RowOutput],
    ...
    *,
    ...
    status_commit_only, staged_tuple = (
        _BASE_IQ1693_MOD.matrix_summary_snapshot_digest_q64_checked_commit_only_preflight_only_parity_commit_only_iq1693(
            rows=rows,
    ...
    if not _BASE_IQ1692_MOD._BASE_IQ1683_MOD._status_is_valid(status_commit_only):
        return GPU_SEC_PERF_ERR_BAD_PARAM, saved_out
    if not _BASE_IQ1692_MOD._BASE_IQ1683_MOD._status_is_valid(status_parity):
        return GPU_SEC_PERF_ERR_BAD_PARAM, saved_out
    if not _BASE_IQ1692_MOD._BASE_IQ1683_MOD._budget_gate_status_is_valid(staged_gate):
        return GPU_SEC_PERF_ERR_BAD_PARAM, saved_out
    if not _BASE_IQ1692_MOD._BASE_IQ1683_MOD._budget_gate_status_is_valid(canonical_gate):
        return GPU_SEC_PERF_ERR_BAD_PARAM, saved_out
...
def test_source_contains_iq1695_symbols() -> None:
    src = Path("src/gpu/security_perf_matrix.HC").read_text(encoding="utf-8")

    assert (
        "I32 GPUSecurityPerfMatrixSummaryQ16CheckedOverheadEnvelopeSecureLocalBudgetGateSnapshotDigestQ64CheckedCommitOnlyPreflightOnlyParityCommitOnlyPreflightOnly("
        in src
    )
    assert (
        "status_commit_only = GPUSecurityPerfMatrixSummaryQ16CheckedOverheadEnvelopeSecureLocalBudgetGateSnapshotDigestQ64CheckedCommitOnlyPreflightOnlyParityCommitOnly("
        in src
    )
    assert (
    ...
    assert "// IQ-1695 zero-write diagnostics companion:" in src


def test_gate_missing_vector() -> None:
    status, out_values = matrix_summary_snapshot_digest_q64_checked_commit_only_preflight_only_parity_commit_only_preflight_only_iq1695(
...
def test_deterministic_secure_on_vector() -> None:
    ...
    first = matrix_summary_snapshot_digest_q64_checked_commit_only_preflight_only_parity_commit_only_preflight_only_iq1695(
        rows=rows,
        secure_local_mode=GPU_SEC_PERF_PROFILE_SECURE_LOCAL,
        iommu_active=1,
        book_of_truth_gpu_hooks=1,
    ...
    second = matrix_summary_snapshot_digest_q64_checked_commit_only_preflight_only_parity_commit_only_preflight_only_iq1695(
        rows=rows,
        secure_local_mode=GPU_SEC_PERF_PROFILE_SECURE_LOCAL,
        iommu_active=1,
        book_of_truth_gpu_hooks=1,
...
if __name__ == "__main__":
    test_source_contains_iq1695_symbols()
