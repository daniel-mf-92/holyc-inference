#!/usr/bin/env python3
"""Reference checks for GGUFMetadataReadU8PairChecked semantics."""
...
def test_null_ptr_and_no_partial_write() -> None:
    ...
    assert (
        gguf_metadata_read_u8_pair_checked(None, 4, cursor, 4, out_first, out_second)
    ...
    assert (
        gguf_metadata_read_u8_pair_checked([1, 2, 3], 3, None, 3, out_first, out_second)
        == GGUF_META_TABLE_ERR_NULL_PTR
    )
    assert out_first == [77]
    assert out_second == [88]
    ...
    assert (
        gguf_metadata_read_u8_pair_checked([1, 2, 3], 3, cursor, 3, None, out_second)
        == GGUF_META_TABLE_ERR_NULL_PTR
    )
    assert cursor == [1]
    assert out_second == [88]
    ...
    assert (
        gguf_metadata_read_u8_pair_checked([1, 2, 3], 3, cursor, 3, out_first, None)
        == GGUF_META_TABLE_ERR_NULL_PTR
    )
    assert cursor == [1]
    assert out_first == [77]
...
def test_short_read_does_not_commit_first_or_cursor() -> None:
    ...
    err = gguf_metadata_read_u8_pair_checked(
        buf,
        len(buf),
        cursor,
        len(buf),
...
def test_success_reads_two_bytes_and_advances() -> None:
    buf = [10, 20, 30, 40]
    cursor = [1]
    out_first = [0]
    out_second = [0]
    ...
    assert out_first[0] == 20
    assert out_second[0] == 30
    assert cursor[0] == 3


def test_randomized_parity() -> None:
    rng = random.Random(20260417_208)
    ...
    for _ in range(4000):
        ...
        if cursor0 + 2 <= table_end and cursor0 + 2 <= n:
            ...
            assert out_first[0] == 123
            assert out_second[0] == 231
...
def run() -> None:
    ...
    test_success_reads_two_bytes_and_advances()
    test_randomized_parity()
    print("gguf_metadata_read_u8_pair_checked_reference_checks=ok")
