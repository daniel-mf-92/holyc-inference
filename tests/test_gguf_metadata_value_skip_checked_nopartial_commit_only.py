...
def gguf_metadata_value_skip_checked_nopartial_commit_only_reference(
    *,
    buf: list[int] | None,
    buf_nbytes: int,
...
def test_source_contains_iq1247_function() -> None:
    ...
    sig_def = (
        "I32 GGUFMetadataValueSkipCheckedNoPartialCommitOnly(U8 *buf,\n"
        "                                                    U64 buf_nbytes,\n"
        "                                                    U64 *cursor,\n"
        "                                                    U64 table_end,\n"
        "                                                    U32 value_type,\n"
...
def test_string_bounds_error_no_partial_commit() -> None:
...
def test_array_string_span_success() -> None:
...
def test_array_string_span_truncated_no_partial() -> None:
    strings = [b"abc", b"def"]
    payload = _u32le(GGUF_TYPE_STRING) + _u64le(len(strings)) + b"".join(_string_payload(s) for s in strings)
...
def test_bad_type_no_partial_commit() -> None:
    buf = [0] * 64
    cursor = [10]
    out_next = [888]
