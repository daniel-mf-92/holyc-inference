#!/usr/bin/env python3
"""Commit-only parity harness for InferenceForward...Parity...ParityCommitOnly (IQ-1077)."""
...
def parity_commit_only_preflight_only_parity_commit_only_preflight_only_parity_commit_only_preflight_only_parity_commit_only_preflight_only_parity_commit_only_reference(
    *,
    embed_capacity: int,
    ...
    staged_fn=parity_commit_only_preflight_only_parity_commit_only_preflight_only_parity_commit_only_preflight_only_parity_commit_only_preflight_only_parity_reference,
    canonical_fn=parity_commit_only_preflight_only_parity_commit_only_preflight_only_parity_commit_only_preflight_only_parity_commit_only_preflight_only_reference,
) -> int:
    if (
...
def test_source_contains_iq_1077_symbol() -> None:
    ...
    assert "if (staged_dense_cells < 0 || staged_required_workspace_cells < 0 ||" in body
...
def test_known_vector_success() -> None:
    ...
    status = parity_commit_only_preflight_only_parity_commit_only_preflight_only_parity_commit_only_preflight_only_parity_commit_only_preflight_only_parity_commit_only_reference(
        embed_capacity=96,
...
def test_no_partial_publish_on_staged_mismatch() -> None:
    ...
    assert out_dense == [8711]
    assert out_workspace == [8712]
    assert out_block == [8713]
    assert out_logits == [8714]
...
def test_no_partial_publish_on_canonical_mismatch() -> None:
    ...
    assert status == MODEL_Q16_ERR_BAD_PARAM
    assert out_dense == [9721]
    assert out_workspace == [9722]
    assert out_block == [9723]
    assert out_logits == [9724]


def test_randomized_geometry_vectors() -> None:
    rng = random.Random(20260422_1077)

    for _ in range(1400):
        row_count = rng.randint(0, 196)
        lane_count = rng.randint(0, 196)
        block_count = rng.randint(0, 48)
...
if __name__ == "__main__":
    test_source_contains_iq_1077_symbol()
