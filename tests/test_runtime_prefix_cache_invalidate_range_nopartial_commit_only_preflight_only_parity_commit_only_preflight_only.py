#!/usr/bin/env python3
"""Host-side harness for IQ-1340 PrefixCacheInvalidateRangeCheckedNoPartialCommitOnlyPreflightOnlyParityCommitOnlyPreflightOnly."""
...
def test_iq1340_preflight_only_matches_commit_only_parity_tuple():
    ...
    status, removed = invalidate_range_commit_only_preflight_only_parity_commit_only_preflight_only(cache, 12, 30)
...
def test_iq1340_preflight_only_zero_length_invalidate_is_noop():
    ...
    status, removed = invalidate_range_commit_only_preflight_only_parity_commit_only_preflight_only(cache, 12, 0)
    assert status == PREFIX_CACHE_OK
    assert removed == 0
...
def test_iq1340_preflight_only_guard_keeps_state_unchanged_on_error():
    ...
    status, removed = invalidate_range_commit_only_preflight_only_parity_commit_only_preflight_only(cache, 0, 10)
...
def test_holyc_function_body_and_contract_markers_present():
    ...
    assert (
        "status_parity = PrefixCacheInvalidateRangeCheckedNoPartialCommitOnlyPreflightOnlyParity(cache," in body
    )
    assert "if (commit_removed_count != parity_removed_count ||" in body
    assert "*out_removed_count = commit_removed_count;" in body


if __name__ == "__main__":
    test_iq1340_preflight_only_matches_commit_only_parity_tuple()
    test_iq1340_preflight_only_zero_length_invalidate_is_noop()
    test_iq1340_preflight_only_guard_keeps_state_unchanged_on_error()
