#!/usr/bin/env python3
"""Harness for IQ-1743 fast-path batch audit commit-only wrapper."""
...
def _cross_gate_model(
    ...
    if (
        ...
        return GPU_SEC_PERF_ERR_POLICY_GUARD, 0, GPU_SEC_PERF_FAST_PATH_DISABLE_REASON_OVERHEAD_BUDGET_BREACH
...
def iq1743_model(
    ...
    dispatch_transcript_parity: int,
    *,
    initial_out: tuple[int, int, int, int] = (211, 223, 227, 229),
    ...
    if status_primary == GPU_SEC_PERF_OK:
        if tuple_primary[0] < 0 or tuple_primary[1] < 0 or tuple_primary[2] < 0:
            return GPU_SEC_PERF_ERR_BAD_PARAM, saved_out
    ...
    if status_parity == GPU_SEC_PERF_OK:
        if tuple_parity[0] < 0 or tuple_parity[1] < 0 or tuple_parity[2] < 0:
            return GPU_SEC_PERF_ERR_BAD_PARAM, saved_out
    ...
    return GPU_SEC_PERF_OK, tuple_primary


def test_source_contains_iq1743_symbols() -> None:
    src = Path("src/gpu/security_perf_matrix.HC").read_text(encoding="utf-8")
    assert f"I32 {HC_FN_IQ1743}(" in src
    assert f"status_primary = {HC_FN_IQ1742}(" in src
    assert f"status_parity = {HC_FN_IQ1742}(" in src
    assert "if (status_primary != status_parity)" in src
    assert "// IQ-1743 commit-only hardening wrapper over batch audit:" in src


def test_gate_missing_vector() -> None:
    status, out_values = iq1743_model(
...
def test_digest_drift_vector() -> None:
    status, out_values = iq1743_model(
        ...
        policy_digest_parity=1,
        dispatch_transcript_parity=1,
        force_digest_drift_second=True,
    )
    assert status == GPU_SEC_PERF_ERR_BAD_PARAM
    assert out_values == (211, 223, 227, 229)
...
def test_status_domain_drift_vector() -> None:
    status, out_values = iq1743_model(
        ...
        policy_digest_parity=1,
        dispatch_transcript_parity=1,
        force_status_domain_drift_second=True,
    )
    assert status == GPU_SEC_PERF_ERR_BAD_PARAM
    assert out_values == (211, 223, 227, 229)
...
if __name__ == "__main__":
    test_source_contains_iq1743_symbols()
