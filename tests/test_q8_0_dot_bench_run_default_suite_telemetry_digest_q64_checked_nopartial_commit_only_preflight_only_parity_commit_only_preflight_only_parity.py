#!/usr/bin/env python3
"""Strict parity diagnostics gate checks for IQ-1331 telemetry digest wrapper."""
...
def telemetry_digest_q64_checked_nopartial_commit_only_preflight_only_parity_commit_only_preflight_only(
    ...
    parity_probe = [0]
    status = telemetry_digest_q64_checked_nopartial_commit_only_preflight_only_parity_commit_only(
        total_ops,
        total_cycles,
        cycles_per_op,
        remainder_cycles,
...
def telemetry_digest_q64_checked_nopartial_commit_only_preflight_only_parity_commit_only_preflight_only_parity(
    ...
    out_ptr: list[int] | None,
    *,
    force_preflight_digest: int | None = None,
) -> int:
    if out_ptr is None:
        return ERR_NULL_PTR
    ...
    preflight_probe = [0x1122334455667788]
    ...
    status, preflight_digest = telemetry_digest_q64_checked(
        total_ops,
        total_cycles,
        cycles_per_op,
        remainder_cycles,
    ...
    if status != ERR_OK:
        return status
    if force_preflight_digest is not None:
        preflight_digest = force_preflight_digest
    ...
    if preflight_digest != commit_probe[0]:
        return ERR_BAD_PARAM

    out_ptr[0] = preflight_digest
    return ERR_OK


def test_source_contains_iq1331_function_and_guards() -> None:
    ...
    assert "if (preflight_digest_q64 != commit_digest_q64)" in body
    assert "*out_digest_q64 = preflight_digest_q64;" in body
...
if __name__ == "__main__":
    test_source_contains_iq1331_function_and_guards()
