#!/usr/bin/env python3
"""Reference checks for GGUFMetadataReadI16PairChecked semantics."""
...
def gguf_metadata_read_i16_pair_checked(
    buf: list[int] | None,
    buf_nbytes: int,
    cursor_ref: list[int] | None,
    table_end: int,
...
def test_null_ptr_and_no_partial_write() -> None:
    ...
    assert (
        gguf_metadata_read_i16_pair_checked(
            None,
            8,
            cursor,
            8,
...
def test_second_scalar_fail_does_not_commit_first_or_cursor() -> None:
    first = reinterpret_u16_as_i16(0xFFAA)
    buf = _le_u16(0xFFAA) + [0x20]

    cursor = [0]
    out_first = [-1]
    out_second = [-2]

    err = gguf_metadata_read_i16_pair_checked(
        buf,
        len(buf),
        cursor,
        len(buf),
...
def test_success_reads_two_i16_and_advances() -> None:
    first_raw = 0x7FFF
    second_raw = 0x8001
    first = reinterpret_u16_as_i16(first_raw)
    second = reinterpret_u16_as_i16(second_raw)
    buf = [0x33] + _le_u16(first_raw) + _le_u16(second_raw) + [0x44]
...
def test_randomized_parity() -> None:
    rng = random.Random(20260417_222)
    ...
    for _ in range(5000):
        ...
        if cursor0 + 4 <= table_end and cursor0 + 4 <= n:
            raw_first = buf[cursor0] | (buf[cursor0 + 1] << 8)
            raw_second = buf[cursor0 + 2] | (buf[cursor0 + 3] << 8)
            expect_first = reinterpret_u16_as_i16(raw_first)
            expect_second = reinterpret_u16_as_i16(raw_second)
            assert err == GGUF_META_TABLE_OK
...
def run() -> None:
    ...
    test_success_reads_two_i16_and_advances()
    test_overflow_passthrough_and_no_commit()
    test_randomized_parity()
    print("gguf_metadata_read_i16_pair_checked_reference_checks=ok")
