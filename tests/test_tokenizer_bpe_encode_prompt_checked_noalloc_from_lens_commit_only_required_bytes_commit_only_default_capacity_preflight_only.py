#!/usr/bin/env python3
"""Parity harness for ...CommitOnlyDefaultCapacityPreflightOnly."""
...
from test_tokenizer_bpe_encode_prompt_checked_noalloc_from_lens_commit_only_required_bytes_commit_only_default_capacity import (
    tokenizer_bpe_encode_prompt_checked_noalloc_from_lens_commit_only_required_bytes_commit_only_default_capacity,
)
...
def tokenizer_bpe_encode_prompt_checked_noalloc_from_lens_commit_only_required_bytes_commit_only_default_capacity_preflight_only(
    data: list[int] | None,
    byte_len: int,
...
def test_source_contains_signature_and_parity_calls() -> None:
    ...
    sig = "I32 TokenizerBPEEncodePromptCheckedNoAllocFromLensCommitOnlyRequiredBytesCommitOnlyDefaultCapacityPreflightOnly("
...
def test_success_error_no_write_parity() -> None:
    ...
    err_a = tokenizer_bpe_encode_prompt_checked_noalloc_from_lens_commit_only_required_bytes_commit_only_default_capacity_preflight_only(
        payload,
        len(payload),
        cur_a,
        len(payload),
...
if __name__ == "__main__":
    test_source_contains_signature_and_parity_calls()
    test_success_error_no_write_parity()
