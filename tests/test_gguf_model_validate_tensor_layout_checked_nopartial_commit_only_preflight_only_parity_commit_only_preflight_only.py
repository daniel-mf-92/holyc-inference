#!/usr/bin/env python3
"""Harness for IQ-1127 tensor-layout parity commit-only preflight wrapper."""
...
from test_gguf_model_validate_tensor_layout_checked_nopartial_commit_only_preflight_only_parity_commit_only import (  # noqa: E402
    gguf_model_validate_tensor_layout_checked_nopartial_commit_only_preflight_only_parity_commit_only_reference,
)


def gguf_model_validate_tensor_layout_checked_nopartial_commit_only_preflight_only_parity_commit_only_preflight_only_reference(
    *,
...
def test_source_contains_iq1127_signature_and_contract() -> None:
    source = Path("src/gguf/validator.HC").read_text(encoding="utf-8")
    sig = "I64 GGUFModelValidateTensorLayoutCheckedNoPartialCommitOnlyPreflightOnlyParityCommitOnlyPreflightOnly("
    ...
    assert "GGUFModelValidateTensorLayoutCheckedNoPartialCommitOnlyPreflightOnlyParityCommitOnly(" in body
...
def test_success_matches_parity_and_preflight_publish() -> None:
    ...
    rc = gguf_model_validate_tensor_layout_checked_nopartial_commit_only_preflight_only_parity_commit_only_preflight_only_reference(
        tensor_types=tensor_types,
        tensor_offsets=tensor_offsets,
        tensor_n_dims=tensor_n_dims,
        tensor_dims_flat=tensor_dims_flat,
...
def run() -> None:
    test_source_contains_iq1127_signature_and_contract()
