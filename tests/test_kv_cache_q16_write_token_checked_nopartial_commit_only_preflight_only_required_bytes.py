#!/usr/bin/env python3
"""Parity harness for KVCacheQ16WriteToken...PreflightOnlyRequiredBytes (IQ-884)."""
...
def kv_cache_q16_write_token_checked_nopartial_commit_only_preflight_only_required_bytes(
...
def test_source_contains_required_bytes_helper() -> None:
    ...
    sig = "I32 KVCacheQ16WriteTokenCheckedNoPartialCommitOnlyPreflightOnlyRequiredBytes("
    assert sig in source
    body = source.split(sig, 1)[1]
    assert "KVCacheQ16WriteTokenCheckedNoPartialCommitOnlyPreflightOnly(" in body
...
def test_known_vector_required_bytes_and_zero_write_semantics() -> None:
    ...
    err = kv_cache_q16_write_token_checked_nopartial_commit_only_preflight_only_required_bytes(
        k_cache,
        len(k_cache),
        v_cache,
        len(v_cache),
    ...
    expected_base = ((3 * token_capacity) + 1) * span
...
def test_alias_null_and_no_partial_output_contracts() -> None:
    ...
    assert (
        kv_cache_q16_write_token_checked_nopartial_commit_only_preflight_only_required_bytes(
            k_cache,
            len(k_cache),
            v_cache,
            len(v_cache),
...
def test_randomized_parity_vs_explicit_composition_adversarial() -> None:
    ...
    for _ in range(800):
        ...
        k_src = [rng.randint(-50, 50) for _ in range(max(1, min(span, 64)))]
        v_src = [rng.randint(-50, 50) for _ in range(max(1, min(span, 64)))]
        ...
        if rng.random() < 0.15:
            shrink = rng.randint(1, max(1, k_src_capacity))
            k_src_capacity = max(0, k_src_capacity - shrink)
            v_src_capacity = max(0, v_src_capacity - shrink)

        # Force required-bytes overflow path while keeping list allocations tiny.
        if rng.random() < 0.05:
            layer_count = 1
            token_capacity = 1
            layer_idx = 0
            ...
            head_dim = 1
            k_src_capacity = kv_heads
            v_src_capacity = kv_heads
        ...
        err_a = kv_cache_q16_write_token_checked_nopartial_commit_only_preflight_only_required_bytes(
            k_cache,
            k_cache_capacity,
            v_cache,
            v_cache_capacity,
