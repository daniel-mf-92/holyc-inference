#!/usr/bin/env python3
"""Reference checks for FPQ16MulDivArrayRoundedByPositiveIntFromQ16DenChecked semantics."""
...
def test_encoded_denominator_contracts() -> None:
    one = FP_Q16_ONE

    err, out = fpq16_mul_div_array_rounded_by_positive_int_from_q16den_checked([one], [one], [0])
    assert err == FP_Q16_ERR_DOMAIN
    assert out is None

    err, out = fpq16_mul_div_array_rounded_by_positive_int_from_q16den_checked([one], [one], [-one])
    assert err == FP_Q16_ERR_DOMAIN
    assert out is None

    err, out = fpq16_mul_div_array_rounded_by_positive_int_from_q16den_checked([one], [one], [one + 7])
    assert err == FP_Q16_ERR_BAD_PARAM
    assert out is None


def test_no_partial_write_preflight_model() -> None:
    one = FP_Q16_ONE
    a = [one, one, one]
    b = [one, one, one]
    d = [one, one + 3, one]

    err, out = fpq16_mul_div_array_rounded_by_positive_int_from_q16den_checked(a, b, d)
    assert err == FP_Q16_ERR_BAD_PARAM
    assert out is None


def test_randomized_lane_parity() -> None:
    rng = random.Random(20260417_191)
    ...
    for _ in range(6000):
        ...
        err, out = fpq16_mul_div_array_rounded_by_positive_int_from_q16den_checked(a, b, d_q16)
        if err != FP_Q16_OK:
            assert err in (FP_Q16_ERR_BAD_PARAM, FP_Q16_ERR_DOMAIN, FP_Q16_ERR_OVERFLOW)
            assert out is None
            continue

        assert out is not None
        for ai, bi, di_q16, got in zip(a, b, d_q16, out):
...
def run() -> None:
    test_encoded_denominator_contracts()
    test_no_partial_write_preflight_model()
    test_randomized_lane_parity()
    print("fixedpoint_q16_muldiv_array_positive_int_from_q16den_checked_reference_checks=ok")
