#!/usr/bin/env python3
"""Harness for IQ-1612 zero-write diagnostics companion over IQ-1610 and IQ-1609."""
...
def _fast_path_switch_checked_audit_parity_iq1609(
    secure_local_mode: int,
    iommu_active: int,
    book_of_truth_gpu_hooks: int,
    policy_digest_parity: int,
...
def _fast_path_switch_checked_audit_parity_iq1610(
    secure_local_mode: int,
    iommu_active: int,
    book_of_truth_gpu_hooks: int,
    policy_digest_parity: int,
    ...
    status_parity, parity_enabled, parity_reason = _fast_path_switch_checked_audit_parity_iq1609(
        secure_local_mode,
        iommu_active,
        book_of_truth_gpu_hooks,
        policy_digest_parity,
...
def fast_path_switch_checked_audit_parity_iq1612(
    secure_local_mode: int,
    iommu_active: int,
    book_of_truth_gpu_hooks: int,
    policy_digest_parity: int,
    ...
    *,
    ...
    status_commit_only, staged_enabled, staged_reason = _fast_path_switch_checked_audit_parity_iq1610(
        secure_local_mode,
        iommu_active,
        book_of_truth_gpu_hooks,
        policy_digest_parity,
    ...
    status_parity, canonical_enabled, canonical_reason = _fast_path_switch_checked_audit_parity_iq1609(
        secure_local_mode,
        iommu_active,
        book_of_truth_gpu_hooks,
        policy_digest_parity,
...
def test_source_contains_iq1612_symbols() -> None:
    src = Path("src/gpu/security_perf_matrix.HC").read_text(encoding="utf-8")

    assert "I32 GPUSecurityPerfFastPathSwitchCheckedAuditParityCommitOnlyPreflightOnlyParityCommitOnlyPreflightOnlyParityCommitOnlyPreflightOnly(" in src
...
def test_gate_missing_and_audit_parity_breach_vectors() -> None:
    status, enabled, reason = fast_path_switch_checked_audit_parity_iq1612(
...
def test_status_domain_drift_no_write_parity_and_secure_on_budget_vectors() -> None:
    status, enabled, reason = fast_path_switch_checked_audit_parity_iq1612(
        secure_local_mode=GPU_SEC_PERF_PROFILE_SECURE_LOCAL,
        iommu_active=1,
        book_of_truth_gpu_hooks=1,
    ...
    # No-write parity vector: IQ-1610 and IQ-1609 both fail-closed under secure-pass tuple,
    # and IQ-1612 must preserve caller slots exactly.
    status, enabled, reason = fast_path_switch_checked_audit_parity_iq1612(
        secure_local_mode=GPU_SEC_PERF_PROFILE_SECURE_LOCAL,
        iommu_active=1,
        book_of_truth_gpu_hooks=1,
    ...
    # Secure-on overhead-budget vector: keep secure-local gates enabled and verify deterministic no-write behavior.
    status, enabled, reason = fast_path_switch_checked_audit_parity_iq1612(
        secure_local_mode=GPU_SEC_PERF_PROFILE_SECURE_LOCAL,
        iommu_active=1,
        book_of_truth_gpu_hooks=1,
