#!/usr/bin/env python3
"""Harness for IQ-1470 row-gate preflight-only parity diagnostics wrapper."""
...
def test_source_contains_iq1470_symbols() -> None:
    src = Path("src/gpu/security_perf_matrix.HC").read_text(encoding="utf-8")

    assert "I32 GPUSecurityPerfMatrixRowGateCheckedNoPartialPreflightOnlyParity(" in src
...
def test_null_alias_capacity_vectors() -> None:
    status, reason, allowed = gpu_security_perf_matrix_row_gate_checked_nopartial_preflight_only_parity(
...
def test_gate_missing_and_reason_code_parity_vectors() -> None:
    ...
    for vector in vectors:
        parity_status, _, _ = gpu_security_perf_matrix_row_gate_checked_nopartial_preflight_only_parity(
            *vector,
        ...
        assert parity_status == preflight_status
...
if __name__ == "__main__":
    test_source_contains_iq1470_symbols()
