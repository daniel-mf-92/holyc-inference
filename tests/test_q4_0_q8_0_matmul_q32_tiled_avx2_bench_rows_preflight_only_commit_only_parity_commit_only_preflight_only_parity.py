#!/usr/bin/env python3
"""Parity harness for ...PreflightOnlyCommitOnlyParityCommitOnlyPreflightOnlyParity (IQ-856)."""
...
from test_q4_0_q8_0_matmul_q32_tiled_avx2_bench_rows_preflight_only_commit_only_parity_commit_only_preflight_only import (
    q4_0_q8_0_matmul_q32_tiled_avx2_bench_rows_preflight_only_commit_only_parity_commit_only_preflight_only,
)


def q4_0_q8_0_matmul_q32_tiled_avx2_bench_rows_preflight_only_commit_only_parity_commit_only_preflight_only_parity(
    lhs_q4_blocks,
    ...
    if preflight_cells_per_iter[0] != commit_cells_per_iter[0]:
        return Q4_0_Q8_0_AVX2_ERR_BAD_LEN
    if preflight_block_dots_per_iter[0] != commit_block_dots_per_iter[0]:
        return Q4_0_Q8_0_AVX2_ERR_BAD_LEN
    if preflight_total_cells[0] != commit_total_cells[0]:
        return Q4_0_Q8_0_AVX2_ERR_BAD_LEN
    if preflight_total_block_dots[0] != commit_total_block_dots[0]:
        return Q4_0_Q8_0_AVX2_ERR_BAD_LEN
...
def explicit_checked_composition(*args):
    return q4_0_q8_0_matmul_q32_tiled_avx2_bench_rows_preflight_only_commit_only_parity_commit_only_preflight_only_parity(
        *args
    )
...
def test_source_contains_signature_and_parity_gate() -> None:
    ...
    sig = "I32 Q4_0Q8_0MatMulQ32TiledAVX2BenchRowsPreflightOnlyCommitOnlyParityCommitOnlyPreflightOnlyParity("
    assert sig in source
    body = source.split(sig, 1)[1]

    assert "Q4_0Q8_0MatMulQ32TiledAVX2BenchRowsPreflightOnlyCommitOnlyParityCommitOnlyPreflightOnly(" in body
    assert "Q4_0Q8_0MatMulQ32TiledAVX2BenchRowsPreflightOnlyCommitOnlyParityCommitOnly(" in body
    assert "if (preflight_cells_per_iter != commit_cells_per_iter)" in body
    assert "if (preflight_block_dots_per_iter != commit_block_dots_per_iter)" in body
    assert "if (preflight_total_cells != commit_total_cells)" in body
    assert "if (preflight_total_block_dots != commit_total_block_dots)" in body
...
def test_null_output_ptr_rejected() -> None:
    rng = random.Random(20260421_8562)
...
def test_fuzz_parity_matches_explicit_checked_composition() -> None:
    ...
    for i in range(2600):
        ...
        rng = random.Random(20260421_1856000 + i)
        ...
        err_a = q4_0_q8_0_matmul_q32_tiled_avx2_bench_rows_preflight_only_commit_only_parity_commit_only_preflight_only_parity(
            lhs,
            lhs_capacity,
            row_count,
            lhs_stride,
