#!/usr/bin/env python3
"""Parity harness for KVCacheQ16ReserveTokenSpan...RequiredBytesParityCommitOnlyPreflightOnly (IQ-1072)."""
...
def explicit_composition(
    ...
    if cells[0] != cells2[0] or bytes_[0] != bytes2[0] or last[0] != last2[0]:
...
def test_source_contains_required_bytes_parity_commit_only_preflight_only_helpers() -> None:
    ...
    sig_target = "I32 KVCacheQ16ReserveTokenSpanCheckedNoPartialCommitOnlyPreflightOnlyRequiredBytesParityCommitOnlyPreflightOnlyParity("
    ...
    assert "KVCacheQ16ReserveTokenSpanCheckedNoPartialCommitOnlyPreflightOnlyRequiredBytesParityCommitOnlyPreflightOnly(" in body_target
    assert "KVCacheQ16ReserveTokenSpanCheckedNoPartialCommitOnlyPreflightOnlyRequiredBytesParityCommitOnly(" in body_target
    assert "if (staged_preflight_required_cells != staged_commit_required_cells)" in body_target
