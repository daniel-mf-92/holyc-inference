#!/usr/bin/env python3
"""Parity harness for IQ-1180 ...PreflightOnlyCommitOnlyParityCommitOnlyPreflightOnly."""
...
def q8_0_dot_bench_run_default_suite_preflight_only_commit_only_parity_commit_only_preflight_only(
    cpu_hz: int,
    ...
    commit_shape_count = [out_shape_count[0]]
    commit_max_blocks = [out_max_blocks[0]]
    commit_max_rows = [out_max_rows[0]]
    commit_max_row_matrix_blocks = [out_max_row_matrix_blocks[0]]
    ...
    if (
        commit_shape_count[0] != canonical_shape_count
        or commit_max_blocks[0] != canonical_max_blocks
        or commit_max_rows[0] != canonical_max_rows
        or commit_max_row_matrix_blocks[0] != canonical_max_row_matrix_blocks
    ):
        return Q8_0_DOT_BENCH_ERR_BAD_PARAM

    out_shape_count[0] = commit_shape_count[0]
    out_max_blocks[0] = commit_max_blocks[0]
    out_max_rows[0] = commit_max_rows[0]
    out_max_row_matrix_blocks[0] = commit_max_row_matrix_blocks[0]
...
def explicit_checked_composition(
    ...
    return q8_0_dot_bench_run_default_suite_preflight_only_commit_only_parity_commit_only_preflight_only(
        cpu_hz,
        out_shape_count,
        out_max_blocks,
        out_max_rows,
...
def test_source_contains_iq1180_function_and_guards() -> None:
    source = Path("src/bench/q8_0_dot_bench.HC").read_text(encoding="utf-8")
    sig = "I32 Q8_0DotBenchRunDefaultSuitePreflightOnlyCommitOnlyParityCommitOnlyPreflightOnly("
    ...
    assert "if (commit_shape_count != canonical_shape_count ||" in body
    assert "*out_shape_count = commit_shape_count;" in body
    assert "*out_max_blocks = commit_max_blocks;" in body
    assert "*out_max_rows = commit_max_rows;" in body
    assert "*out_max_row_matrix_blocks = commit_max_row_matrix_blocks;" in body


def test_preflight_only_commit_only_parity_commit_only_preflight_only_no_partial_on_error() -> None:
    assert (
...
def test_preflight_only_commit_only_parity_commit_only_preflight_only_happy_path() -> None:
    out_shape = [-1]
    out_blocks = [-1]
    out_rows = [-1]
    out_row_matrix = [-1]
...
def test_preflight_only_commit_only_parity_commit_only_preflight_only_randomized_parity() -> None:
    rng = random.Random(20260422_1180)
...
if __name__ == "__main__":
    test_source_contains_iq1180_function_and_guards()
    test_preflight_only_commit_only_parity_commit_only_preflight_only_no_partial_on_error()
    test_preflight_only_commit_only_parity_commit_only_preflight_only_happy_path()
    test_preflight_only_commit_only_parity_commit_only_preflight_only_randomized_parity()
    print("ok")
