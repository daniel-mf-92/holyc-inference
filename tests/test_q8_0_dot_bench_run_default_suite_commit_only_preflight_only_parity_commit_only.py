#!/usr/bin/env python3
"""IQ-1302: parity-commit-only wrapper checks for default Q8_0 dot bench suite."""
...
def test_source_contains_iq1302_function_and_guards() -> None:
    source = Path("src/bench/q8_0_dot_bench.HC").read_text(encoding="utf-8")
    sig = "I32 Q8_0DotBenchRunDefaultSuiteCommitOnlyPreflightOnlyParityCommitOnly("
    ...
    assert "status = Q8_0DotBenchRunDefaultSuiteCommitOnlyPreflightOnly(" in body
    assert "if (parity_total_ops != commit_total_ops ||" in body
    assert "*out_total_ops = parity_total_ops;" in body
    assert "*out_total_cycles = parity_total_cycles;" in body
    assert "*out_cycles_per_op = parity_cycles_per_op;" in body
    assert "*out_remainder_cycles = parity_remainder_cycles;" in body
    assert "*out_suite_checksum = parity_suite_checksum;" in body


def test_null_and_alias_guards() -> None:
    assert parity_commit_only(3_200_000_000, None, [1], [1], [1], [1]) == Q8_0_DOT_BENCH_ERR_NULL_PTR

    same = [77]
    status = parity_commit_only(3_200_000_000, same, same, [3], [4], [5])
...
def test_parity_commit_tuple_matches_canonical() -> None:
    out_total_ops = [0]
    out_total_cycles = [0]
    out_cycles_per_op = [0]
...
if __name__ == "__main__":
    test_source_contains_iq1302_function_and_guards()
    test_null_and_alias_guards()
    test_bad_frequency_no_partial_commit()
    test_parity_commit_tuple_matches_canonical()
    print("ok")
