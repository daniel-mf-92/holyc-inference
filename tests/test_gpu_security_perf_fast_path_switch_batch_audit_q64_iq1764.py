#!/usr/bin/env python3
"""Harness for IQ-1764 fast-path batch audit zero-write diagnostics companion."""
...
import test_gpu_security_perf_fast_path_switch_batch_audit_q64_iq1763 as iq1763

GPU_SEC_PERF_OK = iq1763.GPU_SEC_PERF_OK
GPU_SEC_PERF_ERR_BAD_PARAM = iq1763.GPU_SEC_PERF_ERR_BAD_PARAM
GPU_SEC_PERF_PROFILE_SECURE_LOCAL = iq1763.GPU_SEC_PERF_PROFILE_SECURE_LOCAL

Scenario = iq1763.Scenario

HC_FN_IQ1764 = ""
HC_FN_IQ1763 = iq1763.HC_FN_IQ1763
HC_FN_IQ1762 = iq1763.HC_FN_IQ1762


def _seed_iq1762_tuple(
    ...
    seed_out = iq1763.iq1762._seed_iq1760_tuple(
        scenarios=scenarios,
    ...
    status, out_tuple = iq1763.iq1762.iq1762_model(
        scenarios=scenarios,
        secure_local_mode=secure_local_mode,
        iommu_active=iommu_active,
        book_of_truth_gpu_hooks=book_of_truth_gpu_hooks,
...
def iq1764_model(
    ...
    *,
    ...
    status_companion, staged_tuple = iq1763.iq1763_model(
        scenarios=scenarios,
        secure_local_mode=secure_local_mode,
        iommu_active=iommu_active,
        book_of_truth_gpu_hooks=book_of_truth_gpu_hooks,
    ...
    status_canonical, canonical_tuple = iq1763.iq1762.iq1762_model(
        scenarios=scenarios,
        secure_local_mode=secure_local_mode,
        iommu_active=iommu_active,
        book_of_truth_gpu_hooks=book_of_truth_gpu_hooks,
    ...
    if not iq1763.iq1762.iq1761.iq1760.iq1759.iq1758.iq1757.iq1756.iq1755.iq1754.iq1753.iq1752.iq1751.iq1750.iq1749.iq1748.iq1747.iq1746._status_is_valid(status_companion):
        return GPU_SEC_PERF_ERR_BAD_PARAM, saved_out
    if not iq1763.iq1762.iq1761.iq1760.iq1759.iq1758.iq1757.iq1756.iq1755.iq1754.iq1753.iq1752.iq1751.iq1750.iq1749.iq1748.iq1747.iq1746._status_is_valid(status_canonical):
        return GPU_SEC_PERF_ERR_BAD_PARAM, saved_out
...
def test_source_contains_iq1764_symbols() -> None:
    src = Path("src/gpu/security_perf_matrix.HC").read_text(encoding="utf-8")
    assert f"I32 {HC_FN_IQ1764}(" in src
    assert f"status_companion = {HC_FN_IQ1763}(" in src
    assert f"status_canonical = {HC_FN_IQ1762}(" in src
    assert "if (status_companion != status_canonical)" in src
    assert "// IQ-1764 zero-write diagnostics companion:" in src


def test_gate_missing_vector() -> None:
    scenarios = [Scenario(8, 16, 32, 64), Scenario(10, 20, 40, 80), Scenario(12, 24, 48, 96)]
    saved_out = _seed_iq1762_tuple(
        scenarios=scenarios,
    ...
    status, out_values = iq1764_model(
        scenarios=scenarios,
        secure_local_mode=GPU_SEC_PERF_PROFILE_SECURE_LOCAL,
        iommu_active=0,
        book_of_truth_gpu_hooks=1,
...
if __name__ == "__main__":
    test_source_contains_iq1764_symbols()
