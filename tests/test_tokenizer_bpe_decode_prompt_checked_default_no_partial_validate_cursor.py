#!/usr/bin/env python3
"""Parity harness for TokenizerBPEDecodePromptCheckedDefaultNoPartialValidateCursor."""
...
def tokenizer_bpe_decode_prompt_checked_default_no_partial_validate_cursor(
...
def run_case(token_ids: list[int], token_cursor: int, pieces: list[bytes], max_piece_bytes: int) -> None:
    ...
    err_wrapped = tokenizer_bpe_decode_prompt_checked_default_no_partial_validate_cursor(
        token_ids,
        len(token_ids),
        cursor_wrapped,
        blob,
...
def test_validate_cursor_rejects_overflow_and_oob() -> None:
    ...
    cursor = [4]
    err = tokenizer_bpe_decode_prompt_checked_default_no_partial_validate_cursor(
        [0, 1, 2],
        3,
        cursor,
        blob,
...
def test_capacity_and_null_contracts() -> None:
    ...
    assert (
        tokenizer_bpe_decode_prompt_checked_default_no_partial_validate_cursor(
            None,
            0,
            cursor,
            blob,
...
def test_randomized_parity_against_explicit_staged_composition() -> None:
    rng = random.Random(20260418_423)
    pieces = [
        b"a",
        b"bc",
        b"def",
...
if __name__ == "__main__":
    ...
    print("test_tokenizer_bpe_decode_prompt_checked_default_no_partial_validate_cursor: ok")
