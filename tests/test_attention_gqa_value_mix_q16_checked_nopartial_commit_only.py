#!/usr/bin/env python3
"""Reference checks for GQAAttentionValueMixQ16CheckedNoPartialCommitOnly (IQ-1374)."""
...
def gqa_attention_value_mix_q16_checked_nopartial_commit_only(
    scores_q16,
    scores_capacity: int,
    query_rows: int,
    key_rows: int,
...
def explicit_commit_only_composition(*args, **kwargs) -> int:
    return gqa_attention_value_mix_q16_checked_nopartial_commit_only(*args, **kwargs)
...
def test_fixed_vector_reference() -> None:
    ...
    out_a = [777] * (query_rows * value_dim)
    out_b = out_a.copy()

    err_a = gqa_attention_value_mix_q16_checked_nopartial_commit_only(
        scores_q16,
        len(scores_q16),
        query_rows,
        key_rows,
...
def test_null_alias_capacity_and_overflow_contracts() -> None:
    seed = [9191] * 8
...
def test_randomized_parity() -> None:
    rng = random.Random(20260425_1374)
    ...
    for _ in range(220):
        ...
        out_a = [rng.randint(-777, 777) for _ in range(out_cells)]
        out_b = out_a.copy()
...
def test_source_contract_markers() -> None:
    ...
    assert "I32 GQAAttentionValueMixQ16CheckedNoPartialCommitOnly(" in source
    body = source.split("I32 GQAAttentionValueMixQ16CheckedNoPartialCommitOnly(", 1)[1]
...
if __name__ == "__main__":
    ...
    test_null_alias_capacity_and_overflow_contracts()
    test_randomized_parity()
    test_source_contract_markers()
    print("gqa_attention_value_mix_q16_checked_nopartial_commit_only_reference_checks=ok")
