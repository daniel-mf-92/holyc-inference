#!/usr/bin/env python3
"""Harness for FPQ16Sqrt...ParityCommitOnlyPreflightOnly (IQ-1238)."""
...
from test_intsqrt_q16_checked_nopartial_commit_only_preflight_only_parity import (  # noqa: E402
    fpq16_sqrt_checked_nopartial_commit_only_preflight_only_parity_reference,
)
...
def fpq16_sqrt_checked_nopartial_commit_only_preflight_only_parity_commit_only_preflight_only_reference(
    x_q16: int,
    ...
    *,
    ...
    status = fpq16_sqrt_checked_nopartial_commit_only_preflight_only_parity_reference(
        x_q16,
        canonical_required,
    )
    if status != FP_Q16_OK:
...
def test_source_contains_iq1238_function() -> None:
    source = Path("src/math/intsqrt.HC").read_text(encoding="utf-8")
    sig = "I32 FPQ16SqrtCheckedNoPartialCommitOnlyPreflightOnlyParityCommitOnlyPreflightOnly("
    ...
    assert "status = FPQ16SqrtCheckedNoPartialCommitOnlyPreflightOnlyParity(" in body
...
def test_snapshot_and_tuple_parity_guards() -> None:
    out = [11]
    err = fpq16_sqrt_checked_nopartial_commit_only_preflight_only_parity_commit_only_preflight_only_reference(
        q16_from_float(49.0),
        out,
...
def test_success_and_explicit_composition_parity() -> None:
    ...
    for x_q16 in vectors:
        ...
        err_a = fpq16_sqrt_checked_nopartial_commit_only_preflight_only_parity_commit_only_preflight_only_reference(
            x_q16,
            out_a,
        )
...
def test_randomized_parity_against_component() -> None:
    rng = random.Random(20260423_1238)
    ...
    for _ in range(12000):
        ...
        out_parity = [0]
        err_parity = fpq16_sqrt_checked_nopartial_commit_only_preflight_only_parity_reference(
            x_q16,
            out_parity,
        )
...
if __name__ == "__main__":
    test_source_contains_iq1238_function()
    ...
    print("intsqrt_q16_checked_nopartial_commit_only_preflight_only_parity_commit_only_preflight_only=ok")
