#!/usr/bin/env python3
"""Reference checks for GGUFMetadataReadU64TripleChecked semantics."""
...
def test_null_ptr_and_no_partial_write() -> None:
    ...
    assert (
        gguf_metadata_read_u64_triple_checked(
            None,
            64,
            cursor,
            64,
...
def test_third_scalar_fail_does_not_commit_anything() -> None:
    ...
    err = gguf_metadata_read_u64_triple_checked(
        buf,
        len(buf),
        cursor,
        len(buf),
...
def test_randomized_parity() -> None:
    rng = random.Random(20260417_280)
    ...
    for _ in range(6000):
        ...
        if cursor0 + 24 <= table_end:
            ...
            assert out_third[0] == 0x3333333333333333
