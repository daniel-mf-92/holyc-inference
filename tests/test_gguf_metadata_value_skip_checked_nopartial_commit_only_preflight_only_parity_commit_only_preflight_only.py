#!/usr/bin/env python3
"""Harness for ...ParityCommitOnlyPreflightOnly (IQ-1251)."""
...
def gguf_metadata_value_skip_checked_nopartial_commit_only_preflight_only_parity_commit_only_preflight_only_reference(
    *,
    buf: list[int] | None,
    buf_nbytes: int,
...
def test_source_contains_iq1251_function_and_strict_publish() -> None:
    source = Path("src/gguf/metadata.HC").read_text(encoding="utf-8")

    sig = "I32 GGUFMetadataValueSkipCheckedNoPartialCommitOnlyPreflightOnlyParityCommitOnlyPreflightOnly(U8 *buf,"
    sig_def = (
        "I32 GGUFMetadataValueSkipCheckedNoPartialCommitOnlyPreflightOnlyParityCommitOnlyPreflightOnly(U8 *buf,\n"
        "                                                                                               U64 buf_nbytes,\n"
        "                                                                                               U64 *cursor,\n"
        "                                                                                               U64 table_end,\n"
        "                                                                                               U32 value_type,\n"
...
def test_randomized_parity_vectors() -> None:
    rng = random.Random(20260423_1251)
    ...
    for _ in range(1200):
        ...
        cursor_a = [cursor0]
        out_a = [0xA1]
        status_a = gguf_metadata_value_skip_checked_nopartial_commit_only_preflight_only_parity_commit_only_preflight_only_reference(
            buf=buf,
            buf_nbytes=len(buf),
            cursor_ref=cursor_a,
            table_end=len(buf),
...
if __name__ == "__main__":
    test_source_contains_iq1251_function_and_strict_publish()
