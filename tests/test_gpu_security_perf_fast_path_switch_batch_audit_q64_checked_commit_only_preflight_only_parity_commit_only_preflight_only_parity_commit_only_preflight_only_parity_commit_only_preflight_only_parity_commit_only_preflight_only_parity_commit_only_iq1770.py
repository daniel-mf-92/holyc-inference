#!/usr/bin/env python3
"""Harness for IQ-1770 batch-audit commit-only hardening wrapper retag."""
...
HC_FN_IQ1770 = "GPUSecurityPerfFastPathSwitchBatchAuditQ64CheckedCommitOnlyPreflightOnlyParityCommitOnlyPreflightOnlyParityCommitOnlyPreflightOnlyParityCommitOnlyPreflightOnlyParityCommitOnlyPreflightOnlyParityCommitOnly"
HC_FN_SEED_PARITY = iq1768.HC_FN_IQ1768
...
def iq1770_model(
    ...
    *,
    ...
    status_seed, seeded_tuple = iq1768.iq1768_model(
        scenarios=scenarios,
    ...
    status_is_valid = iq1768.iq1767._status_is_valid
    ...
    status_parity, parity_tuple = iq1768.iq1768_model(
        scenarios=scenarios,
        secure_local_mode=secure_local_mode,
        iommu_active=iommu_active,
        book_of_truth_gpu_hooks=book_of_truth_gpu_hooks,
...
def test_source_contains_iq1770_symbols() -> None:
    src = Path("src/gpu/security_perf_matrix.HC").read_text(encoding="utf-8")
    assert f"I32 {HC_FN_IQ1770}(" in src
    assert f"I32 {HC_FN_PRIMARY}(" in src
    assert f"I32 {HC_FN_SEED_PARITY}(" in src
    assert "// IQ-1768 / IQ-1770 commit-only hardening wrapper refresh:" in src


def test_gate_missing_vector() -> None:
    status, out_values = iq1770_model(
...
if __name__ == "__main__":
    test_source_contains_iq1770_symbols()
