#!/usr/bin/env python3
"""Harness for IQ-1590 secure-local budget gate preflight-only parity commit-only wrapper."""
...
def secure_local_budget_gate_preflight_only_parity_commit_only(
    rows: list[RowOutput],
    rows_capacity: int,
    secure_local_mode: int,
    iommu_active: int,
...
def test_source_contains_iq1590_symbols() -> None:
    src = Path("src/gpu/security_perf_matrix.HC").read_text(encoding="utf-8")

    assert "I32 GPUSecurityPerfMatrixSummaryQ16CheckedOverheadEnvelopeSecureLocalBudgetGatePreflightOnlyParityCommitOnly(" in src
    assert "status_parity = GPUSecurityPerfMatrixSummaryQ16CheckedOverheadEnvelopeSecureLocalBudgetGatePreflightOnlyParity(" in src
...
def test_gate_missing_and_threshold_breach_vectors() -> None:
    ...
    status, outputs = secure_local_budget_gate_preflight_only_parity_commit_only(
        rows_gate_missing,
        rows_capacity=3,
...
def test_secure_local_pass_vector() -> None:
    ...
    status, outputs = secure_local_budget_gate_preflight_only_parity_commit_only(
        ...
        caller_outputs=(1234, 5678, GPU_SEC_PERF_BUDGET_GATE_STATUS_THRESHOLD_BREACH),
    )
    assert status == GPU_SEC_PERF_OK
    assert outputs == (4_200, 6_000, GPU_SEC_PERF_BUDGET_GATE_STATUS_PASS)
...
if __name__ == "__main__":
    test_source_contains_iq1590_symbols()
