#!/usr/bin/env python3
"""Harness for IQ-1602 strict diagnostics parity gate over IQ-1601 and IQ-1600."""
...
def secure_local_budget_gate_commit_only_preflight_only_parity_commit_only_preflight_only_parity(
    rows: list[RowOutput],
    rows_capacity: int,
    secure_local_mode: int,
    iommu_active: int,
...
def test_source_contains_iq1602_symbols() -> None:
    src = Path("src/gpu/security_perf_matrix.HC").read_text(encoding="utf-8")
    assert "I32 GPUSecurityPerfMatrixSummaryQ16CheckedOverheadEnvelopeSecureLocalBudgetGateCommitOnlyPreflightOnlyParityCommitOnlyPreflightOnlyParity(" in src
    ...
    assert "if (staged_budget_gate_status != canonical_budget_gate_status)" in src
...
def test_gate_missing_and_threshold_breach_vectors() -> None:
    ...
    status, outputs = secure_local_budget_gate_commit_only_preflight_only_parity_commit_only_preflight_only_parity(
        rows_gate_missing,
        rows_capacity=3,
...
if __name__ == "__main__":
    test_source_contains_iq1602_symbols()
