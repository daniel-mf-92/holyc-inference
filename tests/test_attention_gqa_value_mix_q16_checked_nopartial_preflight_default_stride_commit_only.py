#!/usr/bin/env python3
"""Reference checks for GQAAttentionValueMixQ16CheckedNoPartialPreflightDefaultStrideCommitOnly (IQ-1407)."""
...
def gqa_attention_value_mix_q16_checked_nopartial_preflight_default_stride_commit_only(
    scores_q16,
    scores_capacity: int,
    query_rows: int,
    key_rows: int,
    ...
    if (
        staged_default_score[0] > snapshot_scores_capacity
        or staged_default_value[0] > snapshot_values_capacity
        or staged_default_out[0] > snapshot_out_capacity
    ):
        return ATTN_Q16_ERR_BAD_PARAM
...
def test_parity_with_explicit_composition() -> None:
    rng = random.Random(20260425_1407)
    ...
    for _ in range(220):
        ...
        ref_value = [888]
        ref_out = [999]
        ref_err = gqa_attention_value_mix_q16_checked_nopartial_preflight_default_stride(
            scores,
            required_scores,
            query_rows,
            key_rows,
...
if __name__ == "__main__":
    ...
    test_parity_with_explicit_composition()
    print("ok")
