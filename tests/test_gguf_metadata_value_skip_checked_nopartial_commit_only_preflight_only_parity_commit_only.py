#!/usr/bin/env python3
"""Harness for GGUFMetadataValueSkipCheckedNoPartialCommitOnlyPreflightOnlyParityCommitOnly (IQ-1250)."""
...
from test_gguf_metadata_value_skip_checked_nopartial_commit_only_preflight_only import (
    gguf_metadata_value_skip_checked_nopartial_commit_only_preflight_only_reference,
)
from test_gguf_metadata_value_skip_checked_nopartial_commit_only_preflight_only_parity import (
    gguf_metadata_value_skip_checked_nopartial_commit_only_preflight_only_parity_reference,
)


def gguf_metadata_value_skip_checked_nopartial_commit_only_preflight_only_parity_commit_only_reference(
    *,
    buf: list[int] | None,
    buf_nbytes: int,
    ...
    err = gguf_metadata_value_skip_checked_nopartial_commit_only_preflight_only_parity_reference(
        buf=buf,
    ...
    err = gguf_metadata_value_skip_checked_nopartial_commit_only_preflight_only_reference(
        ...
        table_end=table_end,
        value_type=value_type,
        out_next_cursor_ref=staged_preflight_next,
    )
    if err != GGUF_META_TABLE_OK:
        return err

    if staged_preflight_cursor[0] != staged_preflight_next[0]:
        return GGUF_META_TABLE_ERR_BAD_PARAM
    if staged_preflight_next[0] > buf_nbytes or staged_preflight_next[0] > table_end:
        return GGUF_META_TABLE_ERR_OUT_OF_BOUNDS

    if staged_parity_next[0] != staged_preflight_next[0]:
        return GGUF_META_TABLE_ERR_BAD_PARAM
...
def test_source_contains_iq1250_function_and_strict_commit_publish() -> None:
    source = Path("src/gguf/metadata.HC").read_text(encoding="utf-8")

    sig = "I32 GGUFMetadataValueSkipCheckedNoPartialCommitOnlyPreflightOnlyParityCommitOnly(U8 *buf,"
    sig_def = (
        "I32 GGUFMetadataValueSkipCheckedNoPartialCommitOnlyPreflightOnlyParityCommitOnly(U8 *buf,\n"
        "                                                                                  U64 buf_nbytes,\n"
        "                                                                                  U64 *cursor,\n"
        "                                                                                  U64 table_end,\n"
        "                                                                                  U32 value_type,\n"
    ...
    assert "GGUFMetadataValueSkipCheckedNoPartialCommitOnlyPreflightOnlyParity(" in body
    assert "GGUFMetadataValueSkipCheckedNoPartialCommitOnlyPreflightOnly(" in body
    assert "snapshot_cursor = *cursor;" in body
    assert "staged_parity_next_cursor != staged_preflight_next_cursor" in body
...
def test_bad_type_no_partial_publish() -> None:
    ...
    assert cursor[0] == 8
    assert out_next[0] == 321


def test_array_string_span_success_and_truncation() -> None:
    payload = _u32le(GGUF_TYPE_STRING) + _u64le(2) + _string_payload(b"holy") + _string_payload(b"c")
    buf = [0] * 3 + list(payload) + [0] * 2
    ...
    assert err == GGUF_META_TABLE_OK
    assert cursor[0] == 3 + len(payload)
    assert out_next[0] == 3 + len(payload)

    truncated = buf[: 3 + len(payload) - 1]
    cursor2 = [3]
    out_next2 = [0xDEAD]
    err = gguf_metadata_value_skip_checked_nopartial_commit_only_preflight_only_parity_commit_only_reference(
        buf=truncated,
    ...
    assert err != GGUF_META_TABLE_OK
    assert cursor2[0] == 3
    assert out_next2[0] == 0xDEAD


def test_randomized_parity_vectors() -> None:
    rng = random.Random(20260423_1250)
    ...
    for _ in range(1200):
        ...
        cursor_a = [cursor0]
        out_a = [0xA1]
        status_a = gguf_metadata_value_skip_checked_nopartial_commit_only_preflight_only_parity_commit_only_reference(
            buf=buf,
            buf_nbytes=len(buf),
            cursor_ref=cursor_a,
            table_end=len(buf),
        ...
        cursor_b = [cursor0]
        out_b = [0xB2]
        status_b = gguf_metadata_value_skip_checked_nopartial_commit_only_preflight_only_parity_reference(
            buf=buf,
            buf_nbytes=len(buf),
            cursor_ref=cursor_b,
            table_end=len(buf),
...
if __name__ == "__main__":
    test_source_contains_iq1250_function_and_strict_commit_publish()
