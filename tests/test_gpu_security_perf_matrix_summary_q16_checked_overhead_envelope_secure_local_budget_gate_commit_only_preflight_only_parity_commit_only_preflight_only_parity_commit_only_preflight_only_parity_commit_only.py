#!/usr/bin/env python3
"""Harness for IQ-1606 commit-only hardening wrapper over IQ-1605 and IQ-1604."""
...
def _secure_local_budget_gate_commit_only_preflight_only_parity(
    ...
    *,
    ...
    status_preflight_only, _, staged_tuple = _secure_local_budget_gate_commit_only_preflight_only(
        ...
        force_canonical_status_domain_drift=force_parity_status_domain_drift,
    )
...
def secure_local_budget_gate_commit_only_preflight_only_parity_commit_only_preflight_only_parity_commit_only_preflight_only_parity_commit_only(
    rows: list[RowOutput],
    rows_capacity: int,
    secure_local_mode: int,
    iommu_active: int,
    ...
    *,
    ...
    status_parity, canonical_tuple = _secure_local_budget_gate_commit_only_preflight_only_parity(
        ...
        max_p95_overhead_q16,
        canonical_tuple,
        force_parity_status_domain_drift=force_parity_status_domain_drift,
    )
...
def test_source_contains_iq1606_symbols() -> None:
    src = Path("src/gpu/security_perf_matrix.HC").read_text(encoding="utf-8")
    assert "I32 GPUSecurityPerfMatrixSummaryQ16CheckedOverheadEnvelopeSecureLocalBudgetGateCommitOnlyPreflightOnlyParityCommitOnlyPreflightOnlyParityCommitOnlyPreflightOnlyParityCommitOnly(" in src
    assert "status_preflight_only = GPUSecurityPerfMatrixSummaryQ16CheckedOverheadEnvelopeSecureLocalBudgetGateCommitOnlyPreflightOnlyParityCommitOnlyPreflightOnlyParityCommitOnlyPreflightOnly(" in src
    assert "status_parity = GPUSecurityPerfMatrixSummaryQ16CheckedOverheadEnvelopeSecureLocalBudgetGateCommitOnlyPreflightOnlyParityCommitOnlyPreflightOnlyParityCommitOnlyPreflightOnlyParity(" in src
...
def test_gate_missing_and_threshold_breach_vectors() -> None:
    ...
    status, outputs = secure_local_budget_gate_commit_only_preflight_only_parity_commit_only_preflight_only_parity_commit_only_preflight_only_parity_commit_only(
        rows_gate_missing,
        rows_capacity=3,
...
if __name__ == "__main__":
    test_source_contains_iq1606_symbols()
