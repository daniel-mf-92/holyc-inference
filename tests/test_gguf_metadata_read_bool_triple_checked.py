#!/usr/bin/env python3
"""Reference checks for GGUFMetadataReadBoolTripleChecked semantics."""
...
def test_null_ptr_and_no_partial_write() -> None:
    ...
    assert (
        gguf_metadata_read_bool_triple_checked(
            None,
            4,
            cursor,
            4,
...
def test_short_read_and_noncanonical_third_do_not_commit() -> None:
    cursor = [1]
    out_first = [False]
    out_second = [True]
    out_third = [False]
...
def test_randomized_parity() -> None:
    rng = random.Random(20260417_244)
    ...
    for _ in range(6000):
        ...
        if cursor0 + 3 <= table_end and cursor0 + 3 <= n:
            ...
            if raw_first <= 1 and raw_second <= 1 and raw_third <= 1:
                assert err == GGUF_META_TABLE_OK
...
if __name__ == "__main__":
    ...
    test_short_read_and_noncanonical_third_do_not_commit()
