#!/usr/bin/env python3
"""Parity harness for FPQ16SqrtCheckedNoPartialCommitOnlyPreflightOnly (IQ-1235)."""
...
def fpq16_sqrt_checked_nopartial_commit_only_preflight_only_reference(
    ...
    *,
    ...
    force_value_parity_break: bool = False,
) -> int:
...
def explicit_preflight_composition(x_q16: int, required_out_cells_out: list[int] | None) -> int:
    if required_out_cells_out is None:
        return FP_Q16_ERR_NULL_PTR
    ...
    staged_out = [0]
    status = fpq16_sqrt_checked_nopartial_commit_only_reference(x_q16, staged_out)
    if status != FP_Q16_OK:
        return status

    if staged_out[0] != fpq16_sqrt(x_q16):
        return FP_Q16_ERR_BAD_PARAM

    required_out_cells_out[0] = 1
    return FP_Q16_OK


def test_source_contains_iq1235_function() -> None:
    source = Path("src/math/intsqrt.HC").read_text(encoding="utf-8")
    sig = "I32 FPQ16SqrtCheckedNoPartialCommitOnlyPreflightOnly(I64 x_q16,"
    ...
    assert "canonical_out_q16 = FPQ16Sqrt(x_q16);" in body
    assert "if (canonical_out_q16 != staged_out_q16)" in body
    assert "staged_required_out_cells = 1;" in body
    assert "if (staged_required_out_cells != 1)" in body
    assert "*required_out_cells_out = staged_required_out_cells;" in body


def test_null_domain_overflow_keep_required_zero() -> None:
    assert fpq16_sqrt_checked_nopartial_commit_only_preflight_only_reference(12, None) == FP_Q16_ERR_NULL_PTR

    required = [77]
    assert (
        fpq16_sqrt_checked_nopartial_commit_only_preflight_only_reference(-1, required)
        == FP_Q16_ERR_DOMAIN
    )
    assert required == [0]
    ...
    assert (
        fpq16_sqrt_checked_nopartial_commit_only_preflight_only_reference(too_large, required)
        == FP_Q16_ERR_OVERFLOW
    )
    assert required == [0]


def test_success_and_explicit_preflight_parity() -> None:
    vectors = [
        0,
        1,
        q16_from_float(0.25),
    ...
    for x_q16 in vectors:
        req_a = [0]
        req_b = [0]
        err_a = fpq16_sqrt_checked_nopartial_commit_only_preflight_only_reference(x_q16, req_a)
        err_b = explicit_preflight_composition(x_q16, req_b)
...
def test_snapshot_mutation_guard_returns_bad_param() -> None:
    required = [9]
    err = fpq16_sqrt_checked_nopartial_commit_only_preflight_only_reference(
        q16_from_float(9.0), required, mutate_snapshot=True
    )
    assert err == FP_Q16_ERR_BAD_PARAM
    assert required == [0]


def test_value_and_tuple_parity_guards() -> None:
    required = [5]
    err = fpq16_sqrt_checked_nopartial_commit_only_preflight_only_reference(
        q16_from_float(16.0), required, force_value_parity_break=True
    )
    assert err == FP_Q16_ERR_BAD_PARAM
    assert required == [0]
    ...
    required = [6]
    err = fpq16_sqrt_checked_nopartial_commit_only_preflight_only_reference(
        q16_from_float(25.0), required, force_tuple_parity_break=True
    )
    assert err == FP_Q16_ERR_BAD_PARAM
    assert required == [0]
...
def test_randomized_requirement_stability() -> None:
    rng = random.Random(20260423_1235)

    for _ in range(5000):
        x_q16 = q16_from_float(rng.uniform(0.0, 50000.0))
        required = [0]
        err = fpq16_sqrt_checked_nopartial_commit_only_preflight_only_reference(x_q16, required)
        assert err == FP_Q16_OK
        assert required == [1]


def run() -> None:
    test_source_contains_iq1235_function()
    test_null_domain_overflow_keep_required_zero()
    test_success_and_explicit_preflight_parity()
    test_snapshot_mutation_guard_returns_bad_param()
    test_value_and_tuple_parity_guards()
    test_randomized_requirement_stability()
    print("intsqrt_q16_checked_nopartial_commit_only_preflight_only_reference_checks=ok")
