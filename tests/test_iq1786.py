#!/usr/bin/env python3
"""Harness for IQ-1786 short-name commit-only hardening wrapper."""
...
def iq1786_model(
    ...
    *,
    ...
    status_seed, staged = iq1785.iq1784_model(
        scenarios=scenarios,
    ...
    status_primary, staged_after = iq1785.iq1785_model(
        scenarios=scenarios,
        secure_local_mode=secure_local_mode,
        iommu_active=iommu_active,
        book_of_truth_gpu_hooks=book_of_truth_gpu_hooks,
    ...
    status_parity, parity = iq1785.iq1784_model(
        scenarios=scenarios,
        secure_local_mode=secure_local_mode,
        iommu_active=iommu_active,
        book_of_truth_gpu_hooks=book_of_truth_gpu_hooks,
    ...
    if status_seed == GPU_SEC_PERF_OK and not _tuple_ok(staged, len(scenarios)):
        return GPU_SEC_PERF_ERR_BAD_PARAM, (0, 0, 0, 0)
    if status_primary == GPU_SEC_PERF_OK and not _tuple_ok(staged_after, len(scenarios)):
        return GPU_SEC_PERF_ERR_BAD_PARAM, (0, 0, 0, 0)
    if status_parity == GPU_SEC_PERF_OK and not _tuple_ok(parity, len(scenarios)):
        return GPU_SEC_PERF_ERR_BAD_PARAM, (0, 0, 0, 0)
...
def test_source_contains_iq1786_symbols() -> None:
    src = Path("src/gpu/security_perf_matrix.HC").read_text(encoding="utf-8")
    assert f"I32 {HC_FN_IQ1786}(" in src
    assert "#define IQ1786_SEED_FN IQ1785_COMP_FN" in src
    assert "#define IQ1786_PRIMARY_FN GPUSecPerfIQ1785" in src
    assert "status_seed = IQ1786_SEED_FN(" in src
    assert "status_primary = IQ1786_PRIMARY_FN(" in src
    assert "status_parity = IQ1786_SEED_FN(" in src
    assert "// IQ-1786 commit-only hardening wrapper:" in src


def test_gate_missing_vector() -> None:
    status, out_values = iq1786_model(
...
if __name__ == "__main__":
    test_source_contains_iq1786_symbols()
