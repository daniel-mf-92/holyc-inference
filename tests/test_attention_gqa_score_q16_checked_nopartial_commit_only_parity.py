#!/usr/bin/env python3
"""Reference checks for GQAAttentionScoreQ16CheckedNoPartialCommitOnlyParity (IQ-1405)."""
...
def gqa_attention_score_q16_checked_nopartial_commit_only_parity(
    q_rows_q16,
    q_rows_capacity: int,
    q_rows: int,
    k_rows_q16,
    ...
    staged_nopartial_scores = [0] * required_out_cells
    ...
    for idx in range(required_out_cells):
        if staged_commit_scores[idx] != staged_nopartial_scores[idx]:
            return ATTN_Q16_ERR_BAD_PARAM

    for idx in range(required_out_cells):
        out_scores_q32[idx] = staged_commit_scores[idx]

    return ATTN_Q16_OK


def explicit_commit_only_parity_composition(*args, **kwargs) -> int:
    return gqa_attention_score_q16_checked_nopartial_commit_only_parity(*args, **kwargs)
...
def test_fixed_vector_reference() -> None:
    ...
    out_a = [777] * (q_rows * seq_len)
    out_b = out_a.copy()

    err_a = gqa_attention_score_q16_checked_nopartial_commit_only_parity(
        q,
        len(q),
        q_rows,
        k,
...
def test_randomized_parity_vectors() -> None:
    rng = random.Random(20260425_1405)
