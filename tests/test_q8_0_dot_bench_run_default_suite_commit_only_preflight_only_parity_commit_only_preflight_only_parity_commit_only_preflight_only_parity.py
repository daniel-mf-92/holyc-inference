...
def parity_commit_only_preflight_only_parity(
    cpu_hz: int,
...
def test_source_contains_iq1307_function_and_guards() -> None:
    source = Path("src/bench/q8_0_dot_bench.HC").read_text(encoding="utf-8")
    sig = "I32 Q8_0DotBenchRunDefaultSuiteCommitOnlyPreflightOnlyParityCommitOnlyPreflightOnlyParityCommitOnlyPreflightOnlyParity("
    ...
    assert "status = Q8_0DotBenchRunDefaultSuiteCommitOnlyPreflightOnlyParityCommitOnlyPreflightOnlyParityCommitOnlyPreflightOnly(" in body
    assert "status = Q8_0DotBenchRunDefaultSuiteCommitOnlyPreflightOnlyParityCommitOnlyPreflightOnlyParityCommitOnly(" in body
    assert "if (preflight_total_ops != commit_total_ops ||" in body
    assert "*out_total_ops = preflight_total_ops;" in body
    assert "*out_total_cycles = preflight_total_cycles;" in body
    assert "*out_cycles_per_op = preflight_cycles_per_op;" in body
    assert "*out_remainder_cycles = preflight_remainder_cycles;" in body
    assert "*out_suite_checksum = preflight_suite_checksum;" in body


def test_null_and_alias_guards() -> None:
    assert parity_commit_only_preflight_only_parity(3_200_000_000, None, [1], [1], [1], [1]) == Q8_0_DOT_BENCH_ERR_NULL_PTR

    same = [77]
    status = parity_commit_only_preflight_only_parity(3_200_000_000, same, same, [3], [4], [5])
...
def test_bad_frequency_no_partial_commit() -> None:
    ...
    status = parity_commit_only_preflight_only_parity(
        0,
        out_total_ops,
        out_total_cycles,
        out_cycles_per_op,
...
if __name__ == "__main__":
    test_source_contains_iq1307_function_and_guards()
