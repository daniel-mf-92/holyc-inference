#!/usr/bin/env python3
"""Parity harness for GGUFReadTensorDataQ4_0CheckedNoPartialCommitOnly (IQ-933)."""
...
def gguf_read_tensor_data_q4_0_checked_no_partial(
    ...
    if gguf_bytes is None or tensor_info is None or out_q4_0_blocks is None:
...
def gguf_read_tensor_data_q4_0_checked_no_partial_commit_only(
    gguf_bytes: bytes | None,
    gguf_nbytes: int,
    tensor_data_base: int,
    tensor_info: TensorInfoQ40 | None,
    ...
    err, staged_blocks, staged_bytes = gguf_read_tensor_data_q4_0_checked_no_partial(
        gguf_bytes,
        gguf_nbytes,
        tensor_data_base,
        tensor_info,
...
def test_source_contains_iq933_function_and_commit_only_staging() -> None:
    source = Path("src/gguf/reader.HC").read_text(encoding="utf-8")
    sig = "I32 GGUFReadTensorDataQ4_0CheckedNoPartialCommitOnly("
...
def test_null_and_truncation_do_not_publish_diagnostics() -> None:
    ...
    out = bytearray(b"\xAA" * 36)

    out_blocks = [111]
    out_bytes = [222]

    err = gguf_read_tensor_data_q4_0_checked_no_partial_commit_only(
        None,
        0,
        0,
        info,
...
def test_capacity_and_snapshot_mismatch_vectors() -> None:
    payload = bytes((idx * 9) & 0xFF for idx in range(36))
...
def test_success_publishes_exact_blocks_and_bytes() -> None:
    ...
    assert bytes(out[36:]) == bytes([0x77] * (len(out) - 36))


def run() -> None:
    test_source_contains_iq933_function_and_commit_only_staging()
    ...
    print("gguf_read_tensor_data_q4_0_checked_nopartial_commit_only=ok")
