#!/usr/bin/env python3
"""Harness for ...ParityCommitOnlyPreflightOnlyParityCommitOnlyPreflightOnlyParityCommitOnlyPreflightOnlyParity (IQ-1131)."""
...
from test_inference_generate_tokens_checked_topk_topp_commit_only_preflight_only_parity_commit_only_preflight_only_parity_commit_only_preflight_only import (
...
from test_inference_generate_tokens_checked_topk_topp_commit_only_preflight_only_parity_commit_only_preflight_only_parity_commit_only_preflight_only_parity_commit_only import (
...
def inference_generate_tokens_checked_topk_topp_commit_only_preflight_only_parity_commit_only_preflight_only_parity_commit_only_preflight_only_parity_commit_only_preflight_only_parity_reference(
    *,
    ...
    parity_fn=inference_generate_tokens_checked_topk_topp_commit_only_preflight_only_parity_commit_only_preflight_only_parity_commit_only_preflight_only_reference,
) -> int:
    if (
    ...
    if snapshot_top_k <= 0 or snapshot_top_k > snapshot_vocab_size:
        return SAMPLING_Q16_ERR_BAD_PARAM
    if snapshot_workspace_stage_logits_capacity < snapshot_vocab_size:
        return SAMPLING_Q16_ERR_BAD_PARAM

    if canonical_required_logits > snapshot_step_logits_capacity:
        return SAMPLING_Q16_ERR_BAD_PARAM
    ...
    if canonical_required_tokens > snapshot_generated_capacity:
        return SAMPLING_Q16_ERR_BAD_PARAM
    if canonical_required_tokens > snapshot_random_q16_capacity:
        return SAMPLING_Q16_ERR_BAD_PARAM

    if canonical_final_token_count > snapshot_token_history_capacity:
        return SAMPLING_Q16_ERR_BAD_PARAM

    if (
        staged_commit_required_logits[0] != staged_parity_required_logits[0]
        or staged_commit_required_tokens[0] != staged_parity_required_tokens[0]
        or staged_commit_final_token_count[0] != staged_parity_final_token_count[0]
    ):
        return SAMPLING_Q16_ERR_BAD_PARAM

    if (
        staged_commit_required_logits[0] != canonical_required_logits
        or staged_commit_required_tokens[0] != canonical_required_tokens
        or staged_commit_final_token_count[0] != canonical_final_token_count
    ):
        return SAMPLING_Q16_ERR_BAD_PARAM

    out_required_logits[0] = staged_commit_required_logits[0]
    out_required_tokens[0] = staged_commit_required_tokens[0]
    out_final_token_count[0] = staged_commit_final_token_count[0]
    return SAMPLING_Q16_OK


def test_source_contains_iq_1131_symbol() -> None:
    ...
    sig = (
        ...
        "ParityCommitOnlyPreflightOnlyParityCommitOnlyPreflightOnlyParityCommitOnlyPreflightOnlyParity("
    )
    assert sig in source
    assert len(re.findall(re.escape(sig), source)) >= 1
    body = source.rsplit(sig, 1)[1]
    assert "InferenceGenerateTokensCheckedTopKTopPCommitOnlyPreflightOnlyParityCommitOnlyPreflightOnlyParityCommitOnlyPreflightOnlyParityCommitOnly(" in body
    assert "InferenceGenerateTokensCheckedTopKTopPCommitOnlyPreflightOnlyParityCommitOnlyPreflightOnlyParityCommitOnlyPreflightOnlyParityCommitOnlyPreflightOnly(" in body
    assert "if (snapshot_step_logits_capacity < 0 ||" in body
    assert "if (snapshot_workspace_stage_logits_capacity < snapshot_vocab_size)" in body
    assert "if (staged_preflight_required_logits != staged_commit_required_logits ||" in body
    assert "if (staged_preflight_required_logits != recomputed_required_logits ||" in body
...
def test_known_vector_success() -> None:
    ...
    status = (
        inference_generate_tokens_checked_topk_topp_commit_only_preflight_only_parity_commit_only_preflight_only_parity_commit_only_preflight_only_parity_commit_only_preflight_only_parity_reference(
...
def test_workspace_stage_capacity_rejected_without_publish() -> None:
    ...
    status = (
        inference_generate_tokens_checked_topk_topp_commit_only_preflight_only_parity_commit_only_preflight_only_parity_commit_only_preflight_only_parity_commit_only_preflight_only_parity_reference(
            ...
            workspace_stage_logits_q16=[0] * 7,
    ...
    assert out_required_tokens == [222]
    assert out_final_token_count == [333]


def test_step_logits_capacity_rejected_without_publish() -> None:
    out_required_logits = [444]
    out_required_tokens = [555]
    out_final_token_count = [666]
    ...
    assert status == SAMPLING_Q16_ERR_BAD_PARAM
    assert out_required_logits == [444]
    assert out_required_tokens == [555]
    assert out_final_token_count == [666]


def test_parity_mismatch_rejected() -> None:
    def bad_commit_fn(**kwargs: object) -> int:
        out_required_logits = kwargs["out_required_logits"]
        out_required_tokens = kwargs["out_required_tokens"]
        out_final_token_count = kwargs["out_final_token_count"]
        assert isinstance(out_required_logits, list)
        assert isinstance(out_required_tokens, list)
        assert isinstance(out_final_token_count, list)
        out_required_logits[0] = 9
        out_required_tokens[0] = 1
        out_final_token_count[0] = 5
        return SAMPLING_Q16_OK
...
def test_overflow_guard() -> None:
    ...
    status = (
        inference_generate_tokens_checked_topk_topp_commit_only_preflight_only_parity_commit_only_preflight_only_parity_commit_only_preflight_only_parity_commit_only_preflight_only_parity_reference(
            step_logits_q16=[0],
            step_logits_capacity=(1 << 63) - 1,
            vocab_size=huge,
            max_new_tokens=3,
            token_history=[0],
            token_history_capacity=(1 << 63) - 1,
            ...
            repetition_penalty_q16=SAMPLING_Q16_ONE,
            random_q16_values=[0],
            random_q16_capacity=(1 << 63) - 1,
            workspace_stage_logits_q16=[0],
            workspace_stage_logits_capacity=(1 << 63) - 1,
...
def test_fuzz_valid_contracts() -> None:
    rng = random.Random(1131)
    for _ in range(200):
        vocab_size = rng.randint(1, 48)
        max_new_tokens = rng.randint(0, 10)
        ...
        out_required_logits = [9991]
        out_required_tokens = [9992]
        out_final_token_count = [9993]

        status = (
            inference_generate_tokens_checked_topk_topp_commit_only_preflight_only_parity_commit_only_preflight_only_parity_commit_only_preflight_only_parity_commit_only_preflight_only_parity_reference(
                step_logits_q16=[0] * max(1, required_logits),
                ...
                top_p_q16=SAMPLING_Q16_ONE,
                repetition_penalty_q16=SAMPLING_Q16_ONE,
                random_q16_values=[0] * max(1, max_new_tokens),
                ...
                out_generated_tokens=[0] * max(1, max_new_tokens),
        ...
        assert status == SAMPLING_Q16_OK
        assert out_required_logits == [required_logits]
        assert out_required_tokens == [max_new_tokens]
        assert out_final_token_count == [token_history_count + max_new_tokens]


if __name__ == "__main__":
    test_source_contains_iq_1131_symbol()
    test_known_vector_success()
    test_workspace_stage_capacity_rejected_without_publish()
    test_step_logits_capacity_rejected_without_publish()
    test_parity_mismatch_rejected()
    test_overflow_guard()
    test_fuzz_valid_contracts()
    print("ok")
