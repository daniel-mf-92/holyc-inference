#!/usr/bin/env python3
"""Parity harness for ...CommitCapacityAliasSafeDefaultCapacityPreflightOnly."""
...
import test_ffn_q16_swiglu_apply_rows_checked_nopartial_strided_noalloc_required_bytes_commit_capacity_alias_safe_default_capacity as alias_safe_default
...
def ffn_q16_swiglu_apply_rows_checked_nopartial_strided_noalloc_commit_capacity_alias_safe_default_capacity_preflight_only(
    gate_q16,
    gate_capacity: int,
    up_q16,
    up_capacity: int,
    ...
    *,
    ...
    return alias_safe_default.ffn_q16_swiglu_apply_rows_checked_nopartial_strided_noalloc_required_bytes_commit_capacity_alias_safe_default_capacity(
        gate_q16,
        gate_capacity,
        up_q16,
        up_capacity,
...
def explicit_checked_alias_safe_default_capacity_preflight_composition(
    gate_q16,
    gate_capacity: int,
    up_q16,
    up_capacity: int,
...
def test_source_contains_alias_safe_default_capacity_preflight_wrapper() -> None:
    source = Path("src/model/ffn.HC").read_text(encoding="utf-8")
    signature = (
        "I32 FFNQ16SwiGLUApplyRowsCheckedNoPartialStridedNoAllocCommitCapacityAliasSafeDefaultCapacityPreflightOnly("
    )
    assert signature in source
    body = source.split(signature, 1)[1]

    assert "if (!out_required_in_cells || !out_commit_required_stage_cells ||" in body
...
def test_known_vectors_and_overlap_rejection() -> None:
    gate = [1] * 96
    up = [2] * 96
    out = [0] * 128
    stage = [0] * 128
    ...
    err = ffn_q16_swiglu_apply_rows_checked_nopartial_strided_noalloc_commit_capacity_alias_safe_default_capacity_preflight_only(
        gate,
        96,
        up,
        96,
...
def test_randomized_parity_vs_explicit_composition() -> None:
    ...
    for _ in range(350):
        ...
        err_got = ffn_q16_swiglu_apply_rows_checked_nopartial_strided_noalloc_commit_capacity_alias_safe_default_capacity_preflight_only(
            gate,
            gate_capacity,
            up,
            up_capacity,
        ...
        err_exp = explicit_checked_alias_safe_default_capacity_preflight_composition(
            gate,
            gate_capacity,
            up,
            up_capacity,
...
if __name__ == "__main__":
    test_source_contains_alias_safe_default_capacity_preflight_wrapper()
    test_known_vectors_and_overlap_rejection()
