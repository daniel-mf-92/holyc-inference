#!/usr/bin/env python3
"""Harness for IQ-1483 commit-only hardening wrapper over row-gate preflight/no-partial."""
...
def gpu_security_perf_matrix_row_gate_checked_nopartial_preflight_only_commit_only(
    secure_local_mode: int,
    iommu_active: int,
    book_of_truth_gpu_hooks: int,
    policy_digest_parity: int,
...
def test_source_contains_iq1483_symbols() -> None:
    src = Path("src/gpu/security_perf_matrix.HC").read_text(encoding="utf-8")

    assert "I32 GPUSecurityPerfMatrixRowGateCheckedNoPartialPreflightOnlyCommitOnly(" in src
...
def test_null_alias_capacity_vectors() -> None:
    status, reason, allowed = gpu_security_perf_matrix_row_gate_checked_nopartial_preflight_only_commit_only(
...
def test_reason_code_parity_drift_vector() -> None:
    ...
    assert (status, reason, allowed) == (GPU_SEC_PERF_ERR_BAD_PARAM, 701, 702)
