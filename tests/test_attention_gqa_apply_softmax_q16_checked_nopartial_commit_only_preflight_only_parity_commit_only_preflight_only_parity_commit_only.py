...
from test_attention_gqa_apply_softmax_q16_checked_nopartial_commit_only_preflight_only_parity_commit_only_preflight_only import (
    gqa_attention_apply_softmax_q16_checked_nopartial_commit_only_preflight_only_parity_commit_only_preflight_only,
)
...
def gqa_attention_apply_softmax_q16_checked_nopartial_commit_only_preflight_only_parity_commit_only_preflight_only_parity_commit_only(
    scores_q32,
    scores_capacity: int,
    query_rows: int,
    key_rows: int,
    ...
    err = gqa_attention_apply_softmax_q16_checked_nopartial_commit_only_preflight_only_parity_commit_only_preflight_only(
        scores_q32,
        scores_capacity,
        query_rows,
        key_rows,
    ...
    for idx in range(required_out_cells):
        if staged_parity[idx] != staged_commit[idx]:
            return ATTN_Q16_ERR_BAD_PARAM
...
def test_source_contract_markers() -> None:
    ...
    assert "status = GQAAttentionApplySoftmaxQ16CheckedNoPartialCommitOnlyPreflightOnlyParityCommitOnlyPreflightOnly(" in body
    assert "if (out_begin < scores_end && scores_begin < out_end)" in body
    assert "if (staged_parity_probs_q16[copy_index] != staged_commit_probs_q16[copy_index])" in body
