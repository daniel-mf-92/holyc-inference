#!/usr/bin/env python3
"""Harness for IQ-1620 commit-only hardening wrapper over IQ-1619 and IQ-1616."""
...
def fast_path_switch_checked_audit_parity_iq1620(
    secure_local_mode: int,
    iommu_active: int,
    book_of_truth_gpu_hooks: int,
    policy_digest_parity: int,
...
def test_source_contains_iq1620_symbols() -> None:
...
def test_gate_missing_and_audit_parity_breach_vectors() -> None:
    status, enabled, reason = fast_path_switch_checked_audit_parity_iq1620(
        secure_local_mode=GPU_SEC_PERF_PROFILE_SECURE_LOCAL,
        iommu_active=0,
        book_of_truth_gpu_hooks=1,
        policy_digest_parity=1,
        dispatch_transcript_parity=1,
        current_fast_path_enabled=301,
        current_disable_reason_code=302,
    )
    assert (status, enabled, reason) == (GPU_SEC_PERF_ERR_POLICY_GUARD, 301, 302)
    ...
    status, enabled, reason = fast_path_switch_checked_audit_parity_iq1620(
        ...
        current_fast_path_enabled=303,
        current_disable_reason_code=304,
    )
    assert (status, enabled, reason) == (GPU_SEC_PERF_ERR_POLICY_GUARD, 303, 304)
...
def test_status_domain_drift_and_tuple_parity_vectors() -> None:
    status, enabled, reason = fast_path_switch_checked_audit_parity_iq1620(
        ...
        policy_digest_parity=1,
        dispatch_transcript_parity=1,
        current_fast_path_enabled=311,
        current_disable_reason_code=312,
        force_status_domain_drift=True,
    )
    assert (status, enabled, reason) == (GPU_SEC_PERF_ERR_BAD_PARAM, 311, 312)
    ...
    status, enabled, reason = fast_path_switch_checked_audit_parity_iq1620(
        ...
        policy_digest_parity=1,
        dispatch_transcript_parity=1,
        current_fast_path_enabled=313,
        current_disable_reason_code=314,
        force_tuple_drift=True,
    )
    assert (status, enabled, reason) == (GPU_SEC_PERF_ERR_BAD_PARAM, 313, 314)


def test_deterministic_tuple_parity_vectors() -> None:
    first = fast_path_switch_checked_audit_parity_iq1620(
        secure_local_mode=GPU_SEC_PERF_PROFILE_SECURE_LOCAL,
        iommu_active=1,
        book_of_truth_gpu_hooks=1,
        policy_digest_parity=1,
    ...
    second = fast_path_switch_checked_audit_parity_iq1620(
        secure_local_mode=GPU_SEC_PERF_PROFILE_SECURE_LOCAL,
        iommu_active=1,
        book_of_truth_gpu_hooks=1,
        policy_digest_parity=1,
...
if __name__ == "__main__":
    test_source_contains_iq1620_symbols()
