#!/usr/bin/env python3
"""Parity checks for GGUFMetadataReadF64BitsQuadCheckedDefault."""
...
def gguf_metadata_read_f64bits_quad_checked_default(
    buf: list[int] | None,
    buf_nbytes: int,
    cursor_ref: list[int] | None,
    out_first_ref: list[int] | None,
...
def test_null_ptr_and_no_partial_write() -> None:
    ...
    out_fourth = [0xFFEEDDCCBBAA9988]

    assert (
        gguf_metadata_read_f64bits_quad_checked_default(
            None,
            64,
            cursor,
            out_first,
...
def test_default_end_rejects_short_payload_without_commit() -> None:
    ...
    err = gguf_metadata_read_f64bits_quad_checked_default(
        buf,
        len(buf),
        cursor,
        out_first,
...
def test_randomized_parity_against_checked_core() -> None:
    rng = random.Random(20260418_288)
    ...
    for _ in range(6000):
        ...
        out_default_fourth = [0xD4D4D4D4D4D4D4D4]
        ...
        out_checked_fourth = [0xD4D4D4D4D4D4D4D4]

        err_default = gguf_metadata_read_f64bits_quad_checked_default(
            buf,
            n,
            cursor_default,
            out_default_first,
        ...
        assert out_default_fourth == out_checked_fourth
...
def run() -> None:
    ...
    print("gguf_metadata_read_f64bits_quad_checked_default_parity=ok")
