#!/usr/bin/env python3
"""Reference checks for GQAAttentionValueMixQ16CheckedNoPartialCommitOnlyParityCommitOnlyPreflightOnlyParityCommitOnly (IQ-1434)."""
...
def gqa_attention_value_mix_q16_checked_nopartial_commit_only_parity_commit_only_preflight_only_parity_commit_only(
    scores_q16,
    scores_capacity: int,
    query_rows: int,
    key_rows: int,
    ...
    if (
        staged_parity_required_score_cells != staged_preflight_required_score_cells
        or staged_parity_required_value_cells != staged_preflight_required_value_cells
        or staged_parity_required_out_cells != staged_preflight_required_out_cells
    ):
        return ATTN_Q16_ERR_BAD_PARAM

    if (
        staged_parity_required_score_cells != required_score_cells
        or staged_parity_required_value_cells != required_value_cells
        or staged_parity_required_out_cells != required_out_cells
    ):
        return ATTN_Q16_ERR_BAD_PARAM
    ...
    out_required_score_cells[0] = staged_parity_required_score_cells
    out_required_value_cells[0] = staged_parity_required_value_cells
    out_required_out_cells[0] = staged_parity_required_out_cells
    return ATTN_Q16_OK


def explicit_commit_composition(*args, **kwargs) -> int:
    return gqa_attention_value_mix_q16_checked_nopartial_commit_only_parity_commit_only_preflight_only_parity_commit_only(
        *args, **kwargs
    )


def test_fixed_vector_publishes_required_tuple_without_output_writes() -> None:
    query_rows = 4
    key_rows = 3
    value_dim = 2
    head_groups = 2
    ...
    assert req_score == [((query_rows - 1) * row_stride) + key_rows]
    assert req_value == [(query_rows // head_groups) * key_rows * value_dim]
    assert req_out == [query_rows * value_dim]
...
def test_randomized_parity_vectors_and_source_signature() -> None:
    rng = random.Random(20260425_1434)
    ...
    for _ in range(120):
        ...
        out_a = [rng.randint(-3000, 3000) for _ in range(out_cells)]
        out_b = out_a.copy()
        req_a = [rng.randint(-100, 100)]
        req_b = req_a.copy()
        val_a = [rng.randint(-100, 100)]
        val_b = val_a.copy()
        outreq_a = [rng.randint(-100, 100)]
        outreq_b = outreq_a.copy()

        err_a = gqa_attention_value_mix_q16_checked_nopartial_commit_only_parity_commit_only_preflight_only_parity_commit_only(
            scores,
            len(scores),
            query_rows,
            key_rows,
        ...
        assert req_a == req_b
        assert val_a == val_b
        assert outreq_a == outreq_b

    attention_hc = Path(__file__).resolve().parents[1] / "src" / "model" / "attention.HC"
    body = attention_hc.read_text(encoding="utf-8")
    signature = "I32 GQAAttentionValueMixQ16CheckedNoPartialCommitOnlyParityCommitOnlyPreflightOnlyParityCommitOnly("
    assert signature in body
    assert "status = GQAAttentionValueMixQ16CheckedNoPartialCommitOnlyParityCommitOnlyPreflightOnlyParity(" in body
    assert "status = GQAAttentionValueMixQ16CheckedNoPartialCommitOnlyParityCommitOnlyPreflightOnly(" in body


if __name__ == "__main__":
    test_fixed_vector_publishes_required_tuple_without_output_writes()
