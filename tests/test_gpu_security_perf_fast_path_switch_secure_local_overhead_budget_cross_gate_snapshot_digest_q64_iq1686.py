#!/usr/bin/env python3
"""Harness for IQ-1686 snapshot-digest preflight-parity commit-only wrapper."""
...
def fast_path_switch_secure_local_overhead_budget_cross_gate_snapshot_digest_q64_checked_commit_only_preflight_only_parity_commit_only_iq1686(
    ...
    *,
    ...
    _, staged_enabled, staged_reason, staged_digest_q64 = (
        _BASE_IQ1684_MOD._BASE_IQ1680_MOD.fast_path_switch_secure_local_overhead_budget_cross_gate_snapshot_digest_q64_checked_commit_only_iq1680(
            secure_local_mode=secure_local_mode,
            iommu_active=iommu_active,
            book_of_truth_gpu_hooks=book_of_truth_gpu_hooks,
            policy_digest_parity=policy_digest_parity,
    ...
    _, parity_enabled, parity_reason, parity_digest_q64 = (
        _BASE_IQ1684_MOD._BASE_IQ1680_MOD.fast_path_switch_secure_local_overhead_budget_cross_gate_snapshot_digest_q64_checked_commit_only_iq1680(
            secure_local_mode=secure_local_mode,
            iommu_active=iommu_active,
            book_of_truth_gpu_hooks=book_of_truth_gpu_hooks,
...
def test_source_contains_iq1686_symbols() -> None:
    src = Path("src/gpu/security_perf_matrix.HC").read_text(encoding="utf-8")

    assert (
        "I32 GPUSecurityPerfFastPathSwitchSecureLocalOverheadBudgetCrossGateSnapshotDigestQ64CheckedCommitOnlyPreflightOnlyParityCommitOnly("
        in src
    )
    assert (
...
def test_gate_missing_vector() -> None:
    status, enabled, reason, digest_q64 = (
        fast_path_switch_secure_local_overhead_budget_cross_gate_snapshot_digest_q64_checked_commit_only_preflight_only_parity_commit_only_iq1686(
...
if __name__ == "__main__":
    test_source_contains_iq1686_symbols()
