#!/usr/bin/env python3
"""Reference checks for GQAAttentionValueMixQ16CheckedNoPartialPreflightDefaultStrideParityCommitOnlyPreflightOnly (IQ-1429)."""
...
def gqa_attention_value_mix_q16_checked_nopartial_preflight_default_stride_parity_commit_only_preflight_only(
    scores_q16,
    scores_capacity: int,
    query_rows: int,
    key_rows: int,
    ...
    snapshot_out_vals = out_values_q16[:recomputed_required_out_cells]

    staged_commit_score = [0]
    staged_commit_value = [0]
    staged_commit_out = [0]
    ...
    if (
        staged_commit_score[0] != staged_parity_score[0]
        or staged_commit_value[0] != staged_parity_value[0]
    ...
    if (
        ...
        or staged_commit_value[0] != recomputed_required_value_cells
    ...
    if (
        ...
        or staged_commit_value[0] > snapshot_values_capacity
    ...
    if out_values_q16[:recomputed_required_out_cells] != snapshot_out_vals:
        return ATTN_Q16_ERR_BAD_PARAM
...
def test_randomized_zero_write_parity_vectors() -> None:
    rng = random.Random(20260425_1429)
...
def test_source_contract_markers() -> None:
    ...
    assert "status = GQAAttentionValueMixQ16CheckedNoPartialPreflightDefaultStrideParity(" in body
...
if __name__ == "__main__":
    ...
    print(
        "gqa_attention_value_mix_q16_checked_nopartial_preflight_default_stride_parity_commit_only_preflight_only_reference_checks=ok"
    )
