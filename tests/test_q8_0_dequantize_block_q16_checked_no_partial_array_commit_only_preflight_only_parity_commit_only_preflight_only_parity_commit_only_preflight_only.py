#!/usr/bin/env python3
"""Harness for IQ-960 diagnostics-only preflight companion."""
...
_COMMIT_PATH = Path(
    "tests/test_q8_0_dequantize_block_q16_checked_no_partial_array_commit_only_preflight_only_parity_commit_only_preflight_only_parity_commit_only.py"
)

_spec_commit = importlib.util.spec_from_file_location("q8_iq960_commit", _COMMIT_PATH)
...
commit_only = (
    _commit.q8_0_dequantize_block_q16_checked_no_partial_array_commit_only_preflight_only_parity_commit_only_preflight_only_parity_commit_only
)
try_mul_i64_nonneg = _commit.try_mul_i64_nonneg


def q8_0_dequantize_block_q16_checked_no_partial_array_commit_only_preflight_only_parity_commit_only_preflight_only_parity_commit_only_preflight_only(
    src_blocks,
...
def test_source_contains_iq960_preflight_only_contract() -> None:
    source = Path("src/quant/q8_0.HC").read_text(encoding="utf-8")
    sig = "I32 Q8_0DequantizeBlockQ16CheckedNoPartialArrayCommitOnlyPreflightOnlyParityCommitOnlyPreflightOnlyParityCommitOnlyPreflightOnly("
    assert source.count(sig) == 1
    body = source.split(sig, 1)[1]

    assert (
        "status = Q8_0DequantizeBlockQ16CheckedNoPartialArrayCommitOnlyPreflightOnlyParityCommitOnlyPreflightOnlyParityCommitOnly("
        in body
    )
    assert "Diagnostics-only no-write companion over IQ-959 commit-only wrapper." in source
...
def test_success_and_no_write_publish() -> None:
    ...
    err = q8_0_dequantize_block_q16_checked_no_partial_array_commit_only_preflight_only_parity_commit_only_preflight_only_parity_commit_only_preflight_only(
        src_blocks,
        12,
        2,
        6,
...
def test_randomized_adversarial_vectors() -> None:
    ...
    for _ in range(180):
        ...
        dst_q16 = [707] * max(dst_capacity, 1)
