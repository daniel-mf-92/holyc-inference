#!/usr/bin/env python3
"""Parity harness for ...PreflightOnlyCommitOnlyParityCommitOnlyParity (IQ-851)."""
...
from test_inference_generate_tokens_checked_topk_default_preflight_only_commit_only_parity_commit_only import (
    inference_generate_tokens_checked_topk_default_preflight_only_commit_only_parity_commit_only_reference,
)
from test_inference_generate_tokens_checked_topk_default_preflight_only_commit_only_parity_preflight_only import (
    inference_generate_tokens_checked_topk_default_preflight_only_commit_only_parity_preflight_only_reference,
)


def inference_generate_tokens_checked_topk_default_preflight_only_commit_only_parity_commit_only_parity_reference(
    *,
    ...
    commit_required_step_logits_cells = [out_required_step_logits_cells[0]]
    commit_required_generated_capacity = [out_required_generated_capacity[0]]
    commit_required_random_capacity = [out_required_random_capacity[0]]

    preflight_required_step_logits_cells = [out_required_step_logits_cells[0]]
    preflight_required_generated_capacity = [out_required_generated_capacity[0]]
    preflight_required_random_capacity = [out_required_random_capacity[0]]

    status = inference_generate_tokens_checked_topk_default_preflight_only_commit_only_parity_commit_only_reference(
    ...
    status = inference_generate_tokens_checked_topk_default_preflight_only_commit_only_parity_preflight_only_reference(
        step_logits_q16=step_logits_q16,
        step_logits_capacity=step_logits_capacity,
        vocab_size=vocab_size,
        max_new_tokens=max_new_tokens,
        ...
        out_generated_tokens=out_generated_tokens,
        generated_capacity=generated_capacity,
        out_required_step_logits_cells=preflight_required_step_logits_cells,
        out_required_generated_capacity=preflight_required_generated_capacity,
        out_required_random_capacity=preflight_required_random_capacity,
    )
    if status != SAMPLING_Q16_OK:
        return status
    ...
    if commit_required_step_logits_cells[0] != preflight_required_step_logits_cells[0]:
        return SAMPLING_Q16_ERR_BAD_PARAM
    if commit_required_generated_capacity[0] != preflight_required_generated_capacity[0]:
        return SAMPLING_Q16_ERR_BAD_PARAM
    if commit_required_random_capacity[0] != preflight_required_random_capacity[0]:
        return SAMPLING_Q16_ERR_BAD_PARAM
...
def test_source_contains_commit_only_parity_commit_only_parity_helper() -> None:
    source = Path("src/model/sampling.HC").read_text(encoding="utf-8")
    sig = "I32 InferenceGenerateTokensCheckedTopKDefaultPreflightOnlyCommitOnlyParityCommitOnlyParity("
    ...
    assert "*out_required_random_capacity = commit_required_random_capacity;" in body


def test_known_vector_and_error_no_publish() -> None:
    out_step = [7]
    out_gen = [8]
    out_rand = [9]

    status = (
        inference_generate_tokens_checked_topk_default_preflight_only_commit_only_parity_commit_only_parity_reference(
    ...
    status = (
        inference_generate_tokens_checked_topk_default_preflight_only_commit_only_parity_commit_only_parity_reference(
            ...
            out_generated_tokens=[0] * 4,
            generated_capacity=4,
            out_required_step_logits_cells=out_step_fail,
            out_required_generated_capacity=out_gen_fail,
            out_required_random_capacity=out_rand_fail,
        )
    )

    assert status == SAMPLING_Q16_ERR_BAD_PARAM
    assert out_step_fail == [101]
    assert out_gen_fail == [102]
    assert out_rand_fail == [103]


def test_randomized_commit_only_parity_vs_explicit_composition() -> None:
    rng = random.Random(20260421_851)
    ...
    for _ in range(900):
        ...
        token_history_capacity = token_history_count + max_new_tokens + rng.randint(0, 4)
        step_logits_capacity = vocab_size * max_new_tokens + rng.randint(0, 4)
        random_capacity = max_new_tokens + rng.randint(0, 3)
        generated_capacity = max_new_tokens + rng.randint(0, 3)
        stage_capacity = vocab_size + rng.randint(0, 4)
        topk_logits_capacity = vocab_size + rng.randint(0, 4)
        topk_index_capacity = vocab_size + rng.randint(0, 4)
        ...
        status_a = (
            inference_generate_tokens_checked_topk_default_preflight_only_commit_only_parity_commit_only_parity_reference(
                out_required_step_logits_cells=out_step_a,
                out_required_generated_capacity=out_gen_a,
                out_required_random_capacity=out_rand_a,
                **kwargs,
            )
        )

        status_b = explicit_parity_composition(
            out_required_step_logits_cells=out_step_b,
            out_required_generated_capacity=out_gen_b,
            out_required_random_capacity=out_rand_b,
            **kwargs,
        )

        assert status_a == status_b
        assert out_step_a == out_step_b
        assert out_gen_a == out_gen_b
        assert out_rand_a == out_rand_b


if __name__ == "__main__":
    test_source_contains_commit_only_parity_commit_only_parity_helper()
    test_known_vector_and_error_no_publish()
