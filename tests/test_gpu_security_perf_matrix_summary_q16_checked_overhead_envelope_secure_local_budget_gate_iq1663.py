#!/usr/bin/env python3
"""Harness for IQ-1663 commit-only hardening wrapper over IQ-1662 strict parity + IQ-1661 preflight-only."""
...
def test_source_contains_iq1663_symbols() -> None:
    ...
    assert "// IQ-1663 commit-only hardening wrapper over IQ-1662 strict diagnostics parity + IQ-1661 preflight-only:" in src
...
def test_gate_missing_and_threshold_breach_vectors() -> None:
    ...
    status, outputs = secure_local_budget_gate_iq1663(
        rows_gate_missing,
        rows_capacity=3,
...
def test_status_domain_drift_and_deterministic_tuple_parity_vectors() -> None:
    ...
    status, outputs = secure_local_budget_gate_iq1663(
        ...
        max_p50_overhead_q16=6_000,
        max_p95_overhead_q16=7_000,
        caller_outputs=(271, 272, GPU_SEC_PERF_BUDGET_GATE_STATUS_POLICY_BLOCK),
        force_preflight_tuple_drift=True,
    )
    assert status == GPU_SEC_PERF_ERR_BAD_PARAM
    assert outputs == (271, 272, GPU_SEC_PERF_BUDGET_GATE_STATUS_POLICY_BLOCK)
...
if __name__ == "__main__":
    test_source_contains_iq1663_symbols()
