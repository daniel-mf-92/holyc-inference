#!/usr/bin/env python3
"""Harness for IQ-1594 zero-write diagnostics companion over IQ-1593 and IQ-1592."""
...
def fast_path_switch_checked_audit_parity_iq1594(
    secure_local_mode: int,
    iommu_active: int,
    book_of_truth_gpu_hooks: int,
    policy_digest_parity: int,
...
def test_source_contains_iq1594_symbols() -> None:
...
def test_gate_missing_and_audit_parity_breach_vectors() -> None:
    status, enabled, reason = fast_path_switch_checked_audit_parity_iq1594(
...
def test_status_domain_drift_no_write_parity_and_secure_on_budget_vectors() -> None:
    ...
    # and IQ-1594 must preserve caller slots exactly.
    status, enabled, reason = fast_path_switch_checked_audit_parity_iq1594(
        secure_local_mode=GPU_SEC_PERF_PROFILE_SECURE_LOCAL,
        iommu_active=1,
        book_of_truth_gpu_hooks=1,
