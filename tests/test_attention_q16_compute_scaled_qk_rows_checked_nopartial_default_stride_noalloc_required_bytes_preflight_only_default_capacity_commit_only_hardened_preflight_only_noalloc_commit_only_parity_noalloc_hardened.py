#!/usr/bin/env python3
"""Parity harness for ...HardenedPreflightOnlyNoAllocCommitOnlyParityNoAllocHardened."""
...
from test_attention_q16_compute_scaled_qk_rows_checked_nopartial_default_stride_noalloc_required_bytes_preflight_only_default_capacity_commit_only_hardened_preflight_only_noalloc_commit_only_parity import (
    attention_q16_compute_scaled_qk_rows_checked_nopartial_default_stride_noalloc_required_bytes_preflight_only_default_capacity_commit_only_hardened_preflight_only_noalloc_commit_only_parity,
)
...
def attention_q16_compute_scaled_qk_rows_checked_nopartial_default_stride_noalloc_required_bytes_preflight_only_default_capacity_commit_only_hardened_preflight_only_noalloc_commit_only_parity_noalloc_hardened(
    ...
    hardened_k = [0]
    hardened_out = [0]
    hardened_stage_cells = [0]
    hardened_stage_bytes = [0]
    ...
    err = attention_q16_compute_scaled_qk_rows_checked_nopartial_default_stride_noalloc_required_bytes_preflight_only_default_capacity_commit_only_hardened_preflight_only_noalloc_commit_only_parity(
        ...
        hardened_k,
        hardened_out,
        hardened_stage_cells,
        hardened_stage_bytes,
    )
    if err != ATTN_Q16_OK:
        return err
    ...
    if (
        canonical_q[0] != hardened_q[0]
        or canonical_k[0] != hardened_k[0]
    ...
    out_required_stage_cells[0] = hardened_stage_cells[0]
    out_required_stage_bytes[0] = hardened_stage_bytes[0]
    return ATTN_Q16_OK
...
def test_source_contains_parity_noalloc_hardened_wrapper() -> None:
    source = Path("src/model/attention.HC").read_text(encoding="utf-8")
    signature = "I32 AttentionQ16ComputeScaledQKRowsCheckedNoPartialDefaultStrideNoAllocRequiredBytesPreflightOnlyDefaultCapacityCommitOnlyHardenedPreflightOnlyNoAllocCommitOnlyParityNoAllocHardened("
    ...
    assert "AttentionQ16ComputeScaledQKRowsCheckedNoPartialDefaultStrideNoAllocRequiredBytesPreflightOnlyDefaultCapacityCommitOnlyHardenedPreflightOnlyNoAllocCommitOnlyParity(" in body
...
def test_known_vector_and_failure_sentinel_immutability() -> None:
    ...
    err = attention_q16_compute_scaled_qk_rows_checked_nopartial_default_stride_noalloc_required_bytes_preflight_only_default_capacity_commit_only_hardened_preflight_only_noalloc_commit_only_parity_noalloc_hardened(
        q_rows,
        len(q_rows),
        3,
        k_rows,
...
def test_overflow_and_capacity_sentinel_vectors() -> None:
    ...
    for query_row_count, token_count, head_dim in vectors:
        ...
        assert out_k == [602]
        assert out_out == [603]
        assert out_stage_cells == [604]
        assert out_stage_bytes == [605]


def test_randomized_parity_and_no_partial() -> None:
    rng = random.Random(20260420_732)
    ...
    for _ in range(3000):
        ...
        err_got = attention_q16_compute_scaled_qk_rows_checked_nopartial_default_stride_noalloc_required_bytes_preflight_only_default_capacity_commit_only_hardened_preflight_only_noalloc_commit_only_parity_noalloc_hardened(
            q_rows,
            q_capacity,
            query_row_count,
            k_rows,
...
if __name__ == "__main__":
    test_source_contains_parity_noalloc_hardened_wrapper()
