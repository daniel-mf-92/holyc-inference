#!/usr/bin/env python3
"""Harness for FPQ16Sqrt...PreflightOnlyParityCommitOnly (IQ-1240)."""
...
def fpq16_sqrt_checked_nopartial_commit_only_preflight_only_parity_commit_only_preflight_only_parity_commit_only_reference(
    x_q16: int,
...
def test_source_contains_iq1240_function() -> None:
    source = Path("src/math/intsqrt.HC").read_text(encoding="utf-8")
    sig = "I32 FPQ16SqrtCheckedNoPartialCommitOnlyPreflightOnlyParityCommitOnlyPreflightOnlyParityCommitOnly("
...
def test_snapshot_and_tuple_parity_guards() -> None:
    out = [11]
    err = fpq16_sqrt_checked_nopartial_commit_only_preflight_only_parity_commit_only_preflight_only_parity_commit_only_reference(
        q16_from_float(49.0),
        out,
...
def test_success_and_explicit_composition_parity() -> None:
    ...
    for x_q16 in vectors:
        ...
        err_a = fpq16_sqrt_checked_nopartial_commit_only_preflight_only_parity_commit_only_preflight_only_parity_commit_only_reference(
            x_q16,
            out_a,
        )
...
def test_randomized_parity_against_components() -> None:
    rng = random.Random(20260423_1240)

    for _ in range(12000):
        x_q16 = q16_from_float(rng.uniform(0.0, 50000.0))

        out_commit = [9]
        err_commit = fpq16_sqrt_checked_nopartial_commit_only_preflight_only_parity_commit_only_preflight_only_parity_commit_only_reference(
            x_q16,
            out_commit,
        )
...
if __name__ == "__main__":
    test_source_contains_iq1240_function()
    ...
    print("intsqrt_q16_checked_nopartial_commit_only_preflight_only_parity_commit_only_preflight_only_parity_commit_only=ok")
