#!/usr/bin/env python3
"""Reference checks for GGUFMetadataReadF32BitsLEChecked semantics."""
...
def test_null_ptr_and_no_partial_write() -> None:
    ...
    assert (
        gguf_metadata_read_f32bitsle_checked(None, 16, cursor, 16, out_bits)
    ...
    assert (
        gguf_metadata_read_f32bitsle_checked([0] * 16, 16, None, 16, out_bits)
        == GGUF_META_TABLE_ERR_NULL_PTR
    )
    assert out_bits[0] == 0xDEADBEEF

    assert (
        gguf_metadata_read_f32bitsle_checked([0] * 16, 16, cursor, 16, None)
        == GGUF_META_TABLE_ERR_NULL_PTR
    )
    assert cursor[0] == 3
...
def test_short_reads_do_not_advance_or_write() -> None:
    ...
    for table_end in range(0, 4):
        cursor = [0]
        err = gguf_metadata_read_f32bitsle_checked(
            buf,
            len(buf),
            cursor,
            table_end,
...
def test_nan_payload_bits_are_preserved_exactly() -> None:
    nan_bits = 0x7FC12345
    payload = list(struct.pack("<I", nan_bits))
...
def test_i64_domain_overflow_surfaces_and_no_commit() -> None:
    ...
    cursor = [0]
    out_bits = [out_seed]
    err = gguf_metadata_read_f32bitsle_checked(buf, (1 << 63), cursor, 16, out_bits)
    ...
    cursor = [0]
    out_bits = [out_seed]
    err = gguf_metadata_read_f32bitsle_checked(buf, len(buf), cursor, (1 << 63), out_bits)
    assert err == GGUF_META_TABLE_ERR_OVERFLOW
    assert cursor[0] == 0
    assert out_bits[0] == out_seed
    ...
    cursor = [(1 << 63)]
    out_bits = [out_seed]
    err = gguf_metadata_read_f32bitsle_checked(buf, len(buf), cursor, len(buf), out_bits)
    assert err == GGUF_META_TABLE_ERR_OVERFLOW
...
def test_randomized_parity() -> None:
    rng = random.Random(20260417_215)
    ...
    for _ in range(5000):
        ...
        err = gguf_metadata_read_f32bitsle_checked(buf, n, cursor, table_end, out_bits)
...
def run() -> None:
    ...
    print("gguf_metadata_read_f32bitsle_checked_reference_checks=ok")
