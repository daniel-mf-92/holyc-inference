#!/usr/bin/env python3
"""Parity harness for ...PreflightOnlyCommitOnlyParityPreflightOnlyParity (IQ-848)."""
...
def q4_0_q8_0_matmul_q32_tiled_avx2_bench_rows_preflight_only_commit_only_parity_preflight_only_parity(
    lhs_q4_blocks,
    ...
    preflight_cells_per_iter = [0x8111]
    preflight_block_dots_per_iter = [0x8222]
    preflight_total_cells = [0x8333]
    preflight_total_block_dots = [0x8444]

    parity_cells_per_iter = [0x9111]
    parity_block_dots_per_iter = [0x9222]
    parity_total_cells = [0x9333]
    parity_total_block_dots = [0x9444]
    ...
    if preflight_cells_per_iter[0] != parity_cells_per_iter[0]:
        return Q4_0_Q8_0_AVX2_ERR_BAD_LEN
    if preflight_block_dots_per_iter[0] != parity_block_dots_per_iter[0]:
        return Q4_0_Q8_0_AVX2_ERR_BAD_LEN
    if preflight_total_cells[0] != parity_total_cells[0]:
        return Q4_0_Q8_0_AVX2_ERR_BAD_LEN
    if preflight_total_block_dots[0] != parity_total_block_dots[0]:
        return Q4_0_Q8_0_AVX2_ERR_BAD_LEN

    out_cells_per_iter[0] = preflight_cells_per_iter[0]
    out_block_dots_per_iter[0] = preflight_block_dots_per_iter[0]
    out_total_cells[0] = preflight_total_cells[0]
    out_total_block_dots[0] = preflight_total_block_dots[0]
    return Q4_0_Q8_0_AVX2_OK


def explicit_checked_composition(*args):
    return q4_0_q8_0_matmul_q32_tiled_avx2_bench_rows_preflight_only_commit_only_parity_preflight_only_parity(
        *args
    )


def test_source_contains_signature_and_strict_parity_gate() -> None:
    source = Path("src/matmul/q4_0_q8_0_matmul.HC").read_text(encoding="utf-8")
    sig = "I32 Q4_0Q8_0MatMulQ32TiledAVX2BenchRowsPreflightOnlyCommitOnlyParityPreflightOnlyParity("
    ...
    assert "if (preflight_cells_per_iter != parity_cells_per_iter)" in body
    assert "if (preflight_block_dots_per_iter != parity_block_dots_per_iter)" in body
    assert "if (preflight_total_cells != parity_total_cells)" in body
    assert "if (preflight_total_block_dots != parity_total_block_dots)" in body
    assert "*out_cells_per_iter = preflight_cells_per_iter;" in body
    assert "*out_block_dots_per_iter = preflight_block_dots_per_iter;" in body
    assert "*out_total_cells = preflight_total_cells;" in body
    assert "*out_total_block_dots = preflight_total_block_dots;" in body
...
def test_known_vector_and_no_publish_on_failure() -> None:
    ...
    rng = random.Random(20260421_8481)
    ...
    err = q4_0_q8_0_matmul_q32_tiled_avx2_bench_rows_preflight_only_commit_only_parity_preflight_only_parity(
        lhs,
        len(lhs),
        row_count,
        lhs_stride,
...
def test_null_output_ptr_rejected() -> None:
    ...
    rng = random.Random(20260421_8482)
...
def test_fuzz_parity_matches_explicit_checked_composition() -> None:
    ...
    for i in range(2600):
        ...
        rng = random.Random(20260421_1848000 + i)
        ...
        err_a = q4_0_q8_0_matmul_q32_tiled_avx2_bench_rows_preflight_only_commit_only_parity_preflight_only_parity(
            lhs,
            lhs_capacity,
            row_count,
            lhs_stride,
...
if __name__ == "__main__":
    test_source_contains_signature_and_strict_parity_gate()
