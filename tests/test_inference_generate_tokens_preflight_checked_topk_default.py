#!/usr/bin/env python3
"""Reference checks for InferenceGenerateTokensPreflightCheckedTopKDefault (IQ-789)."""
...
def inference_generate_tokens_preflight_topk_default_reference(
    *,
    step_logits_capacity: int,
    vocab_size: int,
    max_new_tokens: int,
...
def test_source_contains_preflight_topk_default_wrapper() -> None:
    source = Path("src/model/sampling.HC").read_text(encoding="utf-8")
    assert "I32 InferenceGenerateTokensPreflightCheckedTopKDefault(" in source
...
def test_preflight_topk_default_matches_explicit_reference_randomized() -> None:
    rng = random.Random(20260420_789)
    for _ in range(1000):
...
def test_preflight_topk_default_preserves_failure_classification() -> None:
    cases = [
        dict(
    ...
    for case in cases:
        ...
        status_default, diag_default = (
            inference_generate_tokens_preflight_topk_default_reference(**case)
        )
        status_explicit, diag_explicit = inference_generate_tokens_preflight_reference(
            **case
        )
...
def test_preflight_topk_default_nullptr_contract_in_source() -> None:
    source = Path("src/model/sampling.HC").read_text(encoding="utf-8")
    wrapper_anchor = source.index("I32 InferenceGenerateTokensPreflightCheckedTopKDefault(")
...
def test_preflight_topk_default_success_diagnostics_exact() -> None:
...
if __name__ == "__main__":
    test_source_contains_preflight_topk_default_wrapper()
    test_preflight_topk_default_matches_explicit_reference_randomized()
    test_preflight_topk_default_preserves_failure_classification()
    test_preflight_topk_default_nullptr_contract_in_source()
    test_preflight_topk_default_success_diagnostics_exact()
    print("inference_generate_tokens_preflight_checked_topk_default_reference_checks=ok")
