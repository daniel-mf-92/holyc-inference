#!/usr/bin/env python3
"""Harness for IQ-1524 overhead-envelope commit-only parity-commit preflight-only wrapper."""
...
def _overhead_envelope_nopartial_commit_only_preflight_only(
    rows: list[RowOutput],
    rows_capacity: int,
    out_capacity: int,
    secure_local_mode: int,
...
def _overhead_envelope_nopartial_commit_only_preflight_only_parity(
    rows: list[RowOutput],
    rows_capacity: int,
    out_capacity: int,
    secure_local_mode: int,
    ...
    preflight = _overhead_envelope_nopartial_commit_only_preflight_only(
        rows,
        rows_capacity,
        out_capacity,
        secure_local_mode,
...
def _overhead_envelope_nopartial_commit_only_preflight_only_parity_commit_only(
    ...
    parity_status, parity_tuple = _overhead_envelope_nopartial_commit_only_preflight_only_parity(
        rows,
        rows_capacity,
        out_capacity,
        secure_local_mode,
    ...
    if preflight[0] != parity_status:
        return GPU_SEC_PERF_ERR_BAD_PARAM, (0, 0, 0)
    if parity_status != GPU_SEC_PERF_OK:
        return parity_status, (0, 0, 0)
...
def gpu_security_perf_matrix_summary_q16_checked_overhead_envelope_checked_nopartial_commit_only_preflight_only_parity_commit_only_preflight_only(
    ...
    commit_only_status, staged_tuple = _overhead_envelope_nopartial_commit_only_preflight_only_parity_commit_only(
        rows,
        rows_capacity,
        out_capacity,
        secure_local_mode,
    ...
    if commit_only_status != parity_status:
        return GPU_SEC_PERF_ERR_BAD_PARAM, caller_outputs, (0, 0, 0)
    if commit_only_status != GPU_SEC_PERF_OK:
        return commit_only_status, caller_outputs, (0, 0, 0)
...
def test_source_contains_iq1524_symbols() -> None:
    src = Path("src/gpu/security_perf_matrix.HC").read_text(encoding="utf-8")

    assert "I32 GPUSecurityPerfMatrixSummaryQ16CheckedOverheadEnvelopeCheckedNoPartialCommitOnlyPreflightOnlyParityCommitOnlyPreflightOnly(" in src
    assert "status_commit_only = GPUSecurityPerfMatrixSummaryQ16CheckedOverheadEnvelopeCheckedNoPartialCommitOnlyPreflightOnlyParityCommitOnly(" in src
    assert "status_parity = GPUSecurityPerfMatrixSummaryQ16CheckedOverheadEnvelopeCheckedNoPartialCommitOnlyPreflightOnlyParity(" in src
...
def test_preflight_only_success_preserves_outputs_and_emits_tuple() -> None:
    rows = [
        RowOutput(100_000, 1_600, 10_000),
        RowOutput(98_000, 1_800, 10_400),
        RowOutput(96_000, 2_000, 10_800),
    ]
    status, preserved, staged = gpu_security_perf_matrix_summary_q16_checked_overhead_envelope_checked_nopartial_commit_only_preflight_only_parity_commit_only_preflight_only(
        rows,
        rows_capacity=3,
        out_capacity=3,
...
def test_null_alias_capacity_gate_monotonicity_and_parity_vectors() -> None:
    ...
    status, *_ = gpu_security_perf_matrix_summary_q16_checked_overhead_envelope_checked_nopartial_commit_only_preflight_only_parity_commit_only_preflight_only(
        rows,
        rows_capacity=1,
        out_capacity=3,
        secure_local_mode=GPU_SEC_PERF_PROFILE_SECURE_LOCAL,
    ...
    assert (status, preserved, staged) == (GPU_SEC_PERF_ERR_CAPACITY, (10, 11, 12), (0, 0, 0))
    ...
    assert (status, preserved, staged) == (GPU_SEC_PERF_ERR_POLICY_GUARD, (10, 11, 12), (0, 0, 0))
    ...
    assert (status, preserved, staged) == (GPU_SEC_PERF_ERR_BAD_PARAM, (5, 6, 7), (0, 0, 0))


def test_overflow_vector_returns_overflow_and_preserves_outputs() -> None:
    ...
    status, preserved, staged = gpu_security_perf_matrix_summary_q16_checked_overhead_envelope_checked_nopartial_commit_only_preflight_only_parity_commit_only_preflight_only(
        ...
        book_of_truth_gpu_hooks=1,
        policy_digest_parity=1,
        caller_outputs=(13, 14, 15),
    )
    assert (status, preserved, staged) == (GPU_SEC_PERF_ERR_OVERFLOW, (13, 14, 15), (0, 0, 0))
...
def test_digest_drift_returns_bad_param_and_preserves_outputs() -> None:
    ...
    assert (status, preserved, staged) == (GPU_SEC_PERF_ERR_BAD_PARAM, (21, 22, 23), (0, 0, 0))
