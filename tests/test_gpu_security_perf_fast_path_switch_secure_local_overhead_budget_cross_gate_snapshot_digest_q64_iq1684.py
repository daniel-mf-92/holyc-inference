#!/usr/bin/env python3
"""Harness for IQ-1684 snapshot-digest commit-only preflight-only wrapper."""
...
_BASE_IQ1679_PATH = Path(
    "tests/test_gpu_security_perf_fast_path_switch_secure_local_overhead_budget_cross_gate_snapshot_digest_q64_iq1679.py"
)
_BASE_IQ1679_SPEC = importlib.util.spec_from_file_location("iq1679_models", _BASE_IQ1679_PATH)
assert _BASE_IQ1679_SPEC is not None and _BASE_IQ1679_SPEC.loader is not None
_BASE_IQ1679_MOD = importlib.util.module_from_spec(_BASE_IQ1679_SPEC)
sys.modules[_BASE_IQ1679_SPEC.name] = _BASE_IQ1679_MOD
_BASE_IQ1679_SPEC.loader.exec_module(_BASE_IQ1679_MOD)
...
GPU_SEC_PERF_OK = _BASE_IQ1679_MOD.GPU_SEC_PERF_OK
GPU_SEC_PERF_ERR_BAD_PARAM = _BASE_IQ1679_MOD.GPU_SEC_PERF_ERR_BAD_PARAM
GPU_SEC_PERF_ERR_POLICY_GUARD = _BASE_IQ1679_MOD.GPU_SEC_PERF_ERR_POLICY_GUARD

GPU_SEC_PERF_PROFILE_SECURE_LOCAL = _BASE_IQ1679_MOD.GPU_SEC_PERF_PROFILE_SECURE_LOCAL
GPU_SEC_PERF_FAST_PATH_DISABLE_REASON_ALLOW = _BASE_IQ1679_MOD.GPU_SEC_PERF_FAST_PATH_DISABLE_REASON_ALLOW
GPU_SEC_PERF_FAST_PATH_DISABLE_REASON_PROFILE_GUARD = (
    _BASE_IQ1679_MOD.GPU_SEC_PERF_FAST_PATH_DISABLE_REASON_PROFILE_GUARD
)
...
def _disable_reason_is_valid(reason_code: int) -> bool:
    return (
        _BASE_IQ1679_MOD.GPU_SEC_PERF_FAST_PATH_DISABLE_REASON_ALLOW
        <= reason_code
        <= GPU_SEC_PERF_FAST_PATH_DISABLE_REASON_OVERHEAD_BUDGET_BREACH
    )


def fast_path_switch_secure_local_overhead_budget_cross_gate_snapshot_digest_q64_checked_commit_only_preflight_only_iq1684(
    ...
    *,
    initial_out: tuple[int, int, int] = (111, 7, 999),
    ...
    status_canonical, canonical_enabled, canonical_reason, canonical_digest_q64 = (
        _BASE_IQ1679_MOD.fast_path_switch_secure_local_overhead_budget_cross_gate_snapshot_digest_q64_checked_iq1679(
            secure_local_mode=secure_local_mode,
            iommu_active=iommu_active,
            book_of_truth_gpu_hooks=book_of_truth_gpu_hooks,
            policy_digest_parity=policy_digest_parity,
...
def test_source_contains_iq1684_symbols() -> None:
    src = Path("src/gpu/security_perf_matrix.HC").read_text(encoding="utf-8")

    assert (
        "I32 GPUSecurityPerfFastPathSwitchSecureLocalOverheadBudgetCrossGateSnapshotDigestQ64CheckedCommitOnlyPreflightOnly("
        in src
    )
    assert (
...
def test_gate_missing_vector() -> None:
    status, out_values = (
        fast_path_switch_secure_local_overhead_budget_cross_gate_snapshot_digest_q64_checked_commit_only_preflight_only_iq1684(
...
if __name__ == "__main__":
    test_source_contains_iq1684_symbols()
