#!/usr/bin/env python3
"""Parity harness for ...RequiredBytesParityPreflightOnlyCommitOnly (IQ-844)."""
...
def attention_q16_compute_scaled_qk_rows_checked_default_stride_nopartial_commit_only_preflight_only_required_bytes_parity_preflight_only_commit_only(
    query_row_count: int,
    token_count: int,
    staged_scores_q32,
    ...
    if canonical_required_stage_cells[0] != preflight_required_stage_cells[0]:
        return ATTN_Q16_ERR_BAD_PARAM
    if canonical_required_stage_bytes[0] != preflight_required_stage_bytes[0]:
        return ATTN_Q16_ERR_BAD_PARAM
    if canonical_required_out_cells[0] != preflight_required_out_cells[0]:
        return ATTN_Q16_ERR_BAD_PARAM
    if canonical_last_out_index[0] != preflight_last_out_index[0]:
        return ATTN_Q16_ERR_BAD_PARAM
...
def test_source_contains_required_bytes_parity_preflight_only_commit_only_wrapper() -> None:
    ...
    assert (
        "AttentionQ16ComputeScaledQKRowsCheckedDefaultStrideNoPartialCommitOnlyPreflightOnlyRequiredBytes(" in body
    )
    assert (
...
def test_randomized_commit_only_vs_explicit_composition() -> None:
    rng = random.Random(20260421_844)
    ...
    for _ in range(1200):
        ...
        err_new = attention_q16_compute_scaled_qk_rows_checked_default_stride_nopartial_commit_only_preflight_only_required_bytes_parity_preflight_only_commit_only(
            query_row_count,
            token_count,
            staged,
            staged_cap,
...
if __name__ == "__main__":
    test_source_contains_required_bytes_parity_preflight_only_commit_only_wrapper()
    test_known_vector_outputs()
    test_error_paths_preserve_outputs()
    test_randomized_commit_only_vs_explicit_composition()
    print(
        "attention_q16_compute_scaled_qk_rows_checked_default_stride_nopartial_commit_only_preflight_only_required_bytes_parity_preflight_only_commit_only=ok"
    )
