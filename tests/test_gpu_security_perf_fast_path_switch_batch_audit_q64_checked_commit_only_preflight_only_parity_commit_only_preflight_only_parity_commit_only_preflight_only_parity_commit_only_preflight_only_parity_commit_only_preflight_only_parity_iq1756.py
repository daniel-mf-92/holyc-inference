#!/usr/bin/env python3
"""Harness for IQ-1756 fast-path batch audit strict diagnostics parity gate."""
...
import test_gpu_security_perf_fast_path_switch_batch_audit_q64_checked_commit_only_preflight_only_parity_commit_only_preflight_only_parity_commit_only_preflight_only_parity_commit_only_preflight_only_parity_commit_only_preflight_only_iq1755 as iq1755

GPU_SEC_PERF_OK = iq1755.GPU_SEC_PERF_OK
GPU_SEC_PERF_ERR_BAD_PARAM = iq1755.GPU_SEC_PERF_ERR_BAD_PARAM
GPU_SEC_PERF_PROFILE_SECURE_LOCAL = iq1755.GPU_SEC_PERF_PROFILE_SECURE_LOCAL

Scenario = iq1755.Scenario

HC_FN_IQ1756 = "GPUSecurityPerfFastPathSwitchBatchAuditQ64CheckedCommitOnlyPreflightOnlyParityCommitOnlyPreflightOnlyParityCommitOnlyPreflightOnlyParityCommitOnlyPreflightOnlyParityCommitOnlyPreflightOnlyParity"
HC_FN_IQ1755 = iq1755.HC_FN_IQ1755
HC_FN_IQ1754 = iq1755.HC_FN_IQ1754
...
def iq1756_model(
    ...
    *,
    ...
    status_canonical, canonical_tuple = iq1755.iq1754.iq1754_model(
        scenarios=scenarios,
    ...
    status_primary, staged_tuple = iq1755.iq1755_model(
        scenarios=scenarios,
        secure_local_mode=secure_local_mode,
        iommu_active=iommu_active,
        book_of_truth_gpu_hooks=book_of_truth_gpu_hooks,
    ...
    if not iq1755.iq1754.iq1753.iq1752.iq1751.iq1750.iq1749.iq1748.iq1747.iq1746._status_is_valid(status_primary):
        return GPU_SEC_PERF_ERR_BAD_PARAM, saved_out
    if not iq1755.iq1754.iq1753.iq1752.iq1751.iq1750.iq1749.iq1748.iq1747.iq1746._status_is_valid(status_canonical):
        return GPU_SEC_PERF_ERR_BAD_PARAM, saved_out
...
def test_source_contains_iq1756_symbols() -> None:
    src = Path("src/gpu/security_perf_matrix.HC").read_text(encoding="utf-8")
    assert f"I32 {HC_FN_IQ1756}(" in src
    assert f"status_primary = {HC_FN_IQ1755}(" in src
    assert f"status_canonical = {HC_FN_IQ1754}(" in src
    assert "if (status_primary != status_canonical)" in src
    assert "// IQ-1756 strict diagnostics parity gate:" in src


def test_gate_missing_vector() -> None:
    saved_out = (51, 52, 53, 54)
    status, out_values = iq1756_model(
...
if __name__ == "__main__":
    test_source_contains_iq1756_symbols()
