#!/usr/bin/env python3
"""Parity harness for TokenizerBPEDecodePromptCheckedDefaultValidateCursorNoPartial."""
...
from test_tokenizer_bpe_decode_prompt_checked_default_no_partial import (
    tokenizer_bpe_decode_prompt_checked_default_no_partial,
)


def tokenizer_bpe_decode_prompt_checked_default_validate_cursor_no_partial(
...
def explicit_staged_wrapper_composition(
    token_ids: list[int],
    token_count: int,
...
def run_case(token_ids: list[int], token_cursor: int, pieces: list[bytes], max_piece_bytes: int) -> None:
    ...
    count_ref = [0xCDCD]
    count_wrapped = [0xCDCD]
    cursor_ref = [token_cursor]
    cursor_wrapped = [token_cursor]

    err_ref = explicit_staged_wrapper_composition(
        token_ids,
        len(token_ids),
        cursor_ref,
        blob,
    ...
    err_wrapped = tokenizer_bpe_decode_prompt_checked_default_validate_cursor_no_partial(
        token_ids,
        len(token_ids),
        cursor_wrapped,
        blob,
...
def test_multilingual_success_parity_vs_explicit_wrapper_composition() -> None:
    pieces = [
        b"hello",
        b" ",
        b"world",
...
def test_cursor_overflow_and_mul_overflow_reject_no_partial() -> None:
    ...
    cursor = [4]
    err = tokenizer_bpe_decode_prompt_checked_default_validate_cursor_no_partial(
        [0, 1, 2],
        3,
        cursor,
        blob,
...
def test_capacity_and_null_contracts() -> None:
    ...
    assert (
        tokenizer_bpe_decode_prompt_checked_default_validate_cursor_no_partial(
            None,
            0,
            cursor,
            blob,
...
def test_randomized_parity_against_explicit_wrapper_composition() -> None:
    rng = random.Random(20260418_434)
    pieces = [
        b"a",
        b"bc",
        b"def",
...
if __name__ == "__main__":
    test_multilingual_success_parity_vs_explicit_wrapper_composition()
    test_cursor_overflow_and_mul_overflow_reject_no_partial()
    test_capacity_and_null_contracts()
    test_randomized_parity_against_explicit_wrapper_composition()
    print("test_tokenizer_bpe_decode_prompt_checked_default_validate_cursor_no_partial: ok")
