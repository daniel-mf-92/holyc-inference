#!/usr/bin/env python3
"""Reference checks for GQAAttentionValueMixQ16CheckedNoPartialCommitOnlyParityCommitOnly (IQ-1389)."""
...
def gqa_attention_value_mix_q16_checked_nopartial_commit_only_parity_commit_only(
    scores_q16,
    scores_capacity: int,
    query_rows: int,
    key_rows: int,
    ...
    if staged_parity != staged_commit:
        return ATTN_Q16_ERR_BAD_PARAM
...
def explicit_commit_only_parity_composition(*args, **kwargs) -> int:
    scores_q16 = args[0]
    ...
    out_values_q16[:out_cells] = staged_parity
    return ATTN_Q16_OK
...
def test_fixed_vector_reference() -> None:
    ...
    out_a = [123456] * (query_rows * value_dim)
    out_b = out_a.copy()

    err_a = gqa_attention_value_mix_q16_checked_nopartial_commit_only_parity_commit_only(
        scores_q16,
        len(scores_q16),
        query_rows,
        key_rows,
    ...
    err_b = explicit_commit_only_parity_composition(
        scores_q16,
        len(scores_q16),
        query_rows,
        key_rows,
...
def test_randomized_commit_only_parity_publish_path() -> None:
    rng = random.Random(20260425_1389)
    ...
    for _ in range(240):
        ...
        out = [rng.randint(-7777, 7777) for _ in range(max(1, out_cells))]
...
def test_source_contract_markers() -> None:
    ...
    marker = "I32 GQAAttentionValueMixQ16CheckedNoPartialCommitOnlyParityCommitOnly("
    assert marker in source
    body = source.split(marker, 1)[1]
    assert "status = GQAAttentionValueMixQ16CheckedNoPartialCommitOnlyParity(" in body
    ...
    assert "if (staged_parity_out_values_q16[copy_index] != staged_commit_out_values_q16[copy_index])" in body


if __name__ == "__main__":
    test_fixed_vector_reference()
    test_error_contract_null_alias_capacity_overflow()
    test_randomized_commit_only_parity_publish_path()
    test_source_contract_markers()
    print("gqa_attention_value_mix_q16_checked_nopartial_commit_only_parity_commit_only_reference_checks=ok")
