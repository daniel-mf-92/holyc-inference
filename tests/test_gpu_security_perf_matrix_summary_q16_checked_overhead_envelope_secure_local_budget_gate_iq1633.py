#!/usr/bin/env python3
"""Harness for IQ-1633 zero-write diagnostics companion over IQ-1632 + IQ-1630 parity."""
...
def secure_local_budget_gate_iq1633(
    rows: list[RowOutput],
    rows_capacity: int,
    secure_local_mode: int,
    iommu_active: int,
    ...
    *,
    ...
    status_commit_only, staged_tuple = _iq1632_commit_only(
        rows,
        rows_capacity,
        secure_local_mode,
        iommu_active,
        ...
        force_tuple_parity_drift=force_commit_tuple_drift,
    )
    status_canonical, canonical_tuple = _iq1630_parity(
        rows,
        rows_capacity,
        secure_local_mode,
        iommu_active,
        ...
        force_tuple_parity_drift=force_canonical_tuple_drift,
    )
...
def test_source_contains_iq1633_symbols() -> None:
    src = Path("src/gpu/security_perf_matrix.HC").read_text(encoding="utf-8")
    assert "I32 GPUSecurityPerfMatrixSummaryQ16CheckedOverheadEnvelopeSecureLocalBudgetGateCommitOnlyPreflightOnlyParityCommitOnlyPreflightOnlyParityCommitOnlyPreflightOnlyParityCommitOnlyPreflightOnlyParityCommitOnlyPreflightOnlyParityCommitOnlyPreflightOnly(" in src
    assert "status_commit_only = GPUSecurityPerfMatrixSummaryQ16CheckedOverheadEnvelopeSecureLocalBudgetGateCommitOnlyPreflightOnlyParityCommitOnlyPreflightOnlyParityCommitOnlyPreflightOnlyParityCommitOnlyPreflightOnlyParityCommitOnlyPreflightOnlyParityCommitOnly(" in src
    assert "status_canonical = GPUSecurityPerfMatrixSummaryQ16CheckedOverheadEnvelopeSecureLocalBudgetGateCommitOnlyPreflightOnlyParityCommitOnlyPreflightOnlyParityCommitOnlyPreflightOnlyParityCommitOnlyPreflightOnlyParityCommitOnlyPreflightOnlyParity(" in src
    assert "// IQ-1633 zero-write diagnostics companion over IQ-1632 + canonical parity:" in src
...
def test_gate_missing_and_threshold_breach_vectors() -> None:
    ...
    status, outputs = secure_local_budget_gate_iq1633(
        rows_gate_missing,
        rows_capacity=3,
...
def test_no_write_parity_and_secure_on_vectors() -> None:
    rows = [
        RowOutput(90_000, 3_000, 41_000),
        RowOutput(88_000, 4_000, 42_000),
        RowOutput(86_000, 5_000, 43_000),
    ...
    first = secure_local_budget_gate_iq1633(
        rows,
        rows_capacity=4,
        secure_local_mode=GPU_SEC_PERF_PROFILE_SECURE_LOCAL,
        iommu_active=1,
        ...
        max_p50_overhead_q16=6_000,
        max_p95_overhead_q16=7_000,
        caller_outputs=(701, 702, GPU_SEC_PERF_BUDGET_GATE_STATUS_THRESHOLD_BREACH),
    )
    second = secure_local_budget_gate_iq1633(
        rows,
        rows_capacity=4,
        secure_local_mode=GPU_SEC_PERF_PROFILE_SECURE_LOCAL,
        iommu_active=1,
    ...
    assert first == (GPU_SEC_PERF_OK, (701, 702, GPU_SEC_PERF_BUDGET_GATE_STATUS_THRESHOLD_BREACH))
    assert second == (GPU_SEC_PERF_OK, (801, 802, GPU_SEC_PERF_BUDGET_GATE_STATUS_POLICY_BLOCK))
