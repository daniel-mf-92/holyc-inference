...
def tokenizer_bpe_decode_prompt_checked_default_capacity_validate_cursor_noalloc_nopartial_from_max_piece_preflight(
...
def wrapper_using_preflight(
    ...
    err, _max_piece_bytes, derived_capacity = (
        tokenizer_bpe_decode_prompt_checked_default_capacity_validate_cursor_noalloc_nopartial_from_max_piece_preflight(
            token_ids,
            token_count,
            io_token_cursor,
            vocab_piece_bytes,
...
def test_source_contains_preflight_and_nopartial_wrapper_delegation() -> None:
    source = Path("src/tokenizer/bpe.HC").read_text(encoding="utf-8")
    assert "TokenizerBPEDecodePromptCheckedDefaultCapacityValidateCursorNoAllocNoPartialFromMaxPiecePreflight" in source
    body = source.split(
        "I32 TokenizerBPEDecodePromptCheckedDefaultCapacityValidateCursorNoAllocNoPartialFromMaxPiecePreflight", 1
    )[1].split(
    ...
    assert "if (token_count && max_piece_bytes > 0x7FFFFFFFFFFFFFFF / token_count)" in body
    ...
    assert "TokenizerBPEDecodePromptCheckedDefaultCapacityValidateCursorNoAllocNoPartialFromMaxPiecePreflight" in wrapper_body
...
def test_null_cursor_overflow_and_output_ptr_surfaces() -> None:
    ...
    assert (
        tokenizer_bpe_decode_prompt_checked_default_capacity_validate_cursor_noalloc_nopartial_from_max_piece_preflight(
            ...
            out,
            out_count,
            out_derived_out_byte_capacity_present=False,
        )[0]
        == TOKENIZER_BPE_ERR_NULL_PTR
    )
...
def test_multilingual_and_adversarial_preflight_parity() -> None:
    ...
    for cursor in [0, 4, len(token_ids), len(token_ids) + 1]:
        got = tokenizer_bpe_decode_prompt_checked_default_capacity_validate_cursor_noalloc_nopartial_from_max_piece_preflight(
            token_ids,
            len(token_ids),
            [cursor],
            blob,
...
def test_randomized_preflight_and_wrapper_parity() -> None:
    rng = random.Random(20260419_507)
    pieces = [
        b"",
        b"a",
        b"bc",
    ...
    for _ in range(4000):
        ...
        got_pre = tokenizer_bpe_decode_prompt_checked_default_capacity_validate_cursor_noalloc_nopartial_from_max_piece_preflight(
            token_ids,
            token_count,
            cursor_a,
            blob,
        ...
        if err != TOKENIZER_BPE_OK:
            ...
            err_staged = tokenizer_bpe_decode_prompt_checked_default_capacity_validate_cursor_noalloc_nopartial(
                token_ids,
                token_count,
                cursor_staged,
                blob,
...
def run() -> None:
    test_source_contains_preflight_and_nopartial_wrapper_delegation()
    ...
    print(
        "tokenizer_bpe_decode_prompt_checked_default_capacity_validate_cursor_noalloc_nopartial_from_max_piece_preflight=ok"
    )
