#!/usr/bin/env python3
"""Parity harness for ...ParityNoAllocHardenedDefaultPreflightOnly."""
...
from test_tokenizer_bpe_encode_prompt_checked_noalloc_from_lens_commit_only_required_bytes_preflight_only_default_capacity_commit_only_parity_noalloc_hardened_default import (
...
def tokenizer_bpe_encode_prompt_checked_noalloc_from_lens_commit_only_required_bytes_preflight_only_default_capacity_commit_only_parity_noalloc_hardened_default_preflight_only(
    data: list[int] | None,
    byte_len: int,
...
def test_source_contains_signature_and_default_preflight_wrapper() -> None:
    source = Path("src/tokenizer/bpe.HC").read_text(encoding="utf-8")
    sig = "I32 TokenizerBPEEncodePromptCheckedNoAllocFromLensCommitOnlyRequiredBytesPreflightOnlyDefaultCapacityCommitOnlyParityNoAllocHardenedDefaultPreflightOnly("
    ...
    assert "TokenizerBPEEncodePromptCheckedNoAllocFromLensCommitOnlyRequiredBytesPreflightOnlyDefaultCapacityCommitOnlyParityNoAllocHardenedDefault(" in body
...
def test_success_error_no_write_parity_and_sentinel_preservation() -> None:
    ...
    err_a = tokenizer_bpe_encode_prompt_checked_noalloc_from_lens_commit_only_required_bytes_preflight_only_default_capacity_commit_only_parity_noalloc_hardened_default_preflight_only(
        payload,
        len(payload),
        cur_a,
        good_prompt_nbytes,
