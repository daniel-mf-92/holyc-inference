#!/usr/bin/env python3
"""Parity harness for ...NoAllocRequiredBytesCommitCapacity."""
...
def attention_q16_apply_score_scale_rows_checked_nopartial_default_stride_noalloc_required_bytes_commit_capacity(
    in_scores_q32,
    ...
    if commit_required_stage_bytes[0] > commit_stage_byte_capacity:
        return ATTN_Q16_ERR_BAD_PARAM

    out_commit_required_stage_cells[0] = commit_required_stage_cells[0]
    out_commit_required_stage_bytes[0] = commit_required_stage_bytes[0]
...
def test_source_contains_noalloc_required_bytes_commit_capacity_helper() -> None:
    ...
    signature = "I32 AttentionQ16ApplyScoreScaleRowsCheckedNoPartialDefaultStrideNoAllocRequiredBytesCommitCapacity("
...
def test_error_paths() -> None:
    ...
    assert (
        attention_q16_apply_score_scale_rows_checked_nopartial_default_stride_noalloc_required_bytes_commit_capacity(
            [0], 1, 1, 1, [0], 1, 1, 8, None, out_commit_bytes, out_out_cells
        )
        == ATTN_Q16_ERR_NULL_PTR
    )
...
def test_randomized_parity_against_explicit_composition() -> None:
    rng = random.Random(20260420_623)
    ...
    for _ in range(7000):
        ...
        if row_count >= 0 and token_count >= 0 and row_count < (1 << 31) and token_count < (1 << 31):
            required_cells = row_count * token_count
            commit_stage_cell_capacity = max(0, required_cells + rng.randint(-32, 128))
            required_bytes = required_cells * 8
            commit_stage_byte_capacity = max(0, required_bytes + rng.randint(-256, 1024))
        else:
            commit_stage_cell_capacity = rng.randint(0, 1 << 20)
            commit_stage_byte_capacity = rng.randint(0, 1 << 24)

        if rng.random() < 0.04:
            commit_stage_cell_capacity = -rng.randint(1, 300)
        if rng.random() < 0.04:
        ...
        err_new = attention_q16_apply_score_scale_rows_checked_nopartial_default_stride_noalloc_required_bytes_commit_capacity(
            in_scores,
            in_capacity,
            row_count,
            token_count,
