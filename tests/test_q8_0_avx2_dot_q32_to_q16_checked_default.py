#!/usr/bin/env python3
"""Parity checks for Q8_0DotBlocksAVX2Q32ToQ16CheckedDefault semantics."""
...
def q8_0_dot_blocks_avx2_q32_to_q16_checked_ptr(
    lhs_blocks,
    rhs_blocks,
    block_count: int,
    out_holder,
    ...
    err, dot_q16 = q8_0_dot_blocks_avx2_q32_to_q16_checked(lhs_blocks, rhs_blocks, block_count)
    if err != Q8_0_AVX2_OK:
        return err

    out_holder["value"] = dot_q16
    return Q8_0_AVX2_OK


def q8_0_dot_blocks_avx2_q32_to_q16_checked_default(
    lhs_blocks,
    rhs_blocks,
    block_count: int,
    out_holder,
    ...
    return q8_0_dot_blocks_avx2_q32_to_q16_checked_ptr(lhs_blocks, rhs_blocks, block_count, out_holder)


def test_default_wrapper_matches_checked_core_success_and_errors() -> None:
    rng = random.Random(2026041808)
    fp16_scales = [
        0x0000,
        0x1800,
        0x2800,
    ...
    for _ in range(320):
        ...
        lhs = [
            make_block(rng.choice(fp16_scales), [rng.randint(-128, 127) for _ in range(32)])
        ...
        err_default = q8_0_dot_blocks_avx2_q32_to_q16_checked_default(
            lhs,
            rhs,
            block_count,
            out_default,
        )
        err_core = q8_0_dot_blocks_avx2_q32_to_q16_checked_ptr(
            lhs,
            rhs,
            block_count,
            out_core,
...
def test_default_wrapper_negative_count_no_partial_write() -> None:
    ...
    err = q8_0_dot_blocks_avx2_q32_to_q16_checked_default([block], [block], -1, out_holder)
...
def test_default_wrapper_extent_shortfall_no_partial_write() -> None:
    ...
    err = q8_0_dot_blocks_avx2_q32_to_q16_checked_default([], [], 1, out_holder)
    assert err == Q8_0_AVX2_ERR_BAD_LEN
...
def test_default_wrapper_null_ptr_paths() -> None:
    ...
    err = q8_0_dot_blocks_avx2_q32_to_q16_checked_default(None, [block], 0, out_holder)
    assert err == Q8_0_AVX2_ERR_NULL_PTR
    assert out_holder["value"] == 5

    err = q8_0_dot_blocks_avx2_q32_to_q16_checked_default([block], None, 0, out_holder)
    assert err == Q8_0_AVX2_ERR_NULL_PTR
    assert out_holder["value"] == 5

    err = q8_0_dot_blocks_avx2_q32_to_q16_checked_default([block], [block], 0, None)
    assert err == Q8_0_AVX2_ERR_NULL_PTR
...
if __name__ == "__main__":
    ...
    print("q8_0_avx2_dot_q32_to_q16_checked_default_reference_checks=ok")
