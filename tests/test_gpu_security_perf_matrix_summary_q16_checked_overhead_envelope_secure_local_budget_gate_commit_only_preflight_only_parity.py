#!/usr/bin/env python3
"""Harness for IQ-1599 strict diagnostics parity gate over IQ-1598 and IQ-1583."""
...
def secure_local_budget_gate_commit_only_preflight_only_parity(
    rows: list[RowOutput],
    rows_capacity: int,
    secure_local_mode: int,
    iommu_active: int,
...
def test_source_contains_iq1599_symbols() -> None:
    src = Path("src/gpu/security_perf_matrix.HC").read_text(encoding="utf-8")
    assert "I32 GPUSecurityPerfMatrixSummaryQ16CheckedOverheadEnvelopeSecureLocalBudgetGateCommitOnlyPreflightOnlyParity(" in src
    ...
    assert "status_commit_only = GPUSecurityPerfMatrixSummaryQ16CheckedOverheadEnvelopeSecureLocalBudgetGateCommitOnly(" in src
...
def test_gate_missing_and_threshold_breach_vectors() -> None:
    ...
    status, outputs = secure_local_budget_gate_commit_only_preflight_only_parity(
        rows_gate_missing,
        rows_capacity=3,
...
def test_status_domain_drift_and_deterministic_parity_vectors() -> None:
    ...
    status, outputs = secure_local_budget_gate_commit_only_preflight_only_parity(
        ...
        force_commit_tuple_parity_drift=True,
    )
    assert status == GPU_SEC_PERF_ERR_BAD_PARAM
    ...
    status, outputs = secure_local_budget_gate_commit_only_preflight_only_parity(
        ...
        max_p50_overhead_q16=6_000,
        max_p95_overhead_q16=7_000,
        caller_outputs=(277, 288, GPU_SEC_PERF_BUDGET_GATE_STATUS_PASS),
    )
    assert status == GPU_SEC_PERF_OK
    assert outputs == (277, 288, GPU_SEC_PERF_BUDGET_GATE_STATUS_PASS)
...
if __name__ == "__main__":
    test_source_contains_iq1599_symbols()
    test_gate_missing_and_threshold_breach_vectors()
    test_status_domain_drift_and_deterministic_parity_vectors()
