#!/usr/bin/env python3
"""Reference checks for GQAAttentionApplySoftmaxQ16CheckedNoPartialPreflightDefaultStrideParityCommitOnlyPreflightOnlyParityCommitOnly (IQ-1441)."""
...
from test_attention_gqa_apply_softmax_q16_checked_nopartial_preflight_default_stride_parity_commit_only_preflight_only_parity import (
    gqa_attention_apply_softmax_q16_checked_nopartial_preflight_default_stride_parity_commit_only_preflight_only_parity,
)


def gqa_attention_apply_softmax_q16_checked_nopartial_preflight_default_stride_parity_commit_only_preflight_only_parity_commit_only(
    scores_q32,
    scores_capacity: int,
    query_rows: int,
    key_rows: int,
...
def test_fixed_vector_reference_tuple_publish() -> None:
    ...
    err = gqa_attention_apply_softmax_q16_checked_nopartial_preflight_default_stride_parity_commit_only_preflight_only_parity_commit_only(
        scores,
        len(scores),
        query_rows,
        key_rows,
...
def test_randomized_equivalence_to_explicit_composition() -> None:
    rng = random.Random(20260425_1441)
    ...
    for _ in range(240):
        ...
        required_score_a = [rng.randint(-100, 100)]
        required_out_a = [rng.randint(-100, 100)]
        required_score_b = [required_score_a[0]]
        required_out_b = [required_out_a[0]]

        err_a = gqa_attention_apply_softmax_q16_checked_nopartial_preflight_default_stride_parity_commit_only_preflight_only_parity_commit_only(
            scores,
            len(scores),
            query_rows,
            key_rows,
...
if __name__ == "__main__":
    ...
    print(
        "gqa_attention_apply_softmax_q16_checked_nopartial_preflight_default_stride_parity_commit_only_preflight_only_parity_commit_only_reference_checks=ok"
    )
