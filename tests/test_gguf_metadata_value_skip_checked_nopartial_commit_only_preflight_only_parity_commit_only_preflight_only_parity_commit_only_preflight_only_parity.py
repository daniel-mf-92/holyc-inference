#!/usr/bin/env python3
"""Harness for ...ParityCommitOnlyPreflightOnlyParityCommitOnlyPreflightOnly (IQ-1327)."""
...
def gguf_metadata_value_skip_checked_nopartial_commit_only_preflight_only_parity_commit_only_preflight_only_parity_commit_only_preflight_only_parity_reference(
    *,
    buf: list[int] | None,
    buf_nbytes: int,
    ...
    staged_preflight_only_cursor = [snapshot_cursor]
    staged_parity_preflight_only_next = [0]
    err = gguf_metadata_value_skip_checked_nopartial_commit_only_preflight_only_parity_commit_only_preflight_only_parity_commit_only_reference(
        ...
        cursor_ref=staged_preflight_only_cursor,
        table_end=table_end,
        value_type=value_type,
        out_next_cursor_ref=staged_parity_preflight_only_next,
    )
    if err != GGUF_META_TABLE_OK:
        return err

    if staged_preflight_only_cursor[0] != staged_parity_preflight_only_next[0]:
        return GGUF_META_TABLE_ERR_BAD_PARAM
    if staged_parity_preflight_only_next[0] > buf_nbytes or staged_parity_preflight_only_next[0] > table_end:
    ...
    if staged_parity_preflight_only_next[0] != staged_canonical_commit_next[0]:
        return GGUF_META_TABLE_ERR_BAD_PARAM
    ...
    out_next_cursor_ref[0] = staged_parity_preflight_only_next[0]
    cursor_ref[0] = staged_parity_preflight_only_next[0]
    return GGUF_META_TABLE_OK


def test_source_contains_iq1327_function_and_strict_publish() -> None:
    source = Path("src/gguf/metadata.HC").read_text(encoding="utf-8")

    sig = "I32 GGUFMetadataValueSkipCheckedNoPartialCommitOnlyPreflightOnlyParityCommitOnlyPreflightOnlyParityCommitOnlyPreflightOnlyParity(U8 *buf,"
    assert sig in source
    assert re.search(
        r"I32 GGUFMetadataValueSkipCheckedNoPartialCommitOnlyPreflightOnlyParityCommitOnlyPreflightOnlyParityCommitOnlyPreflightOnlyParity\(U8 \*buf,\s+"
    ...
    assert "staged_preflight_only_next_cursor != staged_canonical_commit_next_cursor" in body
    assert "*out_next_cursor = staged_preflight_only_next_cursor;" in body
    assert "*cursor = staged_preflight_only_next_cursor;" in body
...
def test_randomized_parity_vectors() -> None:
    rng = random.Random(20260424_1327)
    ...
    for _ in range(1200):
        ...
        cursor_a = [cursor0]
        out_a = [0xAA]
        status_a = gguf_metadata_value_skip_checked_nopartial_commit_only_preflight_only_parity_commit_only_preflight_only_parity_commit_only_preflight_only_parity_reference(
            buf=buf,
            buf_nbytes=len(buf),
            cursor_ref=cursor_a,
            table_end=len(buf),
...
if __name__ == "__main__":
    test_source_contains_iq1327_function_and_strict_publish()
