#!/usr/bin/env python3
"""Parity harness for TokenizerBPEMergeCandidatesBuildCheckedDefaultCapacityNoPartial."""
...
def tokenizer_bpe_merge_candidates_build_checked_default_capacity_no_partial(
...
def run_case(
    ...
    # Explicit staged baseline for no-partial default-capacity composition.
    derived_capacity = 0 if token_count < 2 else token_count - 1
    ...
    err_actual = tokenizer_bpe_merge_candidates_build_checked_default_capacity_no_partial(
        token_ids,
        token_count,
        token_capacity,
        rank_left,
...
def test_known_vectors_parity_vs_explicit_staged_composition() -> None:
    run_case([10, 20, 30, 20], 4, 4, [10, 20, 30], [20, 30, 20], [7, 4, 9], 3, 3, 1)
    run_case([7, 8, 7, 8, 9], 5, 5, [7, 7, 8, 8], [8, 8, 7, 9], [12, 3, 6, 2], 4, 4, 2)
    run_case([42], 1, 1, [1], [2], [3], 1, 1, 3)
    run_case([], 0, 0, [], [], [], 0, 0, 4)
...
def test_error_paths_preserve_exact_output_state() -> None:
    ...
    err = tokenizer_bpe_merge_candidates_build_checked_default_capacity_no_partial(
        [10, 20, 30, 40],
        5,
        4,
        [10, 20, 30],
    ...
    err2 = tokenizer_bpe_merge_candidates_build_checked_default_capacity_no_partial(
        [1, 2],
        2,
        I64_MAX + 1,
        [],
...
def test_randomized_parity() -> None:
    rng = random.Random(20260418_393)
...
if __name__ == "__main__":
    ...
    print("tokenizer_bpe_merge_candidates_build_checked_default_capacity_no_partial_reference_checks=ok")
