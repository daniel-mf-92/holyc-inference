#!/usr/bin/env python3
"""Harness for IQ-1641 secure-local overhead-budget cross-gate commit-only preflight-only wrapper."""
...
def fast_path_switch_secure_local_overhead_budget_cross_gate_commit_only_preflight_only_iq1641(
    secure_local_mode: int,
    iommu_active: int,
    book_of_truth_gpu_hooks: int,
    policy_digest_parity: int,
...
def test_source_contains_iq1641_symbols() -> None:
    src = Path("src/gpu/security_perf_matrix.HC").read_text(encoding="utf-8")

    assert "I32 GPUSecurityPerfFastPathSwitchSecureLocalOverheadBudgetCrossGateCheckedCommitOnlyPreflightOnly(" in src
    assert (
        "status_commit_only = GPUSecurityPerfFastPathSwitchSecureLocalOverheadBudgetCrossGateCheckedCommitOnly(" in src
    )
    assert "status_preflight_only = GPUSecurityPerfFastPathSwitchSecureLocalOverheadBudgetCrossGateChecked(" in src


def test_gate_missing_vector() -> None:
    status, out_enabled, out_reason = (
        fast_path_switch_secure_local_overhead_budget_cross_gate_commit_only_preflight_only_iq1641(
...
def test_deterministic_secure_on_vectors() -> None:
    first = fast_path_switch_secure_local_overhead_budget_cross_gate_commit_only_preflight_only_iq1641(
        secure_local_mode=GPU_SEC_PERF_PROFILE_SECURE_LOCAL,
        iommu_active=1,
        book_of_truth_gpu_hooks=1,
        policy_digest_parity=1,
    ...
    second = fast_path_switch_secure_local_overhead_budget_cross_gate_commit_only_preflight_only_iq1641(
        secure_local_mode=GPU_SEC_PERF_PROFILE_SECURE_LOCAL,
        iommu_active=1,
        book_of_truth_gpu_hooks=1,
        policy_digest_parity=1,
...
if __name__ == "__main__":
    test_source_contains_iq1641_symbols()
