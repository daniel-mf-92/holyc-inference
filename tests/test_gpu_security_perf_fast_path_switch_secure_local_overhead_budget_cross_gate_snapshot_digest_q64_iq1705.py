#!/usr/bin/env python3
"""Harness for IQ-1705 snapshot-digest parity commit-only hardening wrapper."""
...
_BASE_IQ1703_PATH = Path(
    "tests/test_gpu_security_perf_fast_path_switch_secure_local_overhead_budget_cross_gate_snapshot_digest_q64_iq1703.py"
)
_BASE_IQ1703_SPEC = importlib.util.spec_from_file_location("iq1703_models", _BASE_IQ1703_PATH)
assert _BASE_IQ1703_SPEC is not None and _BASE_IQ1703_SPEC.loader is not None
_BASE_IQ1703_MOD = importlib.util.module_from_spec(_BASE_IQ1703_SPEC)
sys.modules[_BASE_IQ1703_SPEC.name] = _BASE_IQ1703_MOD
_BASE_IQ1703_SPEC.loader.exec_module(_BASE_IQ1703_MOD)
...
def fast_path_switch_secure_local_overhead_budget_cross_gate_snapshot_digest_q64_checked_commit_only_preflight_only_parity_commit_only_preflight_only_parity_commit_only_preflight_only_parity_commit_only_iq1705(
    ...
    *,
    ...
    _, staged_enabled, staged_reason, staged_digest_q64 = (
        _BASE_IQ1703_MOD.fast_path_switch_secure_local_overhead_budget_cross_gate_snapshot_digest_q64_checked_commit_only_preflight_only_parity_commit_only_preflight_only_parity_commit_only_iq1703(
            secure_local_mode=secure_local_mode,
            iommu_active=iommu_active,
            book_of_truth_gpu_hooks=book_of_truth_gpu_hooks,
            policy_digest_parity=policy_digest_parity,
...
def test_source_contains_iq1705_symbols() -> None:
    src = Path("src/gpu/security_perf_matrix.HC").read_text(encoding="utf-8")

    assert (
        "I32 GPUSecurityPerfFastPathSwitchSecureLocalOverheadBudgetCrossGateSnapshotDigestQ64CheckedCommitOnlyPreflightOnlyParityCommitOnlyPreflightOnlyParityCommitOnlyPreflightOnlyParityCommitOnly("
        in src
    )
    assert (
    ...
    assert "// IQ-1705 commit-only hardening wrapper:" in src


def test_gate_missing_vector() -> None:
    status, enabled, reason, digest_q64 = (
        fast_path_switch_secure_local_overhead_budget_cross_gate_snapshot_digest_q64_checked_commit_only_preflight_only_parity_commit_only_preflight_only_parity_commit_only_preflight_only_parity_commit_only_iq1705(
...
if __name__ == "__main__":
    test_source_contains_iq1705_symbols()
