#!/usr/bin/env python3
"""Reference checks for GGUFMetadataReadU16LEChecked semantics."""
...
def test_null_ptr_and_no_partial_write() -> None:
    ...
    assert (
        gguf_metadata_read_u16le_checked(None, 5, cursor, 5, out)
    ...
    assert (
        gguf_metadata_read_u16le_checked([1, 2], 2, None, 2, out)
        == GGUF_META_TABLE_ERR_NULL_PTR
    )
    assert out == [0xABCD]

    assert (
        gguf_metadata_read_u16le_checked([1, 2], 2, cursor, 2, None)
        == GGUF_META_TABLE_ERR_NULL_PTR
    )
    assert cursor == [2]
...
def test_short_reads_do_not_advance_cursor() -> None:
    ...
    out = [0x7777]
    assert (
        gguf_metadata_read_u16le_checked(buf, len(buf), cursor, len(buf), out)
    ...
    out = [0xAAAA]
    assert (
        gguf_metadata_read_u16le_checked(buf, 2, cursor, 3, out)
        == GGUF_META_TABLE_ERR_OUT_OF_BOUNDS
    )
    assert cursor == [1]
    assert out == [0xAAAA]
...
def test_success_decodes_little_endian_and_advances() -> None:
    ...
    err = gguf_metadata_read_u16le_checked(buf, len(buf), cursor, len(buf), out)
...
def test_randomized_parity() -> None:
    rng = random.Random(20260417_206)
    ...
    for _ in range(4000):
        ...
        err = gguf_metadata_read_u16le_checked(buf, n, cursor, table_end, out)

        if cursor0 + 2 <= table_end and cursor0 + 2 <= n:
            expect = buf[cursor0] | (buf[cursor0 + 1] << 8)
            assert err == GGUF_META_TABLE_OK
            assert out[0] == expect
...
def test_table_end_overflow_surface() -> None:
    ...
    err = gguf_metadata_read_u16le_checked(buf, len(buf), cursor, I64_MAX + 1, out)
...
def run() -> None:
    ...
    print("gguf_metadata_read_u16le_checked_reference_checks=ok")
