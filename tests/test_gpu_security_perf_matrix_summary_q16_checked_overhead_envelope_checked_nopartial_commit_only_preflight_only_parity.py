#!/usr/bin/env python3
"""Harness for IQ-1509 overhead-envelope no-partial commit-only preflight-only parity gate."""
...
def _overhead_envelope_nopartial(
    ...
    first = _overhead_envelope_checked(
        rows, rows_capacity, secure_local_mode, iommu_active, book_of_truth_gpu_hooks, policy_digest_parity
    )
...
def _overhead_envelope_nopartial_commit_only(
    ...
    parity = _overhead_envelope_nopartial(
        rows, rows_capacity, out_capacity, secure_local_mode, iommu_active, book_of_truth_gpu_hooks, policy_digest_parity
    )
...
def _overhead_envelope_nopartial_commit_only_preflight_only(
    ...
    preflight = _overhead_envelope_nopartial_commit_only(
        rows, rows_capacity, out_capacity, secure_local_mode, iommu_active, book_of_truth_gpu_hooks, policy_digest_parity
    )
    canonical = _overhead_envelope_nopartial(
        rows, rows_capacity, out_capacity, secure_local_mode, iommu_active, book_of_truth_gpu_hooks, policy_digest_parity
    )
    if preflight[0] != canonical[0]:
        return GPU_SEC_PERF_ERR_BAD_PARAM, 0, 0, 0
    if preflight[0] != GPU_SEC_PERF_OK:
        return preflight[0], 0, 0, 0
    if preflight[1:] != canonical[1:]:
        return GPU_SEC_PERF_ERR_BAD_PARAM, 0, 0, 0
    return preflight


def gpu_security_perf_matrix_summary_q16_checked_overhead_envelope_checked_nopartial_commit_only_preflight_only_parity(
    ...
    preflight_tuple = preflight[1:]
    parity_tuple = parity[1:]

    if inject_parity_drift and preflight_status == GPU_SEC_PERF_OK:
        preflight_tuple = (preflight_tuple[0], preflight_tuple[1], preflight_tuple[2] + 1)
    if inject_status_drift:
        parity_status = GPU_SEC_PERF_ERR_BAD_PARAM

    if preflight_status != parity_status:
        return GPU_SEC_PERF_ERR_BAD_PARAM, caller_outputs, (0, 0, 0)
...
def test_source_contains_iq1509_symbols() -> None:
    src = Path("src/gpu/security_perf_matrix.HC").read_text(encoding="utf-8")

    assert "I32 GPUSecurityPerfMatrixSummaryQ16CheckedOverheadEnvelopeCheckedNoPartialCommitOnlyPreflightOnlyParity(" in src
    assert (
        "status_preflight_only = GPUSecurityPerfMatrixSummaryQ16CheckedOverheadEnvelopeCheckedNoPartialCommitOnlyPreflightOnly("
        in src
    )
...
def test_success_zero_write_and_tuple_parity() -> None:
    rows = [
    ...
    status, outputs_after, tuple_diag = (
        gpu_security_perf_matrix_summary_q16_checked_overhead_envelope_checked_nopartial_commit_only_preflight_only_parity(
            rows,
            rows_capacity=4,
            out_capacity=3,
    ...
    assert tuple_diag == (1_600, 2_000, 1_800)


def test_null_and_alias_rejected() -> None:
...
def test_policy_gate_and_capacity_rejected() -> None:
    rows = [RowOutput(100_000, 1_500, 10_000)]
...
def test_parity_and_status_drift_detected() -> None:
    rows = [RowOutput(100_000, 1_500, 10_000), RowOutput(100_000, 1_700, 10_500)]
    ...
    status, outputs_after, tuple_diag = (
        gpu_security_perf_matrix_summary_q16_checked_overhead_envelope_checked_nopartial_commit_only_preflight_only_parity(
            ...
            book_of_truth_gpu_hooks=1,
            policy_digest_parity=1,
            caller_outputs=(8, 9, 10),
            inject_status_drift=True,
        )
    )
    assert status == GPU_SEC_PERF_ERR_BAD_PARAM
    assert outputs_after == (8, 9, 10)
    assert tuple_diag == (0, 0, 0)
