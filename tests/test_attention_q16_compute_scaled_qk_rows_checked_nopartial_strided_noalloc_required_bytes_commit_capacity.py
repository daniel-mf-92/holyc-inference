#!/usr/bin/env python3
"""Parity harness for AttentionQ16ComputeScaledQKRowsCheckedNoPartialStridedNoAllocRequiredBytesCommitCapacity."""
...
def attention_q16_compute_scaled_qk_rows_checked_nopartial_strided_noalloc_required_bytes_commit_capacity(
    q_rows_q16,
    q_rows_capacity: int,
    query_row_count: int,
    ...
    err, commit_required_stage_cells = try_mul_i64_checked(query_row_count, token_count)
    if err != ATTN_Q16_OK:
        return err

    err, commit_required_stage_bytes = try_mul_i64_checked(commit_required_stage_cells, 8)
    if err != ATTN_Q16_OK:
        return err
    ...
    out_commit_required_stage_cells[0] = commit_required_stage_cells
    out_commit_required_stage_bytes[0] = commit_required_stage_bytes
    out_required_out_cells[0] = required_out[0]
    return ATTN_Q16_OK
...
def test_source_contains_strided_noalloc_required_bytes_commit_capacity_helper() -> None:
    source = Path("src/model/attention.HC").read_text(encoding="utf-8")
    signature = "I32 AttentionQ16ComputeScaledQKRowsCheckedNoPartialStridedNoAllocRequiredBytesCommitCapacity("
...
def test_known_vectors_and_commit_capacity_rejection() -> None:
    query_row_count = 3
    token_count = 4
    head_dim = 7
    ...
    err = attention_q16_compute_scaled_qk_rows_checked_nopartial_strided_noalloc_required_bytes_commit_capacity(
        q_rows,
        q_capacity,
        query_row_count,
        query_row_stride_q16,
    ...
    expected_required_out_cells = (query_row_count - 1) * out_row_stride + token_count
    assert out_out_cells == [expected_required_out_cells]
...
def test_randomized_parity_against_explicit_checked_composition() -> None:
    rng = random.Random(20260420_632)
    ...
    for _ in range(5000):
        ...
        commit_stage_cell_capacity = max(
            0, query_row_count * token_count + rng.randint(-80, 80)
        )
        commit_stage_byte_capacity = max(
            0, commit_stage_cell_capacity * 8 + rng.randint(-256, 256)
        )
        ...
        err_got = attention_q16_compute_scaled_qk_rows_checked_nopartial_strided_noalloc_required_bytes_commit_capacity(
            q_rows,
            q_capacity,
            query_row_count,
            query_row_stride_q16,
...
if __name__ == "__main__":
    test_source_contains_strided_noalloc_required_bytes_commit_capacity_helper()
    test_known_vectors_and_commit_capacity_rejection()
