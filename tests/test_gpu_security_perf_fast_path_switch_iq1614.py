#!/usr/bin/env python3
"""Harness for IQ-1614 zero-write diagnostics companion over IQ-1613 and IQ-1610."""
...
def _target_parity(
    ...
    # Parity diagnostics path is zero-write: caller tuple remains unchanged.
...
def fast_path_switch_checked_audit_parity_iq1614(
    secure_local_mode: int,
    iommu_active: int,
    book_of_truth_gpu_hooks: int,
    policy_digest_parity: int,
    ...
    *,
    ...
    status_preflight_only, canonical_enabled, canonical_reason = _target_parity(
        secure_local_mode,
        iommu_active,
        book_of_truth_gpu_hooks,
        policy_digest_parity,
    ...
    if force_tuple_drift:
        ...
        if canonical_reason == staged_reason:
    ...
    if (
        status_commit_only != status_preflight_only
        or staged_enabled != canonical_enabled
        or staged_reason != canonical_reason
    ):
        return GPU_SEC_PERF_ERR_BAD_PARAM, current_fast_path_enabled, current_disable_reason_code

    # IQ-1614 is diagnostics-only: preserve caller tuple even on success.
    return status_commit_only, current_fast_path_enabled, current_disable_reason_code


def test_source_contains_iq1614_symbols() -> None:
    src = Path("src/gpu/security_perf_matrix.HC").read_text(encoding="utf-8")

    assert "I32 GPUSecurityPerfFastPathSwitchCheckedAuditParityCommitOnlyPreflightOnlyParityCommitOnlyPreflightOnlyParityCommitOnlyPreflightOnlyParityCommitOnlyPreflightOnly(" in src
    ...
    assert "status_preflight_only = GPUSecurityPerfFastPathSwitchCheckedAuditParityCommitOnlyPreflightOnlyParityCommitOnlyPreflightOnlyParityCommitOnlyPreflightOnlyParity(" in src
    assert "if (*out_fast_path_enabled != saved_fast_path_enabled" in src


def test_gate_missing_and_audit_parity_breach_vectors() -> None:
    status, enabled, reason = fast_path_switch_checked_audit_parity_iq1614(
    ...
    assert (status, enabled, reason) == (GPU_SEC_PERF_ERR_POLICY_GUARD, 71, 72)
    ...
    status, enabled, reason = fast_path_switch_checked_audit_parity_iq1614(
        ...
        current_fast_path_enabled=73,
        current_disable_reason_code=74,
    )
    assert (status, enabled, reason) == (GPU_SEC_PERF_ERR_POLICY_GUARD, 73, 74)
...
def test_status_domain_drift_and_no_write_parity_vectors() -> None:
    ...
    status, enabled, reason = fast_path_switch_checked_audit_parity_iq1614(
        ...
        policy_digest_parity=1,
        dispatch_transcript_parity=1,
        current_fast_path_enabled=83,
        current_disable_reason_code=84,
        force_tuple_drift=True,
    )
    assert (status, enabled, reason) == (GPU_SEC_PERF_ERR_BAD_PARAM, 83, 84)


def test_secure_on_overhead_budget_vector_and_determinism() -> None:
    # Secure-on budget vector: all trust gates pass and fast-path is available,
    # but IQ-1614 must remain diagnostics-only and preserve caller slots.
    first = fast_path_switch_checked_audit_parity_iq1614(
        secure_local_mode=GPU_SEC_PERF_PROFILE_SECURE_LOCAL,
        iommu_active=1,
        book_of_truth_gpu_hooks=1,
        policy_digest_parity=1,
    ...
    second = fast_path_switch_checked_audit_parity_iq1614(
        secure_local_mode=GPU_SEC_PERF_PROFILE_SECURE_LOCAL,
        iommu_active=1,
        book_of_truth_gpu_hooks=1,
        policy_digest_parity=1,
    ...
    assert first == second
    assert first == (GPU_SEC_PERF_OK, 91, 92)


if __name__ == "__main__":
    test_source_contains_iq1614_symbols()
    test_gate_missing_and_audit_parity_breach_vectors()
    test_status_domain_drift_and_no_write_parity_vectors()
    test_secure_on_overhead_budget_vector_and_determinism()
    print("ok")
