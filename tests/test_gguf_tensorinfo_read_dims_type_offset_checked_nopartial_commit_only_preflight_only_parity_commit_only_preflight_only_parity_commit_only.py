#!/usr/bin/env python3
"""Harness for IQ-1038 dims/type/offset parity-preflight commit-only wrapper."""
...
def parse_dims_type_offset_checked_nopartial_commit_only_preflight_only_parity_commit_only_preflight_only_parity_commit_only(
    buf: bytes | None,
    size: int,
    cursor: int,
    ...
    if parity_end is None or canonical_end is None:
        return GGUF_TENSOR_PARSE_ERR_TRUNCATED

    if parity_end > size or canonical_end > size:
        return GGUF_TENSOR_PARSE_ERR_TRUNCATED
    ...
    if (
        staged_parity_dim_count[0] != canonical_dim_count[0]
        or staged_parity_dims_cells[0] != canonical_dims_cells[0]
        or staged_parity_required_bytes[0] != canonical_required_bytes[0]
        or staged_parity_type_value[0] != canonical_type_value[0]
        or staged_parity_tensor_offset[0] != canonical_tensor_offset[0]
        or staged_parity_last_dim_index[0] != canonical_last_dim_index[0]
    ):
        return GGUF_TENSOR_PARSE_ERR_TRUNCATED

    if staged_parity_type_value[0] not in KNOWN_TYPES or canonical_type_value[0] not in KNOWN_TYPES:
...
def explicit_checked_composition(*args) -> int:
    return parse_dims_type_offset_checked_nopartial_commit_only_preflight_only_parity_commit_only_preflight_only_parity_commit_only(
        *args
    )


def test_source_contains_iq1038_signature_and_contract() -> None:
    source = Path("src/gguf/tensorinfo.HC").read_text(encoding="utf-8")
    sig = "I64 GGUFTensorInfoReadDimsTypeOffsetCheckedNoPartialCommitOnlyPreflightOnlyParityCommitOnlyPreflightOnlyParityCommitOnly("
    assert sig in source
    body = source.split(sig, 1)[1].split("I64 GGUFTensorParseOne(", 1)[0]

    assert "IQ-1038 commit-only diagnostics wrapper for dims parity-preflight gate." in source
    ...
    assert "if (staged_parity_dim_count != canonical_dim_count ||" in body
    assert "staged_parity_required_bytes != canonical_required_bytes" in body
    assert "staged_parity_tensor_offset != canonical_tensor_offset" in body
...
def test_known_vector_success_and_alias_guard() -> None:
    ...
    err = parse_dims_type_offset_checked_nopartial_commit_only_preflight_only_parity_commit_only_preflight_only_parity_commit_only(
        payload,
        len(payload),
        0,
        out_dim_count,
...
def test_randomized_parity_vs_explicit_composition() -> None:
    rng = random.Random(20260422_1038)
    ...
    for _ in range(900):
        ...
        got = parse_dims_type_offset_checked_nopartial_commit_only_preflight_only_parity_commit_only_preflight_only_parity_commit_only(
            buf,
            size,
            cursor,
            out_a[0],
...
if __name__ == "__main__":
    test_source_contains_iq1038_signature_and_contract()
    ...
    print(
        "gguf_tensorinfo_read_dims_type_offset_checked_nopartial_commit_only_preflight_only_parity_commit_only_preflight_only_parity_commit_only=ok"
    )
