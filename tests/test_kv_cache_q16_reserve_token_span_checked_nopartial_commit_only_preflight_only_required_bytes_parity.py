#!/usr/bin/env python3
"""Parity harness for KVCacheQ16ReserveTokenSpan...RequiredBytesParity (IQ-1064)."""
...
def kv_cache_q16_reserve_token_span_checked_nopartial_commit_only_preflight_only_required_bytes_parity(
    layer_count: int,
    head_count: int,
    head_dim: int,
    token_start: int,
    ...
    if staged_required_cells[0] != required_cells:
        return KV_Q16_ERR_BAD_PARAM
    if staged_required_bytes[0] != required_bytes:
        return KV_Q16_ERR_BAD_PARAM
    if staged_last_cell_index[0] != last_cell_index:
        return KV_Q16_ERR_BAD_PARAM

    out_required_cells[0] = staged_required_cells[0]
    out_required_bytes[0] = staged_required_bytes[0]
    out_last_cell_index[0] = staged_last_cell_index[0]
    return KV_Q16_OK
...
def explicit_composition(
    ...
    if (
        staged_cells[0] != required_cells
        or staged_bytes[0] != required_bytes
        or staged_last[0] != last_cell_index
    ):
        return KV_Q16_ERR_BAD_PARAM, (0, 0, 0)
...
def test_source_contains_required_bytes_parity_helper() -> None:
    ...
    sig = "I32 KVCacheQ16ReserveTokenSpanCheckedNoPartialCommitOnlyPreflightOnlyRequiredBytesParity("
    ...
    assert "KVTryMulI64Checked(recomputed_cells_per_token," in body
    assert "KVTryMulI64Checked(token_start," in body
    assert "KVTryMulI64Checked(token_count," in body
    assert "KVTryAddI64Checked(recomputed_base_index," in body
    assert "snapshot_layer_count" in body
    assert "if (staged_required_cells != recomputed_required_cells)" in body
    assert "if (staged_required_bytes != recomputed_required_bytes)" in body
    assert "if (staged_last_cell_index != recomputed_last_cell_index)" in body
...
def test_randomized_parity_vs_explicit_composition() -> None:
    rng = random.Random(20260422_1064)
    ...
    for _ in range(1700):
        ...
        err_new = kv_cache_q16_reserve_token_span_checked_nopartial_commit_only_preflight_only_required_bytes_parity(
            layer_count,
            head_count,
            head_dim,
            token_start,
...
if __name__ == "__main__":
    ...
    print(
        "kv_cache_q16_reserve_token_span_checked_nopartial_commit_only_preflight_only_required_bytes_parity=ok"
    )
