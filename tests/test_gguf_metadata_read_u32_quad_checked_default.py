#!/usr/bin/env python3
"""Parity checks for GGUFMetadataReadU32QuadCheckedDefault."""
...
def gguf_metadata_read_u32_quad_checked_default(
    buf: list[int] | None,
    buf_nbytes: int,
    cursor_ref: list[int] | None,
    out_first_ref: list[int] | None,
    ...
    return gguf_metadata_read_u32_quad_checked(
        buf,
        buf_nbytes,
        cursor_ref,
        buf_nbytes,
...
def test_null_ptr_and_no_partial_write() -> None:
    ...
    assert (
        gguf_metadata_read_u32_quad_checked_default(
            None,
            32,
            cursor,
            out_first,
...
def test_uses_default_end_and_no_commit_on_short_payload() -> None:
    ...
    err = gguf_metadata_read_u32_quad_checked_default(
        buf,
        len(buf),
        cursor,
        out_first,
...
def test_success_reads_four_u32_and_advances() -> None:
    ...
    fourth = 0xCAFEBABE
    prefix = [0xAA]
    buf = prefix + _le_u32(first) + _le_u32(second) + _le_u32(third) + _le_u32(fourth)
...
def test_default_wrapper_matches_checked_core_for_random_inputs() -> None:
    random.seed(0x5532446)

    for _ in range(250):
        values = [random.getrandbits(32) for _ in range(4)]
        prefix_len = random.randint(0, 6)
        suffix_len = random.randint(0, 6)
        ...
        cursor_checked = [prefix_len]
        out_default = [[0], [0], [0], [0]]
        out_checked = [[0], [0], [0], [0]]

        err_default = gguf_metadata_read_u32_quad_checked_default(
            buf,
            len(buf),
            cursor_default,
            out_default[0],
        ...
        err_checked = gguf_metadata_read_u32_quad_checked(
            buf,
            len(buf),
            cursor_checked,
            len(buf),
        ...
        assert [slot[0] for slot in out_default] == [slot[0] for slot in out_checked]


if __name__ == "__main__":
    test_null_ptr_and_no_partial_write()
    test_uses_default_end_and_no_commit_on_short_payload()
    test_success_reads_four_u32_and_advances()
    test_overflow_passthrough_and_no_commit()
    test_default_wrapper_matches_checked_core_for_random_inputs()
    print("ok")
