#!/usr/bin/env python3
"""Harness for IQ-1635 commit-only hardening wrapper over IQ-1623 and IQ-1622."""
...
def fast_path_switch_checked_audit_parity_iq1635(
    secure_local_mode: int,
    iommu_active: int,
    book_of_truth_gpu_hooks: int,
    policy_digest_parity: int,
...
def test_source_contains_iq1635_symbols() -> None:
    src = Path("src/gpu/security_perf_matrix.HC").read_text(encoding="utf-8")

    assert "I32 (" in src
    assert "status_commit_only = (" in src
    assert "status_parity = GPUSecurityPerfFastPathSwitchCheckedAuditParityCommitOnlyPreflightOnlyParityCommitOnlyPreflightOnlyParityCommitOnlyPreflightOnlyParityCommitOnlyPreflightOnlyParityCommitOnlyPreflightOnlyParityCommitOnlyPreflightOnlyParityCommitOnlyPreflightOnlyParity(" in src


def test_gate_missing_and_audit_parity_breach_vectors() -> None:
    status, enabled, reason = fast_path_switch_checked_audit_parity_iq1635(
...
def test_deterministic_tuple_parity_vectors() -> None:
    first = fast_path_switch_checked_audit_parity_iq1635(
        secure_local_mode=GPU_SEC_PERF_PROFILE_SECURE_LOCAL,
        iommu_active=1,
        book_of_truth_gpu_hooks=1,
        policy_digest_parity=1,
    ...
    second = fast_path_switch_checked_audit_parity_iq1635(
        secure_local_mode=GPU_SEC_PERF_PROFILE_SECURE_LOCAL,
        iommu_active=1,
        book_of_truth_gpu_hooks=1,
        policy_digest_parity=1,
    ...
    assert first == (GPU_SEC_PERF_OK, 521, 522)
    assert second == first


def test_secure_on_overhead_budget_vector_keeps_no_write_contract() -> None:
    status, enabled, reason = fast_path_switch_checked_audit_parity_iq1635(
        secure_local_mode=GPU_SEC_PERF_PROFILE_SECURE_LOCAL,
        iommu_active=1,
        book_of_truth_gpu_hooks=1,
    ...
    assert (status, enabled, reason) == (GPU_SEC_PERF_OK, 0x5A5A, 0x0B0B)


if __name__ == "__main__":
    test_source_contains_iq1635_symbols()
    ...
    test_secure_on_overhead_budget_vector_keeps_no_write_contract()
    print("ok")
