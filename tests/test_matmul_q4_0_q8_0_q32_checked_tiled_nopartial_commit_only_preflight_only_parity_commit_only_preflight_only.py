#!/usr/bin/env python3
"""Harness for MatMulQ4_0Q8_0Q32CheckedTiledNoPartial...ParityCommitOnlyPreflightOnly (IQ-1005)."""
...
def matmul_q4_0_q8_0_q32_checked_tiled_nopartial_commit_only_preflight_only_parity_commit_only_preflight_only(
    ...
    if (
        staged_required_out_cells[0] != canonical_required_out_cells[0]
        or staged_required_out_bytes[0] != canonical_required_out_bytes[0]
        or staged_tile_rows[0] != canonical_tile_rows[0]
        or staged_tile_cols[0] != canonical_tile_cols[0]
    ):
        return Q4_0_Q8_0_AVX2_ERR_BAD_LEN
...
def explicit_checked_composition(*args):
    staged_required_out_cells = [0x81]
    staged_required_out_bytes = [0x82]
    staged_tile_rows = [0x83]
    staged_tile_cols = [0x84]
    ...
    canonical_required_out_cells = [0x91]
    canonical_required_out_bytes = [0x92]
    canonical_tile_rows = [0x93]
    canonical_tile_cols = [0x94]
...
def test_source_contains_iq1005_signature_and_staged_parity_contract() -> None:
    ...
    assert "// IQ-1005 diagnostics-only no-write companion:" in body
