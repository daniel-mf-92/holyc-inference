#!/usr/bin/env python3
"""Parity harness for KVCacheQ16ZeroTokenSpan...RequiredBytesCommitOnlyPreflightOnly (IQ-899)."""
...
from test_kv_cache_q16_zero_token_span_checked_nopartial_commit_only_preflight_only_required_bytes import (
    kv_cache_q16_zero_token_span_checked_nopartial_commit_only_preflight_only_required_bytes,
)
from test_kv_cache_q16_zero_token_span_checked_nopartial_commit_only_preflight_only_required_bytes_commit_only import (
    kv_cache_q16_zero_token_span_checked_nopartial_commit_only_preflight_only_required_bytes_commit_only,
)


def kv_cache_q16_zero_token_span_checked_nopartial_commit_only_preflight_only_required_bytes_commit_only_preflight_only(
    ...
    staged_k_base_commit = [0]
    staged_v_base_commit = [0]
    staged_end_commit = [0]
    err = kv_cache_q16_zero_token_span_checked_nopartial_commit_only_preflight_only_required_bytes_commit_only(
        k_cache_q16,
        k_cache_capacity,
        v_cache_q16,
        v_cache_capacity,
        ...
        staged_commit,
        staged_bytes_commit,
        staged_k_base_commit,
        staged_v_base_commit,
        staged_end_commit,
    )
    if err != KV_Q16_OK:
        return err
    ...
    staged_preflight = [0]
    staged_bytes_preflight = [0]
    staged_k_base_preflight = [0]
    staged_v_base_preflight = [0]
    staged_end_preflight = [0]
    err = kv_cache_q16_zero_token_span_checked_nopartial_commit_only_preflight_only_required_bytes(
        ...
        head_dim,
        staged_preflight,
        staged_bytes_preflight,
        staged_k_base_preflight,
        staged_v_base_preflight,
        staged_end_preflight,
    )
    if err != KV_Q16_OK:
        return err

    if (
        staged_commit[0] != staged_preflight[0]
        or staged_bytes_commit[0] != staged_bytes_preflight[0]
        or staged_k_base_commit[0] != staged_k_base_preflight[0]
        or staged_v_base_commit[0] != staged_v_base_preflight[0]
        or staged_end_commit[0] != staged_end_preflight[0]
    ):
        return KV_Q16_ERR_BAD_PARAM
    ...
    out_required_span_cells[0] = staged_commit[0]
    out_required_token_bytes[0] = staged_bytes_commit[0]
    out_k_base_index[0] = staged_k_base_commit[0]
    out_v_base_index[0] = staged_v_base_commit[0]
    out_end_index[0] = staged_end_commit[0]
    return KV_Q16_OK


def explicit_preflight_only_composition(*args, **kwargs) -> tuple[int, tuple[int, int, int, int, int]]:
    commit_span = [0]
    commit_bytes = [0]
    commit_k_base = [0]
    commit_v_base = [0]
    ...
    err = kv_cache_q16_zero_token_span_checked_nopartial_commit_only_preflight_only_required_bytes(
        *args,
        out_required_span_cells=pre_span,
        out_required_token_bytes=pre_bytes,
        out_k_base_index=pre_k_base,
        out_v_base_index=pre_v_base,
        out_end_index=pre_end,
        **kwargs,
    )
    if err != KV_Q16_OK:
        return err, (0, 0, 0, 0, 0)
    ...
    if (
        commit_span[0] != pre_span[0]
        or commit_bytes[0] != pre_bytes[0]
        or commit_k_base[0] != pre_k_base[0]
        or commit_v_base[0] != pre_v_base[0]
        or commit_end[0] != pre_end[0]
    ):
        return KV_Q16_ERR_BAD_PARAM, (0, 0, 0, 0, 0)

    return KV_Q16_OK, (commit_span[0], commit_bytes[0], commit_k_base[0], commit_v_base[0], commit_end[0])


def test_source_contains_required_bytes_commit_only_preflight_only_helper() -> None:
    source = Path("src/model/kv_cache.HC").read_text(encoding="utf-8")
    sig = "I32 KVCacheQ16ZeroTokenSpanCheckedNoPartialCommitOnlyPreflightOnlyRequiredBytesCommitOnlyPreflightOnly("
    assert sig in source
    body = source.split(sig, 1)[1]
    assert "KVCacheQ16ZeroTokenSpanCheckedNoPartialCommitOnlyPreflightOnlyRequiredBytesCommitOnly(" in body
    assert "KVCacheQ16ZeroTokenSpanCheckedNoPartialCommitOnlyPreflightOnlyRequiredBytes(" in body
    assert "KVCacheQ16ComputeLayerTokenBaseIndexChecked(layer_idx," in body
    assert "KVTryMulI64Checked(staged_span_cells_recomputed," in body
    ...
    assert "if (staged_required_span_cells != staged_span_cells_recomputed)" in body
...
def test_known_vector_preflight_only_required_bytes_and_zero_write() -> None:
    layer_count = 3
    token_capacity = 5
    kv_heads = 2
    head_dim = 4
...
def test_alias_null_and_no_partial_contracts() -> None:
    ...
    assert (
        kv_cache_q16_zero_token_span_checked_nopartial_commit_only_preflight_only_required_bytes_commit_only_preflight_only(
            k_cache,
            len(k_cache),
            v_cache,
            len(v_cache),
...
def test_randomized_parity_vs_explicit_composition_adversarial() -> None:
    ...
    for _ in range(1000):
        ...
        # Force required-bytes overflow while keeping host allocations tiny.
        if rng.random() < 0.06:
            layer_count = 1
            token_capacity = 1
            layer_idx = 0
        ...
        err_a = kv_cache_q16_zero_token_span_checked_nopartial_commit_only_preflight_only_required_bytes_commit_only_preflight_only(
            k_cache,
            k_cache_capacity,
            v_cache,
            v_cache_capacity,
