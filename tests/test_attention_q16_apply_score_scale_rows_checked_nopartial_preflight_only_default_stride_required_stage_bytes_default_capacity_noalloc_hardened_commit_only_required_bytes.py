#!/usr/bin/env python3
"""Parity harness for ...NoAllocHardenedCommitOnlyRequiredBytes (IQ-793)."""
...
def attention_q16_apply_score_scale_rows_checked_nopartial_preflight_only_default_stride_required_stage_bytes_default_capacity_noalloc_hardened_commit_only_required_bytes(
    in_scores_q32,
...
def explicit_checked_composition(
    ...
    if commit_required_in_cells[0] != recomputed_required_stage_cells:
        return ATTN_Q16_ERR_BAD_PARAM
    if commit_required_out_cells[0] != recomputed_required_stage_cells:
        return ATTN_Q16_ERR_BAD_PARAM
    if commit_required_stage_cells[0] != recomputed_required_stage_cells:
        return ATTN_Q16_ERR_BAD_PARAM
    if commit_required_stage_bytes[0] != recomputed_required_stage_bytes:
        return ATTN_Q16_ERR_BAD_PARAM
    if commit_last_in_index[0] != expected_last_index:
        return ATTN_Q16_ERR_BAD_PARAM
    if commit_last_out_index[0] != expected_last_index:
        return ATTN_Q16_ERR_BAD_PARAM
...
def test_source_contains_default_capacity_noalloc_hardened_commit_only_required_bytes() -> None:
    ...
    assert "IQ-793 required-bytes diagnostics contract" in source


def test_commit_only_required_bytes_nullptr_and_alias_contracts() -> None:
    out_a = [111]
    out_b = [222]
    out_c = [333]
    out_d = [444]
    ...
    err = attention_q16_apply_score_scale_rows_checked_nopartial_preflight_only_default_stride_required_stage_bytes_default_capacity_noalloc_hardened_commit_only_required_bytes(
        [0],
        1,
        1,
        1,
...
def test_commit_only_required_bytes_overflow_propagation() -> None:
...
def test_commit_only_required_bytes_matches_explicit_checked_composition_randomized() -> None:
    rng = random.Random(20260420_793)
    ...
    for _ in range(1000):
        ...
        err_a = attention_q16_apply_score_scale_rows_checked_nopartial_preflight_only_default_stride_required_stage_bytes_default_capacity_noalloc_hardened_commit_only_required_bytes(
            in_scores,
            in_scores_capacity,
            row_count,
            token_count,
...
if __name__ == "__main__":
    test_source_contains_default_capacity_noalloc_hardened_commit_only_required_bytes()
    test_commit_only_required_bytes_nullptr_and_alias_contracts()
    test_commit_only_required_bytes_overflow_propagation()
    test_commit_only_required_bytes_matches_explicit_checked_composition_randomized()
    print(
        ...
        "hardened_commit_only_required_bytes=ok"
    )
