#!/usr/bin/env python3
"""Harness for IQ-1468 secure-on GPU per-row gate no-partial wrapper."""
...
def test_source_contains_iq1468_symbols() -> None:
    src = Path("src/gpu/security_perf_matrix.HC").read_text(encoding="utf-8")

    assert "I32 GPUSecurityPerfMatrixRowGateCheckedNoPartial(" in src
    assert "status_preflight = GPUSecurityPerfMatrixRowGateCheckedCommitOnly(" in src
    assert "status_commit = GPUSecurityPerfMatrixRowGateCheckedCommitOnly(" in src
...
def test_null_alias_capacity_vectors() -> None:
    status, reason, allowed = gpu_security_perf_matrix_row_gate_checked_nopartial(
...
def test_gate_missing_and_reason_code_parity_vectors() -> None:
    ...
    for vector in vectors:
        nopartial_status, _, _ = gpu_security_perf_matrix_row_gate_checked_nopartial(
            *vector,
        ...
        assert nopartial_status == commit_status
...
if __name__ == "__main__":
    test_source_contains_iq1468_symbols()
