...
def gpu_security_perf_matrix_summary_q16_checked_latency_window_checked_nopartial_preflight_only_parity_commit_only_preflight_only_parity_commit_only(
...
def test_commit_only_success_publishes_duplicate_latency_tuple() -> None:
    rows = [
    ...
    status, persisted, staged = gpu_security_perf_matrix_summary_q16_checked_latency_window_checked_nopartial_preflight_only_parity_commit_only_preflight_only_parity_commit_only(
        rows,
        rows_capacity=5,
        out_capacity=4,
    ...
    assert persisted == (10_000, 10_000, 30_000, 80_000)
...
def test_null_alias_capacity_gate_missing_and_parity_drift_vectors() -> None:
    ...
    assert (status, persisted, staged) == (GPU_SEC_PERF_ERR_NULL_PTR, (1, 2, 3, 4), (0, 0, 0, 0))

    status, *_ = gpu_security_perf_matrix_summary_q16_checked_latency_window_checked_nopartial_preflight_only_parity_commit_only_preflight_only_parity_commit_only(
        rows,
        rows_capacity=1,
        out_capacity=4,
        secure_local_mode=GPU_SEC_PERF_PROFILE_SECURE_LOCAL,
