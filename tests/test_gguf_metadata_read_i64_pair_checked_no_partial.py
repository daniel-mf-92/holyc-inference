#!/usr/bin/env python3
"""Reference checks for GGUFMetadataReadI64PairCheckedNoPartial semantics."""
...
def gguf_metadata_read_i64_pair_checked_no_partial(
    buf: list[int] | None,
    buf_nbytes: int,
    cursor_ref: list[int] | None,
    table_end: int,
...
def test_null_ptr_and_no_partial_write() -> None:
    ...
    assert (
        gguf_metadata_read_i64_pair_checked_no_partial(
            None,
            32,
            cursor,
            32,
...
def test_second_scalar_fail_does_not_commit_first_or_cursor() -> None:
    first_value = -1
    buf = _le_u64(_i64_to_u64_bits(first_value)) + [0xAA] * 7

    cursor = [0]
    out_first = [0x12345]
    out_second = [0x67890]

    err = gguf_metadata_read_i64_pair_checked_no_partial(
        buf,
        len(buf),
        cursor,
        len(buf),
    ...
    assert cursor == [0]
    assert out_first == [0x12345]
    assert out_second == [0x67890]


def test_success_reads_two_i64_and_advances() -> None:
    first = -(1 << 63)
    second = (1 << 63) - 1
    buf = [0xCC] + _le_u64(_i64_to_u64_bits(first)) + _le_u64(_i64_to_u64_bits(second)) + [0xDD]
...
def test_randomized_parity() -> None:
    rng = random.Random(20260417_227)
...
def run() -> None:
    ...
    test_success_reads_two_i64_and_advances()
    test_randomized_parity()
    print("gguf_metadata_read_i64_pair_checked_no_partial_reference_checks=ok")
