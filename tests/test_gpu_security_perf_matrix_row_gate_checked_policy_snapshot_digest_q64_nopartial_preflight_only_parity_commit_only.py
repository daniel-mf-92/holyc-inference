#!/usr/bin/env python3
"""Harness for IQ-1485 commit-only hardening over policy-snapshot preflight-only parity."""
...
def _row_gate_policy_snapshot_digest_q64_nopartial_preflight_only_parity(
    secure_local_mode: int,
    iommu_active: int,
    book_of_truth_gpu_hooks: int,
    policy_digest_parity: int,
...
def gpu_security_perf_matrix_row_gate_checked_policy_snapshot_digest_q64_nopartial_preflight_only_parity_commit_only(
    secure_local_mode: int,
    iommu_active: int,
    book_of_truth_gpu_hooks: int,
    policy_digest_parity: int,
    ...
    status_parity, parity_digest = _row_gate_policy_snapshot_digest_q64_nopartial_preflight_only_parity(
        secure_local_mode,
        iommu_active,
        book_of_truth_gpu_hooks,
        policy_digest_parity,
...
def test_source_contains_iq1485_symbols() -> None:
    src = Path("src/gpu/security_perf_matrix.HC").read_text(encoding="utf-8")

    assert "I32 GPUSecurityPerfMatrixRowGateCheckedPolicySnapshotDigestQ64NoPartialPreflightOnlyParityCommitOnly(" in src
...
def test_null_alias_overflow_vectors() -> None:
    status, digest = gpu_security_perf_matrix_row_gate_checked_policy_snapshot_digest_q64_nopartial_preflight_only_parity_commit_only(
        ...
        current_snapshot_digest_q64=5100,
        has_null_output=True,
    )
    assert (status, digest) == (GPU_SEC_PERF_ERR_NULL_PTR, 5100)

    # Single-output slot wrapper: alias-like behavior is represented by strict seed preservation on failure.
    status, digest = gpu_security_perf_matrix_row_gate_checked_policy_snapshot_digest_q64_nopartial_preflight_only_parity_commit_only(
        secure_local_mode=GPU_SEC_PERF_PROFILE_SECURE_LOCAL,
        iommu_active=1,
        book_of_truth_gpu_hooks=1,
        ...
        current_snapshot_digest_q64=6200,
    )
    assert (status, digest) == (GPU_SEC_PERF_ERR_BAD_PARAM, 6200)
    ...
    assert (status, digest) == (GPU_SEC_PERF_ERR_OVERFLOW, 7300)


def test_gate_missing_parity_vectors() -> None:
    vectors = [
        (0, 1, 1),
        (1, 0, 1),
        (1, 1, 0),
...
def test_bit_flip_sensitivity_vectors() -> None:
    base_canonical_status, base_canonical_digest = _row_gate_policy_snapshot_digest_q64(
        secure_local_mode=GPU_SEC_PERF_PROFILE_SECURE_LOCAL,
        iommu_active=1,
        book_of_truth_gpu_hooks=1,
        policy_digest_parity=1,
    ...
    prompt_canonical_status, prompt_canonical_digest = _row_gate_policy_snapshot_digest_q64(
        secure_local_mode=GPU_SEC_PERF_PROFILE_SECURE_LOCAL,
        iommu_active=1,
        book_of_truth_gpu_hooks=1,
        policy_digest_parity=1,
    ...
    batch_canonical_status, batch_canonical_digest = _row_gate_policy_snapshot_digest_q64(
        secure_local_mode=GPU_SEC_PERF_PROFILE_SECURE_LOCAL,
        iommu_active=1,
        book_of_truth_gpu_hooks=1,
        policy_digest_parity=1,
    ...
    assert base_canonical_status == GPU_SEC_PERF_OK
    assert prompt_canonical_status == GPU_SEC_PERF_OK
    assert batch_canonical_status == GPU_SEC_PERF_OK
    assert len({base_canonical_digest, prompt_canonical_digest, batch_canonical_digest}) == 3

    status_base, digest_base = gpu_security_perf_matrix_row_gate_checked_policy_snapshot_digest_q64_nopartial_preflight_only_parity_commit_only(
        secure_local_mode=GPU_SEC_PERF_PROFILE_SECURE_LOCAL,
        iommu_active=1,
        book_of_truth_gpu_hooks=1,
        policy_digest_parity=1,
    ...
    status_prompt, digest_prompt = gpu_security_perf_matrix_row_gate_checked_policy_snapshot_digest_q64_nopartial_preflight_only_parity_commit_only(
        secure_local_mode=GPU_SEC_PERF_PROFILE_SECURE_LOCAL,
        iommu_active=1,
        book_of_truth_gpu_hooks=1,
        policy_digest_parity=1,
    ...
    status_batch, digest_batch = gpu_security_perf_matrix_row_gate_checked_policy_snapshot_digest_q64_nopartial_preflight_only_parity_commit_only(
        secure_local_mode=GPU_SEC_PERF_PROFILE_SECURE_LOCAL,
        iommu_active=1,
        book_of_truth_gpu_hooks=1,
        policy_digest_parity=1,
    ...
    assert (digest_base, digest_prompt, digest_batch) == (0, 0, 0)


def test_digest_drift_rejected() -> None:
    status, digest = gpu_security_perf_matrix_row_gate_checked_policy_snapshot_digest_q64_nopartial_preflight_only_parity_commit_only(
        secure_local_mode=GPU_SEC_PERF_PROFILE_SECURE_LOCAL,
        iommu_active=1,
        book_of_truth_gpu_hooks=1,
...
if __name__ == "__main__":
    test_source_contains_iq1485_symbols()
    test_null_alias_overflow_vectors()
    test_gate_missing_parity_vectors()
