#!/usr/bin/env python3
"""Harness for IQ-1583 secure-local overhead budget gate commit-only wrapper."""
...
def _secure_local_budget_gate_commit_only(
    ...
    *,
    ...
    out_p50 = 0
    out_p95 = 0
    out_gate = GPU_SEC_PERF_BUDGET_GATE_STATUS_POLICY_BLOCK
    ...
    if not _status_is_valid(status_primary) or not _status_is_valid(status_parity):
        return GPU_SEC_PERF_ERR_BAD_PARAM, out_p50, out_p95, out_gate
    ...
    if status_primary != status_parity:
        return GPU_SEC_PERF_ERR_BAD_PARAM, out_p50, out_p95, out_gate
    if staged_p50 != parity_p50 or staged_p95 != parity_p95 or staged_gate != parity_gate:
        return GPU_SEC_PERF_ERR_BAD_PARAM, out_p50, out_p95, out_gate

    if status_primary != GPU_SEC_PERF_OK:
        return status_primary, out_p50, out_p95, out_gate

    out_p50 = staged_p50
    out_p95 = staged_p95
    out_gate = staged_gate
    return GPU_SEC_PERF_OK, out_p50, out_p95, out_gate


def test_source_contains_iq1583_symbols() -> None:
    src = Path("src/gpu/security_perf_matrix.HC").read_text(encoding="utf-8")

    assert "I32 GPUSecurityPerfMatrixSummaryQ16CheckedOverheadEnvelopeSecureLocalBudgetGateCommitOnly(" in src
    ...
    assert "if (!GPUSecurityPerfStatusIsValid(status_primary) ||" in src
...
def test_gate_missing_vectors_fail_closed() -> None:
    ...
    status, p50, p95, gate = _secure_local_budget_gate_commit_only(
        rows,
        rows_capacity=3,
...
def test_threshold_breach_vectors_fail_closed_publish() -> None:
    rows = [
...
def test_status_domain_drift_vector() -> None:
    rows = [
        RowOutput(100_000, 2_000, 40_000),
        RowOutput(100_000, 3_000, 42_000),
        RowOutput(100_000, 4_000, 44_000),
    ]
    ...
    status, p50, p95, gate = _secure_local_budget_gate_commit_only(
        ...
        book_of_truth_gpu_hooks=1,
        policy_digest_parity=1,
        max_p50_overhead_q16=4_000,
        max_p95_overhead_q16=5_000,
...
if __name__ == "__main__":
    test_source_contains_iq1583_symbols()
    test_gate_missing_vectors_fail_closed()
    test_threshold_breach_vectors_fail_closed_publish()
