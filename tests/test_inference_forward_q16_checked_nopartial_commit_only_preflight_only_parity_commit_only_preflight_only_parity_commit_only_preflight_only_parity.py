#!/usr/bin/env python3
"""Parity harness for InferenceForwardQ16...ParityCommitOnlyPreflightOnlyParityCommitOnlyPreflightOnlyParity (IQ-1063)."""
...
from test_inference_forward_q16_checked_nopartial_commit_only_preflight_only_parity_commit_only_preflight_only_parity_commit_only import (
    parity_commit_only_preflight_only_parity_commit_only_reference,
)
from test_inference_forward_q16_checked_nopartial_commit_only_preflight_only_parity_commit_only_preflight_only_parity_commit_only_preflight_only import (
    parity_commit_only_preflight_only_parity_commit_only_preflight_only_reference,
)


def parity_commit_only_preflight_only_parity_commit_only_preflight_only_parity_reference(
    *,
    embed_capacity: int,
    ...
    preflight_only_fn=parity_commit_only_preflight_only_parity_commit_only_preflight_only_reference,
    commit_only_fn=parity_commit_only_preflight_only_parity_commit_only_reference,
) -> int:
    if (
...
def test_source_contains_iq_1063_symbol() -> None:
    ...
    assert (
        "status =\n"
        "        InferenceForwardQ16CheckedNoPartialCommitOnlyPreflightOnlyParityCommitOnlyPreflightOnlyParityCommitOnlyPreflightOnly("
        in body
    )
    assert (
        "status =\n"
        "        InferenceForwardQ16CheckedNoPartialCommitOnlyPreflightOnlyParityCommitOnlyPreflightOnlyParityCommitOnly("
        in body
    )
...
def test_known_vector_success() -> None:
    ...
    status = parity_commit_only_preflight_only_parity_commit_only_preflight_only_parity_reference(
        embed_capacity=128,
...
def test_no_partial_publish_on_commit_mismatch() -> None:
    ...
    def _bad_commit_only(**kwargs: int) -> int:
        ...
        out_dense_cells[0] = 32
        out_required_workspace_cells[0] = 64
        out_required_block_tensor_cells[0] = 128
    ...
    assert out_dense == [701]
    assert out_workspace == [702]
    assert out_block == [703]
    assert out_logits == [704]


def test_randomized_geometry_capacity_overflow_vectors() -> None:
    rng = random.Random(20260422_1063)
...
if __name__ == "__main__":
    test_source_contains_iq_1063_symbol()
