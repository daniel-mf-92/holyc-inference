#!/usr/bin/env python3
"""Harness for IQ-962 commit-only wrapper over IQ-961 parity preflight."""
...
_PREV_PATH = Path(
    "tests/test_q8_0_dequantize_block_q16_checked_no_partial_array_commit_only_preflight_only_parity_commit_only_preflight_only_parity_commit_only_preflight_only_parity.py"
)

_spec_prev = importlib.util.spec_from_file_location("q8_iq962_prev", _PREV_PATH)
...
preflight_parity = (
    _prev.q8_0_dequantize_block_q16_checked_no_partial_array_commit_only_preflight_only_parity_commit_only_preflight_only_parity_commit_only_preflight_only_parity
)
...
def q8_0_dequantize_block_q16_checked_no_partial_array_commit_only_preflight_only_parity_commit_only_preflight_only_parity_commit_only_preflight_only_parity_commit_only(
    ...
    ok, expected_required_src_bytes = try_mul_i64_nonneg(staged_required_src_blocks[0], Q8_0_BLOCK_BYTES)
    if not ok:
        return Q8_0_ERR_OVERFLOW
    ...
    if (
        staged_required_src_blocks[0] != recomputed_required_src_blocks
...
def test_source_contains_iq962_commit_only_contract() -> None:
    source = Path("src/quant/q8_0.HC").read_text(encoding="utf-8")
    sig = "I32 Q8_0DequantizeBlockQ16CheckedNoPartialArrayCommitOnlyPreflightOnlyParityCommitOnlyPreflightOnlyParityCommitOnlyPreflightOnlyParityCommitOnly("
    assert source.count(sig) == 1
    body = source.split(sig, 1)[1]

    assert (
        "status = Q8_0DequantizeBlockQ16CheckedNoPartialArrayCommitOnlyPreflightOnlyParityCommitOnlyPreflightOnlyParityCommitOnlyPreflightOnlyParity("
        in body
    )
    assert "// Commit-only wrapper contract (IQ-962):" in body
...
def test_success_and_atomic_publish_no_write() -> None:
    rng = random.Random(962)
    src_blocks = [make_q8_block(rng) for _ in range(11)]
    dst_q16 = [818] * 600
    before = dst_q16[:]
...
def test_snapshot_mismatch_no_publish() -> None:
    ...
    src_blocks = [make_q8_block(rng) for _ in range(7)]
    dst_q16 = [919] * 320
...
def test_randomized_adversarial_vectors() -> None:
    ...
    for _ in range(220):
        ...
        dst_q16 = [717] * max(dst_capacity, 1)
...
def run() -> None:
    test_source_contains_iq962_commit_only_contract()
    test_success_and_atomic_publish_no_write()
    test_snapshot_mismatch_no_publish()
    test_randomized_adversarial_vectors()
    print("iq962_commit_only=ok")
