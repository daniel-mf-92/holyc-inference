#!/usr/bin/env python3
"""Reference checks for GGUFMetadataReadI32PairChecked semantics."""
...
def _le_i32(value: int) -> list[int]:
    return _le_u32(value & 0xFFFFFFFF)
...
def test_null_ptr_and_no_partial_write() -> None:
    ...
    assert (
        gguf_metadata_read_i32_pair_checked(
            None,
            16,
            cursor,
            16,
    ...
    assert cursor == [2]
    assert out_first == [-123456]
    assert out_second == [654321]
...
def test_second_scalar_fail_does_not_commit_first_or_cursor() -> None:
    first = -2147483648
    buf = _le_i32(first) + [0xAA, 0xBB, 0xCC]
...
def test_success_reads_two_i32_and_advances() -> None:
    first = -123456789
    second = 2147483647
    buf = [0xEE] + _le_i32(first) + _le_i32(second) + [0x77]
...
def test_randomized_parity() -> None:
    rng = random.Random(20260417_223)
    ...
    for _ in range(5000):
        ...
        if cursor0 + 8 <= table_end and cursor0 + 8 <= n:
            ...
            assert out_first[0] == _to_i32(first_u32)
            assert out_second[0] == _to_i32(second_u32)
            ...
            assert out_first[0] == 11111
            assert out_second[0] == 22222
...
def run() -> None:
    ...
    print("gguf_metadata_read_i32_pair_checked_reference_checks=ok")
