#!/usr/bin/env python3
"""Harness for IQ-1517 strict commit parity gate over row-gate digest wrappers."""
...
def gpu_security_perf_matrix_row_gate_checked_policy_snapshot_digest_q64_commit_only_preflight_only_parity_commit_only_parity(
    secure_local_mode: int,
    iommu_active: int,
    book_of_truth_gpu_hooks: int,
    policy_digest_parity: int,
    ...
    staged_digest = commit_only_digest
    if inject_digest_drift:
        staged_digest += 1

    status_parity, _ = _row_gate_policy_snapshot_digest_q64_commit_only_preflight_only_parity(
        secure_local_mode,
        iommu_active,
        book_of_truth_gpu_hooks,
        policy_digest_parity,
    ...
    if status_commit_only != GPU_SEC_PERF_OK:
        return status_commit_only, current_snapshot_digest_q64
    return GPU_SEC_PERF_OK, commit_only_digest


def test_source_contains_iq1517_symbols() -> None:
    src = Path("src/gpu/security_perf_matrix.HC").read_text(encoding="utf-8")

    assert "I32 GPUSecurityPerfMatrixRowGateCheckedPolicySnapshotDigestQ64CommitOnlyPreflightOnlyParityCommitOnlyParity(" in src
...
def test_secure_on_perf_overhead_measurement_plan_vectors() -> None:
    # IQ-1517 secure-on perf-overhead measurement plan:
    ...
    for prompt_tokens, batch_size, quant_profile in plan_vectors:
        status, digest = gpu_security_perf_matrix_row_gate_checked_policy_snapshot_digest_q64_commit_only_preflight_only_parity_commit_only_parity(
...
def test_digest_bit_flip_sensitivity() -> None:
    ...
    status_base, digest_base = gpu_security_perf_matrix_row_gate_checked_policy_snapshot_digest_q64_commit_only_preflight_only_parity_commit_only_parity(
        secure_local_mode=GPU_SEC_PERF_PROFILE_SECURE_LOCAL,
        iommu_active=1,
        book_of_truth_gpu_hooks=1,
        policy_digest_parity=1,
    ...
    status_prompt, digest_prompt = gpu_security_perf_matrix_row_gate_checked_policy_snapshot_digest_q64_commit_only_preflight_only_parity_commit_only_parity(
        secure_local_mode=GPU_SEC_PERF_PROFILE_SECURE_LOCAL,
        iommu_active=1,
        book_of_truth_gpu_hooks=1,
        policy_digest_parity=1,
...
if __name__ == "__main__":
    test_source_contains_iq1517_symbols()
