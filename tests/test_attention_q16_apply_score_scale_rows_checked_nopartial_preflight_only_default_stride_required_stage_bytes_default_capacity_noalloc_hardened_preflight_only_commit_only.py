#!/usr/bin/env python3
"""Parity harness for ...NoAllocHardenedPreflightOnlyCommitOnly (IQ-723)."""
...
def attention_q16_apply_score_scale_rows_checked_nopartial_preflight_only_default_stride_required_stage_bytes_default_capacity_noalloc_hardened_preflight_only_commit_only(
    in_scores_q32,
    ...
    if canonical_required_in_cells[0] != parity_required_in_cells[0]:
        return ATTN_Q16_ERR_BAD_PARAM
    if canonical_required_out_cells[0] != parity_required_out_cells[0]:
        return ATTN_Q16_ERR_BAD_PARAM
    if canonical_required_stage_cells[0] != parity_required_stage_cells[0]:
        return ATTN_Q16_ERR_BAD_PARAM
    if canonical_required_stage_bytes[0] != parity_required_stage_bytes[0]:
        return ATTN_Q16_ERR_BAD_PARAM
    if canonical_last_in_index[0] != parity_last_in_index[0]:
        return ATTN_Q16_ERR_BAD_PARAM
    if canonical_last_out_index[0] != parity_last_out_index[0]:
        return ATTN_Q16_ERR_BAD_PARAM
...
def test_source_contains_hardened_preflight_only_commit_only() -> None:
    ...
    assert (
        ...
        "RequiredStageBytesDefaultCapacityNoAllocHardenedPreflightOnly("
    ) in source
    assert (
        "AttentionQ16ApplyScoreScaleRowsCheckedNoPartialPreflightOnlyDefaultStride"
        "RequiredStageBytesDefaultCapacityNoAllocHardened("
    ) in source
    assert "snapshot_in_scores_capacity" in source
    assert "canonical_required_stage_bytes" in source
    assert "parity_required_stage_bytes" in source


def test_preflight_only_commit_only_nullptr_and_alias_contracts() -> None:
    out_a = [111]
    out_b = [222]
    out_c = [333]
    out_d = [444]
...
def test_preflight_only_commit_only_overflow_propagation() -> None:
...
def test_preflight_only_commit_only_matches_explicit_staged_composition_randomized() -> None:
    rng = random.Random(20260420_723)
    ...
    for _ in range(1000):
        ...
        err_a = attention_q16_apply_score_scale_rows_checked_nopartial_preflight_only_default_stride_required_stage_bytes_default_capacity_noalloc_hardened_preflight_only_commit_only(
            in_scores,
            in_scores_capacity,
            row_count,
            token_count,
...
if __name__ == "__main__":
    test_source_contains_hardened_preflight_only_commit_only()
    test_preflight_only_commit_only_nullptr_and_alias_contracts()
    test_preflight_only_commit_only_overflow_propagation()
    test_preflight_only_commit_only_matches_explicit_staged_composition_randomized()
    print(
