#!/usr/bin/env python3
"""Host-side harness for IQ-1337 PrefixCacheReplayGuardCheckedNoPartialCommitOnlyPreflightOnlyParityCommitOnlyPreflightOnlyParity."""
...
def replay_guard_commit_only_preflight_only_parity_commit_only_preflight_only(
    cache: PrefixCache | None,
    entry_index: int,
    access_tick: int,
    profile_id: int,
...
def replay_guard_commit_only_preflight_only_parity_commit_only_preflight_only_parity(
    cache: PrefixCache | None,
    entry_index: int,
    access_tick: int,
    profile_id: int,
    ...
    preflight_out = [0, 0, 0]
    status = replay_guard_commit_only_preflight_only_parity_commit_only_preflight_only(
        cache,
        entry_index,
        access_tick,
        profile_id,
...
def test_parity_commit_only_preflight_only_parity_secure_local_monotonic_roundtrip():
    cache = _cache_with_tick(700)
    out = [-1, -1, -1]
    status = replay_guard_commit_only_preflight_only_parity_commit_only_preflight_only_parity(
        cache,
        entry_index=1,
        access_tick=701,
...
def test_parity_commit_only_preflight_only_parity_dev_local_allows_rollback():
...
def test_parity_commit_only_preflight_only_parity_secure_local_policy_failure_keeps_output_slots_unchanged():
...
def test_parity_commit_only_preflight_only_parity_rejects_bad_vectors():
    cache = _cache_with_tick(3)
    out = [0, 0, 0]

    assert (
        replay_guard_commit_only_preflight_only_parity_commit_only_preflight_only_parity(
            None,
            1,
            2,
...
def test_holyc_function_body_and_contract_markers_present():
    ...
    sig = "I32 PrefixCacheReplayGuardCheckedNoPartialCommitOnlyPreflightOnlyParityCommitOnlyPreflightOnlyParity(PrefixCache *cache,"
    assert sig in source
    body = source.split(sig, 1)[1]
    assert "status_preflight = PrefixCacheReplayGuardCheckedNoPartialCommitOnlyPreflightOnlyParityCommitOnlyPreflightOnly(cache," in body
    assert "cache->entries[entry_index].last_used_tick = snapshot_previous_tick;" in body
    assert "status_commit = PrefixCacheReplayGuardCheckedNoPartialCommitOnlyPreflightOnlyParityCommitOnly(cache," in body
    assert "if (preflight_previous_tick != commit_previous_tick ||" in body
    assert "*out_previous_tick = preflight_previous_tick;" in body
    assert "*out_guard_passed = preflight_guard_passed;" in body


if __name__ == "__main__":
    test_parity_commit_only_preflight_only_parity_secure_local_monotonic_roundtrip()
    test_parity_commit_only_preflight_only_parity_dev_local_allows_rollback()
    test_parity_commit_only_preflight_only_parity_secure_local_policy_failure_keeps_output_slots_unchanged()
    test_parity_commit_only_preflight_only_parity_rejects_bad_vectors()
