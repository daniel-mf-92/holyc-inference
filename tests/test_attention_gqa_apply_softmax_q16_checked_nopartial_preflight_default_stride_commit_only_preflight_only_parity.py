#!/usr/bin/env python3
"""Reference checks for GQAAttentionApplySoftmaxQ16CheckedNoPartialPreflightDefaultStrideCommitOnlyPreflightOnlyParity (IQ-1423)."""
...
def gqa_attention_apply_softmax_q16_checked_nopartial_preflight_default_stride_commit_only_preflight_only_parity(
    scores_q32,
    scores_capacity: int,
    query_rows: int,
    key_rows: int,
...
def test_randomized_zero_write_success() -> None:
    rng = random.Random(20260425_1423)
...
def test_source_contract_markers() -> None:
    ...
    assert (
        "status = GQAAttentionApplySoftmaxQ16CheckedNoPartialPreflightDefaultStrideCommitOnlyPreflightOnly(" in body
    )
...
if __name__ == "__main__":
    ...
    print(
        "gqa_attention_apply_softmax_q16_checked_nopartial_preflight_default_stride_commit_only_preflight_only_parity_reference_checks=ok"
    )
