#!/usr/bin/env python3
"""Reference checks for GGUFMetadataReadU32TripleChecked semantics."""
...
def test_null_ptr_and_no_partial_write() -> None:
    ...
    assert (
        gguf_metadata_read_u32_triple_checked(
            None,
            16,
            cursor,
            16,
...
def test_random_reference_parity() -> None:
    rng = random.Random(0x9234_1234)

    for _ in range(300):
        first = rng.randrange(0, 1 << 32)
        second = rng.randrange(0, 1 << 32)
        third = rng.randrange(0, 1 << 32)

        prefix = [rng.randint(0, 255) for _ in range(rng.randint(0, 6))]
        suffix = [rng.randint(0, 255) for _ in range(rng.randint(0, 6))]
        buf = prefix + _le_u32(first) + _le_u32(second) + _le_u32(third) + suffix
...
if __name__ == "__main__":
    ...
    test_success_reads_three_u32_and_advances()
