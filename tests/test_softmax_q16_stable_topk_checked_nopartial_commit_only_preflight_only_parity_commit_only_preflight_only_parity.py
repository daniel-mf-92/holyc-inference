#!/usr/bin/env python3
"""Harness for SoftmaxQ16StableTopKCheckedNoPartialCommitOnlyPreflightOnlyParityCommitOnlyPreflightOnlyParity (IQ-1087)."""
...
from test_softmax_q16_stable_topk_checked_nopartial_commit_only_preflight_only_parity_commit_only_preflight_only import (
    softmax_q16_stable_topk_checked_nopartial_commit_only_preflight_only_parity_commit_only_preflight_only,
)


def softmax_q16_stable_topk_checked_nopartial_commit_only_preflight_only_parity_commit_only_preflight_only_parity(
    ...
    staged_fn=softmax_q16_stable_topk_checked_nopartial_commit_only_preflight_only_parity_commit_only_preflight_only,
    canonical_fn=softmax_q16_stable_topk_checked_nopartial_commit_only_preflight_only_parity,
) -> int:
    if (
        logits_q16 is None
...
def test_source_contains_signature_and_composition_chain() -> None:
    ...
    sig = "I32 SoftmaxQ16StableTopKCheckedNoPartialCommitOnlyPreflightOnlyParityCommitOnlyPreflightOnlyParity("
    assert source.count(sig) == 1
    body = source.split(sig, 1)[1].split(
        "I32 FPQ16TopKSelectLogitsCheckedNoAlias(",
        1,
    )[0]
    assert "SoftmaxQ16StableTopKCheckedNoPartialCommitOnlyPreflightOnlyParityCommitOnlyPreflightOnly(" in body
...
def test_known_vector() -> None:
    ...
    err = softmax_q16_stable_topk_checked_nopartial_commit_only_preflight_only_parity_commit_only_preflight_only_parity(
        logits,
        logits_count=4,
        top_k=2,
...
def test_fuzz_parity_vs_components() -> None:
    rng = random.Random(20260422_1087)
