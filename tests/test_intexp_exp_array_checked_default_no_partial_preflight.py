#!/usr/bin/env python3
"""Parity checks for FPQ16ExpArrayCheckedDefaultNoPartialPreflight."""
...
def fpq16_exp_array_checked_default_no_partial_preflight(
...
def test_source_contains_preflight_shape_and_wrapper_delegation() -> None:
    ...
    assert "FPQ16ExpArrayCheckedDefaultNoPartialPreflight" in source
    assert "if (!out_staging_bytes)" in source
    assert "status = FPQ16ExpArrayCheckedDefaultNoPartialPreflight(input_q16," in source


def test_null_bad_count_and_zero_surfaces() -> None:
    assert fpq16_exp_array_checked_default_no_partial_preflight(None, [0], 1)[0] == FP_Q16_ERR_NULL_PTR
    assert fpq16_exp_array_checked_default_no_partial_preflight([0], None, 1)[0] == FP_Q16_ERR_NULL_PTR
    assert fpq16_exp_array_checked_default_no_partial_preflight([0], [0], -1)[0] == FP_Q16_ERR_BAD_PARAM

    err, staging = fpq16_exp_array_checked_default_no_partial_preflight([1, 2], [0, 0], 0)
    assert err == FP_Q16_OK
    assert staging == 0

    err, staging = fpq16_exp_array_checked_default_no_partial_preflight(
        [1],
        [0],
        1,
...
def test_overflow_and_pointer_guard_parity() -> None:
    ...
    got = fpq16_exp_array_checked_default_no_partial_preflight(inp, out, huge_count)
    ref = inline_wrapper_preflight_equivalent(inp, out, huge_count)
    assert got == ref == (FP_Q16_ERR_OVERFLOW, 0)

    near_end = U64_MAX_VALUE - ((len(inp) - 1) << 3) + 1
    got = fpq16_exp_array_checked_default_no_partial_preflight(
        inp,
        out,
        len(inp),
...
def test_boundary_and_randomized_parity_vs_inline_preflight() -> None:
    for count in [1, 2, 31, 32, 33, 127, 256, 1024]:
        inp = [0] * count
        out = [0] * count
        got = fpq16_exp_array_checked_default_no_partial_preflight(inp, out, count)
        ref = inline_wrapper_preflight_equivalent(inp, out, count)
        assert got == ref
        assert got[0] == FP_Q16_OK
        assert got[1] == count << 3

    rng = random.Random(20260419_516)
    for _ in range(2000):
        count = rng.randint(-8, 1024)
        valid_count = max(0, count)
        inp = [rng.randint(-(1 << 20), (1 << 20)) for _ in range(valid_count)]
        out = [0x66] * valid_count

        in_addr = rng.randrange(0, 1 << 16)
        out_addr = rng.randrange(0, 1 << 16)

        got = fpq16_exp_array_checked_default_no_partial_preflight(
            inp if count >= 0 else inp,
            out if count >= 0 else out,
            count,
...
def run() -> None:
    ...
    print("intexp_exp_array_checked_default_no_partial_preflight=ok")
