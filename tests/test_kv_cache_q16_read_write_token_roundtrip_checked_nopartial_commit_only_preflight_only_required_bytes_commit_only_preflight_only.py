#!/usr/bin/env python3
"""Parity harness for ...RoundTrip...RequiredBytesCommitOnlyPreflightOnly (IQ-920)."""
...
def kv_cache_q16_read_write_token_roundtrip_checked_nopartial_commit_only_preflight_only_required_bytes_commit_only_preflight_only(
    ...
    if (
        staged_commit_span[0] != staged_canonical_span[0]
        or staged_commit_bytes[0] != staged_canonical_bytes[0]
        or staged_commit_k_base[0] != staged_canonical_k_base[0]
        or staged_commit_v_base[0] != staged_canonical_v_base[0]
    ):
        return KV_Q16_ERR_BAD_PARAM
    ...
    out_required_span_cells[0] = staged_commit_span[0]
    out_required_token_bytes[0] = staged_commit_bytes[0]
    out_k_base_index[0] = staged_commit_k_base[0]
    out_v_base_index[0] = staged_commit_v_base[0]
    return KV_Q16_OK
...
def explicit_preflight_only_composition(*args, **kwargs) -> tuple[int, tuple[int, int, int, int]]:
    ...
    if (
        commit_span[0] != canonical_span[0]
        or commit_bytes[0] != canonical_bytes[0]
        or commit_k_base[0] != canonical_k_base[0]
        or commit_v_base[0] != canonical_v_base[0]
    ):
        return KV_Q16_ERR_BAD_PARAM, (0, 0, 0, 0)

    return KV_Q16_OK, (commit_span[0], commit_bytes[0], commit_k_base[0], commit_v_base[0])


def test_source_contains_iq920_function() -> None:
    source = Path("src/model/kv_cache.HC").read_text(encoding="utf-8")
    sig = "I32 KVCacheQ16ReadWriteTokenRoundTripCheckedNoPartialCommitOnlyPreflightOnlyRequiredBytesCommitOnlyPreflightOnly("
...
def test_randomized_parity() -> None:
    rng = random.Random(20260421_920)
    ...
    for _ in range(2000):
        ...
        err_impl = kv_cache_q16_read_write_token_roundtrip_checked_nopartial_commit_only_preflight_only_required_bytes_commit_only_preflight_only(
            *args,
            out_span,
            out_bytes,
            out_k,
...
if __name__ == "__main__":
    test_source_contains_iq920_function()
