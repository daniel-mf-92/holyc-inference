#!/usr/bin/env python3
"""Host-side harness for IQ-1350 PrefixCacheReplayGuardCheckedNoPartialCommitOnlyPreflightOnlyParityCommitOnlyPreflightOnlyParityCommitOnlyPreflightOnly."""
...
def replay_guard_commit_only_preflight_only_parity_commit_only_preflight_only_parity_commit_only_preflight_only(
    cache,
    entry_index: int,
    access_tick: int,
    profile_id: int,
    ...
    snapshot_out_previous = outputs[slot_previous]
    snapshot_out_applied = outputs[slot_applied]
    snapshot_out_guard = outputs[slot_guard]
    ...
    if (
        ...
        return PREFIX_CACHE_ERR_BAD_PARAM
    if outputs[slot_previous] != snapshot_out_previous:
        return PREFIX_CACHE_ERR_BAD_PARAM
    if outputs[slot_applied] != snapshot_out_applied:
        return PREFIX_CACHE_ERR_BAD_PARAM
    if outputs[slot_guard] != snapshot_out_guard:
        return PREFIX_CACHE_ERR_BAD_PARAM
    ...
    if (
        snapshot_capacity != cache.capacity
        or snapshot_count != cache.count
        or snapshot_entry_index != entry_index
        or snapshot_access_tick != access_tick
        or snapshot_profile_id != profile_id
        or snapshot_out_previous != outputs[slot_previous]
        or snapshot_out_applied != outputs[slot_applied]
        or snapshot_out_guard != outputs[slot_guard]
    ):
        return PREFIX_CACHE_ERR_BAD_PARAM
...
def test_zero_write_preflight_only_dev_local_allows_rollback():
...
def test_zero_write_preflight_only_secure_local_policy_failure_keeps_outputs_unchanged():
...
def test_zero_write_preflight_only_rejects_bad_vectors():
...
def test_holyc_function_body_and_zero_write_contract_markers_present():
    ...
    assert "*out_previous_tick =" not in body
    assert "*out_applied_tick =" not in body
    assert "*out_guard_passed =" not in body


if __name__ == "__main__":
    test_zero_write_preflight_only_secure_local_monotonic_roundtrip()
    test_zero_write_preflight_only_dev_local_allows_rollback()
    test_zero_write_preflight_only_secure_local_policy_failure_keeps_outputs_unchanged()
    test_zero_write_preflight_only_rejects_bad_vectors()
    test_holyc_function_body_and_zero_write_contract_markers_present()
    print("ok")
