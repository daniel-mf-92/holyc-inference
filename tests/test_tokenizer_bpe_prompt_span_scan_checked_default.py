#!/usr/bin/env python3
"""Parity harness for TokenizerBPEPromptSpanScanCheckedDefault."""
...
def tokenizer_bpe_prompt_span_scan_checked_default(
    data: list[int] | None,
    byte_len: int,
...
def run_parity_case(payload: list[int], cursor_start: int, prompt_nbytes: int) -> None:
    ...
    err_def = tokenizer_bpe_prompt_span_scan_checked_default(
        payload,
        len(payload),
        cursor_def,
        prompt_nbytes,
...
def test_zero_length_prompt_parity() -> None:
    payload = list(b"abc")
    run_parity_case(payload, 1, 0)


def test_default_wrapper_guard_parity() -> None:
    starts = [0xAAAA] * 8
    lengths = [0xBBBB] * 8
    classes = [0xCC] * 8
    count = [0xDDDD]
    ...
    cursor = [4]
    err = tokenizer_bpe_prompt_span_scan_checked_default(
        [ord("a"), ord("b"), ord("c")],
        3,
        cursor,
        0,
...
def test_randomized_parity_against_explicit_capacity_core() -> None:
    rng = random.Random(20260418_403)
...
if __name__ == "__main__":
    ...
    test_default_wrapper_guard_parity()
    test_randomized_parity_against_explicit_capacity_core()
    print("test_tokenizer_bpe_prompt_span_scan_checked_default: ok")
