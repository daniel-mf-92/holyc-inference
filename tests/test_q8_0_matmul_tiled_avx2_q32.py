...
def q8_0_matmul_tiled_avx2_q32_checked(
    ...
    while tile_row_start < lhs_rows:
        err, tile_row_end = compute_span_bounds_checked(tile_row_start, tile_rows, lhs_rows)
        if err != Q8_0_AVX2_OK:
            return err, []

        tile_col_start = 0
        while tile_col_start < rhs_cols:
            err, tile_col_end = compute_span_bounds_checked(tile_col_start, tile_cols, rhs_cols)
            if err != Q8_0_AVX2_OK:
                return err, []
...
def test_compute_span_bounds_checked_contract() -> None:
    err, tile_end = compute_span_bounds_checked(1, 4, 9)
    assert err == Q8_0_AVX2_OK
    assert tile_end == 5

    err, tile_end = compute_span_bounds_checked(8, 4, 9)
    assert err == Q8_0_AVX2_OK
    assert tile_end == 9

    err, _ = compute_span_bounds_checked(-1, 2, 9)
    assert err == Q8_0_AVX2_ERR_BAD_LEN
    err, _ = compute_span_bounds_checked(1, -2, 9)
    assert err == Q8_0_AVX2_ERR_BAD_LEN
    err, _ = compute_span_bounds_checked(1, 2, -9)
    ...
    err, _ = compute_span_bounds_checked(I64_MAX - 2, 5, I64_MAX)
