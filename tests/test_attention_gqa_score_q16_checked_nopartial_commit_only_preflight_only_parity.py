#!/usr/bin/env python3
"""Reference checks for GQAAttentionScoreQ16CheckedNoPartialCommitOnlyPreflightOnlyParity (IQ-1375)."""
...
from test_attention_gqa_score_q16_checked_nopartial_commit_only_preflight_only import (
    gqa_attention_score_q16_checked_nopartial_commit_only_preflight_only,
)

I64_MAX = (1 << 63) - 1


def gqa_attention_score_q16_checked_nopartial_commit_only_preflight_only_parity(
    q_rows_q16,
    q_rows_capacity: int,
    q_rows: int,
    k_rows_q16,
    ...
    staged_commit_scores = [0] * max(1, required_out_cells)
    staged_pre_q = [required_q_cells]
    staged_pre_k = [required_k_cells]
    staged_pre_out = [required_out_cells]
    ...
    err = gqa_attention_score_q16_checked_nopartial_commit_only(
        ...
        seq_len,
        head_dim,
        staged_commit_scores if required_out_cells else out_scores_q32,
        required_out_cells if required_out_cells else out_capacity,
    )
    if err != ATTN_Q16_OK:
        return err
    ...
    if (
        staged_pre_q[0] != required_q_cells
        or staged_pre_k[0] != required_k_cells
        or staged_pre_out[0] != required_out_cells
    ):
        return ATTN_Q16_ERR_BAD_PARAM
    ...
    out_required_q_cells[0] = staged_pre_q[0]
    out_required_k_cells[0] = staged_pre_k[0]
    out_required_out_cells[0] = staged_pre_out[0]
    return ATTN_Q16_OK


def test_fixed_vector_reference_publishes_required_tuple() -> None:
    q_rows = 4
    k_rows = 2
    group_count = 2
    seq_len = 3
...
def test_randomized_parity_success() -> None:
    rng = random.Random(20260425_1375)
...
def test_source_contract_markers() -> None:
    ...
    sig = "I32 GQAAttentionScoreQ16CheckedNoPartialCommitOnlyPreflightOnlyParity("
    assert sig in source
    body = source.split(sig, 1)[1]
    assert "status = GQAAttentionScoreQ16CheckedNoPartialCommitOnlyPreflightOnly(" in body
    ...
    assert "*out_required_q_cells = staged_preflight_required_q_cells;" in body


if __name__ == "__main__":
    test_fixed_vector_reference_publishes_required_tuple()
    test_error_contract_preserves_outputs()
    test_randomized_parity_success()
    test_source_contract_markers()
    print("gqa_attention_score_q16_checked_nopartial_commit_only_preflight_only_parity_reference_checks=ok")
