#!/usr/bin/env python3
"""Parity harness for ...PreflightOnlyCommitOnlyParityCommitOnly (IQ-850)."""
...
from test_inference_generate_tokens_checked_topk_default_preflight_only_commit_only_parity import (
    inference_generate_tokens_checked_topk_default_preflight_only_commit_only_parity_reference,
)


def inference_generate_tokens_checked_topk_default_preflight_only_commit_only_parity_commit_only_reference(
    *,
    ...
    staged_required_step_logits_cells = [out_required_step_logits_cells[0]]
    staged_required_generated_capacity = [out_required_generated_capacity[0]]
    staged_required_random_capacity = [out_required_random_capacity[0]]
    ...
    status = inference_generate_tokens_checked_topk_default_preflight_only_commit_only_parity_reference(
        ...
        out_required_generated_capacity=staged_required_generated_capacity,
        out_required_random_capacity=staged_required_random_capacity,
    )
    ...
    out_required_step_logits_cells[0] = staged_required_step_logits_cells[0]
    out_required_generated_capacity[0] = staged_required_generated_capacity[0]
    out_required_random_capacity[0] = staged_required_random_capacity[0]
    return SAMPLING_Q16_OK


def explicit_commit_only_composition(*, out_required_step_logits_cells, out_required_generated_capacity, out_required_random_capacity, **kwargs) -> int:
    staged_required_step_logits_cells = [out_required_step_logits_cells[0]]
    staged_required_generated_capacity = [out_required_generated_capacity[0]]
    staged_required_random_capacity = [out_required_random_capacity[0]]
...
def test_source_contains_commit_only_parity_commit_only_helper() -> None:
    ...
    sig = "I32 InferenceGenerateTokensCheckedTopKDefaultPreflightOnlyCommitOnlyParityCommitOnly("
    assert sig in source
    body = source.split(sig, 1)[1]

    assert "InferenceGenerateTokensCheckedTopKDefaultPreflightOnlyCommitOnlyParity(" in body
    assert "snapshot_vocab_size = vocab_size;" in body
    assert "snapshot_max_new_tokens = max_new_tokens;" in body
    assert "snapshot_workspace_topk_index_capacity = workspace_topk_index_capacity;" in body
    assert "*out_required_step_logits_cells = staged_required_step_logits_cells;" in body
    assert "*out_required_generated_capacity = staged_required_generated_capacity;" in body
    assert "*out_required_random_capacity = staged_required_random_capacity;" in body
...
def test_randomized_commit_only_parity_vs_explicit_staged_composition() -> None:
    rng = random.Random(20260421_850)
    ...
    for _ in range(900):
        ...
        status_a = inference_generate_tokens_checked_topk_default_preflight_only_commit_only_parity_commit_only_reference(
        ...
        status_b = explicit_commit_only_composition(
...
if __name__ == "__main__":
    ...
    test_randomized_commit_only_parity_vs_explicit_staged_composition()
    print("ok")
