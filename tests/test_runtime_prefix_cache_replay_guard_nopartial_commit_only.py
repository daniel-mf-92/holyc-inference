#!/usr/bin/env python3
"""Host-side harness for IQ-1320 PrefixCacheReplayGuardCheckedNoPartialCommitOnly."""
...
def test_commit_only_secure_local_monotonic_roundtrip():
    cache = _cache_with_tick(250)
    out = [-7, -7, -7]
    ...
    assert out == [250, 251, 1]
    assert cache.entries[1].last_used_tick == 251


def test_commit_only_dev_local_allows_rollback():
    cache = _cache_with_tick(800)
    out = [11, 12, 13]
    status = replay_guard_commit_only(
        cache,
        entry_index=1,
    ...
    assert status == PREFIX_CACHE_OK
    assert out == [800, 600, 1]
    assert cache.entries[1].last_used_tick == 600


def test_commit_only_secure_local_policy_failure_keeps_output_slots_unchanged():
    cache = _cache_with_tick(500)
    out = [101, 102, 103]
    status = replay_guard_commit_only(
        cache,
        entry_index=1,
    ...
    assert cache.entries[1].last_used_tick == 500


def test_commit_only_rejects_bad_vectors():
    cache = _cache_with_tick(1)
    out = [0, 0, 0]

    assert (
        replay_guard_commit_only(None, 1, 1, PREFIX_CACHE_PROFILE_SECURE_LOCAL, out, 0, 1, 2)
        == PREFIX_CACHE_ERR_NULL_PTR
    )
    assert (
        replay_guard_commit_only(cache, 1, 1, PREFIX_CACHE_PROFILE_SECURE_LOCAL, None, 0, 1, 2)
        == PREFIX_CACHE_ERR_NULL_PTR
    )
    assert (
        replay_guard_commit_only(cache, 1, 1, PREFIX_CACHE_PROFILE_SECURE_LOCAL, out, 0, 0, 2)
        == PREFIX_CACHE_ERR_BAD_PARAM
    )
    assert (
        replay_guard_commit_only(cache, -1, 1, PREFIX_CACHE_PROFILE_SECURE_LOCAL, out, 0, 1, 2)
        == PREFIX_CACHE_ERR_BAD_PARAM
    )
    assert (
        replay_guard_commit_only(cache, 1, -1, PREFIX_CACHE_PROFILE_SECURE_LOCAL, out, 0, 1, 2)
    ...
    assert replay_guard_commit_only(cache, 1, 1, 999, out, 0, 1, 2) == PREFIX_CACHE_ERR_BAD_PARAM
...
def test_holyc_function_body_and_contract_markers_present():
    ...
    sig = "I32 PrefixCacheReplayGuardCheckedNoPartialCommitOnly(PrefixCache *cache,"
    assert sig in source
    body = source.split(sig, 1)[1]
    assert "status = PrefixCacheReplayGuardChecked(cache," in body
    assert "if (cache->count != snapshot_count ||" in body
    assert "if (staged_applied_tick != access_tick)" in body
    assert "if (cache->entries[entry_index].last_used_tick != staged_applied_tick)" in body
    assert "*out_previous_tick = staged_previous_tick;" in body
    assert "*out_guard_passed = staged_guard_passed;" in body


if __name__ == "__main__":
    test_commit_only_secure_local_monotonic_roundtrip()
    test_commit_only_dev_local_allows_rollback()
    test_commit_only_secure_local_policy_failure_keeps_output_slots_unchanged()
    test_commit_only_rejects_bad_vectors()
