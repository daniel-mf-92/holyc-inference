#!/usr/bin/env python3
"""Harness for IQ-1553 preflight-only digest-locked overhead-envelope diagnostics companion."""
...
def gpu_security_perf_matrix_summary_q16_checked_overhead_envelope_checked_nopartial_commit_only_preflight_only_parity_commit_only_preflight_only_digest_locked_parity_commit_only_preflight_only(
    ...
    status_commit_only, staged_tuple = _overhead_envelope_preflight_only_parity_commit_only_preflight_only_digest_locked_parity_commit_only(
        rows,
        rows_capacity,
        out_capacity,
        secure_local_mode,
    ...
    status_canonical, preserved_outputs, canonical_tuple = _overhead_envelope_preflight_only(
        rows,
        rows_capacity,
        out_capacity,
        secure_local_mode,
...
def test_source_contains_iq1553_symbols() -> None:
    src = Path("src/gpu/security_perf_matrix.HC").read_text(encoding="utf-8")
    assert (
        "I32 GPUSecurityPerfMatrixSummaryQ16CheckedOverheadEnvelopeCheckedNoPartialCommitOnlyPreflightOnlyParityCommitOnlyPreflightOnlyDigestLockedParityCommitOnlyPreflightOnly("
        in src
    )
    assert (
        "status_commit_only = GPUSecurityPerfMatrixSummaryQ16CheckedOverheadEnvelopeCheckedNoPartialCommitOnlyPreflightOnlyParityCommitOnlyPreflightOnlyDigestLockedParityCommitOnly("
        in src
    )
    assert (
        "status_canonical = GPUSecurityPerfMatrixSummaryQ16CheckedOverheadEnvelopeCheckedNoPartialCommitOnlyPreflightOnlyParityCommitOnlyPreflightOnlyDigestLockedParity("
        in src
    )
...
def test_secure_on_success_atomic_publish() -> None:
    ...
    status, preserved, published = gpu_security_perf_matrix_summary_q16_checked_overhead_envelope_checked_nopartial_commit_only_preflight_only_parity_commit_only_preflight_only_digest_locked_parity_commit_only_preflight_only(
        rows,
        rows_capacity=3,
        out_capacity=3,
...
def test_null_alias_capacity_gate_and_monotonicity_vectors() -> None:
    ...
    status, _, _ = gpu_security_perf_matrix_summary_q16_checked_overhead_envelope_checked_nopartial_commit_only_preflight_only_parity_commit_only_preflight_only_digest_locked_parity_commit_only_preflight_only(
        rows,
        rows_capacity=1,
        out_capacity=3,
        secure_local_mode=GPU_SEC_PERF_PROFILE_SECURE_LOCAL,
