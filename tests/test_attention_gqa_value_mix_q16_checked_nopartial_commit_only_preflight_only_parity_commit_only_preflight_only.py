...
def gqa_attention_value_mix_q16_checked_nopartial_commit_only_preflight_only_parity_commit_only_preflight_only(
    scores_q16,
    scores_capacity: int,
    query_rows: int,
    key_rows: int,
...
def explicit_preflight_only_composition(*args, **kwargs) -> int:
    return gqa_attention_value_mix_q16_checked_nopartial_commit_only_preflight_only_parity_commit_only_preflight_only(
        *args, **kwargs
    )
...
def test_fixed_vector_reference() -> None:
    ...
    seed = [5252] * (query_rows * value_dim)
    out_a = seed.copy()
    out_b = seed.copy()

    err_a = gqa_attention_value_mix_q16_checked_nopartial_commit_only_preflight_only_parity_commit_only_preflight_only(
        scores_q16,
        len(scores_q16),
        query_rows,
        key_rows,
