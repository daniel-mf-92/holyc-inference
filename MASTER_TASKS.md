...
- [ ] IQ-071 Implement HolyC helper `GGUFTensorLookupSpanByIndexAndRelRangeDefault` in `src/gguf/tensor_data_base.HC` as a `GGUF_DEFAULT_ALIGNMENT` wrapper over index+relative range span lookup, with host parity tests in `tests/test_gguf_tensor_data_base.py` (WS2-04)
