...
- [ ] IQ-126 Implement HolyC helper `Q8_0MatMulTiledComputeRhsColBaseChecked` in `src/matmul/q8_0_matmul.HC` to centralize checked `col_index*rhs_col_stride_blocks` math across scalar/AVX2 tiled paths (WS4-03)
- [ ] IQ-127 Implement HolyC helper `Q8_0MatMulTiledComputeSpanBoundsChecked` in `src/matmul/q8_0_matmul.HC` to unify tile span validation and `min(start+span,len)` semantics for row/col loops (WS4-03)
