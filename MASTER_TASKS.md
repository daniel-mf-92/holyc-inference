...
- [ ] IQ-153 Implement HolyC `FPQ16MulDivRoundedByPositiveIntFromQ16DenChecked` in `src/math/fixedpoint.HC` to accept positive integer denominators encoded as Q16 whole numbers (`den_q16 = n<<16`) and route through the positive-int fast path with strict divisibility/domain contracts for RMSNorm call sites (WS1-01)
