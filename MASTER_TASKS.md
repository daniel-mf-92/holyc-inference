...
- [ ] IQ-072 Implement HolyC helper `GGUFTensorInfoLoadPayloadWindowByIndexRelDefault` in `src/gguf/tensor_data_base.HC` to resolve index+relative window (default alignment) into checked `[abs_start,abs_end)` plus in-tensor span for parser-side chunked reads (WS2-04)
