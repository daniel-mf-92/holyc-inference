...
- [ ] IQ-070 Implement HolyC helper `GGUFTensorLookupSpanByIndexAndRelRange` in `src/gguf/tensor_data_base.HC` to validate caller-selected tensor index against a relative byte window (via tensor-data base + alignment) and return checked in-tensor span offsets (WS2-04)
