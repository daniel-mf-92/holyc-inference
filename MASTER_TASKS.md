...
- [ ] IQ-167 Implement HolyC helper `FPQ16ExpArrayFromClampedInputChecked` in `src/math/intexp.HC` to evaluate elementwise Q16 exp for pre-clamped vectors and fail fast on first out-of-domain lane without partial writes for split-phase softmax kernels (WS1-02)
