...
- [ ] IQ-172 Implement HolyC helper `FPQ16SoftmaxNormalizePhaseChecked` in `src/math/softmax.HC` to convert precomputed exp-lanes into normalized probabilities (`p_i = exp_i / sum_exp`) with checked zero/overflow guards and deterministic remainder correction to keep `sum(p)==1.0_q16` (WS1-03)
