...
- [ ] IQ-148 Implement HolyC `RoPEQ16RotateHeadRangeByTokenWindowPreflightedChecked` in `src/model/rope.HC` to run range-capacity + range-span preflight once, then apply checked per-token window rotation without partial writes on span faults (WS5-01)
