...
- [ ] IQ-174 Implement HolyC helper `FPQ16SoftmaxNormalizePhaseNoAliasChecked` in `src/math/softmax.HC` to enforce `exp_lanes_q16 != probs_q16` and delegate to `FPQ16SoftmaxNormalizePhaseChecked` so immutable exp-lane callers keep identical normalization/remainder semantics with explicit alias rejection (WS1-03)
