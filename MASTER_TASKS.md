...
- [ ] IQ-142 Implement HolyC helper `RoPEQ16ValidateHeadSpanForDimChecked` in `src/model/rope.HC` to centralize checked `{head_base + ((head_dim/2)-1)*pair_stride + 1 < head_cell_capacity}` bounds validation for per-head rotate kernels before pair iteration (WS5-01)\n
## Progress Ledger
