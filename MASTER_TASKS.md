...
- [ ] IQ-146 Implement HolyC helper `RoPEQ16RotateHeadRangeByTokenWindowChecked` in `src/model/rope.HC` to apply checked head-range rotation across `token_count` positions with checked `position_start + token_index*position_step` math and fail-fast preflight reuse (WS5-01)
