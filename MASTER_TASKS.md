...
- [ ] IQ-131 Implement HolyC helper `Q8_0MatMulTiledValidateOutIndexCapacityChecked` in `src/matmul/q8_0_matmul.HC` to centralize checked `{out_row_base + col_index < out_cell_capacity}` invariants and unify BAD_LEN/OVERFLOW behavior across scalar and AVX2 tiled writers (WS4-03)
- [ ] IQ-132 Add host-side parity harness `tests/test_q8_0_matmul_tiled_row_slice_preflight.py` to assert scalar/AVX2 tiled entrypoints surface identical BAD_LEN/OVERFLOW contracts for checked row-slice bounds (`row_base + k_block_count`) (WS4-03)
