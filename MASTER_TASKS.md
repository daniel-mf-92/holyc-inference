...
- [ ] IQ-143 Implement HolyC helper `RoPEQ16ValidateHeadRangeSpanForDimChecked` in `src/model/rope.HC` to preflight per-head last-pair span bounds (`head_base + ((head_dim/2)-1)*pair_stride + 1`) across `head_count` before multi-head rotation loops (WS5-01)
- [ ] IQ-144 Add host-side parity harness `tests/test_rope_q16_validate_head_range_span_for_dim.py` for range-span preflight helper with overflow and capacity adversarial cases (WS5-01)
- [ ] IQ-145 Implement HolyC helper `RoPEQ16RotateHeadRangeByPositionPreflightedChecked` in `src/model/rope.HC` that composes range-capacity + range-span preflight helpers then applies per-head rotation for one position (WS5-01)
