...
- [ ] IQ-149 Implement HolyC `RoPEQ16RotateHeadRangeByTokenWindowPreflightedHeadBasesChecked` in `src/model/rope.HC` to precompute and validate per-head base indices once, then reuse them across token-window loops with overflow-safe checked position stepping (WS5-01)
