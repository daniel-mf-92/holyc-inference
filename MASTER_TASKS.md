...
- [ ] IQ-168 Implement HolyC helper `FPQ16ExpArrayCheckedNoAliasChecked` in `src/math/intexp.HC` to enforce non-aliasing (`input_q16 != output_q16`) with shared checked span math for callers that require strict source immutability during softmax staging (WS1-02)
